...
# Status: LOCKED 2026-09-06 (v116) — preserve literal XDG path components during trailing-slash cleanup.
#
# Covers:
...
#   - Just-Perfection v37 build-time install (SHA256-pinned download from
...
[org/gnome/shell]
# Blocks InstallRemoteExtension D-Bus requests and loading home extensions
# at next login (GNOME 50 schema). Already-loaded extensions are unaffected.
# System extensions remain available; this is not a sandbox for their code.
...
# Disable external thumbnailer programs using this GNOME setting.
# Reduces automatic parsing; opening files and independent renderers remain.
# Trade-off: previews supplied by these external thumbnailers are unavailable.
disable-all=true
...
# GNOME-hosted and GPL-3.0-only. Static review of the pinned v37 payload found no
# autonomous network client or telemetry sender. Its user-invoked donation URL
# action is documented in M29; this statement is scoped to the pinned payload.
...
JP_SHA256="c54ccd627540d86e6baf06c699f624ba7d5229aedfc8b00b0dc7182c6538eda8"
JP_SIZE="237442"
JP_ENTRY_COUNT="77"
JP_EXPANDED_SIZE="888116"
...
# Lock the remote-install/home-extension policy (effective on next login).
...
for obj in objects:
    ...
    if "media.class" in props and not isinstance(media_class, str):
        raise SystemExit(1)
    if media_class == "Audio/Source" or (
            isinstance(media_class, str) and media_class.startswith("Audio/Source/")):
        node_id = obj.get("id")
        if type(node_id) is not int or not 0 <= node_id < 2**32:
            raise SystemExit(1)
        ids.add(node_id)
...
if grep -qF 'noid.microphone.privacy = required' \
   ...
   && grep -qF 'mixer:call ("set-volume"' \
       /usr/local/share/wireplumber/scripts/noid-microphone-privacy.lua \
   && grep -qF 'if not isinstance(objects, list):' \
       /usr/local/bin/noid-toggle-microphone \
   && grep -qF 'if type(node_id) is not int or not 0 <= node_id < 2**32:' \
       /usr/local/bin/noid-toggle-microphone \
...
# #339, cross-distro). Generators normalize only the path spelling.
# Legacy env.d file removed below.
log "Step 6b/8: XDG_DATA_DIRS trailing-slash cleanup via systemd env-generators (cosmetic-only on duplicate-name spam — upstream-bug remains)"

# Cleanup the legacy env.d file
...
remaining=$xdg
while :; do
    # Split only at literal colons: never expand pathname patterns or spaces.
    component=${remaining%%:*}
    while [ "$component" != / ] && [ "${component%/}" != "$component" ]; do
        component=${component%/}
    done
...
if [ ! -f "$JP_EXT_DIR/metadata.json" ]; then
    ...
    # pipefail-safe extraction (mirrors the appindicator handling). A grep
...
if [ ! -f "$APPINDICATOR_EXT_DIR/metadata.json" ]; then
    ...
    log "         M26 should install gnome-shell-extension-appindicator RPM)"
else
    ...
    # Different shape from Just-Perfection metadata.json. Query rpm for
...
# for path spelling to be normalized across GDM-greeter and user sessions.
# This does not fix the independent dbus-broker duplicate-name diagnostic.
for gen_path in \
