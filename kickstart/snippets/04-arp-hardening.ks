...
# Status: LOCKED 2026-09-07 (v2.52) — bound the chrony transition lock acquisition.
#
# Covers:
...
lock_chrony_transition() {
    ...
    # Bounded wait: both consumers run on the boot path with Type=oneshot, whose
    # systemd start timeout is disabled by default, so an unbounded acquisition
    # would have no ceiling at any level. The critical section is chronyc-only
    # and self-limiting (1s initial timeout, doubled, 2 retries), so 300s is the
    # project's ordinary boot-mutation ceiling with ample headroom. errexit
    # turns a timeout into a fail-closed exit that leaves the sources offline.
    /usr/bin/flock --exclusive -w 300 9
}
...
begin_transaction() {
    ...
    if [ -e "$lock_path" ] || [ -L "$lock_path" ]; then
        ...
        # Concurrent first-boot/dispatcher callers may both see an absent
        # lock. Exclusive creation preserves the winner's inode and flock;
        # install would replace that inode and allow overlapping transactions.
        if ! ( set -o noclobber; : > "$lock_path" ) 2>/dev/null; then
            [ -f "$lock_path" ] && [ ! -L "$lock_path" ] || {
                err "cannot create a regular transaction lock"
                return 1
            }
        fi
    fi
...
cmd_status() {
    ...
    local interface_present=1 status_failed=0 neighbour_query_ok=1
    if [ ! -d "$SYS_CLASS_NET/$WAN_IFACE" ]; then
        echo "  kernel pin:   unavailable [interface not present]"
        interface_present=0
        status_failed=1
    else
        if ! current_record=$(ip -4 neigh show to "$GATEWAY_IP" \
                dev "$WAN_IFACE" 2>/dev/null); then
            neighbour_query_ok=0
        fi
    ...
    if [ "$interface_present" -eq 0 ]; then
        :
    elif [ "$neighbour_query_ok" -eq 0 ]; then
        echo "  kernel pin:   unavailable [neighbour query failed]"
        status_failed=1
    elif [ "$ENABLED" = 0 ]; then
...
cmd_disable() {
    require_root
    local current_record remaining_pin
...
STATE_EOF
    ...
    if ! current_record=$(ip -4 neigh show to "$TX_OLD_GATEWAY_IP" \
            dev "$TX_OLD_IFACE"); then
        err "cannot query the gateway neighbour before removing its pin"
        return 1
    fi
    ...
    if ! remaining_pin=$(exact_permanent_neighbour_mac \
            "$TX_OLD_GATEWAY_IP" "$TX_OLD_IFACE"); then
        err "cannot verify the gateway neighbour after removing its pin"
        return 1
    fi
...
if [ "$#" -gt 1 ]; then
    err "expected exactly one command after optional --silent"
    usage >&2
    exit 2
fi
cmd="${1-help}"
