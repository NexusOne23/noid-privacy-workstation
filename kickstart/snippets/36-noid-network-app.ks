...
# Status: LOCKED 2026-09-06 (v54) — deliver section requests to the already-running window.
#
...
class NoIDNetworkApp(noid_ui.NoIDApplication):
    def __init__(self, section=None):
        ...
        self.set_flags(self.get_flags() | Gio.ApplicationFlags.HANDLES_COMMAND_LINE)
        self._section = section

    def do_command_line(self, command_line):
        # GApplication delivers secondary-process arguments to this primary
        # instance. Validate here too, before changing the visible page.
        try:
            self._section = _requested_section(command_line.get_arguments()[1:])
        except ValueError as error:
            command_line.printerr_literal('noid-network: %s\n' % error)
            return 2
        self.activate()
        return 0
...
# plus the outer IP and UDP headers. Floor the target to 16 bytes so a peer
# with a larger tunnel MTU cannot add padding beyond this local budget.
# Linux caps padding at the sending tunnel's MTU; the peer's MTU is unknown.
...
audit_mtu() {
    ...
    fmt_banner "WireGuard tunnel MTU audit" \
        "Tunnel MTU · outer endpoint route · conservative aligned target"
    fmt_note "Read-only: this mode inspects link and route state and changes nothing."

    if ! tunnels=$("$IP" -d -o link show type wireguard 2>/dev/null \
        | "$AWK" -F': ' '{print $2}' | "$AWK" '{print $1}'); then
        fmt_err "Cannot enumerate WireGuard interfaces; MTU comparison is unassessed"
        overall=1
        return
    fi
    if [ -z "$tunnels" ]; then
    ...
    while read -r iface; do
        ...
        if ! fwmark=$(mtu_tunnel_fwmark "$iface"); then
            fmt_err "WireGuard fwmark for $iface is unreadable or invalid; outer route unassessed"
            overall=1
            continue
        fi
        ...
        if ! endpoints=$("$WG" show "$iface" endpoints 2>/dev/null | "$AWK" '{print $2}'); then
            fmt_err "Cannot read peer endpoints for $iface; MTU comparison is unassessed"
            overall=1
            continue
        fi
        if [ -z "$endpoints" ]; then
        ...
        fmt_info "Conservative local MTU target: $worst_safe ($worst_detail; 16-byte aligned)"
        if [ "$tun_mtu" -le "$worst_safe" ]; then
            if [ "$unresolved" -gt 0 ]; then
                fmt_info "$iface fits every evaluated peer, but the verdict stays open"
            else
                fmt_ok "$iface fits the conservative local MTU target"
            fi
            continue
        fi

        fmt_err "$iface exceeds the conservative local target by $((tun_mtu - worst_safe)) bytes"
        fmt_info "Outer packets may fragment, including replies padded by a peer with"
        fmt_info "a larger tunnel MTU; large TLS handshakes can then stall or fail."
        overall=1
        if [ "$worst_safe" -lt 1280 ]; then
            ...
            if [ "$ipv6_state" = yes ]; then
                ...
                fmt_info "computed aligned target is $worst_safe. Lowering the tunnel would"
