...
# Status: LOCKED 2026-09-06 (v48) — validate MEI arguments and label configured NoID Privacy policy separately from driver state.
#
# Covers:
...
#     Intel serial-controller class and PCI function 3).
...
1. Requires a successful Snapper pre-snapshot before publishing a policy change
   or rebuilding initramfs; a missing or failing snapshot helper stops the change
2. Atomically publishes the reviewed `blacklist mei` + `blacklist mei_me` +
   matching `install ... /bin/false` policy in
   `/etc/modprobe.d/noid-mei-submodules.conf`
3. Atomically regenerates every installed initramfs through M21's shared
...
case "$#:${1:-}" in
    0:|1:--status|1:--list|1:status|1:--undo|1:undo|1:--help|1:-h) ;;
    *) fail "Usage: noid-mei-lockdown [--status|--undo|--help]" ;;
esac
...
if [ "${1:-}" = "--status" ] || [ "${1:-}" = "--list" ] || [ "${1:-}" = "status" ]; then
    ...
    for mod in $CORE_MODS; do
        if grep -qE "^blacklist ${mod}\$" "$CONF" && grep -qE "^install ${mod} /bin/false\$" "$CONF"; then
            policy_state="NoID Privacy ordinary-load block configured"
        else
            policy_state="no NoID Privacy core load block"
        fi
        if [ -d "/sys/module/${mod}" ]; then
            echo "  ${mod}: loaded; ${policy_state} (policy does not unload a running driver)"
        else
            echo "  ${mod}: not loaded; ${policy_state}"
        fi
    done
...
if [ "$CPU_VENDOR" = "intel" ]; then
    ...
    if [ "$mei_me_bound" = "yes" ]; then
        # A policy change does not unbind an already running driver.
        MEI_STATE="mei-me-bound"
        MEI_FWUPD_VISIBILITY="available-platform-results-vary"
    elif [ -d /sys/module/mei ]; then
        MEI_STATE="loaded-unbound"
        MEI_FWUPD_VISIBILITY="conditional"
    elif [ "$MEI_CORE_POLICY" = blacklisted ]; then
        MEI_STATE="blocked-by-policy"
        MEI_FWUPD_VISIBILITY="unavailable-by-policy"
    elif modinfo -F filename mei >/dev/null 2>&1; then
        # Metadata establishes presence, not loadability. A modprobe dry run
        # can succeed solely on an install override for a nonexistent module.
        ...
        # A failed metadata query alone cannot establish module absence.
        MEI_STATE="unknown"
        MEI_FWUPD_VISIBILITY="unknown"
    fi
    ...
    if [ -d /sys/module/ccp ]; then
        CCP_STATE="loaded"
    elif modinfo -F filename ccp >/dev/null 2>&1; then
...
# Sandbox. NoNewPrivileges is deliberately omitted: the kmod tools enter
...
# Keep /usr/lib/modules visible for `modinfo` while retaining the two
...
if [ -f /etc/modprobe.d/noid-mei-submodules.conf ]; then
    echo "  [OK] noid-mei-submodules.conf present (default-empty config)"
    ...
    if [ "$found_bl" -eq 0 ] && [ "$found_inst" -eq 0 ]; then
        echo "  [OK] no default MEI sub-module blacklists (opt-in only)"
    else
        echo "  [FAIL] default-policy regression: $found_bl blacklist + $found_inst install lines present (should be 0 in default)"
        fail=$((fail + 1))
    fi
else
...
for f in /etc/modprobe.d/noid-mei-submodules.conf \
    ...
    if [ -f "$f" ]; then
        ...
        echo "  [FAIL] $f missing — Steps 1-3 must always ship"
        fail=$((fail + 1))
    fi
done
...
if [ -f /usr/share/doc/noid-privacy/15-intel-me-hardware-layer.md ]; then
    ...
    echo "  [FAIL] Intel hardware-layer doc missing — Step 4 must always ship"
    fail=$((fail + 1))
fi
...
if [ "$CPU_VENDOR" = "intel" ]; then
    ...
    echo ""
    echo "Hardware-specific operator checklist (not applied by this image):"
    echo "  - Check OEM documentation for DASH/AIM-T management and its network paths"
    echo "  - Deprovision/disable management and remove every management-capable link"
    echo "  - Verify any replacement adapter is outside those management paths"
    echo "  - A vendor-specific PSP Support toggle does not attest ASP shutdown"
