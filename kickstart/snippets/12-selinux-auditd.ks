...
# Status: LOCKED 2026-09-06 (v64) — preserve notification health evidence and validate the recorded plugin lifecycle.
#
# Covers:
...
# 99-hardening.rules would LOAD but NEVER FIRE. Known Fedora gotcha: per
# auditctl(8), a `task` rule with action `never` suppresses every task-event
# record, so the shipped default silently defeats the syscall rule set.
#
...
#!/bin/bash
# Atomically toggle the reviewed auditd plugin and check its recorded child.
set -euo pipefail
PATH=/usr/sbin:/usr/bin:/sbin:/bin
export PATH
if [ "$#" -gt 1 ]; then
...
plugin_pid() {
    local pid
    [ -f "$HEALTH" ] && [ ! -L "$HEALTH" ] \
        && [ "$(stat -c '%U:%G:%a:%h' "$HEALTH" 2>/dev/null)" = root:root:640:1 ] \
        || return 1
    pid=$(sed -n 's/^pid=//p' "$HEALTH") || return 1
    # Multiple records remain newline-separated and fail this exact match.
...
plugin_process() {
    ...
    local -a argv=()
    [[ "$pid" =~ ^[1-9][0-9]*$ ]] || return 1
    # The fixed Python shebang and argument-free auditd configuration produce
    # exactly two argv records. A path in another process's arguments, or a
    # newline inside one argument, is not evidence of the plugin running.
    mapfile -d '' -t argv 2>/dev/null < "/proc/$pid/cmdline" || return 1
    [ "${#argv[@]}" -eq 2 ] && [ "${argv[0]}" = /usr/bin/python3 ] \
        && [ "${argv[1]}" = "$PLUGIN" ] \
        && [ "/proc/$pid/exe" -ef /usr/bin/python3 ] || return 1
    parent=$(awk '/^PPid:/ {print $2}' "/proc/$pid/status" 2>/dev/null) || return 1
    [[ "$parent" =~ ^[1-9][0-9]*$ ]] \
...
plugin_running() {
    local pid state
    pid=$(plugin_pid) || return 1
    state=$(sed -n 's/^state=//p' "$HEALTH") || return 1
    [ "$state" = running ] && plugin_process "$pid"
}
...
case "$ACTION" in
    on)
        ...
        ;;
    off)
        # A never-started or failed plugin can have no usable health record.
        # Still publish active=no and ask auditd to unload its configuration.
        old_pid=$(plugin_pid) || old_pid=
        set_active no
        reload_auditd
        if [[ "$old_pid" =~ ^[0-9]+$ ]]; then
            for _ in $(seq 1 15); do
                plugin_process "$old_pid" || exit 0
                sleep 1
            done
