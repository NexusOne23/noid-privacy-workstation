...
# Status: LOCKED 2026-09-06 (v66) — reject ambiguous GRUB password arguments before entering credential handling.
#
# Covers:
...
#   - GRUB config (timeout, terminal output, os-prober off; menu auto-hide
#     stays Fedora-owned — see STEP 3)
...
# Scope: fedora*.repo metalinks only. RPM Fusion's metalinks are written and
# owned by Module 08 STEP 7c; VSCodium and negativo17 use an HTTPS baseurl=
# directly. Idempotent and fail-closed on unsupported syntax.
...
# boot for every LUKS UUID Anaconda's target BLS already exposes. This keeps
# the normal encrypted-install path byte-aligned with the firstboot
# canonicalizer and avoids manufacturing a needless first-session delta. The
# selection is token-exact and covers every rd.luks.uuid= (a separately
# encrypted volume must not be left without its retry policy); the runtime
# contract in tests/01-karg-contract.py requires exactly one policy per UUID.
for LUKS_UUID in $(printf '%s\n' "$current" | tr ' ' '\n' \
    | sed -n 's/^rd\.luks\.uuid=luks-\([0-9a-fA-F-]\{20,\}\)$/\1/p'); do
...
# Token-exact per-UUID selection, identical to the target %post and the
# firstboot helper: every rd.luks.uuid= keeps its own retry policy.
for LUKS_UUID in $(tr ' ' '\n' < "$CMDLINE_FILE" \
    | sed -n 's/^rd\.luks\.uuid=luks-\([0-9a-fA-F-]\{20,\}\)$/\1/p'); do
    LUKS_KARG="rd.luks.options=${LUKS_UUID}=tries=0,discard"
...
# from every active rd.luks.uuid=. systemd also accepts a global option list,
# but the volume-specific form keeps the reviewed policy explicit per selector.
...
for required_arg in $EXTRA; do
    required_kargs+=("$required_arg")
done
...
# form for every live rd.luks.uuid= so each listed volume carries its own
# policy: systemd-cryptsetup-generator applies an unprefixed rd.luks.options=
# only to UUIDs without a per-UUID entry, so one shared token is not a
# substitute. Token-exact selection, identical to the target %post and the
# canonicalizer; discard kept.
luks_uuid_count=0
for LUKS_UUID in $(tr ' ' '\n' < "$CMDLINE_FILE" 2>/dev/null \
    | sed -n 's/^rd\.luks\.uuid=luks-\([0-9a-fA-F-]\{20,\}\)$/\1/p' || true); do
    luks_uuid_count=$((luks_uuid_count + 1))
    LUKS_KARG="rd.luks.options=${LUKS_UUID}=tries=0,discard"
    required_kargs+=("$LUKS_KARG")
    if tr ' ' '\n' < "$CMDLINE_FILE" 2>/dev/null \
            | grep -qxF "$LUKS_KARG"; then
    ...
    if [ -f /etc/default/grub ] && grep -q '^GRUB_CMDLINE_LINUX=' /etc/default/grub; then
        if ! sed -n 's/^GRUB_CMDLINE_LINUX="\(.*\)"$/\1/p' /etc/default/grub \
                | tr ' ' '\n' | grep -qxF "$LUKS_KARG"; then
...
verify_required_kargs() {
    local durable_path="$1" required_arg
    shift
    for required_arg in "$@"; do
        [ -n "$required_arg" ] || continue
        if ! tr ' ' '\n' < "$durable_path" | grep -qxF "$required_arg"; then
...
# target argument, including every per-volume LUKS policy, is present in the
# kernel-install source of truth. /proc/cmdline still represents the current
# boot and cannot prove next-boot state.
if ! verify_required_kargs /etc/kernel/cmdline "${required_kargs[@]}"; then
    exit 1
fi
...
#
# SystemCallFilter remains absent after an earlier firstboot SIGSYS report
# during sed/grubby execution. That report did not identify the denied syscall;
# the installed @privileged/@resources groups do not contain rename or execve.
# Reintroducing a filter requires tracing the actual firstboot plugin chain in
# a candidate image. The filesystem, network and namespace restrictions below
# remain in place; this service runs once per boot and validates its state.
NoNewPrivileges=yes
LockPersonality=yes
...
# The 01_users-generated GRUB block sources this file when GRUB executes it.
...
These controls cover different layers: GRUB edit access, encrypted data at
rest, and signature verification for covered boot components and modules.
They do not jointly authenticate every file on the unencrypted `/boot`.
GRUBDOC_EOF
