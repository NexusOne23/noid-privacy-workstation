...
# Status: LOCKED 2026-09-06 (v45) — reject ambiguous history-toggle arguments before policy writes.
#
...
#     and a fail-visible libdnf5 post-transaction action scoped to the six
#     owning packages (crontabs owns the cron.{hourly,daily,weekly,monthly}
#     directories, cronie owns cron.d). No periodic custom chmod mutator is
#     shipped.
...
post_transaction:crontabs:in:enabled=host-only raise_error=1:/usr/bin/sh -c /usr/bin/systemd-tmpfiles\ --create\ /etc/tmpfiles.d/90-noid-permission-policy.conf\ >/dev/null
...
case "$#:${1:-}" in
    0:|1:ephemeral|1:default|1:status) ;;
    *) printf '%s\n' 'Usage: noid-toggle-bash-history {ephemeral|default|status}' >&2; exit 2 ;;
esac
...
# Detects silent permission-reset via rpm-update of crontabs/cronie/sudo packages.
