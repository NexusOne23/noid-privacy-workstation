# ============================================================================
# Module 14 — USBGuard
# Status: LOCKED 2026-09-07 (v61) — fail closed on malformed device records at every rule-reader call site.
#
# Covers:
...
#   - usbguard.service stop recovery: policy reloads are bounded to 15 seconds
#     and use direct SIGKILL recovery instead of Fedora's ineffective SIGABRT
#     diagnostic path (NoID Privacy disables coredump payloads)
...
#     not prevent its `allow id *:*` injection). M12 popup plugin:
...
# A policy-file replacement requires the upstream-supported daemon restart.
# USBGuard can stall while shutting down if a device disappears during its
# sysfs/realpath walk. Fedora's global 45-second timeout then sends SIGABRT for
# a diagnostic coredump, but NoID Privacy intentionally disables coredump
# payloads. Bound this root mutator's boot impact and go directly to SIGKILL
# after a generous normal-stop interval. RestoreControllerDeviceState=false
# above keeps controller default authorization fail-closed across a forced
# stop; the firstboot state remains retryable until the restarted daemon and
# its policy have passed their postconditions. Use the exact Fedora top-level
# drop-in basename: systemd suppresses the lower-priority service.d copy in
# /usr/lib, whereas differently named drop-ins are ordered lexicographically
# across both the top-level and unit-specific directories.
mkdir -p /etc/systemd/system/usbguard.service.d
cat > /etc/systemd/system/usbguard.service.d/10-timeout-abort.conf <<'USBGUARD_STOP_RECOVERY_EOF'
[Service]
TimeoutStopSec=15s
TimeoutStopFailureMode=kill
USBGUARD_STOP_RECOVERY_EOF
chmod 0644 /etc/systemd/system/usbguard.service.d/10-timeout-abort.conf
chown root:root /etc/systemd/system/usbguard.service.d/10-timeout-abort.conf
restorecon -F /etc/systemd/system/usbguard.service.d/10-timeout-abort.conf 2>/dev/null || true
echo "  [OK] usbguard.service stop recovery bounded to 15 seconds"
...
if [ "$CURRENT_STATE" = "real" ] || [ "$CURRENT_STATE" = "initializing" ]; then
    ...
    if [ "$CURRENT_STATE" = "initializing" ]; then
        PENDING_REAL=1
        # A same-boot retry cannot prove that the already-running daemon loaded
        # the newly published policy. A later boot has loaded it, but one
        # bounded redundant restart is the safe ambiguity resolution.
        USBGUARD_RESTART_REQUIRED=1
...
# BEGIN USBGUARD_SERVICE_ACTIVATION
...
if ! systemctl is-active --quiet usbguard.service; then
    ...
    USBGUARD_DAEMON_CHANGED=1
elif [ "$USBGUARD_RESTART_REQUIRED" -eq 1 ]; then
    log "usbguard.service already running -- restarting to load the replaced policy"
    ...
    log "usbguard.service already running with the unchanged committed policy"
fi
...
elif [ "$USBGUARD_DAEMON_CHANGED" -eq 1 ]; then
    log "usbguard-dbus.service already running -- restarting after daemon transition"
    if ! systemctl restart usbguard-dbus.service; then
        log "ERROR: failed to restart usbguard-dbus.service"
        exit 1
    fi
else
    log "usbguard-dbus.service already running; no daemon transition to reconcile"
fi
...
# END USBGUARD_SERVICE_ACTIVATION
...
# Shared lexical field reader for the native canonical USBGuard rows. Quoted
# names and serials, including escaped quotes, remain indivisible tokens.
# USBGuard remains the policy grammar authority; this reader only separates
# selector fields from their values and the optional condition. Programs passed
# here are fixed helper code, while every device-provided value stays on stdin.
usbguard_rule_awk() {
    local program=$1
    shift
    awk "$@" '
        function split_record(line, fields, gaps, count, i) {
            count=patsplit(line, fields, /"([^"\\]|\\.)*"|[^[:space:]"]+/, gaps)
            for (i=0; i<=count; i++) {
                if (gaps[i] !~ /^[[:space:]]*$/) {
                    parse_error=1
                    return 0
            ...
            if (count < 2 || fields[1] !~ /^[0-9]+:$/ ||
                fields[2] !~ /^(allow|block|reject)$/) {
                parse_error=1
                return 0
            }
            return count
        }
        function attribute_key(token) {
            return token ~ /^(id|serial|name|hash|parent-hash|via-port|with-interface|with-connect-type|label|if)$/
        }
        function raw_attribute(line, key, fields, gaps, count, i, j, value) {
            count=split_record(line, fields, gaps)
            for (i=3; i<=count; i++) {
                if (fields[i] == "if") return ""
                if (fields[i] != key) continue
                value=""
                for (j=i+1; j<=count && !attribute_key(fields[j]); j++) {
                    value=value (value == "" ? "" : " ") fields[j]
                }
                return value
            }
            return ""
        ...
        function field_value(line, key, value) {
            value=raw_attribute(line, key)
            if (value ~ /^"([^"\\]|\\.)*"$/) {
                return substr(value, 2, length(value)-2)
            }
            return value
        }
        function conditional_rule(line, fields, gaps, count, i) {
            count=split_record(line, fields, gaps)
            for (i=3; i<=count; i++) {
                if (fields[i] == "if") return 1
            }
            return 0
        }
        # This backstop must precede the caller program. gawk runs END rules in
        # source order and stops END processing at the first exit, so a call
        # site with its own END would otherwise consume the exit status and
        # report a malformed record as an ordinary empty attribute.
        END {
            if (parse_error) {
                print "Error: malformed USBGuard record; device attributes are unverified." > "/dev/stderr"
                exit 1
            }
        }
    '"$program"
...
# every remaining byte of the canonical USBGuard descriptor fields,
...
device_line_without_state() {
    usbguard_rule_awk '
        match($0, /^[0-9]+: (allow|block|reject) /) {
            prefix_length=RLENGTH
            if (split_record($0, fields, gaps) < 3) exit 1
            print $1 " " substr($0, prefix_length+1)
            matched++
            next
        }
        { bad=1 }
        END { exit(bad || matched != 1) }
    ' <<< "$1"
}

same_runtime_device() {
    local before after
    before=$(device_line_without_state "$1") || return 1
    after=$(device_line_without_state "$2") || return 1
    [ "$before" = "$after" ]
}
...
render_device_rows() {
    ...
    usbguard_rule_awk '
        {
            ...
            product_id=field_value($0, "id")
            if (product_id !~ /^[[:xdigit:]]{4}:[[:xdigit:]]{4}$/) product_id="unknown"

            device_name=field_value($0, "name")
            ...
            interface_id=field_value($0, "with-interface")
            if (interface_id == "") interface_id="unknown"
            if (interface_id ~ /^\{ [[:xdigit:]]{2}:[[:xdigit:]]{2}:[[:xdigit:]]{2} \}$/) {
                sub(/^\{ /, "", interface_id)
                sub(/ \}$/, "", interface_id)
            }
    ...
    ' -v display_mode="$display_mode" -v selected_index="$selected_index"
}

device_vidpid() {
    usbguard_rule_awk '{ print field_value($0, "id") }' <<< "$1"
}

device_via_port() {
    usbguard_rule_awk '{ print field_value($0, "via-port") }' <<< "$1"
}

device_hash() {
    usbguard_rule_awk '{ print field_value($0, "hash") }' <<< "$1"
}

device_parent_hash() {
    usbguard_rule_awk '{ print field_value($0, "parent-hash") }' <<< "$1"
}

device_interfaces() {
    usbguard_rule_awk '{ print field_value($0, "with-interface") }' <<< "$1"
}
...
is_generated_topology_allow_rule() {
    local device_line=$1 policy_line=$2
    printf '%s\n%s\n' "$device_line" "$policy_line" | usbguard_rule_awk '
        NR == 1 {
            source_id=field_value($0, "id")
            source_hash=field_value($0, "hash")
            next
        }
        NR == 2 {
            if ($2 != "allow" || conditional_rule($0) || raw_attribute($0, "label") != "") exit 1
            count=split("serial name hash parent-hash with-connect-type", required, " ")
            for (i=1; i<=count; i++) {
                if (raw_attribute($0, required[i]) !~ /^"([^"\\]|\\.)*"$/) exit 1
            }
            if (raw_attribute($0, "with-interface") == "") exit 1
            matched=(source_id != "" && field_value($0, "id") == source_id &&
                     source_hash != "" && field_value($0, "hash") == source_hash)
        }
        END { exit(NR != 2 || !matched) }
    '
}
...
is_modeswitch_precursor() {
    local line=$1 vidpid interfaces config metadata
    vidpid=$(device_vidpid "$line")
    [[ "$vidpid" =~ ^[[:xdigit:]]{4}:[[:xdigit:]]{4}$ ]] || return 1
    interfaces=$(device_interfaces "$line") || return 1
    grep -Eq '^(\{ )?08:06:50([[:space:]}]|$)' <<< "$interfaces" || return 1
...
modeswitch_target_line() {
    local source_line=$1 device_rows=$2
    printf '%s\n%s\n' "$source_line" "$device_rows" | usbguard_rule_awk '
        NR == 1 {
            source_id=field_value($0, "id")
            source_port=field_value($0, "via-port")
            source_parent=field_value($0, "parent-hash")
            if (source_id == "" || source_port == "" || source_parent == "") exit 1
            next
        }
        {
            id=field_value($0, "id")
            port=field_value($0, "via-port")
            parent=field_value($0, "parent-hash")
...
if [ "$#" -eq 1 ]; then
    ...
    if ! DEV_LINE=$(blocked_device_line "$DEV_ID" "$BLOCKED"); then
        ...
        render_device_rows menu <<< "$BLOCKED" >&2
        exit 1
    fi
else
    # Interactive menu
...
RULE_TOKEN_RE = re.compile(r'\s+|"(?:\\.|[^"\\])*"|[^\s"]+')
...
class Rule:
    ...
    def conditional(self) -> bool:
        return any(token == "if" for _, token in rule_tokens(self.body))
...
def rule_tokens(body: str) -> list[tuple[int, str]]:
    """Locate native rule tokens without interpreting quoted device data."""
    tokens: list[tuple[int, str]] = []
    position = 0
    while position < len(body):
        match = RULE_TOKEN_RE.match(body, position)
        if match is None:
            raise ManagerError("USBGuard returned malformed quoted rule data")
        token = match.group()
...
def attribute(pattern: re.Pattern[str], body: str) -> str:
    # A device's name or serial may contain text such as "with-interface ..."
    # or "id ...". Match only at unquoted token boundaries, keeping the
    # original bytes for the existing escaped-string/value expressions.
    # A condition's arguments are not attributes of this rule's selector.
    for position, token in rule_tokens(body):
        if token == "if":
            break
        if token.startswith('"'):
            continue
        start = position - 1 if position and body[position - 1] == " " else position
...
def label(body: str) -> str:
    value = attribute(LABEL_RE, body)
    return clean_text(value, 48) if value else ""
...
def rule_kind(rule: Rule) -> str:
    ...
    if not attribute(VIDPID_RE, rule.body) or not attribute(HASH_RE, rule.body):
...
def is_managed_revoke_candidate(rule: Rule) -> bool:
    return (
        ...
        and bool(attribute(VIDPID_RE, rule.body))
        and bool(attribute(HASH_RE, rule.body))
...
followed by the vendor/product ID, complete interface set and separately labelled
...
usbguard_stop_recovery=/etc/systemd/system/usbguard.service.d/10-timeout-abort.conf
if [ ! -f "$usbguard_stop_recovery" ] || [ -L "$usbguard_stop_recovery" ] || \
   [ "$(stat -Lc '%U:%G:%a:%h' "$usbguard_stop_recovery" 2>/dev/null || true)" != \
       root:root:644:1 ] || \
   ! matchpathcon -V "$usbguard_stop_recovery" >/dev/null 2>&1 || \
   ! cmp -s "$usbguard_stop_recovery" <(printf '%s\n' \
       '[Service]' \
       'TimeoutStopSec=15s' \
       'TimeoutStopFailureMode=kill'); then
    echo "  [FAIL] USBGuard bounded stop-recovery drop-in missing or drifted"
    fail=$((fail + 1))
else
    echo "  [OK] USBGuard policy-reload stop path is bounded and fail-closed"
fi
