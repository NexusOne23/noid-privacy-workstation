...
# Status: LOCKED 2026-09-06 (v36) — verify remote identities with native GKeyFile semantics.
#
# Covers:
...
with os.fdopen(fd, "r", encoding="utf-8") as stream:
    payload = stream.read()
# Flatpak/OSTree use GKeyFile: case-sensitive keys, native escapes and no
# ConfigParser DEFAULT inheritance. Read only the already-checked inode.
config.load_from_data(payload, len(payload.encode("utf-8")), GLib.KeyFileFlags.NONE)
for group in config.get_groups()[0]:
    keys = config.get_keys(group)[0]
    if len(keys) != len(set(keys)):
        raise SystemExit("duplicate repository configuration key")

expected_names = {"flathub", "flathub-verified"}
remote_sections = {
    section[len('remote "'):-1]: section
    for section in config.get_groups()[0]
...
def value(section, key, default=None):
    return config.get_string(section, key) if key in config.get_keys(section)[0] else default

def enabled_bool(section, key, default):
    return config.get_boolean(section, key) if key in config.get_keys(section)[0] else default

for name in sorted(expected_names):
    section = remote_sections[name]
    actual_keys = set(config.get_keys(section)[0])
...
if config.get_integer(full, "xa.prio") != 1:
...
if config.get_integer(verified, "xa.prio") != 2:
...
if not config.has_group(section):
    raise SystemExit(1)
...
if not required_keys <= actual_keys or not (actual_keys - required_keys) <= optional_keys:
    raise SystemExit(1)
if config.get_string(section, "url") != expected_url:
    raise SystemExit(1)
if config.get_string(section, "xa.title") != "Fedora Flatpaks":
    raise SystemExit(1)
if not config.get_boolean(section, "xa.title-is-set"):
    raise SystemExit(1)
if "xa.prio" in actual_keys and config.get_integer(section, "xa.prio") != 1:
    raise SystemExit(1)
PY_EOF
}
...
check "python3 -c 'from gi.repository import GLib; GLib.KeyFile()'" \
    "native GKeyFile parser available for exact remote verification"
