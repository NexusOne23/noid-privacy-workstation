...
# Status: LOCKED 2026-09-06 (v80) — reject incomplete GPU and connector evidence before selecting renderer or Mutter offload policy.
#
...
#     akmods certificate, including cryptographic verification of the complete
#     uncompressed module payload. Signature metadata alone is insufficient.
...
disable the rule by replacing
`/etc/udev/rules.d/62-noid-mutter-headless-offload.rules` with a symlink to
`/dev/null` (udev then reads an empty rules file; NoID Privacy ships no
lower-priority copy under `/usr/lib/udev/rules.d`). Apply either state at the
next complete logout/login or reboot.
...
| `/usr/libexec/noid-nvidia-verify` | read-only integrity verifier | Requires the complete branch pair, matching akmod/CUDA/kmod EVR, the branch-required module flavor (`Dual MIT/GPL` main, `NVIDIA` R580), and all four modules for the exact kernel. Cryptographically verifies each complete uncompressed module against one captured local akmods certificate; matching signature metadata alone is insufficient. Update workers also require that certificate to be enrolled. Removed by `--rollback`. |
...
| `/usr/libexec/noid-nvidia-initramfs-queue` + `noid-nvidia-reboot-guard.service` | durable queue + inhibitor | Writes a unique persistent task before acquiring a block shutdown/sleep inhibitor and scheduling the worker. Failed tasks, unreadable queues and unsafe pending objects keep the inhibitor; only a successful scan without pending entries releases it. Resume rejects failed queue scans; boot-time resume handles power loss. Removed by `--rollback`. |
...
read_boot_vga() {
    local path="$1/boot_vga" value
    # 3D offload devices may omit this attribute. A present but unreadable,
    # dangling or invalid attribute does not establish a non-primary GPU.
...
if [ "$#" -ne 0 ]; then
    ...
    boot_vga=$(read_boot_vga "$device") || exit 1
    [ "$boot_vga" = 0 ] || exit 1
    ...
    connectors=$(find "$real_card" -mindepth 1 -maxdepth 1 \
        -name "$card_name-*" -printf connector -quit 2>/dev/null) || exit 1
    [ -z "$connectors" ] || exit 1
    ...
    for candidate in "$SYS_ROOT"/class/drm/card*; do
        ...
        [ -e "$candidate" ] || exit 1
        candidate_real=$(readlink -f -- "$candidate" 2>/dev/null) || exit 1
        candidate_device=$(
            readlink -f -- "$candidate_real/device" 2>/dev/null
        ) || exit 1
        [ -n "$candidate_device" ] && [ -d "$candidate_device" ] || exit 1
        [ -z "${card_seen[$candidate_device]:-}" ] || continue
        card_seen[$candidate_device]=1
        boot_vga=$(read_boot_vga "$candidate_device") || exit 1
        [ "$boot_vga" = 1 ] || continue
...
for card in "$SYS_ROOT"/class/drm/card*; do
    ...
    link=$card/device
    [ -e "$link" ] || exit 1
    device=$(readlink -f "$link" 2>/dev/null) || exit 1
    [ -n "$device" ] && [ -d "$device" ] || exit 1
    [ -z "${seen[$device]:-}" ] || continue
    seen[$device]=1
    class=$(cat "$device/class" 2>/dev/null) || exit 1
...
case "$action" in
    apply)
        ...
        manager_environment=$(systemctl --user show-environment) \
            || fail "cannot inspect the user-manager activation environment"
        grep -qxF GSK_RENDERER=gl <<< "$manager_environment" \
    ...
    clear)
        ...
        # An absent value is evidence only after a complete successful query.
        manager_environment=$(systemctl --user show-environment) \
            || fail "cannot inspect the user-manager activation environment"
        if grep -qxF GSK_RENDERER=gl <<< "$manager_environment"; then
            systemctl --user unset-environment GSK_RENDERER
            manager_environment=$(systemctl --user show-environment) \
                || fail "cannot verify the user-manager renderer cleanup"
            if grep -q '^GSK_RENDERER=' <<< "$manager_environment"; then
...
if [ "$MODE" = "rollback" ]; then
    ...
    # dnf remove above, but don't rely on it — leaving rd.driver.blacklist=…
    # / nvidia-drm.modeset=1 behind is untidy and can confuse a future re-install.
    # Both the historical single-driver form and the current Fedora 44 form
    # (`nouveau,nova_core`) are named literally, so the result does not depend
    # on grubby's key-versus-value matching. Idempotent (no-op if already gone).
    if ! sudo grubby --update-kernel=ALL \
            --remove-args="rd.driver.blacklist=nouveau modprobe.blacklist=nouveau rd.driver.blacklist=nouveau,nova_core modprobe.blacklist=nouveau,nova_core nvidia-drm.modeset=1"; then
...
# Freeze one public certificate for every module and the enrollment query.
# This private scratch contains public certificates/module bytes, never a key.
signature_workspace=$(mktemp -d /var/tmp/noid-nvidia-verify.XXXXXX) \
    || fail "cannot allocate signature verification workspace"
trap 'rm -rf -- "$signature_workspace"' EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
trap 'exit 129' HUP
openssl x509 -inform DER -in "$cert" -outform DER \
    -out "$signature_workspace/certificate.der" 2>/dev/null \
    || fail "cannot read the exact akmods certificate"
cert="$signature_workspace/certificate.der"
...
verify_module_signature() {
    # modinfo reads signature metadata; it does not authenticate the payload.
    # Linux signs the complete uncompressed ELF and appends a PKCS#7 envelope:
    # https://docs.kernel.org/admin-guide/module-signing.html
    python3 - "$1" "$cert" "$signature_workspace" <<'MODULE_SIGNATURE_PY'
...
module, certificate, workspace = map(Path, sys.argv[1:])
elf = workspace / "module.ko"
signature = workspace / "signature.der"
magic = b"~Module signature appended~\n"

try:
    if module.name.endswith(".ko"):
        shutil.copyfile(module, elf)
    else:
        decompressors = {".xz": "/usr/bin/xz", ".zst": "/usr/bin/zstd",
                         ".gz": "/usr/bin/gzip"}
        decompressor = decompressors.get(module.suffix)
        if decompressor is None:
            raise ValueError("unsupported compression")
        with elf.open("wb") as output:
            subprocess.run([decompressor, "-dc", "--", str(module)],
                           stdout=output, stderr=subprocess.DEVNULL, check=True)

    with elf.open("r+b") as data:
        if data.read(4) != b"\x7fELF":
            raise ValueError("not an ELF module")
        size = data.seek(0, 2)
        if size <= len(magic) + 12:
            raise ValueError("missing signature trailer")
        data.seek(-len(magic), 2)
        if data.read() != magic:
            raise ValueError("missing signature marker")
        data.seek(-len(magic) - 12, 2)
        header = data.read(12)
        # Linux UAPI struct module_signature: PKCS#7 type, zero legacy
        # algorithm/name/key-id fields and padding, big-endian envelope length.
        if header[:8] != bytes((0, 0, 2, 0, 0, 0, 0, 0)):
            raise ValueError("invalid PKCS#7 trailer")
        length = struct.unpack(">I", header[8:])[0]
        start = size - len(magic) - 12 - length
        if length == 0 or start < 4:
            raise ValueError("invalid signature extent")
        data.seek(start)
    ...
    # Require one complete DER sequence; OpenSSL alone may ignore trailing
    # bytes after a parsed envelope. Native sign-file emits definite DER.
    with signature.open("rb") as data:
        tag = data.read(2)
        if len(tag) != 2 or tag[0] != 0x30:
            raise ValueError("invalid signature sequence")
        if tag[1] < 128:
            encoded_length = 2 + tag[1]
        else:
            count = tag[1] & 127
            if not 1 <= count <= 4:
                raise ValueError("invalid DER length")
            value = data.read(count)
            if len(value) != count or value[0] == 0 or int.from_bytes(value, "big") < 128:
                raise ValueError("noncanonical DER length")
            encoded_length = 2 + count + int.from_bytes(value, "big")
        if encoded_length != length:
            raise ValueError("signature envelope has trailing or missing bytes")

    # -nointern pins signer lookup to this one local certificate, even when a
    # foreign embedded certificate repeats its issuer/serial. -noverify skips
    # S/MIME CA-chain policy, not the payload signature: MOK enrollment is the
    # separate kernel trust gate. Never use -nosigs/-no_content_verify here.
    # https://docs.openssl.org/3.5/man1/openssl-cms/
    subprocess.run(["/usr/bin/openssl", "cms", "-verify", "-binary", "-inform", "DER",
                    "-in", str(signature), "-content", str(elf), "-certfile", str(certificate),
                    "-nointern", "-noverify", "-out", "/dev/null"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
except (OSError, ValueError, subprocess.SubprocessError):
    raise SystemExit("noid-nvidia-verify: module signature verification failed")
MODULE_SIGNATURE_PY
}
...
for module in "${modules[@]}"; do
    ...
    verify_module_signature "$canonical_path" \
        || fail "$module failed cryptographic verification against the exact akmods certificate"
done
...
# Only a successful empty scan may release the inhibitor. A failed scan must
# keep this process alive, rather than rely on a restart that drops the lock.
# Unsafe pending objects still represent unresolved work; the queue validator
# rejects their type before scheduling, but they must not look like no work.
queue_error=0
while :; do
    if [ -d "$queue_dir" ] && [ ! -L "$queue_dir" ] \
            && pending=$(find "$queue_dir" -mindepth 1 -maxdepth 1 \
                -name '*.pending' -printf pending -quit 2>/dev/null); then
        [ -n "$pending" ] || break
        queue_error=0
    else
        if [ "$queue_error" -eq 0 ]; then
            printf 'NoID Privacy: NVIDIA queue scan failed; retaining shutdown/sleep inhibitor\n' >&2
        fi
        queue_error=1
    fi
    sleep 1
...
if [ "$mode" = resume ]; then
    ...
    if [ ! -d "$queue_dir" ] || [ -L "$queue_dir" ] \
            || ! pending=$(find "$queue_dir" -mindepth 1 -maxdepth 1 \
                -name '*.pending' -printf pending -quit); then
        printf 'NoID Privacy: NVIDIA resume queue scan failed\n' >&2
        exit 1
    fi
    if [ -z "$pending" ]; then
        release_boot_mutation
...
# plymouth.use-simpledrm=1 is an M01-managed token that Step 2's reconciliation
# already publishes into BLS and /etc/kernel/cmdline; on a converged system the
# grubby branch below is an idempotent defensive no-op, not a competing writer.
