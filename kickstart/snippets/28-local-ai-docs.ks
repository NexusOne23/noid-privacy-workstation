...
# Status: LOCKED 2026-09-22 (v46) — drop the host hardware identity and the second reference model from the local AI record.
#
...
At the 2026-09-21 review, Fedora 44 still offered `ollama-0.12.11-4.fc44`.
CVE-2026-7482 affects Ollama before 0.17.1; the CVE-2026-5757 record separately
names 0.13.5 as affected. The audited Fedora candidate is therefore
...
2026-09-21 review, the current upstream release was `v0.34.2`; its published
Linux x86-64 artifact was 1,427,542,079 bytes, and the vendor checksum plus
...
ollama_expected=e155b83589986d2c581fdbf1381ea3ebdb16549883679cd5a0627f7cdc05b12b
...
For v0.34.2 the vendor checksum matched the retained pin, the archive's 65
entries had no absolute/traversal paths or special device entries, and the
extracted client was executed offline in a network-unshared sandbox. Its version
probe reported 0.34.2, and `ollama serve --help` retained the
`127.0.0.1:11434` loopback default plus the documented `OLLAMA_NO_CLOUD`
control.

The source review also covered two availability advisories whose complete
affected ranges are unspecified. ZDI-26-403 / CVE-2026-15685 describes an
unchecked array index in `downloadBlob`; the current CVE record identifies
0.7.1 as affected. Upstream commit `9239a254e054` added an empty-digest guard on
2025-05-27, release `v0.8.0` contains that commit, and the reviewed `v0.34.2`
source additionally routes the digest through `manifest.BlobsPath`
(`manifest/paths.go`, `^sha256[:-][0-9a-fA-F]{64}$`) before any `[7:19]`
slice. That validator accepts only a complete `sha256:`/`sha256-`
...
issue 17042 links the repair to merged commit `67b6a1c2d453`; `v0.34.2`
contains it, and its reviewed parser caps strings, arrays and tensor dimensions
before allocation and rejects a tensor offset or offset+size beyond the file. These checks reduce the known parser DoS, but they do not
...
The upstream tensor-redirect SSRF report, issue 17041, remained open at
review time. In `v0.34.2`, `x/transfer/download.go` now validates redirect
schemes and targets; `x/transfer/redirect.go` rejects loopback, private,
link-local and other non-public redirect addresses, and pins the validated
DNS result when connecting. The explicit registry host is exempt, and
`AllowPrivateHosts` relaxes address checks. This source review confirms those
new checks, not a complete SSRF security proof or an upstream issue closure.
Do not let untrusted clients or model references drive `/api/pull`; retain
the loopback API bind and the host egress boundary where stronger containment
is required. The release also adds first-run account setup: choose local use
and retain `OLLAMA_NO_CLOUD=1` for the documented local-only configuration.
...
test -f "$lmstudio_source"
test ! -L "$lmstudio_source"
...
At the 2026-09-02 review, Fedora 44 still offered `b6153-3.fc44`, provided no newer `llama`
...
At the 2026-09-21 review, the newest upstream `b*` release carrying the
documented Ubuntu x86-64 Vulkan artifact was development build `b11067`.
This is the continuous pre-release channel; the stable `v0.4.1` release
publishes only a `nightly-tag.txt` marker, not that binary artifact. Its exact
30,396,256-byte archive and the GitHub release metadata both
resolved to SHA-256
`9e2a3755dd3aaa1b061d2e6395041be06a3f29dabde7de56fa0dfb3a910798a3`.
...
(
set -euo pipefail
llama_cpp_build=b11067
llama_cpp_asset=llama-b11067-bin-ubuntu-vulkan-x64.tar.gz
llama_cpp_expected=9e2a3755dd3aaa1b061d2e6395041be06a3f29dabde7de56fa0dfb3a910798a3
...
test ! -e "$llama_cpp_root"
test ! -L "$llama_cpp_root"
...
The downloaded b11067 binary was also executed offline in a network-unshared
sandbox for `--version`, `--help` and `--list-devices`; the binary identified
development build 11067 at commit `932a68e06` (`0.4.1-dev`). The documented
...
only an exact reviewed VSIX and record its version/hash. At the 2026-09-21
review, Open VSX's latest package was `0.0.67`; its 851,029-byte package,
registry-published SHA-256 and the independently retained review pin resolved
to `2bc07c4b27ade7b14592b211c458761b89d1d1d562a4898a2d37ae6a64088339`.
The manifest, network defaults and source changes since the previous 0.0.64
review were inspected again. The RAG implementation remains unchanged;
optional lifecycle hooks and editor-prompt submission settings are new and
disabled by default. No model benchmark was repeated for this client update.

```bash
(
set -euo pipefail
llama_vscode_version=0.0.67
llama_vscode_expected=2bc07c4b27ade7b14592b211c458761b89d1d1d562a4898a2d37ae6a64088339
...
The exact `0.0.67` source/package audited for this guide requires VS Code API
...
- predefined environments can download models, start `llama serve` commands or
  select remote OpenRouter endpoints or OrcaRouter endpoints; and
...
0.0.67 RAG indexer off. It also points the script loader at Fedora's
...
Leaving both LM token-limit overrides at zero makes audited 0.0.67 query the
...
in this chat/tool profile. The tested reference model below is an instruct
model; FIM control tokens alone are not evidence of FIM quality. When a
...
rules, conversation and an output reserve share the same slot. Audited 0.0.67
...
Do not enable RAG in audited llama-vscode 0.0.67. Its full-workspace indexing
...
classifies as non-modifying. In audited 0.0.67, commands classified as
...
Audited 0.0.67 retains a best-effort workspace check to `edit_file` and
`multi_edit_file`, but it is defense in depth rather than a filesystem
boundary: its absolute-path branch does not cover relative traversal paths,
and its auto-memory exception compares a string prefix instead of path
components. The separate delete tool normalizes the target and checks it with
`path.relative` against the workspace root. For an actual containment boundary,
use the OS-level namespace and bind-mount profile documented below; do not rely
on an editor-extension path check.

Version 0.0.67 retains an external `vscode://ggml-org.llama-vscode/...`
...
The new `hooks_folder` setting loads JSON-defined lifecycle scripts into the
extension's DSL interpreter when configured. Keep it empty unless those
scripts have been reviewed; repository hooks are executable code.
...
modifying is a limitation of audited version 0.0.67, not a NoID Privacy policy.
...
test ! -L "$agent_config/api-key"
if [ ! -e "$agent_config/api-key" ]; then
  umask 077
  openssl rand -hex 32 > "$agent_config/api-key"
fi
test -f "$agent_config/api-key"
test -s "$agent_config/api-key"
...
VRAM/RAM checks; the reference model below passed 32768. Add the exact
...
Ornith 1.5 is now available from the publisher. At the 2026-09-02 review, the
...
|---|---|---:|---|
| `Ornith-1.5-9B-Q4_K_M.gguf` | ornith-ai revision `abdd624b12ebf020b767fff532ff44fe552b28c3`, MIT | 5,780,090,816 bytes | `70c112196e0b7023803c9762752e46d29e612a92c83f995bc3ba1ceb07e8fab6` |
| `Ornith-1.5-35B-Q4_K_M.gguf` | ornith-ai revision `12393612fd4f730ff5aadc23e9b8f9648aa49ceb`, MIT | 21,713,463,040 bytes | `42739874cc2ccfdb8523b23fbe52e29b2a7555c8176737ca9ca0b5d59859d41f` |
...
automatic download policy. The reference host was a Fedora 44 laptop with
32 GiB RAM, a 4-GiB NVIDIA GPU and the then-reviewed llama.cpp `b10173` Vulkan
archive.

The model file was resolved to an immutable publisher revision. Its local size
and full SHA-256 matched the remote linked object:

| Model file | Publisher revision and license | Verified size | SHA-256 |
|---|---|---:|---|
| `ornith-1.0-35b-Q4_K_M.gguf` | ornith-ai revision `383064f72a1ef3087b779f268d3ca117eb989aac`, MIT | 21,166,757,760 bytes | `ff25291b2599fb927a835e624d2b3540106af61761c3fa57ac4264046dbec002` |

The embedded GGUF metadata also matched the claimed architecture: 34.66B
total/approximately 3B active, 40 MoE layers, 256 experts and top-8 routing,
262144 native context and an embedded tool-capable chat template. Publisher
...
At idle after initialization, the Ornith profile used 2485 MiB process VRAM and
left 1255 MiB GPU memory free. It successfully served one 32K slot and returned
the exact requested tool call after parsing all seven built-in schemas. Across
the disposable-agent tests, Ornith selected the exact requested `read_file`,
`write_file` and `exec_shell_command` functions, preserved their arguments, and
completed read/write/shell cycles in a disposable Git repository.
Unauthenticated `/tools` returned HTTP 401, authenticated tool calls worked, and
the WebUI returned HTTP 200 to a gzip-capable browser client.
...
with this exact Q4 file on this 4-GiB GPU, and the guide does not promise it.
...
- CVE-2026-5757 / CERT VU#518910:
  <https://www.kb.cert.org/vuls/id/518910> and
  <https://www.cve.org/CVERecord?id=CVE-2026-5757>
- ZDI-26-403 / CVE-2026-15685:
  <https://www.zerodayinitiative.com/advisories/ZDI-26-403/> and
...
- Ollama tensor-redirect SSRF report:
  ...
  <https://github.com/ollama/ollama/blob/v0.34.2/x/transfer/redirect.go>
