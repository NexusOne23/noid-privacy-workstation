...
# Status: LOCKED 2026-09-17 (v44) — advance the reviewed llama.cpp pin to b11010 and the Ollama evaluation record to v0.34.1.
#
...
At the 2026-09-17 review, Fedora 44 still offered `ollama-0.12.11-4.fc44`.
...
2026-09-17 review, the current upstream release was `v0.34.1`; its published
Linux x86-64 artifact was 1,429,323,296 bytes, and the vendor checksum plus
...
ollama_expected=f361dc3992ec07e4ad429f4bb2d10d4663ba2c295f9a9a688c7d52f4ba650034
...
For v0.34.1 the vendor checksum matched the retained pin, the archive's 65
...
probe reported 0.34.1, and `ollama serve --help` retained the
...
2025-05-27, release `v0.8.0` contains that commit, and the reviewed `v0.34.1`
...
issue 17042 links the repair to merged commit `67b6a1c2d453`; `v0.34.1`
...
blob redirects. The reviewed `v0.34.1` source still returns a cross-host
...
At the 2026-09-17 review, the newest upstream `b*` release carrying the
documented Ubuntu x86-64 Vulkan artifact was development build `b11010`.
...
30,218,634-byte archive and the GitHub release metadata both
resolved to SHA-256
`80b6cf394149e888e6d8f363cb0777d12a423a0da6cf0247e678fd4cc3ab13f4`.
...
(
set -euo pipefail
llama_cpp_build=b11010
llama_cpp_asset=llama-b11010-bin-ubuntu-vulkan-x64.tar.gz
llama_cpp_expected=80b6cf394149e888e6d8f363cb0777d12a423a0da6cf0247e678fd4cc3ab13f4
...
The downloaded b11010 binary was also executed offline in a network-unshared
sandbox for `--version`, `--help` and `--list-devices`; the binary identified
development build 11010 at commit `4bc272fd7` (`0.4.1-dev`). The documented
...
only an exact reviewed VSIX and record its version/hash. At the 2026-09-03
review, Open VSX's latest package was `0.0.64`; its 836,085-byte package,
registry-published SHA-256 and the independently retained review pin resolved
to `0b14816ab43665e1fbc23be0f2aa61ae1171aab970405e65e702f64020592fec`.
Upstream has since released `0.0.65` (2026-09-05) and `0.0.66` (2026-09-16),
both changing agent-view placement and chat behaviour; Open VSX carried `0.0.65`
at the 2026-09-17 check. This guide deliberately stays on the audited `0.0.64`
package because every behavioural statement in this section — the agent command
classification, the RAG indexer default and the token-limit handling — was
verified against exactly those bytes. Repeat that audit before moving the pin:

```bash
(
set -euo pipefail
llama_vscode_version=0.0.64
llama_vscode_expected=0b14816ab43665e1fbc23be0f2aa61ae1171aab970405e65e702f64020592fec
...
The exact `0.0.64` source/package audited for this guide requires VS Code API
...
0.0.64 RAG indexer off. It also points the script loader at Fedora's
...
Leaving both LM token-limit overrides at zero makes audited 0.0.64 query the
...
rules, conversation and an output reserve share the same slot. Audited 0.0.64
...
Do not enable RAG in audited llama-vscode 0.0.64. Its full-workspace indexing
...
classifies as non-modifying. In audited 0.0.64, commands classified as
...
Audited 0.0.64 adds a best-effort workspace check to `edit_file` and
...
Version 0.0.64 also registers an external `vscode://ggml-org.llama-vscode/...`
...
modifying is a limitation of audited version 0.0.64, not a NoID Privacy policy.
