...
# Status: LOCKED 2026-09-21 (v140) — recover extensions across browser version jumps and apply authenticated Thunderbird compatibility natively.
#
...
#   - Reboot ACTIVATION detection is MARKER-LESS (kernel-version-diff
#     `uname -r` vs /lib/modules, world-readable + WAL-immune). Reboot SAFETY
#     is a separate boot-scoped closed record, /run/noid-privacy/reboot-blocked,
#     owned by /usr/libexec/noid-reboot-block-state (root via sudo, no durable
#     /var state, no sudoers drop-in). tests/25 enforces the absence of the
#     retired pending-reboot.txt writer and sudoers heredoc as whole-file
#     negatives.
...
# The published update window binds this exact process to its canonical
# argv: /usr/libexec/noid-update-window-active compares /proc/<pid>/cmdline
# against the absolute installed path. A relative or copied invocation would
# therefore fail only after the sudo prompt, lock and marker publication, with
# a message that does not name the cause. Reject it before any side effect.
if [ "$0" != /usr/local/bin/noid-update-all.sh ]; then
    echo -e "${RED}ERROR${NC}: the updater must be invoked by its installed absolute path"
    echo -e "${YELLOW}→${NC} run it as:"
    echo "    /usr/local/bin/noid-update-all.sh"
    exit 2
fi
...
load_reboot_readiness() {
    local -a args=() record=()
    local reader_status=0 helper_pid
    ...
    mapfile -t record < <(/usr/libexec/noid-reboot-readiness "${args[@]}") \
        || reader_status=$?
    helper_pid=$!
    # mapfile reports its own read status, not the producer's result. Wait for
    # that exact child even if it closed stdout before failing after a record.
    wait "$helper_pid" || return 1
    ...
    # Validate the safety/blocker tuple on locals first. The caller seeds the
    # three globals with fail-closed defaults; publishing an inconsistent
    # record before this check would let the summary and GUI marker present
    # an unvalidated tuple as the verdict.
    local activation safety blocker_summary
    activation=${record[1]#activation=}
    safety=${record[2]#safety=}
    blocker_summary=${record[3]#blockers=}
    case "$safety:$blocker_summary" in
        safe:none) ;;
    ...
    REBOOT_ACTIVATION=$activation
    REBOOT_SAFETY=$safety
    REBOOT_BLOCKER_SUMMARY=$blocker_summary
}
...
THUNDERBIRD_COMPATIBILITY=/usr/local/lib/noid-privacy/noid-thunderbird-compatibility
...
fetch_latest_xpi() {
    ...
    LATEST_XPI_COMPATIBILITY="$LATEST_XPI_WORK/compatibility.json"
    if ! install -m 0600 -- "$MARKETPLACE_COMPATIBILITY" "$LATEST_XPI_COMPATIBILITY"; then
        cleanup_marketplace_xpi
        cleanup_latest_xpi
        return 1
    fi
...
browser_extension_inventory() {
    local product="$1" output="$2" include_managed="${3:-0}"
    python3 - "$product" "$output" "${XDG_CONFIG_HOME:-$HOME/.config}" \
        "$HOME" "$include_managed" <<'BROWSER_EXTENSION_INVENTORY_PY'
import configparser
import json
import os
...
product, output, xdg_config, home = sys.argv[1:5]
include_managed = sys.argv[5] if len(sys.argv) == 6 else "0"
...
if product == "firefox":
    ...
    raise SystemExit(2)
if include_managed == "1":
    excluded = set()
elif include_managed != "0":
    raise SystemExit(2)
...
try:
    ...
    raise SystemExit("profile root cannot be inspected")
try:
...
try:
    with (root / "profiles.ini").open(encoding="utf-8") as registry:
        parser.read_file(registry)
except (OSError, UnicodeError, configparser.Error):
    raise SystemExit("profile registry cannot be read or parsed")
records = []
seen_targets = set()
...
# curl's own --retry covers a timeout and the HTTP 408/429/5xx statuses only
# (curl(1): "Transient error means either: a timeout, an FTP 4xx response code
# or an HTTP 408, 429, 500, 502, 503, 504, 522 or 524 response code"). A CDN
# reset while receiving the response instead ends in a curl exit code outside
# that set, even when response headers and an HTTP status already arrived. The
# observed `curl: (56) OpenSSL SSL_read: ... unexpected eof` therefore ended
# the marketplace refresh on the first reset without retrying at all.
# Retry exactly that transport-abort class here and leave every other failure a
# single attempt: DNS, TLS certificate rejection, an HTTP 404 under -f and the
# filesize cap must stay one-shot hard errors, not repeated requests.
# --retry stays on the curl side so 408/429/5xx keep their Retry-After-aware
# handling; the outer loop adds at most two further complete attempts.
MARKETPLACE_TRANSPORT_RETRIES=2
MARKETPLACE_TRANSPORT_RETRY_DELAY=1
# Both call sites write the payload with -o and use stdout only for curl's
# short -w status, so buffering one attempt is bounded. Emitting only the final
# attempt is load-bearing: curl writes its --write-out string even when the
# transfer aborts, so streaming every attempt would turn a recovered retry into
# a concatenation such as `200200` (or `000200` before any response headers)
# and misreport a successful fetch as an HTTP error.
curl_with_transport_retry() {
    local attempt=0 rc out
    while :; do
        rc=0
        out=$(curl "$@") || rc=$?
        # 18 partial file · 52 empty reply · 56 recv failure · 92 HTTP/2 stream
        case "$rc" in
            18|52|56|92)
                if [ "$attempt" -lt "$MARKETPLACE_TRANSPORT_RETRIES" ]; then
                    attempt=$((attempt + 1))
                    sleep "$MARKETPLACE_TRANSPORT_RETRY_DELAY"
                    continue
                fi
                ;;
        esac
...
' "$identity" "$product_version") || return 1
    ...
    curl_rc=0
    http_status=$(curl_with_transport_retry -sS --proto '=https' --tlsv1.2 \
        --max-time 30 --retry 2 --retry-delay 1 --retry-max-time 20 \
    ...
    MARKETPLACE_ERROR_CLASS=validation
    parser_rc=0
    MARKETPLACE_COMPATIBILITY="$MARKETPLACE_WORK/compatibility.json"
    record=$(python3 - "$metadata" "$marketplace" "$identity" "$MARKETPLACE_COMPATIBILITY" <<'MARKETPLACE_XPI_RELEASE_PY'
import json
import re
import sys
from urllib.parse import urlsplit

path, marketplace, expected_id = sys.argv[1:4]
compatibility_path = sys.argv[4] if len(sys.argv) == 5 else "/dev/null"
...
minimum, maximum = compatibility.get("min"), compatibility.get("max")
if not isinstance(minimum, str) or not re.fullmatch(r"[0-9]+(?:[.][0-9]+)*", minimum) \
        or not isinstance(maximum, str) or not re.fullmatch(
            r"\*|[0-9]+(?:[.][0-9]+)*(?:[.]\*)?", maximum):
    raise SystemExit("marketplace compatibility bounds are invalid")
with open(compatibility_path, "w", encoding="utf-8") as destination:
    json.dump({"addons": {expected_id: {"updates": [{
        "version": version, "update_hash": digest, "applications": {"gecko": {
            "strict_min_version": minimum, "strict_max_version": maximum}}
    }]}}}, destination)
...
MARKETPLACE_XPI_RELEASE_PY
    ...
    MARKETPLACE_ERROR_CLASS=availability
    curl_with_transport_retry -fsSL --proto '=https' --proto-redir '=https' \
        --tlsv1.2 --retry 2 --retry-delay 1 --retry-max-time 20 \
    ...
    if [ "$marketplace" = amo ]; then
        "$WEBEXT_VALIDATOR" "$MARKETPLACE_PATH" "$identity" \
            "$MARKETPLACE_VERSION" 1 "$product_version" 1 "$MARKETPLACE_COMPATIBILITY" >/dev/null || return 1
        ...
        "$WEBEXT_VALIDATOR" "$MARKETPLACE_PATH" "$identity" \
            "$MARKETPLACE_VERSION" 0 "$product_version" 1 "$MARKETPLACE_COMPATIBILITY" >/dev/null || return 1
        MARKETPLACE_ERROR="Thunderbird native compatibility verification failed"
        "$THUNDERBIRD_COMPATIBILITY" "$MARKETPLACE_PATH" "$identity" \
            "$MARKETPLACE_VERSION" "$product_version" "$MARKETPLACE_COMPATIBILITY" \
            >/dev/null || return 1
    fi
    MARKETPLACE_ERROR=
...
update_marketplace_extensions() {
    ...
    local checked=0 failed=0 check_result errors_before=$ERRORS
    ...
    while IFS=$'\t' read -r identity installed_version target profile_name; do
        ...
        actual_version=$("$WEBEXT_VALIDATOR" "$target" "$identity" - \
            "$validator_signature" - 1 2>/dev/null) || {
        ...
        if numeric_version_is_newer "$MARKETPLACE_VERSION" "$installed_version" \
                || { ! numeric_version_is_newer "$installed_version" "$MARKETPLACE_VERSION" \
                    && ! payload_matches "$target" "$MARKETPLACE_SHA256"; }; then
            if atomic_install_profile_xpi "$MARKETPLACE_PATH" "$target" \
                    ...
                    && [ "$("$WEBEXT_VALIDATOR" "$target" "$identity" \
                        ...
                        "$product_version" 1 "$MARKETPLACE_COMPATIBILITY" 2>/dev/null || true)" = "$MARKETPLACE_VERSION" ]; then
            ...
            if ! "$WEBEXT_VALIDATOR" "$target" "$identity" "$installed_version" \
                    "$validator_signature" "$product_version" 1 "$MARKETPLACE_COMPATIBILITY" >/dev/null; then
                echo -e "${RED}ERROR${NC}: retained ${product_label} extension ${identity} is incompatible; no implicit downgrade"
                ERRORS=$((ERRORS + 1))
                continue
            fi
            if [ "$product" = firefox ] \
        ...
        if [ "$product" = thunderbird ] && payload_matches "$target" "$MARKETPLACE_SHA256"; then
            if ! "$THUNDERBIRD_COMPATIBILITY" "$target" "$identity" "$MARKETPLACE_VERSION" \
                    "$product_version" "$MARKETPLACE_COMPATIBILITY" "${target%/extensions/*}" >/dev/null; then
                echo -e "${RED}ERROR${NC}: native Thunderbird compatibility reconciliation failed for ${identity}"
                ERRORS=$((ERRORS + 1))
            fi
        fi
    done < "$inventory"
    ...
    if [ "$failed" -gt 0 ] || [ "$ERRORS" -ne "$errors_before" ]; then
        check_result=failed
...
case "$destination" in
    ...
    /var/lib/noid-privacy/managed-extensions/dkim-compatibility.json)
...
case "${source_file##*/}:${destination##*/}" in
    payload.xpi:dkim_verifier@pl.xpi|compatibility.json:dkim-compatibility.json) ;;
    *) exit 2 ;;
esac
[ "${SUDO_UID:-0}" -gt 0 ] 2>/dev/null \
    && case "$source_file" in /var/tmp/noid-xpi-update.*/payload.xpi|/var/tmp/noid-xpi-update.*/compatibility.json) true ;; *) false ;; esac \
...
# cannot be parsed. Failed queries are not absence evidence. The caller
# supplies only fixed Firefox/Thunderbird names.
browser_process_active() {
    local process_name browser_pid status_file browser_state process_uid candidates query_rc
    if ! process_uid=$(id -u) || [[ ! "$process_uid" =~ ^[0-9]+$ ]]; then
        printf '%s\n' 'Browser process owner could not be determined; refusing profile changes.' >&2
        return 0
    fi
    for process_name in "$@"; do
        if candidates=$(pgrep -u "$process_uid" -x "$process_name" 2>/dev/null); then
            if [ -z "$candidates" ]; then
                printf '%s\n' 'Browser process query returned no usable evidence; refusing profile changes.' >&2
                return 0
            fi
        else
            query_rc=$?
            ...
            printf '%s\n' 'Browser process query failed; refusing profile changes.' >&2
            return 0
        fi
        while IFS= read -r browser_pid; do
            if [[ ! "$browser_pid" =~ ^[1-9][0-9]*$ ]]; then
                printf '%s\n' 'Browser process query returned invalid evidence; refusing profile changes.' >&2
                return 0
            fi
...
if browser_process_active firefox firefox-bin; then
    echo -e "${YELLOW}WARN${NC}: Firefox is running or its process state is unavailable — profile hardening deferred; resolve the process state and re-run to update executable extension bytes"
    WARNINGS=$((WARNINGS + 1))
            ...
            if [ "$ubo_change" = advance ] || [ "$ubo_change" = repair ]; then
                if noid_atomic_install_file "$LATEST_XPI_PATH" "$ubo_target" 644 \
                        ...
                        && [ "$("$WEBEXT_VALIDATOR" "$ubo_target" \
                            ...
                            "$LATEST_XPI_PRODUCT_VERSION" 0 "$LATEST_XPI_COMPATIBILITY" 2>/dev/null)" = "$LATEST_XPI_VERSION" ] \
                ...
                if ! "$WEBEXT_VALIDATOR" "$ubo_target" uBlock0@raymondhill.net \
                        "$current_ubo_version" 1 "$LATEST_XPI_PRODUCT_VERSION" \
                        0 "$LATEST_XPI_COMPATIBILITY" >/dev/null; then
                    echo -e "${RED}ERROR${NC}: retained uBO payload is incompatible; no implicit downgrade"
                    ERRORS=$((ERRORS + 1))
                    continue
                fi
...
# Resolve the replacement independently of cache compatibility. An old seed
# remains integrity evidence, but never gates checking the authenticated channel.
refresh_managed_dkim() {
    local seed_compat=/usr/share/noid-thunderbird/dkim-compatibility.json
    local current_compat=/var/lib/noid-privacy/managed-extensions/dkim-compatibility.json
    local source_compat floor=$TB_DKIM_SEED_VERSION candidate_version metadata_digest
    local dkim_check_result=failed source
    DKIM_DST=/usr/lib64/thunderbird/distribution/extensions/dkim_verifier@pl.xpi
    TB_DKIM_COMPATIBILITY=$seed_compat
    TB_VERSION=$(sudo rpm -q --qf '%{VERSION}\n' thunderbird 2>/dev/null) || return 1
    trusted_root_file "$seed_compat" 644 || return 1
    # Authenticate seed identity separately from whether it still covers the
    # newly installed product. Never label ordinary version ageing corruption.
    "$WEBEXT_VALIDATOR" "$TB_DKIM_SEED" dkim_verifier@pl \
        "$TB_DKIM_SEED_VERSION" 0 - >/dev/null || return 1
    for source in "$TB_DKIM_SEED" "$TB_DKIM_CURRENT" "$DKIM_DST"; do
        trusted_root_file "$source" 644 || continue
        candidate_version=$("$WEBEXT_VALIDATOR" "$source" dkim_verifier@pl - 0 - 2>/dev/null) || continue
        if numeric_version_is_newer "$candidate_version" "$floor"; then floor=$candidate_version; fi
        source_compat=$seed_compat
        if [ "$source" != "$TB_DKIM_SEED" ] && trusted_root_file "$current_compat" 644; then
            source_compat=$current_compat
        fi
        "$WEBEXT_VALIDATOR" "$source" dkim_verifier@pl "$candidate_version" \
            0 "$TB_VERSION" 0 "$source_compat" >/dev/null 2>&1 || continue
        if [ -z "$TB_DKIM_SOURCE" ] || ! numeric_version_is_newer "$TB_DKIM_SOURCE_VERSION" "$candidate_version"; then
            TB_DKIM_SOURCE=$source
            TB_DKIM_SOURCE_VERSION=$candidate_version
            TB_DKIM_SOURCE_SHA256=$(sha256sum "$source" | awk '{print $1}')
            TB_DKIM_COMPATIBILITY=$source_compat
        fi
    done
    if fetch_latest_xpi dkim; then
        if numeric_version_is_newer "$floor" "$LATEST_XPI_VERSION"; then
            if [ -z "$TB_DKIM_SOURCE" ] || numeric_version_is_newer "$floor" "$TB_DKIM_SOURCE_VERSION"; then
                echo -e "${RED}ERROR${NC}: newer local DKIM Verifier is incompatible; refusing an implicit downgrade"
                cleanup_latest_xpi
                record_extension_check thunderbird-dkim failed || true
                return 1
            fi
            echo -e "${GREEN}OK${NC}: compatible local DKIM Verifier ${TB_DKIM_SOURCE_VERSION} is newer than ATN (no downgrade)"
            dkim_check_result=current
        else
            metadata_digest=$(sha256sum "$LATEST_XPI_COMPATIBILITY" | awk '{print $1}')
            if ! payload_matches "$TB_DKIM_CURRENT" "$LATEST_XPI_SHA256" \
                    || ! payload_matches "$DKIM_DST" "$LATEST_XPI_SHA256" \
                    || ! payload_matches "$current_compat" "$metadata_digest"; then
                if ! publish_managed_dkim_xpi "$LATEST_XPI_COMPATIBILITY" "$current_compat" \
                            "$metadata_digest" "$LATEST_XPI_VERSION" \
                        || ! publish_managed_dkim_xpi "$LATEST_XPI_PATH" "$TB_DKIM_CURRENT" \
                            "$LATEST_XPI_SHA256" "$LATEST_XPI_VERSION" \
                        || ! publish_managed_dkim_xpi "$LATEST_XPI_PATH" "$DKIM_DST" \
                            "$LATEST_XPI_SHA256" "$LATEST_XPI_VERSION"; then
                    cleanup_latest_xpi
                    record_extension_check thunderbird-dkim failed || true
                    return 1
                fi
                record_extension_update thunderbird-dkim "$floor" "$LATEST_XPI_VERSION" \
                    "$LATEST_XPI_SHA256" || { cleanup_latest_xpi; return 1; }
            ...
            TB_DKIM_SOURCE_SHA256=$LATEST_XPI_SHA256
            TB_DKIM_COMPATIBILITY=$current_compat
            echo -e "${GREEN}OK${NC}: DKIM Verifier ${LATEST_XPI_VERSION} and exact ATN compatibility authenticated"
        fi
    else
        if [ "${LATEST_XPI_ERROR_CLASS:-validation}" = availability ] \
                && [ -n "$TB_DKIM_SOURCE" ] \
                && ! numeric_version_is_newer "$floor" "$TB_DKIM_SOURCE_VERSION"; then
            echo -e "${YELLOW}WARN${NC}: DKIM marketplace check unavailable; compatible local payload retained (${LATEST_XPI_ERROR:-retry later})"
            WARNINGS=$((WARNINGS + 1))
            DEFERRED_LIST+=("DKIM-Verifier-check")
        else
            echo -e "${RED}ERROR${NC}: no authenticated DKIM replacement available (${LATEST_XPI_ERROR:-validation failed})"
            cleanup_latest_xpi
            record_extension_check thunderbird-dkim failed || true
            return 1
        fi
...
# Keep the real profile's compatibility state under Thunderbird's native
# ownership. No extension database is edited by shell/Python and the worker
# cannot contact the network or the user's desktop session.
reconcile_dkim_profile_compatibility() {
    local inventory identity old_version target profile_name profile
    inventory=$(mktemp /var/tmp/noid-dkim-profiles.XXXXXX) || return 1
    if ! browser_extension_inventory thunderbird "$inventory" 1; then
        rm -f -- "$inventory"
        return 1
    fi
    while IFS=$'\t' read -r identity old_version target profile_name; do
        [ "$identity" = dkim_verifier@pl ] || continue
        old_version=$("$WEBEXT_VALIDATOR" "$target" "$identity" - 0 -) || {
            rm -f -- "$inventory"; return 1;
        }
        if numeric_version_is_newer "$old_version" "$TB_DKIM_SOURCE_VERSION"; then
            "$WEBEXT_VALIDATOR" "$target" "$identity" "$old_version" 0 "$TB_VERSION" \
                >/dev/null || { rm -f -- "$inventory"; return 1; }
            continue
        fi
        if ! payload_matches "$target" "$TB_DKIM_SOURCE_SHA256"; then
            atomic_install_profile_xpi "$TB_DKIM_SOURCE" "$target" "$TB_DKIM_SOURCE_SHA256" \
                || { rm -f -- "$inventory"; return 1; }
        fi
        profile=${target%/extensions/*}
        "$THUNDERBIRD_COMPATIBILITY" "$target" "$identity" "$TB_DKIM_SOURCE_VERSION" \
            "$TB_VERSION" "$TB_DKIM_COMPATIBILITY" "$profile" >/dev/null \
            || { rm -f -- "$inventory"; return 1; }
    done < "$inventory"
    rm -f -- "$inventory"
}
...
if ! sudo rpm -q thunderbird >/dev/null 2>&1; then
    ...
    echo -e "${YELLOW}WARN${NC}: Thunderbird is running or its process state is unavailable — profile hardening deferred; resolve the process state and re-run to replace executable extension bytes"
    WARNINGS=$((WARNINGS + 1))
    ...
    if ! refresh_managed_dkim; then
        echo -e "${RED}ERROR${NC}: DKIM payload/compatibility reconciliation failed"
        ERRORS=$((ERRORS + 1))
    fi
    ...
    if [ -z "$TB_DKIM_SOURCE" ] || \
       ...
       [ "$("$WEBEXT_VALIDATOR" "$DKIM_DST" dkim_verifier@pl \
           "$TB_DKIM_SOURCE_VERSION" 0 "$TB_VERSION" 0 "$TB_DKIM_COMPATIBILITY" 2>/dev/null || true)" \
...
'; then
    ...
    if [ -n "$TB_DKIM_SOURCE" ] && ! reconcile_dkim_profile_compatibility; then
        echo -e "${RED}ERROR${NC}: Thunderbird native DKIM profile compatibility reconciliation failed"
        ERRORS=$((ERRORS + 1))
    fi
...
JP_SEED_VERSION="37"
JP_SEED_SHA256="c54ccd627540d86e6baf06c699f624ba7d5229aedfc8b00b0dc7182c6538eda8"
    ...
    if [ ! -d "$nvidia_state_dir" ] || [ -L "$nvidia_state_dir" ] \
        ...
        if [ -e "$nvidia_queue" ] || [ -L "$nvidia_queue" ]; then
            if [ ! -d "$nvidia_queue" ] || [ -L "$nvidia_queue" ] \
                ...
                add_blocker nvidia-state
            elif ! queue_pending=$(find "$nvidia_queue" -mindepth 1 -maxdepth 1 -type f \
                    \( -name '*.pending' -o -name '*.deferred' -o -name '*.failed' \) \
                    -printf pending -quit 2>/dev/null); then
                add_blocker nvidia-state
            elif [ -n "$queue_pending" ]; then
                add_blocker nvidia
            fi
        fi
    fi
...
# A failed scan may still emit version names. Never treat a partial listing as
# the newest installed kernel; the empty value below becomes boot-inventory.
if ! latest_kernel=$(find /lib/modules -mindepth 1 -maxdepth 1 -type d \
        -printf '%f\n' 2>/dev/null | sort -V | tail -1); then
    latest_kernel=
fi
kernel_pending=0
...
if [ "$readiness_status" -eq 0 ] \
        && [ "${#readiness[@]}" -eq 8 ] && [ "${readiness[0]}" = schema=1 ] \
...
notify-send \
    ...
    --icon=/usr/share/pixmaps/noid-privacy-logo.png \
    --app-name="NoID Privacy" \
    "Verified system changes — restart required" \
    "Boot safety is verified. Restart to activate the prepared kernel or boot policy.\n\nKernel: running=${running_kernel}, prepared=${latest_kernel}\nNVIDIA: running=${nv_run}, prepared=${nv_disk}\n\nCommand: sudo reboot" \
    2>/dev/null || true
...
class UpdateWindow(Adw.ApplicationWindow):
    ...
    def _build_run_page(self):
        ...
        # Keep the disclosure control separate from the revealed payload. A
        # GtkExpander stays a distinct child in body's 16px-spaced layout, so
        # its collapsed header leaves a visible gap before the step list. This
        # zero-spacing section makes every terminal-only margin part of the
        # revealer. The closed toggle owns the same 16px separation from the
        # step list that the open terminal panel owns at its bottom edge.
        log_section = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
        self.log_toggle = Gtk.ToggleButton()
        self.log_toggle.set_halign(Gtk.Align.START)
        self.log_toggle.set_margin_bottom(16)
        self.log_toggle.add_css_class('flat')
        log_toggle_content = Gtk.Box(
            orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        self.log_toggle_icon = Gtk.Image.new_from_icon_name('pan-end-symbolic')
        self.log_toggle_icon.set_accessible_role(
            Gtk.AccessibleRole.PRESENTATION)
        log_toggle_content.append(self.log_toggle_icon)
        log_toggle_content.append(Gtk.Label(label='Show log'))
        self.log_toggle.set_child(log_toggle_content)
        noid_ui.accessible(
            self.log_toggle, 'Show log', 'Show or hide the live update terminal')

        self.log_revealer = Gtk.Revealer()
        self.log_revealer.set_transition_type(
            Gtk.RevealerTransitionType.SLIDE_DOWN)
        self.log_revealer.set_transition_duration(180)
        self.log_revealer.set_reveal_child(False)
        self.log_toggle.connect('toggled', self._on_log_toggled)
        self.log_revealer.connect(
            'notify::child-revealed', self._on_log_reveal_complete)
        log_section.append(self.log_toggle)
        log_section.append(self.log_revealer)
        ...
        self.log_panel = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self.log_panel.set_margin_top(6)
        self.log_panel.set_margin_bottom(16)
        self.log_panel.append(term_scroll)
        self.log_panel.set_visible(False)
        self.log_revealer.set_child(self.log_panel)
        ...
        log_section.append(self.steps_group)
    ...
    def _on_log_toggled(self, toggle):
        ...
        if expanded:
            # The child must be visible before the reveal animation starts.
            self.log_toggle.set_margin_bottom(0)
            self.log_panel.set_visible(True)
        self.log_revealer.set_reveal_child(expanded)
        self.log_toggle_icon.set_from_icon_name(
            'pan-down-symbolic' if expanded else 'pan-end-symbolic')

    def _on_log_reveal_complete(self, revealer, _pspec):
        if not revealer.get_reveal_child() \
                and not revealer.get_child_revealed():
            # GtkRevealer keeps its child in the accessibility tree. Hide the
            # completed collapsed payload as well as measuring it at zero.
            self.log_panel.set_visible(False)
            self.log_toggle.set_margin_bottom(16)

    def _set_log_revealed(self, revealed):
        # set_active emits toggled when the state changes, keeping the
        # disclosure icon and revealer on one canonical state transition.
        self.log_toggle.set_active(bool(revealed))
    ...
    def _on_start(self, _btn):
        ...
        self._set_log_revealed(False)  # PW comes via graphical dialog
    ...
    def _reboot_readiness():
        ...
        if len(rows) != 8 or rows[0] != 'schema=1':
            return ('none', 'blocked', 'state-unsafe')
        values = {}
        fields = ('activation', 'safety', 'blockers', 'running_kernel',
                  'latest_kernel', 'nvidia_running', 'nvidia_installed')
        for expected_key, row in zip(fields, rows[1:]):
            if '=' not in row:
                return ('none', 'blocked', 'state-unsafe')
            ...
            if key != expected_key:
                return ('none', 'blocked', 'state-unsafe')
            values[key] = value
        if any(re.fullmatch(r'[A-Za-z0-9._+-]+', values[key]) is None
               for key in fields[3:]):
            return ('none', 'blocked', 'state-unsafe')
        ...
        if (activation not in ('required', 'recommended', 'none')
                ...
                or (safety == 'blocked' and 'none' in blocker_set)
