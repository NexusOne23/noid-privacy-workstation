...
# Status: LOCKED 2026-09-06 (v65) — correct routing diagnosis and guarded examples.
#
...
- [07-physical-ipv6-boundary.md](07-physical-ipv6-boundary.md) — physical IPv6 is not a release-qualified v1.8 mode
...
- Firewall: `journalctl -u firewalld` (service events only; per-packet drop
  logging stays off by default — see 03-firewall-zones.md).
...
# 3. NVIDIA proprietary driver install (Stage 1 of 2: install + MOK import;
#    Stage 2 is the UEFI MokManager enrollment after reboot)
...
1. Takes a **pre-update snapshot** (so you can roll back if an update
   breaks the system)
2. Runs `dnf upgrade` with a consistent umask and logging (including the
   NVIDIA akmod + dracut rebuild on a kernel change)
3. Runs `flatpak update`
4. Offers firmware updates via `fwupdmgr` (opt-in per run)
5. Re-applies the Firefox and Thunderbird hardening and refreshes the pinned
   uBlock Origin / DKIM Verifier add-ons from their official marketplaces
6. Updates only the opted-in Claude/Codex CLIs, editor extensions and the
   managed GNOME extension over their vendor channels
7. Verifies the repository signature settings
8. Checks against an existing user-owned AIDE baseline and preserves drift
9. Reports reboot readiness, then creates a **post-update snapshot** for
   comparison
...
- **Printing**: install only the drivers you need (`sudo dnf install hplip`
  or `gutenprint-cups`), then activate the masked print stack with
  `sudo noid-toggle-printing on` (see `noid-help 26-optional-packages`)
...
Check the effective route to the test destination. A default route in the main
table alone cannot distinguish full-tunnel from split-tunnel operation:
WireGuard and provider clients can select a separate table through routing rules.

```bash
# Read all routing rules and tables for both address families.
ip -4 rule show
ip -6 rule show
ip -4 route show table all
ip -6 route show table all

# This local lookup sends no packet; inspect its selected device and table.
# Use the actual test destination too if it has a more-specific route.
ip -4 route get 1.1.1.1
```

Compare the selected device with the active tunnel. Explicit application
interface binding, marks, user-specific rules and proxy settings can select a
different path. For a full-tunnel WireGuard profile, inspect its `AllowedIPs`
and effective routes for `0.0.0.0/0` and, when used, `::/0`; a physical default
remaining in the main table does not by itself mean that public traffic bypasses
the VPN.
...
- **Version**: first-install repository pin `37`; a user-initiated M25 Update
...
- **Audit**: the exact pinned v37 source contained no network or telemetry
