...
# Status: LOCKED 2026-09-06 (v41) — reject empty/glob section selections; locate repository deploy keys correctly.
#
...
**Account SSH keys**: https://github.com/settings/keys
**Deploy keys**: each repository → Settings → Deploy keys
...
- GitHub Docs — repository deploy keys:
...
set -euo pipefail
# Section names are words, never pathname patterns from the caller's directory.
set -f
...
_section_cron() {
    ...
    if [ "$unowned_count" -gt 0 ]; then
        noun=entries verb=require
        [ "$unowned_count" -eq 1 ] && { noun=entry; verb=requires; }
        _notice "$unowned_count unowned system cron $noun $verb attribution"
    fi
    ...
    if [ "$user_count" -gt 0 ]; then
        noun=entries verb=require
        [ "$user_count" -eq 1 ] && { noun=entry; verb=requires; }
        _notice "$user_count active current-user crontab $noun $verb review"
    fi
...
_section_suid() {
    ...
    count=$(printf '%s\n' "$suid" | grep -c . || true)
    if [ "$find_rc" -ne 0 ]; then
        _notice "$count SUID/SGID file(s) (PARTIAL — find rc=$find_rc; paths were unreadable or changed; $scan_advice)"
    else
        _notice "$count SUID/SGID file(s); attribute every path to its live package"
    fi
    printf '%s\n' "$suid" | sed 's/^/      /'
}
...
[ -n "$normalized_sections" ] || {
    printf 'at least one section name is required (see --help)\n' >&2
    exit 2
}
