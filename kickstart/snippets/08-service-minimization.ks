...
# Status: LOCKED 2026-09-08 (v176) — require explicit per-run user consent for full-system updates.
#
# Covers:
...
#               units, retained compatibility masks,
...
#   - Bluetooth: NOT in MASK_LIST. Fedora's Type=dbus unit has an [Install]
#     section with WantedBy=bluetooth.target and Alias=dbus-org.bluez.service.
#     Keep those activation paths available for the GNOME BT panel.
#     Default-OFF = rfkill block + flag-gated udev
...
#     When and only when Task 2 first fails, the current resolver is proven to
#     be the intended strict Quad9 filtered path, that path cannot resolve the
#     Cisco distribution name, and Quad9's no-threat-blocking path can, Task 2
#     receives one automatic retry over strict authenticated dns10.quad9.net.
#     The runtime-only override is restored before Task 3; EXIT plus systemd
#     ExecStopPost provide recovery after errors, signals and service stops.
...
# Retained compatibility masks (historically called "ghost-unit" masks):
...
# missing, it can be registered in the not-found state. That alone does not
# make the manager degraded: a native missing-Conflicts-unit probe remains
# running, while an actually failed service changes it to degraded.
#
# Historical reference inventory (Conflicts=/Wants=/Requires= origin):
...
#
# These retained masks explicitly deny activation, including after a later
# package installation. They are not a repair for failed units or a guarantee
# that is-system-running reports running. Diagnose actual failures separately.
apparmor.service
iptables.service
ip6tables.service
ebtables.service
...
# (b) remove the legacy /usr/local direct-exec D-Bus bypass descriptor.
...
#   - bluetooth.service unmasked + stopped at transition completion;
#     its target/D-Bus activation paths remain available
...
case "$ACTION" in
    on|enable)
        ...
        # Preserve Fedora's target/D-Bus enablement policy. Start the service
        # directly and verify every state transition before reporting
    ...
    off|disable)
        ...
        # Keep target/D-Bus activation available for the native panel;
        # the rfkill block + the udev enforcer rule own the radio default-OFF.
        if ! path_exists "$WPCFG"; then
            if ! publish_wireplumber_policy; then
...
#
# `systemctl disable` is deliberately not called: Fedora's [Install] section
# provides bluetooth.target and dbus-org.bluez.service activation links used
# by the native desktop. The bluetooth-disabled.flag (set above in 1f.4) and
# rfkill enforcer own the radio default; M99 and Welcome inspect that state.
echo ""
...
# Extract the systemd-formatted verification key.
...
systemd Forward Secure Sealing authenticates journal records using an evolving
sealing key. A separately retained verification key supports detection of later
changes within the verified sealed intervals. **On Fedora 44 it is currently inert.**
...
- **auditd** (Module 12) locks its rule configuration (`-e 2`) until reboot
  and captures auth/AVC events separately from the journal. This does not
  make its on-disk log records immutable against root access.
...
(
    ...
    sudo cp --update=none-fail -- /etc/noid-privacy/journal-verify.key \
        /mnt/usb/noid-fss-verify.key
    sudo chmod 400 -- /mnt/usb/noid-fss-verify.key
)
```

An existing destination is preserved and makes this copy command fail. Choose
a separate destination for another machine or key generation; never overwrite
an earlier trust anchor to make the command succeed.
...
(
    set -euo pipefail
    mountpoint -q /mnt/usb
    fss_key_file=/mnt/usb/noid-fss-verify.key
    fss_key=$(sudo awk '
        /^[a-f0-9]{6}-[a-f0-9]{6}-[a-f0-9]{6}-[a-f0-9]{6}\/[a-f0-9-]+$/ {
            key=$0; count++
        }
        END { if (count != 1) exit 1; print key }
    ' "$fss_key_file")
    sudo journalctl --verify --verify-key="$fss_key"
)
```

Choose the exact retained key file for this journal. Missing, empty or ambiguous
key files stop the example before verification. `--verify` checks internal
consistency; authenticity additionally requires a sealed file and its matching
verification key. A failure needs investigation and can indicate corruption,
a mismatched key or tampering; `PASS` alone does not prove complete log history.
...
#   - UMask=0077               : remove group/other bits from requested modes
#                                (normally files 0600, directories 0700)
#
...
# Every libdnf5 failure (for example an administrator-owned, unreadable
# .repo file) must surface as the helper's one-line diagnostic contract, not
# as a raw traceback on the DNF actions-plugin control channel.
try:
    base = libdnf5.base.Base()
    base.get_config().get_cachedir_option().set(cache_root)
    base.setup()
    base.get_repo_sack().create_repos_from_system_configuration()
except libdnf5.exception.Error as exc:
    raise SystemExit(
        f"noid-vscodium-repo-key-seed: cannot read DNF repository configuration: {exc}"
    )
...
- These are NoID Privacy defaults, not constraints. Higher-priority client and platform instructions remain in force.
  ...
  1. the explicit confirmations required by the Change protocol in §6; its examples are illustrative and its list of qualifying operations stays open;
...
- The NoID Privacy Workstation repository is multi-license, not repo-wide GPL. Before cross-component code moves/combinations, dependencies, or
  notice changes, inspect `LICENSING.md` and affected SPDX IDs. GPL-2.0-only XDP BPF may coexist with GPL-3.0-or-later components as a separate work
...
- **Never launch `noid-update-all.sh` or an equivalent full-system-update workflow unless the user explicitly requests that invocation.**
  ...
  firmware prompt needs its own explicit user answer. After its own successful DNF transaction, and only when an active baseline exists, the workflow
  invokes the check-only `noid-aide-check.sh` unless the user explicitly skips it; it never creates or replaces the AIDE baseline.
...
`~/.codex/AGENTS.md` adapter, and Gemini CLI reads `~/.gemini/GEMINI.md`.
NoID Privacy-created adapters are absolute symlinks to the same root-owned source;
pre-existing user files or links are preserved and may contain different rules.
The repo-root `AGENTS.md` is additionally tested byte-for-byte
...
- **Native > Hacky** — use maintained native mechanisms for NoID Privacy's
  system configuration; this rule does not restrict the user's own projects
...
provider. Permission controls can prevent the read when enabled; the image's
bypass default does not ask for that approval. NoID Privacy has no automatic
content-redaction layer between a tool result and the provider.

Keep secrets outside the agent's accessible filesystem when they must remain
local, for example by using a separate account or an appropriately restricted
sandbox. A different working directory alone does not restrict file access.
`CLAUDE_CODE_HIDE_CWD=1` hides paths in the banner, not in tool results.
...
Close agent sessions and editor extensions before removing their programs.
If you installed Codex and want to clear its stored authentication, run this
while the CLI is still installed and confirm it succeeds:

```bash
codex logout
```

The CLI and IDE share stored credentials; signing out affects both. See
[Codex authentication](https://learn.chatgpt.com/docs/auth). Removing program
files alone does not clear the operating-system keyring.

For the default paths used by the NoID Privacy installers, after checking
that these are still the managed installation paths:
...
If you installed Codex with a custom `CODEX_HOME`, its package directory is
under that location instead; inspect it before adapting the removal path.
...
DNS_RUNTIME_DIR=/run/noid-privacy
...
valid_dns_runtime_dir() {
    [ -d "$DNS_RUNTIME_DIR" ] && [ ! -L "$DNS_RUNTIME_DIR" ] \
        && [ "$(stat -c '%u:%g:%a' -- "$DNS_RUNTIME_DIR" 2>/dev/null)" = \
             "$EXPECTED_UID:$EXPECTED_GID:755" ]
}

open_dns_mode_lock() {
    local fd_meta path_meta
    [ "$DNS_LOCK_HELD" = 0 ] || return 0
    valid_dns_runtime_dir || return 1
    [ -f "$DNS_MODE_LOCK" ] && [ ! -L "$DNS_MODE_LOCK" ] || return 1
    exec 7<>"$DNS_MODE_LOCK" || return 1
    if ! /usr/bin/flock --exclusive 7; then
        exec 7>&-
        return 1
    fi
    fd_meta=$(stat -Lc '%u:%g:%a:%h:%d:%i' /proc/self/fd/7 \
        2>/dev/null) || {
        exec 7>&-
        return 1
    }
    path_meta=$(stat -Lc '%u:%g:%a:%h:%d:%i' "$DNS_MODE_LOCK" \
        2>/dev/null) || {
        exec 7>&-
        return 1
    }
    if [ "$fd_meta" != "$path_meta" ] \
       || [[ "$fd_meta" != "$EXPECTED_UID:$EXPECTED_GID:600:1:"* ]]; then
        exec 7>&-
        return 1
    fi
    DNS_LOCK_HELD=1
...
close_dns_mode_lock() {
    [ "$DNS_LOCK_HELD" = 1 ] || return 0
    /usr/bin/flock --unlock 7 || return 1
    exec 7>&-
    DNS_LOCK_HELD=0
}

valid_openh264_dns_state() {
    [ -f "$OH264_DNS_STATE" ] && [ ! -L "$OH264_DNS_STATE" ] \
        && [ "$(stat -c '%u:%g:%a:%h' -- "$OH264_DNS_STATE" \
                2>/dev/null)" = \
             "$EXPECTED_UID:$EXPECTED_GID:600:1" ]
}

load_openh264_dns_state() {
    local -a lines=()
    OH264_DNS_SCOPE=""
    OH264_DNS_IFACE=""
    valid_openh264_dns_state || return 1
    mapfile -t lines < "$OH264_DNS_STATE" || return 1
    [ "${#lines[@]}" -eq 3 ] \
        && [ "${lines[0]}" = 'NOID-OH264-DNS-RECOVERY-V1' ] \
        && [[ "${lines[1]}" =~ ^SCOPE=(global|physical)$ ]] \
        || return 1
    OH264_DNS_SCOPE=${BASH_REMATCH[1]}
    [[ "${lines[2]}" =~ ^IFACE=([A-Za-z0-9_.-]{1,15})$ ]] || return 1
    OH264_DNS_IFACE=${BASH_REMATCH[1]}
    [ "$OH264_DNS_IFACE" != . ] && [ "$OH264_DNS_IFACE" != .. ]
}

publish_openh264_dns_state() {
    local scope="$1" iface="$2" tmp
    valid_dns_runtime_dir || return 1
    if path_exists "$OH264_DNS_STATE" && ! valid_openh264_dns_state; then
        return 1
    fi
    tmp=$(mktemp "$DNS_RUNTIME_DIR/.openh264-dns-recovery.XXXXXX") || return 1
    if ! chmod 0600 "$tmp" \
       || ! printf '%s\n' \
            ...
            "IFACE=$iface" > "$tmp" \
       || ! sync -- "$tmp" \
       || ! mv -fT -- "$tmp" "$OH264_DNS_STATE" \
       || ! sync -- "$DNS_RUNTIME_DIR" \
       || ! valid_openh264_dns_state; then
        rm -f -- "$tmp"
        return 1
    fi
}
...
dns_machine_field() {
    ...
    [ "$(grep -c "^${key}=" <<< "$payload")" -eq 1 ] || return 1
    value=$(sed -n "s/^${key}=//p" <<< "$payload")
    [ -n "$value" ] || return 1
    printf '%s\n' "$value"
}

active_physical_interface() {
    local rows line iface type extra
    local -a candidates=()
    rows=$("$NMCLI" -t -e no -f DEVICE,TYPE connection show --active \
        2>/dev/null) || return 1
    while IFS= read -r line; do
        [ -n "$line" ] || continue
        IFS=: read -r iface type extra <<< "$line"
        [ -z "$extra" ] || return 1
        case "$type" in
...
dns_runtime_matches() {
    local expected="$1" iface="$2" payload
    payload=$("$RESOLVECTL" status --json=short 2>/dev/null) || return 1
    printf '%s' "$payload" | "$PYTHON" -c '
import json
import sys

expected, iface = sys.argv[1:]
try:
...
candidate = [row for row in links if row.get("ifname") == iface]
if len(candidate) != 1 or global_row.get("dnsOverTLS") != "yes":
    raise SystemExit(1)
candidate = candidate[0]
if expected == "filtered-global":
    ok = (servers(global_row) == filtered_global
          and not any(servers(row) for row in links)
          and not any(root_route(row) for row in links))
elif expected in {"filtered-link", "unfiltered-link"}:
    wanted = filtered_link if expected == "filtered-link" else unfiltered_link
    routed = [row for row in links if root_route(row)]
    ok = (len(routed) == 1 and routed[0] is candidate
          and candidate.get("dnsOverTLS") == "yes"
          and servers(candidate) == wanted
          and all(not servers(row) for row in links if row is not candidate))
elif expected == "restore-filtered-global":
    # Restoration owns only the temporary candidate-link override. A VPN,
    # VM bridge or second profile may legitimately acquire unrelated DNS
    # state while DNF runs; that must not make a successful revert impossible
    # to prove. Keep the original global path and our candidate link exact.
    ok = (servers(global_row) == filtered_global
          and not servers(candidate)
          and not root_route(candidate))
elif expected == "restore-filtered-link":
    # The physical-scope retry changes only DNS servers on the candidate link.
    # Its strict DoT/root-route contract remains load-bearing; other links are
    # outside this transaction and intentionally ignored during restoration.
    ok = (candidate.get("dnsOverTLS") == "yes"
          and servers(candidate) == filtered_link
          and root_route(candidate))
else:
    ok = False
...
detect_filtered_quad9_scope() {
    local status selection configured runtime scope link_mode iface expected
    status=$("$DNS_MODE_CLI" --status-machine 2>/dev/null) || return 1
    [ "$(head -n 1 <<< "$status")" = 'NOID-DNS-MODE-V2' ] || return 1
    selection=$(dns_machine_field "$status" selection) || return 1
    configured=$(dns_machine_field "$status" configured) || return 1
    runtime=$(dns_machine_field "$status" runtime_global) || return 1
    scope=$(dns_machine_field "$status" scope) || return 1
    link_mode=$(dns_machine_field "$status" link_mode) || return 1
    case "$selection" in default|strict) ;; *) return 1 ;; esac
    [ "$configured" = yes ] && [ "$runtime" = yes ] || return 1
    iface=$(active_physical_interface) || return 1
    case "$scope:$link_mode" in
        global:none) expected=filtered-global ;;
        physical:yes) expected=filtered-link ;;
        *) return 1 ;;
    esac
    dns_runtime_matches "$expected" "$iface" || return 1
    printf '%s:%s\n' "$scope" "$iface"
}

restore_openh264_dns() {
    local restore_rc=0
    path_exists "$OH264_DNS_STATE" || return 0
    if ! load_openh264_dns_state; then
        dns_recovery_note \
            "Task 2: FATAL unsafe temporary DNS recovery evidence; refusing an unverifiable restore"
        return 1
    fi
    case "$OH264_DNS_SCOPE" in
        global)
            "$RESOLVECTL" revert "$OH264_DNS_IFACE" >/dev/null 2>&1 \
                || restore_rc=$?
            ;;
        physical)
            "$RESOLVECTL" dns "$OH264_DNS_IFACE" \
                "${QUAD9_FILTERED_V4[@]}" >/dev/null 2>&1 \
                || restore_rc=$?
            ;;
    esac
    "$RESOLVECTL" flush-caches >/dev/null 2>&1 || restore_rc=$?
    if [ "$OH264_DNS_SCOPE" = global ]; then
        dns_runtime_matches restore-filtered-global "$OH264_DNS_IFACE" \
            || restore_rc=1
    elif ! dns_runtime_matches restore-filtered-link "$OH264_DNS_IFACE" \
         && ! dns_runtime_matches restore-filtered-global "$OH264_DNS_IFACE"; then
        restore_rc=1
    fi
    if [ "$restore_rc" -ne 0 ]; then
        dns_recovery_note \
            "Task 2: FATAL could not prove restoration of the original Quad9 Strict filtered DNS path"
        return 1
    fi
    rm -f -- "$OH264_DNS_STATE" || return 1
    sync -- "$DNS_RUNTIME_DIR" || return 1
    dns_recovery_note \
        "Task 2: original Quad9 Strict filtered DNS path restored"
}

prepare_openh264_dns_retry() {
    local detected scope iface
    detected=$(detect_filtered_quad9_scope) || return 1
    scope=${detected%%:*}
    iface=${detected#*:}
    # A successful uncached lookup means the first DNF failure was not the
    # filtered-only resolver condition. Do not weaken threat blocking.
    if "$RESOLVECTL" query --cache=no --stale-data=no --legend=no \
            "$OH264_DNS_HOST" >/dev/null 2>&1; then
        return 1
    fi
    publish_openh264_dns_state "$scope" "$iface" || return 1
    if [ "$scope" = global ]; then
        "$RESOLVECTL" dns "$iface" "${QUAD9_UNFILTERED_V4[@]}" \
            >/dev/null 2>&1 || return 1
        "$RESOLVECTL" dnsovertls "$iface" yes >/dev/null 2>&1 || return 1
        "$RESOLVECTL" domain "$iface" '~.' >/dev/null 2>&1 || return 1
    else
        "$RESOLVECTL" dns "$iface" "${QUAD9_UNFILTERED_V4[@]}" \
            >/dev/null 2>&1 || return 1
    fi
    "$RESOLVECTL" flush-caches >/dev/null 2>&1 || return 1
    dns_runtime_matches unfiltered-link "$iface" || return 1
    "$RESOLVECTL" query --cache=no --stale-data=no --legend=no \
        "$OH264_DNS_HOST" >/dev/null 2>&1
}

run_openh264_dnf() {
    local first_rc=0 retry_rc=0
    run_system_dnf "$@" 2>&1 | tee -a "$LOG_FILE" || first_rc=$?
    [ "$first_rc" -ne 0 ] || return 0
    log "Task 2: initial Cisco transaction failed; checking the exact Quad9 filtered-only DNS condition"
    if ! open_dns_mode_lock; then
        log "Task 2: automatic DNS recovery unavailable; DNS-mode lock contract failed"
        return "$first_rc"
    fi
    if path_exists "$OH264_DNS_STATE" && ! restore_openh264_dns; then
        OH264_DNS_RESTORE_FAILED=1
        close_dns_mode_lock || true
        return 1
    fi
    if ! prepare_openh264_dns_retry; then
        if path_exists "$OH264_DNS_STATE" && ! restore_openh264_dns; then
            OH264_DNS_RESTORE_FAILED=1
        else
            log "Task 2: no confirmed Quad9 filtered-only Cisco failure; no automatic DNS change made"
        fi
        close_dns_mode_lock || OH264_DNS_RESTORE_FAILED=1
        [ "$OH264_DNS_RESTORE_FAILED" = 0 ] || return 1
        return "$first_rc"
    fi
    log "Task 2: confirmed Quad9 filtered-only Cisco resolution failure"
    log "Task 2: retrying once via strict authenticated dns10.quad9.net without threat blocking"
    run_system_dnf "$@" 2>&1 | tee -a "$LOG_FILE" || retry_rc=$?
    if ! restore_openh264_dns; then
        OH264_DNS_RESTORE_FAILED=1
    fi
    close_dns_mode_lock || OH264_DNS_RESTORE_FAILED=1
    [ "$OH264_DNS_RESTORE_FAILED" = 0 ] || return 1
...
cleanup_openh264_dns() {
    local rc=$? cleanup_rc=0
    trap - EXIT
    if path_exists "$OH264_DNS_STATE"; then
        if ! open_dns_mode_lock || ! restore_openh264_dns; then
            cleanup_rc=1
        fi
    fi
    close_dns_mode_lock || cleanup_rc=1
    [ "$cleanup_rc" -eq 0 ] || rc=1
    exit "$rc"
}
# END OPENH264_DNS_RECOVERY
...
case "$#:${1:-}" in
    ...
    1:--restore-openh264-dns)
        ...
        ;;
    *)
        printf '%s\n' 'noid-firstboot-setup: unsupported argument' >&2
        exit 2
        ;;
esac
...
LOG_READY=1
trap cleanup_openh264_dns EXIT
trap 'exit 129' HUP
trap 'exit 130' INT
trap 'exit 143' TERM
if path_exists "$OH264_DNS_STATE"; then
    if ! open_dns_mode_lock \
       || ! restore_openh264_dns \
       || ! close_dns_mode_lock; then
        log "FAIL: could not recover a prior temporary OpenH264 DNS override"
        exit 1
    fi
fi
...
if [ -z "$oh264_missing" ] && [ -f "$TASK2_RECEIPT" ]; then
    ...
    task2_repair_rc=0
    # shellcheck disable=SC2086
    run_openh264_dnf reinstall -y $oh264_pkgs || task2_repair_rc=$?
    if [ "$OH264_DNS_RESTORE_FAILED" != 0 ]; then
        log "Task 2: FATAL refusing to continue while original DNS restoration is unproven"
        exit 1
    fi
    ...
    install_rc=0
    # shellcheck disable=SC2086
    run_openh264_dnf install -y $oh264_missing || install_rc=$?
    if [ "$OH264_DNS_RESTORE_FAILED" != 0 ]; then
        log "Task 2: FATAL refusing to continue while original DNS restoration is unproven"
        exit 1
    fi
...
# Defense in depth for SIGKILL/timeout paths: the main helper normally restores
# the boot-local resolver override synchronously before Task 3 and from EXIT.
# ExecStopPost gets a fresh process after every service result and consumes the
# same closed recovery record if the original shell could not run its trap.
ExecStopPost=/usr/local/bin/noid-firstboot-setup.sh --restore-openh264-dns
RemainAfterExit=no
...
if [ "$#" -gt 1 ] || { [ "$#" -eq 1 ] && [ -z "$1" ]; }; then
    echo "Usage: noid-complete-setup.sh [--help|--dry-run|--dry]" >&2
    exit 2
fi
case "${1:-}" in
    --help|-h)
...
# .repo files used by later explicit codec transactions. Step 10 verification
...
if rpm -q rpmfusion-free-release >/dev/null 2>&1; then
    ...
    echo "  [OK] rpmfusion-free configured through its pinned fallback .repo"
else
...
if rpm -q rpmfusion-nonfree-release >/dev/null 2>&1; then
    ...
    echo "  [OK] rpmfusion-nonfree configured through its pinned fallback .repo"
else
...
validate_state_file() {
    STATE_REPOS=()
    [ -e "$MINIMAL_STATE" ] || [ -L "$MINIMAL_STATE" ] || return 1
...
write_state_file() {
    local candidate
    # Validate existing state before install -d can follow a symlink or change
    # its target's metadata. Only an absent directory is created here.
    ...
    if ! printf '%s\n' "NOID_THIRDPARTY_MINIMAL_V1" "$@" > "$candidate"; then
        rm -f -- "$candidate"
        return 1
    fi
    chmod 0644 "$candidate" &&
...
case "${1:-}" in
    ...
    restore)
        ...
        if ! rm -f -- "$MINIMAL_STATE"; then
            echo "ERROR: repos restored, but minimal-state cleanup failed" >&2
            exit 1
        fi
        if ! sync -- "$STATE_DIR"; then
            echo "ERROR: repos restored, but state cleanup persistence is unconfirmed" >&2
            exit 1
        fi
    ...
    disable|enable)
        ...
        case "$1" in
            disable) val=0 ;;
            enable)
                if [ -e "$MINIMAL_STATE" ] || [ -L "$MINIMAL_STATE" ]; then
