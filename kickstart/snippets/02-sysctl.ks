...
# Status: LOCKED 2026-09-06 (v38) — reject IPv6 type-2 source routing with the signed kernel disable value.
#
# Covers:
...
# === Source Routing (IPv4 off, IPv6 type-2 processing disabled) ===
...
# IPv6 uses a signed integer: zero still permits type-2 routing-header
# processing, subject to the separate Mobile IPv6 checks. A negative value
# rejects that processing; IPv4's Boolean zero is not equivalent. This
# intentionally excludes Mobile IPv6 type-2 routing. Ordinary routed VPN
# traffic does not require that extension. Other routing-header families
# have their own kernel controls; this is not a blanket extension-header filter.
# https://docs.kernel.org/networking/ip-sysctl.html (accept_source_route)
