...
# Status: LOCKED 2026-09-06 (v64) — verify weak-dependency policy through the native DNF configuration reader.
#
# Covers:
...
# ====================================================================
# CHECK 3: Persistent weak-dependency policy (native DNF main configuration)
# ====================================================================
...
# using DNF's section and precedence rules instead. Transaction-specific
# command-line options and plugins are outside this configuration check.
log "CHECK 3: persistent weak-dependency policy"
if ! dnf_main_config=$(dnf5 --no-plugins --dump-main-config 2>/dev/null) ||
        ! awk -F '=' '
            $1 ~ /^[[:space:]]*install_weak_deps[[:space:]]*$/ {
                count++
                disabled = (NF == 2 && $2 ~ /^[[:space:]]*0[[:space:]]*$/)
            }
            END { exit !(count == 1 && disabled) }
        ' <<<"$dnf_main_config"; then
    verify_fail=$((verify_fail + 1))
    log "  [FAIL] native DNF configuration does not establish disabled weak dependencies"
else
    verify_ok=$((verify_ok + 1))
...
# ====================================================================
log "CHECK 5: Productivity Tier-1 packages"
...
Earlier image revisions also installed `unrar` (RPM Fusion nonfree, RARLAB
payload) and `p7zip-plugins` (RPM Fusion free) at first boot. Both were dropped
...
# CUPS and all activation units are deliberately masked by default. The
# supported switch is the NoID Privacy toggle:
sudo noid-toggle-printing on
# It performs exactly these native steps:
...
```bash
sudo noid-toggle-printing off
# Equivalent native steps:
...
- **Just-Perfection** (`just-perfection-desktop@just-perfection`, v37) —
  ...
  Upstream: gitlab.gnome.org/jrahmatzadeh/just-perfection (GPL-3.0-only).
