...
# Repository notice: docs/trademark-notice.md (kept aligned on three facts;
#   the installed notice is the STEP 7b heredoc, not a generated copy)
# Status: LOCKED 2026-09-06 (v3.65) — stamp v1.8; preserve Plymouth publication failures.
#
# Covers:
...
#   - NOID_VERSION (currently v1.8) bumps ONLY on an explicit release GO.
...
# install-fail; en_*-only Welcome language list).
        ...
        if [ -f "$BGRT_PLY" ] && [ ! -L "$BGRT_PLY" ] \
            ...
            if [ "$watermark_keys" -ne 1 ] || [ "$spinner_keys" -ne 1 ] \
               || [ "$firmware_keys" -lt 1 ]; then
                ...
                # Keep publication outside the condition: Bash otherwise
                # suppresses errexit throughout the publisher function.
                sed -E \
                ...
                if grep -qx 'WatermarkVerticalAlignment=.73' "$BGRT_PLY" \
...
# NOID_VERSION bumps only on an explicit release GO (currently v1.8).
