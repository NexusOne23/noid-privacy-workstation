...
# Status: LOCKED 2026-09-06 (v48) — isolate bootstrap mount ownership and correct recovery mount inspection.
#
...
cleanup_init() {
    ...
    trap - EXIT
    # TOP is assigned only by this invocation's exclusive mktemp allocation.
    # Early exits must never unmount a pre-existing fixed-path mount.
    if [ -n "$TOP" ]; then
        if mountpoint -q "$TOP" && ! umount "$TOP"; then
            logger -t "$LOG_TAG" \
                "ERROR: temporary Btrfs top-level mount could not be removed"
            cleanup_failed=1
        fi
        if [ "$cleanup_failed" -eq 0 ] && ! rmdir -- "$TOP"; then
            logger -t "$LOG_TAG" \
                "ERROR: temporary Btrfs mountpoint directory could not be removed"
            cleanup_failed=1
        fi
    fi
...
require_tools awk btrfs chattr chmod chown cp find grep grubby install lsattr \
    ...
    rmdir stat sync systemctl tr umount
...
TOP=$(mktemp -d /run/noid-snapper-top.XXXXXX) \
    || fail "cannot allocate a private Btrfs top-level mountpoint"
[ -d "$TOP" ] && [ ! -L "$TOP" ] \
...
for path in / /.snapshots /var/lib/libvirt; do
    findmnt --target "$path" --output TARGET,FSTYPE,FSROOT,OPTIONS
done
...
- Inspect each mount with the `findmnt --target` loop above before retrying.
