# ============================================================================
# Module 03 — Firewalld
# Status: LOCKED 2026-09-06 (v5.50) — bind cached XDP admission to subnet membership; preserve one lock across concurrent first refreshes.
#
# Covers:
...
# Print 1/0 for gateway membership; query/parse errors return nonzero.
...
iface_onlink_ipv4() {
    local iface=$1 gw=$2 addresses
    addresses=$(ip -j -4 addr show dev "$iface" scope global 2>/dev/null) \
        || return 1
    python3 - "$gw" "$iface" "$addresses" <<'ONLINK_PY'
import ipaddress, json, sys
try:
    gw = ipaddress.IPv4Address(sys.argv[1])
    rows = json.loads(sys.argv[3])
    # ip -4 suppresses an existing interface without matching addresses.
    # A missing interface is distinguished by the command's nonzero status.
    if rows == []:
        print(0)
        sys.exit(0)
    if (not isinstance(rows, list) or len(rows) != 1
            or not isinstance(rows[0], dict)
            or rows[0].get("ifname") != sys.argv[2]
            or not isinstance(rows[0].get("addr_info"), list)):
        raise ValueError("invalid interface address inventory")
    onlink = False
    for address in rows[0]["addr_info"]:
        if (not isinstance(address, dict) or address.get("family") != "inet"
                or type(address.get("prefixlen")) is not int
                or not 0 <= address["prefixlen"] <= 32
                or not isinstance(address.get("local"), str)):
            raise ValueError("invalid IPv4 address row")
        local = ipaddress.IPv4Address(address["local"])
        network = ipaddress.IPv4Network((local, address["prefixlen"]), strict=False)
        onlink |= gw in network
    print(int(onlink))
...
sync_policy() {
    ...
    local -a fields=()
    declare -A seen_ifaces=() seen_peers=() gateway_ifaces=()
    ...
    if [ -e "$ARP_STATE" ] || [ -L "$ARP_STATE" ]; then
        ...
        # Cache the actual map decision once: subnet changes affect gateway
        # admission even when interface name, index and MAC stay unchanged.
        # Failure is not evidence that the interface is on another subnet.
        for iface in "${ifaces[@]}"; do
            if [ "$iface" = "$GATEWAY_IFACE" ]; then
                gateway_ifaces[$iface]=1
            else
                gateway_ifaces[$iface]=$(iface_onlink_ipv4 "$iface" "$GATEWAY_IP") \
                    || die "cannot assess gateway subnet membership for $iface"
            fi
        done
    fi
    ...
    #   - per interface name, ifindex, local MAC AND gateway subnet membership:
    #     all decide map contents across reconnects and address changes
...
table inet noid_lan_topology {
    ...
    chain input {
        ...
        # This drop is interface-independent: an address that carries an
        # inbound grant is admitted only through the exact granted physical
        # tuples above. The same address arriving over any other link
        # (tunnel, bridge) is dropped too, so a grant never widens beyond its
        # physical tuple.
...
# 13 langpacks (12 non-English), so that is the norm outside English, not an
# edge case.
...
LOGGER_BIN="${NOID_LAN_LOGGER_BIN:-/usr/bin/logger}"
...
# to stderr so the advertised command answers the question it promises. A
# final EXIT guard covers future bare-command regressions under `set -e`; the
# explicit `die` path preserves the concrete reason and requested exit status.
failure_reported=0
fail() {
    failure_reported=1
    "$LOGGER_BIN" -t "$LOG_TAG" "$*" || true
    printf '%s: %s\n' "$LOG_TAG" "$*" >&2 || true
}
die() {
    local status="$1"
...
cleanup_and_report() {
    ...
    [ -z "$batch" ] || rm -f -- "$batch"
    if [ "$status" -ne 0 ] \
       && [ "$status" -ne "$XDP_RETRY_RC" ] \
       && [ "$failure_reported" -eq 0 ]; then
        fail "FAILED: unhandled topology-refresh error at script line $unhandled_line (status $status)"
    fi
    exit "$status"
}
trap 'unhandled_line=$LINENO' ERR
...
case "$RETRY_DEGRADED_XDP" in
    0|1) ;;
    *) die 2 "FAILED: invalid degraded-XDP retry selector" ;;
esac

[[ "$STATE_UID" =~ ^(0|[1-9][0-9]{0,9})$ ]] \
    && [[ "$STATE_GID" =~ ^(0|[1-9][0-9]{0,9})$ ]] || {
    die 1 "FAILED: invalid LAN state owner contract"
}
...
# transitions. Run once under that parent to cover the whole transaction; the
# short original shell propagates the reserved retry status and diagnoses every
# other failure even if flock itself could not start the protected command.
if [ "${NOID_LAN_TOPOLOGY_LOCK_HELD:-0}" != "1" ]; then
    lock_dir=${LOCK_FILE%/*}
    [[ "$LOCK_FILE" == /* && "$LOCK_FILE" != */ ]] \
        || die 1 "FAILED: lock path must be an absolute file path"
    trusted_state_directory "$lock_dir" || {
        die 1 "FAILED: lock parent is not a trusted state directory"
    }
    if [ ! -e "$LOCK_FILE" ] && [ ! -L "$LOCK_FILE" ]; then
        # Concurrent first invocations may both observe absence. Exclusive
        # creation preserves the winner's inode and therefore its flock.
        if ! (set -o noclobber; : > "$LOCK_FILE") 2>/dev/null; then
            [ -f "$LOCK_FILE" ] && [ ! -L "$LOCK_FILE" ] \
                || die 1 "FAILED: cannot create the topology transaction lock"
        fi
    ...
    [ -f "$LOCK_FILE" ] && [ ! -L "$LOCK_FILE" ] \
        ...
        die 1 "FAILED: lock file is not one closed root-private identity"
    }
    [ -x /usr/bin/flock ] && [ -x /usr/bin/env ] && [ -x /bin/bash ] \
        || die 1 "FAILED: locked topology-refresh runtime is unavailable"
    if /usr/bin/flock --close --exclusive "$LOCK_FILE" \
            /usr/bin/env NOID_LAN_TOPOLOGY_LOCK_HELD=1 /bin/bash "$0" "$@"; then
        exit 0
    else
        lock_status=$?
    fi
    [ "$lock_status" -eq "$XDP_RETRY_RC" ] && exit "$lock_status"
    die "$lock_status" \
        "FAILED: locked topology-refresh transaction returned status $lock_status"
fi
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --exclude-peer)
            [ -z "$EXCLUDE_PEER" ] && [ "$#" -ge 2 ] \
                || die 2 "FAILED: invalid duplicate/missing --exclude-peer"
            EXCLUDE_PEER=$2
            shift 2
            ;;
        --invalidate-peer-flows)
            [ -z "$INVALIDATE_FLOW_PEER" ] && [ "$#" -ge 2 ] \
                || die 2 "FAILED: invalid duplicate/missing --invalidate-peer-flows"
            INVALIDATE_FLOW_PEER=$2
            shift 2
            ;;
        --require-xdp)
            [ "$REQUIRE_XDP" -eq 0 ] \
                || die 2 "FAILED: duplicate --require-xdp"
            REQUIRE_XDP=1
            shift
            ;;
        *) die 2 "FAILED: unknown topology-refresh argument: $1" ;;
    esac
done
...
if [ "$guard_enabled" -eq 1 ]; then
    ...
    if [ -n "$EXCLUDE_PEER" ]; then
        ...
        case "$EXCLUDE_PEER" in
            *:*)
                fail "FAILED: excluded peer must be an IPv4 address"
                exit 2
                ;;
        esac
...
publish_global_state INCONSISTENT || {
    fail "FAILED: cannot publish the INCONSISTENT runtime state"
    exit 1
}
{
