...
# Status: LOCKED 2026-09-06 (v34) — bound SSH client known-host privacy claims to native hashing behavior.
#
# Covers:
...
# (Step 1, line `HashKnownHosts yes`). New entries hash the destination
# hostname/address, not the host public key. This conceals directly readable
# names but still permits offline candidate guessing and key correlation.
#
...
- **PQ-hybrid KEX preferred**: ML-KEM-768 + X25519, with another hybrid and
  classical fallbacks. Protection against future quantum decryption depends
  on the algorithm actually negotiated with the peer.
...
This hashes eligible hostname/address fields with salted HMAC-SHA1; the host
public keys stay readable. Wildcard and negated host patterns are not hashed,
so inspect the result for entries that remain plaintext. The original is
backed up to `~/.ssh/known_hosts.old` automatically.
...
An attacker can test guessed hostnames and addresses offline, including lists
of popular services or candidate IP ranges. Readable host public keys may also
identify a server by correlation. Hashing reduces immediate disclosure of the
destination names; it does not make the connection history anonymous or encrypt
the file.
