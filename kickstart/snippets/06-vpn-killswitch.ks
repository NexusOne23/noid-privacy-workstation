...
# Status: LOCKED 2026-09-06 (v74) — preserve endpoint publication consistency and existing bounded pause timers.
#
...
#               5 services + 1 path-unit + 1 timer + 1 udev rule + 2 CLIs +
...
#   - noid-wan-strict-tunnel-scan.service   : udev-triggered oneshot for
#                                             tunnel hotplug (STEP 6d)
#   - /etc/udev/rules.d/71-noid-wan-strict-tunnel-hotplug.rules : starts the
#                                             tunnel-scan unit on wireguard/
#                                             tun link appearance
...
table inet noid_wan_strict {
    ...
    chain output {
        ...
        # Some dynamic clients first install a single-host
        ...
        # profile. The controller validates applied routes and their gateway;
        # a matching saved route does not exclude the one-minute allowance.
        # This exception is local-output-only and address+transport
...
def publish(records: list[Record], candidates: set[tuple[str, str, int]] | None,
    ...
    try:
        ...
        # Once rename succeeds, the visible file already contains the new
        # records. Reverting only nft after a directory-sync failure would
        # create conflicting authorities. Retain the matching visible pair
        # and report that its persistence across power loss is unconfirmed.
        try:
            ...
            raise fail("new endpoint state and nft retained; persistence unconfirmed") from exc
    finally:
        staged.unlink(missing_ok=True)
...
if [ -e "$ARMED_FLAG" ] || [ -L "$ARMED_FLAG" ]; then
    ...
    if ! read_status_mode >/dev/null; then
        block_physical_pre_up \
...
# live kernel interface. The tunnel pre-up copy attempts this before
# NetworkManager reports activation; failures are logged without vetoing it.
# Later physical-link changes use the maintained no-wait path.
...
# Distinguish a complete non-match (1) from an unreadable inventory (2).
wireguard_iface() {
    local iface=$1 item interfaces
    interfaces=$("$WG" show interfaces 2>/dev/null) || return 2
...
# Native wg prints "off" or a 32-bit hexadecimal mark. Only proven "off"
# authorizes an unmarked route lookup; query failure is not an absent mark.
wireguard_fwmark() {
    local mark
    ...
    case "$mark" in
        off) return 0 ;;
        *)
            [[ "$mark" =~ ^0x[0-9a-fA-F]{1,8}$ ]] || return 1
            printf '%s' "$mark"
            ;;
    esac
}
...
reconcile_one() {
    ...
    local peers=0 unresolved=0 ipv6_rc iface_rc applied

    valid_iface "$iface" || {
        log_warn "refused invalid or absent interface name"
        return 1
    }
    if wireguard_iface "$iface"; then
        iface_rc=0
    else
        iface_rc=$?
    fi
    case "$iface_rc" in
        ...
        1) return 0 ;;
        *)
            log_warn "$iface WireGuard interface inventory is unreadable; left unchanged"
            return 1
            ;;
    esac
    ...
    mark=$(wireguard_fwmark "$iface") || {
        log_warn "$iface WireGuard fwmark is unreadable or invalid; live MTU $current retained"
        return 1
    }
...
if ! "$HELPER" "$target" >/dev/null; then
    /usr/bin/logger -p user.warning -t noid-wireguard-mtu -- \
        "event $ACTION could not confirm a safe live MTU; see helper diagnostics; owning profile was not modified"
fi
exit 0
...
cmd_pause() {
    ...
    # fails, retain any existing pause timer. A second pause must never cancel
    # the only deadline of an already-relaxed policy. The fixed unit name
    # makes systemd reject a concurrent/duplicate timer without replacing it.
    if ! systemd-run --collect --on-active="${sec}" --unit="noid-wan-strict-autoresume" \
        --timer-property=AccuracySec=1s \
        --description="NoID Privacy auto-resume WAN-egress-strict" \
        /usr/local/sbin/noid-wan-strict resume >/dev/null 2>&1; then
        echo "ERROR: auto-resume could not be scheduled; existing policy and timer retained" >&2
        echo "       If a pause is already running, resume it before starting another." >&2
        exit 1
    fi
...
Starting another pause while its timer exists preserves the original deadline
and reports the conflict. Run `resume` before requesting a new duration; a
failed scheduling attempt never cancels the existing return timer. Durations
are decimal minutes from 1 through 1440, including inputs with leading zeroes.

Endpoint publication distinguishes a failed rename from a failed directory
synchronization afterward. Before rename, the previous file and firewall are
restored. After rename, the new visible file and matching firewall remain in
place, and the command reports unconfirmed persistence instead of restoring
only the firewall. Such an error requires inspection before relying on reboot
durability.
...
| WireGuard MTU reconciler | `/usr/local/sbin/noid-wireguard-mtu-reconcile` + `pre-up.d/45-noid-wireguard-mtu` (awaited copy) + `no-wait.d/45-noid-wireguard-mtu` (event-only; top-level `45-noid-wireguard-mtu` symlink) | Lower-only live-interface correction from every resolved peer's real outer route; never edits or reconnects an owning profile |
...
if verify_owned_regular /usr/local/sbin/noid-wireguard-mtu-reconcile 755 &&
   ...
   grep -qF 'interfaces=$("$WG" show interfaces 2>/dev/null) || return 2' \
        /usr/local/sbin/noid-wireguard-mtu-reconcile &&
   grep -qF 'mark=$(wireguard_fwmark "$iface") || {' \
        /usr/local/sbin/noid-wireguard-mtu-reconcile &&
