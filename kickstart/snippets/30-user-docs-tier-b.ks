...
# Status: LOCKED 2026-09-06 (v60) — stop dependent recovery actions after failed prerequisites.
#
# Ships:
...
# Why was a specific operation denied? (uses audit2why)
...
case "$ORIGINAL_KPTR" in
    0|1|2) ;;
    *) echo 'Cannot determine the original pointer policy.' >&2; exit 1 ;;
esac
trap 'sudo sysctl -w "kernel.kptr_restrict=$ORIGINAL_KPTR"' EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
...
runtime with permanent state. A reload replaces runtime-only policy with the
reviewed permanent configuration and can change connectivity:
...
# Local daemon state; an absent unit is reported explicitly.
...
address, protocol, process and active firewall zone. A timed-out discovery
client does not establish that Avahi is inactive or that LAN isolation works.
...
Install only the drivers you need, then use the supported toggle:

```bash
sudo dnf install cups-pdf hplip gutenprint-cups bluez-cups

# The toggle unmasks the three core units required for direct printer setup
# (cups.service, cups.socket, cups.path) and enables socket/path activation,
# so cupsd starts only on demand.
# cups-browsed.service is M05's fourth CUPS-family mask; it stays masked unless
# you deliberately install cups-browsed and accept network-printer discovery.
sudo noid-toggle-printing on
# `sudo noid-toggle-printing off` restores the NoID Privacy default.
...
```bash
# Printing
sudo dnf install cups-pdf hplip gutenprint-cups
# Then: sudo noid-toggle-printing on
...
Clock skew is one possible cause; inspect Firefox's exact error and the clock
before changing time or certificate policy. NTS requires a system clock within
the certificate-validity window to validate its TLS connection. Check:
```bash
...
- [chrony 4.9 chronyd(8)](https://chrony-project.org/doc/4.9/chronyd.html)
...
ip -4 rule show                            # policy routing selects a table
ip -4 route show table all                  # includes VPN tables beyond main
