...
# Status: LOCKED 2026-09-06 (v27) — describe IPv6 autoconfiguration risks within the physical support boundary.
#
# Covers:
...
#   5. User-doc 07-physical-ipv6-boundary.md — v1.8 support boundary; no
...
# (Module 02). Physical IPv6 is not a release-qualified v1.8 mode.
...
# ====================================================================
# Physical IPv6 is disabled at multiple independent layers. v1.8 does not have
...
NoID Privacy v1.8 does not support physical-WAN IPv6 as one coherent,
...
An enabled IPv6 stack may process Router Advertisements or DHCPv6 information
even when the user intends to use only IPv4. Whether that changes addresses,
routes or DNS depends on the host's actual autoconfiguration policy. A VPN also
needs an explicit IPv6 routing and filtering contract to prevent traffic from
leaving beside its tunnel. These are configuration risks, not an inherent flaw
in IPv6. VPN-internal IPv6 stays supported — only the physical carrier remains
IPv4.
...
custom fork of all affected modules and accepting that it is outside the v1.8
