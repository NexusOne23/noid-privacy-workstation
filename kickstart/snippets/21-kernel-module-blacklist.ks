...
# Status: LOCKED 2026-09-17 (v49) — retire the appletalk deny entry: Linux 7.2 removed the module upstream, so it is now absent inventory.
#
# Covers:
...
#     for the 52 canonical modules that are actually loadable in the target
...
#     from the completed installed topology outside the login critical path. The
...
EXPECTED_DENY_COUNT=52
EXPECTED_BUILTIN_COUNT=8
EXPECTED_ABSENT_COUNT=44
...
resolve_source_bls() {
    ...
    if ! output=$(grep -F -l -x -- "version $kernel" \
            /boot/loader/entries/*.conf 2>/dev/null); then
        log "cannot find or completely read BLS entries for running kernel $kernel"
        return 1
    fi
...
# portability. The enabled firstboot timer schedules host-only convergence for
# the installed target without making normal login wait for Dracut completion.
...
- 52 `deny-loadable`: currently shipped as canonical loadable modules and
  enforced with both `blacklist` and `install /bin/false`
- 8 `unaffected-builtin`: present in the kernel, but outside modprobe policy
- 44 `unaffected-absent`: not shipped by the target kernel
...
The 52 canonical loadable modules are:

`9p`, `affs`, `atm`, `batman_adv`, `befs`, `ceph`, `cifs`,
...
workstation does not deploy version-fragile `initcall_blacklist=` guesses. The 44
absent identities are likewise inventory, not enforcement. `appletalk` joined
that class with Linux 7.2, which removed `net/appletalk` upstream and dropped
`CONFIG_ATALK` from the Fedora kernel configuration; the loadability gate
rejected the then-stale deny entry instead of emitting a declaration that no
longer matches a module.
...
script or third-party kernel hook can bypass it. The timer permits normal login
while M21 is running; only cooperating writers are serialized.

After host-only publication completes, this transition also preserves the
shutdown-hang fix: Fedora's `dracut-shutdown.service` restores `/run/initramfs`
from the current on-disk image when shutdown restoration is needed. On the
standard NoID Privacy layout, that is the published host-only image. The
validated simple-root image has no mdraid cleanup hook and avoids the
`waiting for mdraid devices to be clean` path. A shutdown before publication
still uses the earlier Generic image; scheduling the timer alone does not prove
that this fix is active.
