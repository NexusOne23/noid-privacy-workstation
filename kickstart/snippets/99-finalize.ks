...
# Status: LOCKED 2026-09-21 (v180) — verify the shared native Thunderbird compatibility contract and recovery path.
#
...
# bluetooth.service stays unmasked so explicit user opt-in remains possible;
# M08 owns the radio-block default. Verified via tests/08-mask-list-structural.sh
...
if [ "$(stat -c '%U:%G:%a' /etc/tmpfiles.d/90-noid-permission-policy.conf 2>/dev/null || true)" != root:root:644 ] \
   ...
   || [ "$(grep -c '^post_transaction:' /etc/dnf/libdnf5-plugins/actions.d/noid-permission-policy.actions 2>/dev/null || true)" -ne 6 ]; then
    log "  FAIL: Module 10 native permission policy/action metadata or trigger count invalid"
    fail=$((fail + 1))
fi
for permission_pkg in util-linux shadow-utils glusterfs-fuse cronie crontabs sudo; do
    if ! grep -qxF \
...
if [ ! -x /usr/local/libexec/noid-bash-history-compact ] \
   ...
   || ! bash -n /etc/profile.d/98-noid-bash-history.sh 2>/dev/null \
   || ! bash -n /usr/local/libexec/noid-bash-history-compact 2>/dev/null \
   || ! bash -n /usr/local/bin/noid-toggle-bash-history 2>/dev/null \
...
if [ ! -f "$m11_offline_readiness_unit" ] \
   ...
   || ! grep -qxF \
        ...
        "$m11_readiness_helper" \
   || ! grep -qxF '    /usr/bin/flock --exclusive -w 300 9' \
        "$m11_readiness_helper" \
...
# BEGIN M99_AIDE_REPORT_CONTRACT
# M13's report writer and M42's exact prune must agree on invocation names.
if grep -qF 'AIDE_REPORT_PREFIX+=".$INVOCATION_ID"' \
        /usr/local/sbin/noid-aide-check.sh \
   && grep -qF 'aide-check-[0-9]{8}-[0-9]{6}\.([0-9a-f]{32}\.)?[A-Za-z0-9]{6}\.log' \
        /usr/local/bin/noid-misc-logs-prune.sh; then
    log "  OK: AIDE invocation report names and retention agree"
else
    log "  FAIL: AIDE report writer/retention contract differs"
    fail=$((fail + 1))
fi
# END M99_AIDE_REPORT_CONTRACT
...
if [ -L "$m16_ubo_xpi" ] \
   || [ "$(stat -Lc '%u:%g:%a:%h' "$m16_ubo_xpi" 2>/dev/null || true)" != \
        0:0:644:1 ] \
   || [ "$(stat -Lc '%s' "$m16_ubo_xpi" 2>/dev/null || true)" != 4650100 ] \
...
# rpmfusion-*-release is split out; accept the active fallback .repo state
...
# fallback writes usable /etc/yum.repos.d/rpmfusion-*.repo definitions with
# pinned local keys. A later codec transaction does not install release RPMs.
for pkg in rpmfusion-free-release rpmfusion-nonfree-release; do
    if rpm -q "$pkg" >/dev/null 2>&1; then
        :
    elif [ -f "/etc/yum.repos.d/${pkg%-release}.repo" ]; then
        :  # active fallback repository definition OK
    else
...
if [ -f "$M21_POLICY" ] && [ -f "$M21_CONFIG" ]; then
    ...
    if [ "$policy_rows" -eq 134 ] && [ "$deny_rows" -eq 52 ] && \
       [ "$builtin_rows" -eq 8 ] && [ "$absent_rows" -eq 44 ] && \
       [ "$alias_rows" -eq 2 ] && [ "$supported_rows" -eq 28 ] && \
       cmp -s "$expected_directives" "$actual_directives"; then
        log "  OK: Module 21 exact policy: 52 deny + 8 built-in + 44 absent + 2 aliases + 28 supported"
    else
...
if ! bash -n /usr/local/bin/noid-update-all.sh 2>/dev/null \
   ...
   || ! grep -qF 'BROWSER_EXTENSION_INVENTORY_PY' \
        /usr/local/bin/noid-update-all.sh 2>/dev/null \
   || ! grep -qF 'refresh_managed_dkim()' \
        /usr/local/bin/noid-update-all.sh 2>/dev/null \
   || ! test -x /usr/local/lib/noid-privacy/noid-thunderbird-compatibility \
   || ! python3 -c 'import pathlib; p=pathlib.Path("/usr/local/lib/noid-privacy/noid-thunderbird-compatibility"); compile(p.read_text(), str(p), "exec")' 2>/dev/null \
   || ! /usr/local/lib/noid-privacy/validate-webextension.py \
        /usr/share/noid-thunderbird/dkim_verifier.xpi dkim_verifier@pl - 0 \
        "$(rpm -q --qf '%{VERSION}' thunderbird)" 0 \
        /usr/share/noid-thunderbird/dkim-compatibility.json >/dev/null 2>&1 \
