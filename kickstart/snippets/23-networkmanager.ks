...
# Status: LOCKED 2026-09-03 (v51) — recover account scoping across wall-clock corrections.
#
# Covers:
...
#     a marker that is recent or lies in the future after a backward clock step
#     becomes eligible only after the same inode remains valid for 60 seconds
#     of boot-monotonic time
...
#     killswitch dispatchers (M06 20-/45-/50-/55-/58-/60-; retired 70- absent) ·
...
#     (20-/45-/50-/55-/58-/60-). The retired 70-pvpn-killswitch-dns-fix name is
...
WAIT_STATE="${NOID_NM_SCOPE_WAIT_STATE:-/run/noid-privacy/nm-scope-account-wait.state}"
UPTIME_FILE="${NOID_NM_SCOPE_UPTIME_FILE:-/proc/uptime}"
...
boot_monotonic_cs() {
    local up _rest
    read -r up _rest < "$UPTIME_FILE" 2>/dev/null || return 1
    [[ "$up" =~ ^([0-9]+)[.]([0-9]{2})$ ]] || return 1
    printf '%s\n' "$((10#${BASH_REMATCH[1]} * 100 + 10#${BASH_REMATCH[2]}))"
}

valid_wait_state() {
    [ -f "$WAIT_STATE" ] && [ ! -L "$WAIT_STATE" ] \
        && [ "$(stat -c '%u:%g:%a:%h:%F' -- "$WAIT_STATE" 2>/dev/null)" = \
             "$STATE_UID:$STATE_GID:600:1:regular file" ]
}

valid_wait_dir() {
    local wait_dir=${WAIT_STATE%/*}
    [ -d "$wait_dir" ] && [ ! -L "$wait_dir" ] \
        && [ "$(stat -c '%u:%g:%a:%F' -- "$wait_dir" 2>/dev/null)" = \
             "$STATE_UID:$STATE_GID:755:directory" ]
}

remove_wait_state() {
    if path_exists "$WAIT_STATE"; then
        valid_wait_state || return 1
        rm -f -- "$WAIT_STATE"
    fi
}

write_wait_state() {
    local uid="$1" device="$2" inode="$3" mtime="$4" started="$5"
    local wait_dir tmp
    wait_dir=${WAIT_STATE%/*}
    valid_wait_dir || return 1
    if path_exists "$WAIT_STATE" && ! valid_wait_state; then
        return 1
    fi
    tmp=$(mktemp "${WAIT_STATE}.XXXXXX") || return 1
    if ! chmod 0600 "$tmp" \
       || ! printf '%s\n' \
            'NOID-NM-SCOPE-WAIT-V1' \
            "UID=$uid" \
            "DEVICE=$device" \
            "INODE=$inode" \
            "MTIME=$mtime" \
            "STARTED_CS=$started" > "$tmp" \
       || ! mv -fT -- "$tmp" "$WAIT_STATE"; then
        rm -f -- "$tmp"
        return 1
    fi
    valid_wait_state
...
load_wait_state() {
    local -a lines=()
    WAIT_UID="" WAIT_DEVICE="" WAIT_INODE="" WAIT_MTIME="" WAIT_STARTED_CS=""
    valid_wait_state || return 1
    mapfile -t lines < "$WAIT_STATE" || return 1
    [ "${#lines[@]}" -eq 6 ] \
        && [ "${lines[0]}" = 'NOID-NM-SCOPE-WAIT-V1' ] \
        && [[ "${lines[1]}" =~ ^UID=([0-9]+)$ ]] \
        || return 1
    WAIT_UID=${BASH_REMATCH[1]}
    [[ "${lines[2]}" =~ ^DEVICE=([0-9]+)$ ]] || return 1
    WAIT_DEVICE=${BASH_REMATCH[1]}
    [[ "${lines[3]}" =~ ^INODE=([0-9]+)$ ]] || return 1
    WAIT_INODE=${BASH_REMATCH[1]}
    [[ "${lines[4]}" =~ ^MTIME=([0-9]+)$ ]] || return 1
    WAIT_MTIME=${BASH_REMATCH[1]}
    [[ "${lines[5]}" =~ ^STARTED_CS=(0|[1-9][0-9]{0,17})$ ]] || return 1
    WAIT_STARTED_CS=${BASH_REMATCH[1]}
}
...
if ! monotonic_now=$(boot_monotonic_cs); then
    logger -t noid-nm-scope \
        "FAIL: cannot obtain boot-monotonic account-completion time"
    exit 1
fi
if path_exists "$WAIT_STATE"; then
    if ! load_wait_state; then
        logger -t noid-nm-scope \
            "FAIL: unsafe account-completion wait evidence"
        exit 1
    fi
    wait_state_loaded=1
fi
while IFS=: read -r name _ uid _ _ home shell; do
    ...
    case "$shell" in */false|*/nologin) continue ;; esac
    marker="$home/.config/gnome-initial-setup-done"
    # Capture owner, device, inode, mtime and type in one checked lstat
    # snapshot. Device+inode+mtime bind the boot-monotonic fallback to the
    # same completion evidence; replacing or rewriting the marker restarts
    # the stability window instead of inheriting elapsed time.
    ...
    if ! marker_stat=$(stat -c '%u:%d:%i:%Y:%F' -- "$marker" 2>/dev/null); then
        continue
    fi
    IFS=: read -r marker_uid marker_device marker_inode marker_mtime \
        marker_type marker_extra \
        <<< "$marker_stat"
    [ -z "$marker_extra" ] || continue
    case "$marker_type" in
        "regular file"|"regular empty file") ;;
    ...
    [[ "$marker_uid" =~ ^[0-9]+$ ]] && \
        ...
        [[ "$marker_mtime" =~ ^[0-9]+$ ]] || continue
    [ "$marker_uid" = "$uid" ] || continue
    marker_bound_to_wait=0
    if [ "$wait_state_loaded" -eq 1 ] \
       && [ "$WAIT_UID" = "$uid" ] \
       && [ "$WAIT_DEVICE" = "$marker_device" ] \
       && [ "$WAIT_INODE" = "$marker_inode" ] \
       && [ "$WAIT_MTIME" = "$marker_mtime" ]; then
        marker_bound_to_wait=1
    fi
    # Once a recent/future marker enters a stability window, a later forward
    # wall-clock correction must not bypass the remaining monotonic delay.
    if [ "$marker_bound_to_wait" -eq 0 ] \
       && [ "$now" -ge "$marker_mtime" ]; then
        marker_age=$((now - marker_mtime))
    ...
    if [ -z "$pending_name" ]; then
        pending_name=$name
        pending_uid=$uid
        pending_device=$marker_device
        pending_inode=$marker_inode
        pending_mtime=$marker_mtime
    fi
done <<< "$passwd_db"

if [ -z "$user_name" ]; then
    if [ -z "$pending_name" ]; then
        if ! remove_wait_state; then
            logger -t noid-nm-scope \
                "FAIL: unsafe account-completion wait evidence"
            exit 1
        fi
    ...
    if [ "${WAIT_UID:-}" != "$pending_uid" ] \
       || [ "${WAIT_DEVICE:-}" != "$pending_device" ] \
       || [ "${WAIT_INODE:-}" != "$pending_inode" ] \
       || [ "${WAIT_MTIME:-}" != "$pending_mtime" ] \
       || [ "${WAIT_STARTED_CS:-}" -gt "$monotonic_now" ] 2>/dev/null; then
        if ! write_wait_state "$pending_uid" "$pending_device" \
                "$pending_inode" "$pending_mtime" "$monotonic_now"; then
            logger -t noid-nm-scope \
                "FAIL: cannot publish account-completion wait evidence"
            exit 1
        fi
        logger -t noid-nm-scope \
            "pending: account-completion marker entered a monotonic stability window"
        exit 0
    fi
    monotonic_age=$((monotonic_now - WAIT_STARTED_CS))
    if [ "$monotonic_age" -lt 6000 ]; then
        remaining=$(((6000 - monotonic_age + 99) / 100))
        logger -t noid-nm-scope \
            "pending: account-completion marker monotonic stability window has ${remaining}s remaining"
        exit 0
    fi
    user_name=$pending_name
    logger -t noid-nm-scope \
        "account-completion marker accepted after monotonic clock-recovery window"
fi

if ! remove_wait_state; then
    logger -t noid-nm-scope "FAIL: cannot retire account-completion wait evidence"
    exit 1
fi
