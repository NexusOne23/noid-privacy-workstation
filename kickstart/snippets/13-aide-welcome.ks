...
# Status: LOCKED 2026-09-17 (v194) — advance the reviewed Claude Code seeds to 2.1.267 and the Codex seeds to 0.154.0 / 26.5908.31748.
#
# Covers:
...
#     user actions produce expected drift · 10 retention-path excludes
#     (/var/log/{aide,anaconda,ks-*.log,libvirt,tuned,swtpm/libvirt/qemu} plus
#     the exact one-shot compose/firstboot logs; see AIDE_EOF) · gpg-agent lockfile
...
# image version.
# Retired helper paths stay listed on purpose (for example the M19-retired
# 55-noid-gsk-renderer generator): AIDE reports a resurrected file as
# `added`, which is the intended watch for a retired privileged path.
SECURE_PATHS=(
    /etc/modprobe.d/
    /etc/udev/rules.d
    /etc/dracut.conf.d/
...
#
# Pattern parallel to the boot.log fix above: Fedora default aide.conf
...
The script handles 9 main steps: Snapper pre-snapshot; DNF upgrade (including
the NVIDIA akmod + dracut rebuild on a kernel change); Flatpak update; firmware
(interactive prompt); Firefox and Thunderbird NoID Privacy hardening re-apply
(plus the marketplace refresh of the pinned uBlock Origin / DKIM Verifier
add-ons); opted-in Claude/Codex CLIs, editor extensions and the managed GNOME
extension; repo gpgcheck audit; AIDE check-only evidence against the
user-owned baseline; reboot check. Total runtime 5-15 minutes depending on
updates available.
...
#   - if monotonic elapsed >= max_wait: timeout → return (caller fires notify
#     anyway); NTP/RTC wall-clock corrections cannot extend the bounded wait

noid_boot_uptime_seconds() {
...
noid_wait_fedora_welcome() {
    ...
    # /proc/uptime is boot-monotonic. A malformed/unavailable kernel clock is
    # non-fatal because this helper only sequences a best-effort notification.
    start=$(noid_boot_uptime_seconds) || return 0

    while :; do
        now=$(noid_boot_uptime_seconds) || return 0
        elapsed=$((now - start))
...
def act_run_updates(_row):
    """Launch the user-operated update path after the VPN setup group.
    Launches the same noid-update-all-launcher.sh that
...
def _xdg_app_dirs():
    ...
    paths = [user_data / 'applications']
    for d in GLib.get_system_data_dirs():
        if Path(d).is_absolute():
            paths.append(Path(d) / 'applications')
    # Flatpak exports (user + system)
    paths.append(user_data / 'flatpak' / 'exports' / 'share' / 'applications')
    ...
    for app_dir in _xdg_app_dirs():
        for f in app_dir.glob('*.desktop'):
            if f.name in autostart_files or f.name in seen_ids:
                continue
            # A higher-priority Hidden/NoDisplay entry suppresses the same ID
            # below it; filtering it must not resurrect the vendor launcher.
            seen_ids.add(f.name)
...
class WelcomeWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        # mic, cam and location are NOT in Module 17's dconf locks file;
        # autorun, thumbnailers and remote desktop are locked. User toggles
        ...
        hw.set_description(
            'Manage microphone capture, GNOME camera permissions, '
            'Bluetooth, and location services. Re-enable features '
        ...
        cam_switch.set_subtitle(
            'Disable camera access for apps that honor GNOME permissions')
        ...
        if not has_nvidia():
            ...
            nvidia_row.set_subtitle(
                'Already installed — NVIDIA CUDA library is present')
        opt.add(nvidia_row)
        page.add(opt)
...
CLAUDE_SHA256="0399c793ff571d5946ef923d80b4f330d05ac4b6842a6b0775468f5d389403c0"
CLAUDE_SIZE="217013744"
CLAUDE_URL="https://downloads.claude.ai/claude-code-releases/2.1.267/linux-x64/claude"
EXT_VERSION="2.1.267"
EXT_SHA256="b3ab8c245d50f5b605bea2ca45eaea6532fb5738c56aff81cc8f4f9044ed8fdd"
EXT_SIZE="101307826"
...
CODEX_SIZE="126585776"
CODEX_URL="https://github.com/openai/codex/releases/download/rust-v0.154.0/codex-package-x86_64-unknown-linux-musl.tar.gz"
...
EXT_VERSION="26.5908.31748"
EXT_SHA256="7a359b93b200e4406eb19858a48fa7e2da6ed557d3579ad8ec6b63ee4d354b45"
EXT_SIZE="249969698"
...
if [ "$ACTION" = uninstall ]; then
    ...
    if [ -e "$REPO_FILE" ] || [ -L "$REPO_FILE" ]; then
        sudo rm -f -- "$REPO_FILE" || fail "Repository file removal failed."
        [ ! -e "$REPO_FILE" ] && [ ! -L "$REPO_FILE" ] \
            || fail "Repository file remains after removal."
        fmt_ok "repo removed"
    else
        fmt_info "no repo file"
    fi
...
Options:
  ...
  -s, --settle SECONDS    additional pause after the link appears, for hosts
                          ...
                          (default 0)
                          Durations are decimal 0..86400 seconds;
                          invalid values use their respective defaults.
...
now_cs() {
    ...
    [[ "$up" =~ ^([0-9]{1,12})\.([0-9]{2})$ ]] || return 1
    printf '%s' "$((10#${BASH_REMATCH[1]} * 100 + 10#${BASH_REMATCH[2]}))"
}

# Every query shares the overall deadline. A stuck client cannot consume an
# unlimited wait or multiply the timeout by the number of network devices.
nm_query() {
    local current_cs remaining_cs budget
    current_cs=$(now_cs) || return 1
    remaining_cs=$((deadline_cs - current_cs))
    [ "$remaining_cs" -gt 0 ] || return 1
    printf -v budget '%d.%02ds' "$((remaining_cs / 100))" "$((remaining_cs % 100))"
    timeout --signal=TERM --kill-after=1s "$budget" nmcli "$@" 2>/dev/null
}
...
GATE_DEVICE=
physical_link_up() {
    local dev typ state devices found=1
    GATE_DEVICE=
    devices=$(nm_query -t -f DEVICE,TYPE device status) || return 1
    ...
    while IFS=: read -r dev typ; do
        ...
        state=$(nm_query -g GENERAL.STATE device show "$dev") || return 1
        state=${state%% *}
        if [ "$state" = "100" ]; then
            GATE_DEVICE=$dev
            found=0
...
# Bound before arithmetic; leading zeroes are decimal, never Bash octal.
normalize_duration() {
    local value=$1 fallback=$2
    if [[ "$value" =~ ^[0-9]{1,6}$ ]] && [ "$((10#$value))" -le 86400 ]; then
        printf '%s\n' "$((10#$value))"
    else
        printf '%s\n' "$fallback"
    fi
}
timeout_s=$(normalize_duration "$timeout_s" "$DEFAULT_TIMEOUT")
settle_s=$(normalize_duration "$settle_s" "$DEFAULT_SETTLE")
...
if [ "$check_only" -eq 1 ]; then
    ...
    if ! command -v timeout >/dev/null 2>&1 || ! started_cs=$(now_cs); then
        echo "network readiness probe unavailable"
        exit 1
    fi
    deadline_cs=$((started_cs + 300))
...
if [ "$timeout_s" -gt 0 ] && command -v nmcli >/dev/null 2>&1 \
   && command -v timeout >/dev/null 2>&1 && started_cs=$(now_cs); then
    deadline_cs=$(( started_cs + timeout_s * 100 ))
    waited=0

    while :; do
        if physical_link_up; then
            current_cs=$(now_cs) || break
            elapsed_ms=$(( ( current_cs - started_cs ) * 10 ))
            if [ "$waited" -eq 1 ]; then
...
read_platform_status() {
    ...
    case "$cpu_vendor:$mei_core_policy:$mei_state" in
        intel:loadable:mei-me-bound|intel:loadable:available-not-loaded|intel:loadable:absent|intel:loadable:loaded-unbound|intel:loadable:unknown|intel:blacklisted:mei-me-bound|intel:blacklisted:loaded-unbound)
            [ "${#seen[@]}" -eq 10 ] \
                && [[ "$mei_regression" =~ ^(yes|no)$ ]] \
                && [[ "$mei_submodules" =~ ^(none|hdcp(,pxp)?(,wdt)?|pxp(,wdt)?|wdt)$ ]] \
                && [[ "$kt_state" =~ ^(enforced|no-device-present|failed)$ ]] \
                && [[ "$fwupd_state" =~ ^(available-platform-results-vary|conditional|unavailable|unknown)$ ]] \
                || invalid=1
            case "$mei_state:$fwupd_state:$mei_regression" in
                mei-me-bound:available-platform-results-vary:no|loaded-unbound:conditional:no|available-not-loaded:conditional:no|absent:unavailable:yes|absent:unavailable:no|unknown:unknown:no) ;;
                *) invalid=1 ;;
            esac
            ;;
...
#         report file via --before='report_url=file:…'. Scheduled reports also
#         carry systemd's invocation ID; creation time can follow a long mutex
#         wait and cannot identify the recorded service start by itself.
...
# Keep every report within M42's exact filename grammar. Direct CLI checks
# without a systemd invocation retain the legacy timestamp/random format.
AIDE_REPORT_STAMP=$(date +%Y%m%d-%H%M%S)
[[ "$AIDE_REPORT_STAMP" =~ ^[0-9]{8}-[0-9]{6}$ ]] || {
    echo "noid-aide-check: invalid report timestamp" >&2
    exit 14
}
AIDE_REPORT_PREFIX="$LOG_DIR/aide-check-$AIDE_REPORT_STAMP"
if [ -n "${INVOCATION_ID:-}" ]; then
    [[ "$INVOCATION_ID" =~ ^[0-9a-f]{32}$ && "$INVOCATION_ID" =~ [1-9a-f] ]] || {
        echo "noid-aide-check: invalid service invocation identity" >&2
        exit 14
    }
    AIDE_REPORT_PREFIX+=".$INVOCATION_ID"
fi
AIDE_CHECK_LOG=$(mktemp "$AIDE_REPORT_PREFIX.XXXXXX.log")
...
# Usage must not require privilege: answer help before the root gate.
case "${1:-}" in
...
chmod 0755 /usr/local/sbin/noid-aide-baseline-review
...
echo "  [OK] /usr/local/sbin/noid-aide-baseline-review installed (0755)"
...
/usr/local/sbin/noid-aide-baseline-review|755
...
# Scheduled report identity must survive the wait/coverage boundary, while
# mktemp retains a private unique report for each execution.
if bash -n /usr/local/sbin/noid-aide-check.sh \
   && grep -qF 'AIDE_REPORT_PREFIX+=".$INVOCATION_ID"' \
        /usr/local/sbin/noid-aide-check.sh \
   && grep -qF 'AIDE_CHECK_LOG=$(mktemp "$AIDE_REPORT_PREFIX.XXXXXX.log")' \
        /usr/local/sbin/noid-aide-check.sh; then
    echo "  [OK] AIDE report writer preserves scheduled invocation identity"
else
    echo "  [FAIL] AIDE report writer identity/allocation contract invalid"
    fail=$((fail + 1))
fi
...
# 10.0c — mutable security controls must never be hidden as runtime noise.
