...
[![Version](https://img.shields.io/badge/version-v1.8-blue.svg)](CHANGELOG.md)
[![Tests](https://img.shields.io/badge/tests-85%2F85%20pass-brightgreen.svg)](tests/)
...
(
set -euo pipefail
gpg --batch --no-options --import noid-privacy-release.asc
gpg --batch --no-options --status-fd=1 --verify SHA256SUMS.asc SHA256SUMS |
  awk -v fpr="1ACBFCE49687FEBB91010E52F8E3F11D6962256F" '
    ...
    END {
      if (total==1 && valid==1 && good==1 && !bad) exit 0
      print "ERROR: expected one valid signature from the release key" > "/dev/stderr"
      exit 1
    }
  '
...
Confirm this fingerprint through an independent channel before trusting the
downloaded key. Run the complete Bash block: it requires exactly one good
signature from that primary key, rejects expired signatures and expired or revoked keys,
and stops before checksum verification if key import or authentication fails.
...
git -C ../noid-privacy-linux checkout 706cb0a92a715181d47a7176f104389ab21d7e73
NOID_AUDIT_SRC=../noid-privacy-linux/noid-privacy-linux.sh bash tests/run-all.sh  # 85 structural tests
...
- **85 structural + 4 smoke regression tests** covering critical module invariants —
  current state **85/85 PASS**.
...
| **Build host** | Fedora 44 with exact `lorax-44.7-1.fc44.x86_64` (enforced by `scripts/stage-lorax-overrides.sh`) and `lorax-lmc-virt` for the default KVM build. Development-only `--no-virt` additionally needs `lorax-lmc-novirt` and is restricted to an SELinux-Enforcing, UEFI-booted virtualized build host. |
...
hardening patterns), [KSPP](https://kspp.github.io/)
