...
the default-drop path. Non-Ethernet links retain the documented L3/firewalld/
WAN-strict fallback with `DEGRADED` health; that fallback does not enforce the
Ethernet XDP raw-packet boundary. See the
[hardware and topology matrix](../../docs/hardware-network-compatibility.md).
