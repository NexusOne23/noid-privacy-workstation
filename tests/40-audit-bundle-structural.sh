...
assert_grep_fixed '| `main` (v1.8 release line) | :white_check_mark: (active maintained line) |' \
    "$SECURITY_POLICY" \
    "security policy identifies main as the active maintained release line"
assert_grep_fixed '| v1.8 | :white_check_mark: (current Fedora 44 release line) |' \
    "$SECURITY_POLICY" \
...
assert_cmd_failure "security policy does not retire the current supported release" \
    grep -qF -- '| v1.8 and older |' "$SECURITY_POLICY"
assert_grep_fixed 'v1.8/Fedora 44 release and the `main` release line' \
    "$SECURITY_POLICY" \
...
assert_grep_fixed '**2026-09-02**. The NoID Privacy Workstation row was refreshed against the' \
    "$COMPARISON_DOC" \
    "comparison document separates the peer and local verification dates"
assert_grep_fixed 'local v1.8 release source on **2026-09-02**:' \
    "$COMPARISON_DOC" \
    "comparison document dates the current local release-source review"
assert_grep_fixed '| NoID Privacy Workstation | local v1.8 release source |' \
    "$COMPARISON_DOC" \
    "comparison document identifies the local v1.8 release source"
...
assert_not_grep_extended 'v1\.8 (development|is not (published|released))' \
    "$SECURITY_POLICY" \
    "security policy has no stale v1.8 pre-release label"
assert_not_grep_extended 'v1\.8 (development|is not (published|released))' \
    "$COMPARISON_DOC" \
    "comparison document has no stale v1.8 pre-release label"
...
assert_grep_extended "^AUDITOR_SOURCE_VERSION='v[0-9]+\\.[0-9]+\\.[0-9]+'$" \
    "$SUPPORT_BUILDER" "offline support media identifies the pinned release tag"
...
assert_cmd_success "CI fetches tags for the exact auditor checkout" \
    awk '
        /^[[:space:]]*- name: Check out exact M40 auditor source$/ { inside=1; next }
        inside && /^[[:space:]]*- (name:|uses:)/ { inside=0 }
        inside && $0 == "          fetch-tags: true" { found=1 }
        END { exit !found }
    ' "$CI_WORKFLOW"
assert_cmd_success "CI pin extraction and structural suite run inside the primary source" \
    ...
    ' "$CI_WORKFLOW"
assert_grep_fixed 'NOID_AUDIT_SRC: /var/tmp/noid-ci-workspace/noid-privacy-linux/noid-privacy-linux.sh' \
    "$CI_WORKFLOW" "CI supplies the exact sibling path to the structural suite"
assert_grep_fixed 'runuser --user noid-ci -- bash -c' "$CI_WORKFLOW" \
    "CI executes desktop and permission fixtures as an ordinary user"
assert_grep_fixed 'cd /var/tmp/noid-ci-workspace/noid-privacy-fedora && exec bash tests/run-all.sh' \
    "$CI_WORKFLOW" "CI executes the copied exact source tree"
# Test the declared tool set without depending on line wrapping or ordering.
assert_cmd_success "full source suite declares every mandatory native tool" \
    python3 - "$RUN_ALL" <<'PREFLIGHT_TOOLS_EOF'
...
loops = re.findall(r"for tool in (.*?); do", source, flags=re.S)
assert len(loops) == 1, "ambiguous prerequisite loop"
tools = shlex.split(loops[0].replace("\\\n", " "))
assert all(re.fullmatch(r"[a-z][a-z0-9_-]*", tool) for tool in tools)
assert len(tools) == len(set(tools)), "duplicate prerequisite"
required = set("""aide base64 bwrap checkmodule clang dbus-run-session dconf
 desktop-file-validate git gsettings gzip jq ksvalidator modinfo openssl patch
 pgrep pipewire pw-cli pw-dump python3 semodule_package sha256sum setfacl
 shellcheck spa-json-dump strip systemd-analyze systemd-tmpfiles udevadm
 usbguard usbguard-notifier usbguard-rule-parser visudo wireplumber wpctl
 xz zstd""".split())
assert required <= set(tools), "missing native tools: " + ", ".join(sorted(required - set(tools)))
PREFLIGHT_TOOLS_EOF
...
assert_cmd_success "semantic CI job uses Fedora 44 and installs its exact prerequisites" \
    awk '
        ...
        in_tests && /systemd-udev usbguard usbguard-tools usbguard-notifier clang libbpf-devel kernel-headers binutils/ {
            packages_c=1
        }
        in_tests && /dbus-daemon pipewire pipewire-utils wireplumber/ {
            packages_audio=1
        }
        END {
            exit(seen_tests && container && shell && packages_a && packages_b && packages_c && packages_audio ? 0 : 1)
        }
    ' "$CI_WORKFLOW"
...
support_commit=$(sed -n \
    ...
    "$SUPPORT_BUILDER")
support_version=$(sed -n \
    "s/^AUDITOR_SOURCE_VERSION='\\(v[0-9.]*\\)'$/\\1/p" "$SUPPORT_BUILDER")
support_size=$(sed -n \
...
assert_eq "$ks_version" "$support_version" \
    "support-media and install-stage auditor release tags match"
...
assert_grep_fixed 'create_auditor_bundle "$STAGE/noid-privacy-linux.bundle"' \
    "$SUPPORT_BUILDER" \
    "support media carries the pinned auditor repository history"
assert_grep_fixed 'auditor bundle must advertise only pinned HEAD and its release tag' \
    "$SUPPORT_BUILDER" "auditor bundle binds the exact commit and release-tag refs"
assert_grep_fixed 'is not the pinned commit' "$SUPPORT_BUILDER" \
    "auditor bundle creation verifies the checkout position itself"
...
assert_grep_fixed 'auditor bundle checkout release tag drifted' "$SUPPORT_BUILDER" \
    "auditor release tag must survive the offline clone"
...
# Execute the workflow's real suite-result evaluator. Only the suite launcher
# is replaced with bounded text/exit fixtures; Bash, tee and log parsing stay
# native. Losing the log must fail even when the suite itself returned zero.
ci_fixture="$TMPDIR/ci-evaluation"
mkdir -p "$ci_fixture"
awk '
    /^      - name: Run all structural tests$/ { step=1; next }
    step && /^        run: \|$/ { copy=1; next }
    copy && /^          / { sub(/^          /, ""); print; next }
    copy { exit }
' "$CI_WORKFLOW" > "$ci_fixture/workflow-step.sh"
assert_file_min_size "$ci_fixture/workflow-step.sh" 100 \
    "CI result fixture extracts the actual workflow step"
cat > "$ci_fixture/driver.sh" <<'CI_RESULT_DRIVER_EOF'
#!/bin/bash
runuser() {
    printf '%s\n' "$CI_FIXTURE_OUTPUT"
    return "$CI_FIXTURE_RC"
}
export -f runuser
exec bash "$1"
CI_RESULT_DRIVER_EOF
for ci_case in passed regression cleanup-regression harness-error log-write-error; do
    ci_case_root="$ci_fixture/$ci_case"
    mkdir -p "$ci_case_root"
    ci_expected=0
    ci_suite_rc=0
    ci_output='PASS  17-user-firstrun-fixture'
    case "$ci_case" in
        regression)
            ci_expected=1
            ci_suite_rc=1
            ci_output='FAIL  17-user-firstrun-fixture'
            ;;
        cleanup-regression)
            ci_expected=1
            ci_suite_rc=1
            ci_output='FAIL  41-anaconda-cleanup-structural'
            ;;
        harness-error)
            ci_expected=2
            ci_suite_rc=2
        ...
        log-write-error)
            ci_expected=1
            mkdir "$ci_case_root/run-all.log"
            ;;
    esac
    ci_rc=0
    ( cd "$ci_case_root" && CI_FIXTURE_RC="$ci_suite_rc" \
        CI_FIXTURE_OUTPUT="$ci_output" bash "$ci_fixture/driver.sh" \
        "$ci_fixture/workflow-step.sh" ) \
        > "$ci_case_root/result.log" 2>&1 || ci_rc=$?
    assert_eq "$ci_expected" "$ci_rc" "CI result evaluator: $ci_case"
done

# The runner's own verbose log is a separate boundary from the CI tee above.
# This fixture copies the real runner and executes only synthetic test scripts.
assert_cmd_success "test runner distinguishes test failures from lost logs" \
    python3 "$PROJECT_ROOT/tests/fixtures/test_runner.py" "$RUN_ALL"
...
# Exercise the production runtime guard with native filesystem metadata.
# Only SELinux label verification is stubbed for the isolated fixture paths.
# A private tmpfs root has the namespace's actual root ownership; the host's
# root UID is unmapped when a non-root caller creates this user namespace.
sed -n '/^trusted_root_dir() {$/,/^}$/p' "$TMPDIR/noid-audit" \
    > "$TMPDIR/runtime-parent-helper.sh"
cat > "$TMPDIR/runtime-parent-fixture.sh" <<'M40_RUNTIME_PARENT_FIXTURE_EOF'
#!/bin/bash
set -euo pipefail
READLINK=/usr/bin/readlink
STAT=/usr/bin/stat
MATCHPATHCON=/usr/bin/true
. /mnt/runtime-parent-helper.sh
mkdir -p /mnt/closed/child /mnt/open-world/child /mnt/open-group/child \
    /mnt/sticky/child
chmod 0755 /mnt/closed /mnt/closed/child /mnt/open-world/child \
    /mnt/open-group/child /mnt/sticky/child
chmod 0777 /mnt/open-world
chmod 0775 /mnt/open-group
chmod 1777 /mnt/sticky
ln -s /mnt/closed /mnt/linked
trusted_root_dir /mnt/closed/child 0:0:755
failures=0
for path in /mnt/open-world/child /mnt/open-group/child /mnt/sticky/child \
        /mnt/linked/child /mnt/closed//child /mnt/closed/../closed/child \
        mnt/closed/child /mnt/closed/missing; do
    if trusted_root_dir "$path" 0:0:755; then
        printf 'Unexpectedly trusted fixture path: %s\n' "$path" >&2
        failures=$((failures + 1))
    fi
done
if trusted_root_dir /mnt/closed/child 0:0:700; then
    printf '%s\n' 'Unexpectedly trusted mismatching leaf metadata' >&2
    failures=$((failures + 1))
fi
[ "$failures" -eq 0 ]
M40_RUNTIME_PARENT_FIXTURE_EOF
chmod 0700 "$TMPDIR/runtime-parent-fixture.sh"
if command -v bwrap >/dev/null 2>&1 && \
   bwrap --unshare-user --uid 0 --gid 0 --die-with-parent \
       --ro-bind / / --dev-bind /dev /dev --proc /proc /bin/true \
       >/dev/null 2>&1; then
    assert_cmd_success "runtime parent guard rejects writable ancestors with native metadata" \
        bwrap --unshare-user --uid 0 --gid 0 --die-with-parent \
            --tmpfs / --ro-bind /usr /usr --symlink usr/bin /bin \
            --symlink usr/lib /lib --symlink usr/lib64 /lib64 \
            --dev-bind /dev /dev --proc /proc \
            --bind "$TMPDIR" /mnt /mnt/runtime-parent-fixture.sh
else
    printf '%s\n' \
        '  [SKIP] runtime parent namespace fixture unavailable; structural gates retained'
fi
...
# The argument/environment fixtures below live under writable /var/tmp.
# Model only that shared ancestor as protected; the native namespace fixture
# above independently checks real ancestor permissions without this override.
cat > "$TMPDIR/fake-bin/stat" <<'FAKE_STAT_EOF'
#!/bin/bash
if [ "$#" -eq 4 ] && [ "$1" = -Lc ] && [ "$2" = '%u:%g:%a' ] \
        && [ "$3" = -- ] && [ "$4" = /var/tmp ]; then
    printf '%s\n' 0:0:755
else
    exec /usr/bin/stat "$@"
...
FAKE_STAT_EOF
chmod 0755 "$TMPDIR/fake-bin/stat"
sed -i "s|^readonly STAT=.*|readonly STAT=\"$TMPDIR/fake-bin/stat\"|" \
    "$TMPDIR/noid-audit"
...
# The support builder can only carry the pinned commit as the bundle's HEAD, so
# it requires this checkout to sit on that commit; CI provisions the sibling
# with `ref: <pinned commit>`. Assert that position on its own so an off-pin
# working copy is named here instead of surfacing as a bundle ref mismatch.
assert_eq "$ks_commit" \
    "$(git -C "$sibling_repo" rev-parse --verify HEAD)" \
    "sibling auditor checkout HEAD is at the pinned commit"
...
# Run the support builder's actual bundle creation and verification functions
# with native Git. This exercises the offline clone contract without building
# an ISO, downloading packages or invoking the auditor.
{
    sed -n '/^verify_auditor_bundle() {$/,/^}$/p' "$SUPPORT_BUILDER"
    sed -n '/^create_auditor_bundle() {$/,/^}$/p' "$SUPPORT_BUILDER"
} > "$TMPDIR/support-bundle-functions.sh"
assert_cmd_success "support bundle retains the pinned tag through a native offline clone" \
    bash -euo pipefail -c '
        . "$1/support-bundle-functions.sh"
        die() { printf "ERROR: %s\n" "$*" >&2; exit 1; }
        AUDITOR_REPO=$2 AUDITOR_SOURCE_COMMIT=$3 AUDITOR_SOURCE_VERSION=$4
        create_auditor_bundle "$1/auditor.bundle"
        git -c core.hooksPath=/dev/null clone --no-hardlinks --quiet \
            "$1/auditor.bundle" "$1/bundle-clone"
        [ "$(git -C "$1/bundle-clone" rev-parse --verify HEAD)" = "$3" ]
        [ "$(git -C "$1/bundle-clone" rev-parse --verify "refs/tags/$4^{commit}")" = "$3" ]
        [ -z "$(git -C "$1/bundle-clone" status --porcelain=v1 --untracked-files=all)" ]
        cmp -s "$2/noid-privacy-linux.sh" "$1/bundle-clone/noid-privacy-linux.sh"
    ' _ "$TMPDIR" "$sibling_repo" "$ks_commit" "$ks_version"
assert_cmd_success "support bundle verification rejects the former HEAD-only bundle" \
    bash -euo pipefail -c '
        . "$1/support-bundle-functions.sh"
        die() { printf "ERROR: %s\n" "$*" >&2; exit 1; }
        AUDITOR_REPO=$2 AUDITOR_SOURCE_COMMIT=$3 AUDITOR_SOURCE_VERSION=$4
        git -C "$2" bundle create "$1/head-only.bundle" HEAD
        set +e
        output=$(verify_auditor_bundle "$1/head-only.bundle" 2>&1)
        result=$?
        [ "$result" -eq 1 ] && [[ "$output" == *"only pinned HEAD and its release tag"* ]]
    ' _ "$TMPDIR" "$sibling_repo" "$ks_commit" "$ks_version"
# An off-pin checkout must be named by the creation guard itself and must not
# leave a bundle behind. Reproduce that case in a throwaway repository so the
# assertion holds regardless of where the real sibling checkout points.
assert_cmd_success "support bundle creation rejects an off-pin auditor checkout" \
    bash -euo pipefail -c '
        . "$1/support-bundle-functions.sh"
        die() { printf "ERROR: %s\n" "$*" >&2; exit 1; }
        repo="$1/offpin-auditor"
        git init -q -b main "$repo"
        git -C "$repo" -c user.email=t@invalid -c user.name=t \
            commit -q --allow-empty -m pinned
        pinned=$(git -C "$repo" rev-parse --verify HEAD)
        git -C "$repo" -c user.email=t@invalid -c user.name=t \
            tag -a v0.0.0 -m v0.0.0
        git -C "$repo" -c user.email=t@invalid -c user.name=t \
            commit -q --allow-empty -m later
        AUDITOR_REPO=$repo AUDITOR_SOURCE_COMMIT=$pinned AUDITOR_SOURCE_VERSION=v0.0.0
        set +e
        output=$(create_auditor_bundle "$1/offpin.bundle" 2>&1)
        result=$?
        set -e
        [ "$result" -eq 1 ] || exit 1
        case "$output" in *"is not the pinned commit"*) ;; *) exit 1 ;; esac
        [ ! -e "$1/offpin.bundle" ]
    ' _ "$TMPDIR"
...
assert_grep_fixed 'Scan completed:' "$TMPDIR/noid-privacy-linux.sh" \
    "exact auditor reports section completion in its summary"
assert_grep_fixed 'SECTIONS WITH INCOMPLETE SCORING EVIDENCE:' "$TMPDIR/noid-privacy-linux.sh" \
    "exact auditor distinguishes completed sections from incomplete evidence"
