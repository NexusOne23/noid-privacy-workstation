...
assert_grep_fixed "EXPECTED_NEVRA='lorax-44.7-1.fc44.x86_64'" \
...
assert_grep_fixed "EXPECTED_CREATOR_SHA256='1c21bbecb29d8c8d561e2cce47d05d5aab5caba75ab0d59b0d0b5e74c77266ff'" \
...
assert_cmd_success "Lorax staging rejects incomplete inventories and unreadable destinations" \
    python3 "$PROJECT_ROOT/tests/fixtures/lorax_inventory.py" \
        "$LORAX_STAGE" "$LORAX_TEMPLATE_STAGE"
...
assert_grep_fixed "EXPECTED_NEVRA='lorax-templates-generic-44.7-1.fc44.x86_64'" \
...
assert_grep_fixed '# BEGIN SELINUX_KERNEL_POLICY_SCHEMA_GATE' \
    "$ENFORCING_AVC_GATE" "installed SELinux gate owns a bounded kernel/policy schema check"
assert_grep_fixed 'visit_memfd_extension_paths observe_selinux_extension_path' \
    "$ENFORCING_AVC_GATE" "installed SELinux gate inventories the new memfd security class"
for extension_schema_permission in \
    ...
    'bpf prog_load_as' \
    'io_uring allowed'; do
    assert_grep_fixed "$extension_schema_permission" "$ENFORCING_AVC_GATE" \
        "installed SELinux gate inventories $extension_schema_permission"
done
for required_policy_capability in memfd_class bpf_token_perms; do
    assert_grep_fixed \
        "read_selinux_policy_capability $required_policy_capability" \
        "$ENFORCING_AVC_GATE" \
        "installed SELinux gate records $required_policy_capability"
done
assert_grep_fixed 'kernel-policy-baseline=complete' "$ENFORCING_AVC_GATE" \
    "installed SELinux success output names the hard schema postcondition"
assert_grep_fixed 'extension-coverage=$SELINUX_SCHEMA_COVERAGE' \
    "$ENFORCING_AVC_GATE" "installed SELinux output discloses extension coverage"
assert_grep_fixed 'the Fedora 44 kernel-facing schema baseline' \
    "$RELEASE_DOC" "release checklist blocks regressions against the supported baseline"
assert_grep_fixed 'partial vendor boundary' "$RELEASE_DOC" \
    "release checklist records the supported Fedora extension limitation"
...
SELINUX_SCHEMA_BLOCK="$TMPDIR/selinux-schema-gate.sh"
SELINUX_SCHEMA_ROOT="$TMPDIR/selinuxfs"
SELINUX_SCHEMA_LOG="$TMPDIR/selinux-schema.log"
sed -n '/^# BEGIN SELINUX_KERNEL_POLICY_SCHEMA_GATE$/,/^# END SELINUX_KERNEL_POLICY_SCHEMA_GATE$/p' \
    "$ENFORCING_AVC_GATE" \
    | sed "s|^SELINUXFS_ROOT=/sys/fs/selinux$|SELINUXFS_ROOT=$SELINUX_SCHEMA_ROOT|" \
    > "$SELINUX_SCHEMA_BLOCK"
assert_cmd_success "SELinux schema fixture extracts the production gate" \
    bash -n "$SELINUX_SCHEMA_BLOCK"

mkdir -p "$SELINUX_SCHEMA_ROOT/class/memfd_file/perms" \
    "$SELINUX_SCHEMA_ROOT/policy_capabilities"
sed -n "/^done <<'BASELINE_PERMISSIONS_EOF'$/,/^BASELINE_PERMISSIONS_EOF$/p" \
    "$ENFORCING_AVC_GATE" | sed '1d;$d' \
    | while IFS=' ' read -r security_class permission; do
        mkdir -p "$SELINUX_SCHEMA_ROOT/class/$security_class/perms"
        : > "$SELINUX_SCHEMA_ROOT/class/$security_class/perms/$permission"
    done
sed -n "/^[[:space:]]*done <<'MEMFD_PERMISSIONS_EOF'$/,/^MEMFD_PERMISSIONS_EOF$/p" \
    "$ENFORCING_AVC_GATE" | sed '1d;$d' \
    | while IFS= read -r permission; do
        : > "$SELINUX_SCHEMA_ROOT/class/memfd_file/perms/$permission"
    done
sed -n "/^done <<'KERNEL_PERMISSIONS_EOF'$/,/^KERNEL_PERMISSIONS_EOF$/p" \
    "$ENFORCING_AVC_GATE" | sed '1d;$d' \
    | while IFS=' ' read -r security_class permission; do
        mkdir -p "$SELINUX_SCHEMA_ROOT/class/$security_class/perms"
        : > "$SELINUX_SCHEMA_ROOT/class/$security_class/perms/$permission"
    done
# selinuxfs exposes these values without a trailing newline. Mirror that detail
# so a conventional `read` EOF status cannot regress into a false gate failure.
printf '1' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/memfd_class"
printf '1' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/bpf_token_perms"
printf '0' > "$SELINUX_SCHEMA_ROOT/deny_unknown"
printf '0' > "$SELINUX_SCHEMA_ROOT/reject_unknown"

assert_cmd_success "complete SELinux kernel/policy schema passes" bash -c '
    set -euo pipefail
    TEST_NAME=fixture
    . "$1"
    [ "$SELINUX_SCHEMA_COVERAGE" = complete ]
    [ "$HANDLE_UNKNOWN" = allow ]
' _ "$SELINUX_SCHEMA_BLOCK"

rm -rf "$SELINUX_SCHEMA_ROOT/class/memfd_file"
sed -n "/^done <<'KERNEL_PERMISSIONS_EOF'$/,/^KERNEL_PERMISSIONS_EOF$/p" \
    "$ENFORCING_AVC_GATE" | sed '1d;$d' \
    | while IFS=' ' read -r security_class permission; do
        rm -f "$SELINUX_SCHEMA_ROOT/class/$security_class/perms/$permission"
    done
printf '0' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/memfd_class"
printf '0' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/bpf_token_perms"
if bash -c 'set -euo pipefail; TEST_NAME=fixture; . "$1"' \
    _ "$SELINUX_SCHEMA_BLOCK" > "$SELINUX_SCHEMA_LOG" 2>&1; then
    _pass "current Fedora 44 partial extension coverage remains releasable"
else
    _fail "current Fedora 44 partial extension coverage remains releasable"
fi
assert_grep_fixed 'SELinux kernel-extension coverage is partial' \
    "$SELINUX_SCHEMA_LOG" \
    "current Fedora extension limitation is an explicit warning"

rm -f "$SELINUX_SCHEMA_ROOT/class/bpf/perms/prog_run"
if bash -c 'set -euo pipefail; TEST_NAME=fixture; . "$1"' \
    _ "$SELINUX_SCHEMA_BLOCK" > "$SELINUX_SCHEMA_LOG" 2>&1; then
    _fail "SELinux Fedora baseline regression fails closed"
else
    assert_grep_fixed \
        'regressed below the Fedora 44 schema baseline at class/bpf/perms/prog_run' \
        "$SELINUX_SCHEMA_LOG" \
        "SELinux baseline regression has an exact diagnostic"
fi
: > "$SELINUX_SCHEMA_ROOT/class/bpf/perms/prog_run"

printf '1' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/memfd_class"
if bash -c 'set -euo pipefail; TEST_NAME=fixture; . "$1"' \
    _ "$SELINUX_SCHEMA_BLOCK" > "$SELINUX_SCHEMA_LOG" 2>&1; then
    _fail "enabled SELinux capability with absent schema fails closed"
else
    assert_grep_fixed \
        'enables memfd_class but lacks its associated extension schema at class/memfd_file' \
        "$SELINUX_SCHEMA_LOG" \
        "enabled SELinux memfd capability cannot hide missing schema"
fi
printf '0' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/memfd_class"
...
if bash -c 'set -euo pipefail; TEST_NAME=fixture; . "$1"' \
    _ "$SELINUX_SCHEMA_BLOCK" > "$SELINUX_SCHEMA_LOG" 2>&1; then
    _fail "enabled SELinux BPF capability with absent schema fails closed"
else
    assert_grep_fixed \
        'enables bpf_token_perms but lacks its associated extension schema at class/bpf/perms/map_create_as' \
        "$SELINUX_SCHEMA_LOG" \
        "enabled SELinux BPF capability cannot hide missing schema"
fi
printf '0' > "$SELINUX_SCHEMA_ROOT/policy_capabilities/bpf_token_perms"

printf 'invalid\n' > "$SELINUX_SCHEMA_ROOT/deny_unknown"
if bash -c 'set -euo pipefail; TEST_NAME=fixture; . "$1"' \
    _ "$SELINUX_SCHEMA_BLOCK" > "$SELINUX_SCHEMA_LOG" 2>&1; then
    _fail "malformed SELinux unknown-handling state fails closed"
else
    assert_grep_fixed 'invalid SELinux deny_unknown value' \
        "$SELINUX_SCHEMA_LOG" \
        "malformed SELinux unknown-handling state has an exact diagnostic"
fi
...
python3 - "$LOG_POLICY" "$TMPDIR" <<'PY'
import copy
import json
import pathlib
import sys

policy_path, output = map(pathlib.Path, sys.argv[1:])
base = json.loads(policy_path.read_text(encoding="utf-8"))
for case in ("legacy", "floating", "null", "unknown", "success-row", "same-line"):
    policy = copy.deepcopy(base)
    shutdown = next(row for row in policy["allowed_events"]
                    if row["id"] == "anaconda_tmux_shutdown_exit")
    marker = next(row for row in policy["success_markers"]
                  if row["id"] == "m99_complete")
    if case == "legacy":
        policy["schema_version"] = 3
    elif case == "floating":
        policy["schema_version"] = 4.0
    elif case == "null":
        shutdown["after_success_marker"] = None
    elif case == "unknown":
        shutdown["after_success_marker"] = "missing_marker"
    elif case == "success-row":
        marker["after_success_marker"] = "anaconda_version"
    else:
        marker["regex"] = shutdown["regex"]
    (output / f"compose-order-{case}.json").write_text(json.dumps(policy), encoding="utf-8")
PY
for invalid_order in legacy floating null unknown success-row same-line; do
    assert_cmd_failure "compose success-order contract rejects $invalid_order evidence" \
        python3 "$LOG_AUDITOR" --policy "$TMPDIR/compose-order-$invalid_order.json" \
            --log "$TMPDIR/compose-tmux-interleaved.log" \
            --report "$TMPDIR/compose-order-$invalid_order-report.json"
done

python3 - "$TMPDIR/compose-valid.log" "$TMPDIR/compose-tmux-interleaved.log" \
    "$TMPDIR/compose-tmux-premature.log" "$TMPDIR/compose-tmux-late.log" <<'PY'
import pathlib
import sys

base_path, shutdown_path, early_path, late_path = map(pathlib.Path, sys.argv[1:])
base = base_path.read_text(encoding="utf-8")
shutdown = "\n".join(
    line for line in shutdown_path.read_text(encoding="utf-8").splitlines()
    if "anaconda-tmux@tty1.service:" in line
) + "\n"
early_path.write_text(shutdown + base, encoding="utf-8")
late_path.write_text(
    base + "18:00:00,007 INFO systemd:unrelated completion tail\n" * 300 + shutdown,
    encoding="utf-8",
)
PY
assert_cmd_failure "console failures before M99 completion cannot become shutdown exceptions" \
    python3 "$LOG_AUDITOR" --policy "$LOG_POLICY" \
        --log "$TMPDIR/compose-tmux-premature.log" \
        --report "$TMPDIR/compose-tmux-premature.json"
assert_cmd_success "shutdown ordering tolerates a long successful completion tail" \
    python3 "$LOG_AUDITOR" --policy "$LOG_POLICY" \
        --log "$TMPDIR/compose-tmux-late.log" \
        --report "$TMPDIR/compose-tmux-late.json"
...
assert_eq \
    'sandbox path contract exact: 43 strict unit(s), 19 sandboxed runtime write(s) writable' \
