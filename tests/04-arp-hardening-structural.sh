...
assert_grep_fixed '/usr/bin/flock --exclusive -w 300 9' \
    "$TMPDIR/network-readiness.sh" \
    "chrony transition lock is exclusive and bounded"
# Both consumers are Type=oneshot on the boot path, where systemd disables the
# start timeout by default, so an unbounded acquisition would have no ceiling at
# any level. Prove the flag actually bounds the wait rather than trusting it.
assert_cmd_success "flock -w bounds a contended exclusive acquisition" \
    bash -c '
        lock=$(mktemp "${TMPDIR:-/var/tmp}/noid-flock-bound.XXXXXXXX")
        trap '\''rm -f -- "$lock"'\'' EXIT
        ( exec 9>"$lock"; /usr/bin/flock --exclusive 9; sleep 3 ) &
        holder=$!
        sleep 0.5
        start=$(date +%s)
        ( exec 9>"$lock"; /usr/bin/flock --exclusive -w 1 9 ) && exit 1
        end=$(date +%s)
        wait "$holder"
        [ "$((end - start))" -le 2 ]
    '
...
assert_grep_fixed 'if [ ! -d "$SYS_CLASS_NET/$WAN_IFACE" ]; then' \
    "$TMPDIR/tool.sh" \
