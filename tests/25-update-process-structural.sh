...
cleanup_test() {
    ...
    # Permit cleanup if interrupted during a native traversal-denial fixture.
    for directory in "$TMPDIR/inventory-home" "$TMPDIR/inventory-home/.config"; do
        [ ! -d "$directory" ] || chmod 0700 "$directory"
    done
    rm -rf "$TMPDIR"
}
...
assert_cmd_success "all reboot readers reject failed helpers and malformed records" \
    python3 "$PROJECT_ROOT/tests/fixtures/reboot_readers.py" "$KS_FILE"
assert_cmd_success "reboot readiness rejects partial kernel and unavailable queue inventories" \
    python3 "$PROJECT_ROOT/tests/fixtures/reboot_inventory.py" "$KS_FILE"
...
assert_grep_fixed 'Verified system changes — restart required' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "activation notifier covers verified installation and update changes"
assert_grep_fixed 'Restart to activate the prepared kernel or boot policy.' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "activation notifier states its concise activation purpose"
assert_grep_fixed '--icon=/usr/share/pixmaps/noid-privacy-logo.png' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "safe activation notifier uses the shared NoID Privacy logo"
assert_grep_fixed '--urgency=normal' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "safe activation notification does not request critical persistence"
assert_grep_fixed 'Kernel: running=${running_kernel}, prepared=${latest_kernel}' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "concise activation notice retains running and prepared kernel evidence"
assert_grep_fixed 'NVIDIA: running=${nv_run}, prepared=${nv_disk}' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "concise activation notice retains running and prepared NVIDIA evidence"
assert_not_grep '--transient' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "safe activation notice remains available in GNOME's notification list"
assert_grep_fixed '--urgency=critical' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "unsafe boot and audit-storage findings remain critical"
assert_grep_fixed '--expire-time=0' \
    "$TMPDIR/noid-pending-reboot-check.sh" \
    "unsafe boot and audit-storage findings remain persistent"
...
assert_eq 2 "$(grep -cF -- '--retry 2 --retry-delay 1 --retry-max-time 20' \
    "$TMPDIR/noid-update-all.sh")" \
    "metadata and XPI requests both retry bounded transient marketplace failures"
assert_not_grep_fixed '--retry-all-errors' "$TMPDIR/noid-update-all.sh" \
    "marketplace recovery never retries every failure indiscriminately"
...
assert_grep_fixed 'Firefox is running or its process state is unavailable — profile hardening deferred' \
    "$TMPDIR/noid-update-all.sh" \
...
assert_cmd_success "native browser process-query fixture requires pgrep" command -v pgrep
run_browser_query_fixture() {
    env BROWSER_GUARD="$TMPDIR/browser-process-active.sh" \
        BROWSER_QUERY_MODE="$1" BROWSER_QUERY_PRODUCT="$2" \
        BROWSER_MISSING_PROGRAM="$TMPDIR/absent-bin/pgrep" bash -c '
        . "$BROWSER_GUARD"
        case "$BROWSER_QUERY_MODE" in
            failed-uid) id() { return 1; } ;;
            malformed-uid) id() { printf "%s\n" invalid; } ;;
        esac
        pgrep() {
            case "$BROWSER_QUERY_MODE" in
                absent) command pgrep -p "$$" -x noid-no-match ;;
                native-error) command pgrep --noid-fixture-invalid-option ;;
                missing-command) "$BROWSER_MISSING_PROGRAM" ;;
                partial-error) printf "%s\n" "$$"; return 2 ;;
                inconsistent-no-match) printf "%s\n" "$$"; return 1 ;;
                empty-success) return 0 ;;
                malformed-success) printf "%s\n" invalid ;;
                zero-pid) printf "%s\n" 0 ;;
                failed-uid|malformed-uid) return 1 ;;
            esac
        }
        browser_process_active "$BROWSER_QUERY_PRODUCT" "$BROWSER_QUERY_PRODUCT-bin"
    '
}
for query_product in firefox thunderbird; do
    assert_cmd_failure "native no-match query permits inactive $query_product" \
        run_browser_query_fixture absent "$query_product"
    for query_case in native-error missing-command partial-error inconsistent-no-match \
            empty-success malformed-success zero-pid failed-uid malformed-uid; do
        assert_cmd_success "$query_product mutation is blocked for $query_case process evidence" \
            run_browser_query_fixture "$query_case" "$query_product"
    done
done
...
assert_grep_fixed 'Thunderbird is running or its process state is unavailable — profile hardening deferred' \
    "$TMPDIR/noid-update-all.sh" \
...
assert_grep_fixed 'refresh_managed_dkim()' "$TMPDIR/noid-update-all.sh" \
    "missing/invalid DKIM current state enters the authenticated repair path"
assert_grep_fixed 'DKIM Verifier ${LATEST_XPI_VERSION} and exact ATN compatibility authenticated' "$TMPDIR/noid-update-all.sh" \
        ...
        env FIXTURE_COMPONENT="$component" \
            ...
            bash -c '
                ...
                fetch_marketplace_xpi() {
                    ...
                    MARKETPLACE_COMPATIBILITY="$marketplace_root/compatibility.json"
...
assert_eq advance "$(bash -c '. "$1"; ubo_candidate_action 1.75.0 1.73.0 0 0' \
...
assert_eq keep "$(bash -c '. "$1"; ubo_candidate_action 1.73.0 1.75.0 1 0' \
    _ "$UBO_CANDIDATE_ACTION")" \
    "a compatible manually newer uBO copy is preserved"
assert_eq reject "$(bash -c '. "$1"; ubo_candidate_action 1.73.0 1.75.0 0 0' \
    _ "$UBO_CANDIDATE_ACTION")" \
...
# Native permission denial must not turn an unavailable registry into a
# successful empty inventory. Each product retains a populated positive
# control and a genuinely absent root; no real user profile is inspected.
for inventory_product in firefox thunderbird; do
    if [ "$inventory_product" = firefox ]; then
        inventory_registry="$TMPDIR/inventory-home/.config/mozilla/firefox/profiles.ini"
        inventory_ancestor="$TMPDIR/inventory-home/.config"
    else
        inventory_registry="$TMPDIR/inventory-home/.thunderbird/profiles.ini"
        inventory_ancestor="$TMPDIR/inventory-home"
    fi
    chmod 000 "$inventory_registry"
    assert_cmd_failure "$inventory_product registry fixture is actually unreadable" \
        cat "$inventory_registry"
    assert_cmd_failure "$inventory_product rejects an unreadable registered-profile inventory" \
        python3 "$BROWSER_EXTENSION_INVENTORY" "$inventory_product" \
            "$TMPDIR/$inventory_product-unreadable.tsv" \
            "$TMPDIR/inventory-home/.config" "$TMPDIR/inventory-home"
    chmod 0600 "$inventory_registry"
    chmod 000 "$inventory_ancestor"
    assert_cmd_failure "$inventory_product ancestor fixture actually denies traversal" \
        stat "$inventory_registry"
    assert_cmd_failure "$inventory_product cannot mistake inaccessible ancestors for an absent root" \
        python3 "$BROWSER_EXTENSION_INVENTORY" "$inventory_product" \
            "$TMPDIR/$inventory_product-inaccessible.tsv" \
            "$TMPDIR/inventory-home/.config" "$TMPDIR/inventory-home"
    chmod 0700 "$inventory_ancestor"
    assert_cmd_success "$inventory_product inventory recovers after access is restored" \
        python3 "$BROWSER_EXTENSION_INVENTORY" "$inventory_product" \
            "$TMPDIR/$inventory_product-restored.tsv" \
            "$TMPDIR/inventory-home/.config" "$TMPDIR/inventory-home"
    assert_eq 1 "$(wc -l < "$TMPDIR/$inventory_product-restored.tsv")" \
        "$inventory_product restored inventory still contains its actual extension"
    assert_cmd_success "$inventory_product accepts a genuinely absent profile root" \
        python3 "$BROWSER_EXTENSION_INVENTORY" "$inventory_product" \
            "$TMPDIR/$inventory_product-absent.tsv" \
            "$TMPDIR/absent-home/.config" "$TMPDIR/absent-home"
    assert_eq 0 "$(wc -c < "$TMPDIR/$inventory_product-absent.tsv")" \
        "$inventory_product absent-root inventory is empty"
done
...
assert_not_grep 'Gtk.Expander' "$UPDATE_APP" \
    "Update log no longer leaves the collapsed GtkExpander spacing artifact"
assert_grep_fixed 'self.log_revealer = Gtk.Revealer()' "$UPDATE_APP" \
    "Update log uses GTK4's native zero-size reveal container"
assert_grep_fixed "self.log_toggle.connect('toggled', self._on_log_toggled)" \
    "$UPDATE_APP" "Update log disclosure has one canonical state transition"
assert_cmd_success "collapsed and expanded Update logs own one matching gap" \
...
build = next(node for node in window.body
             if isinstance(node, ast.FunctionDef)
             and node.name == '_build_run_page')
build_text = ast.get_source_segment(text, build)
assert 'log_section = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)' \
       in build_text
assert build_text.index('log_section.append(self.log_toggle)') \
       < build_text.index('log_section.append(self.log_revealer)') \
       < build_text.index('log_section.append(self.steps_group)') \
       < build_text.index('body.append(log_section)')
assert 'body.append(self.steps_group)' not in build_text
assert 'body.append(self.log_revealer)' not in build_text
assert 'self.log_toggle.set_margin_bottom(16)' in build_text
assert 'self.log_panel.set_margin_top(6)' in build_text
assert 'self.log_panel.set_margin_bottom(16)' in build_text
assert 'self.log_panel.set_visible(False)' in build_text
assert 'self.log_revealer.set_child(self.log_panel)' in build_text

toggle = next(node for node in window.body
              if isinstance(node, ast.FunctionDef)
              and node.name == '_on_log_toggled')
toggle_text = ast.get_source_segment(text, toggle)
assert 'expanded = toggle.get_active()' in toggle_text
assert toggle_text.index('self.log_toggle.set_margin_bottom(0)') \
       < toggle_text.index('self.log_panel.set_visible(True)')
assert toggle_text.index('self.log_panel.set_visible(True)') \
       < toggle_text.index('self.log_revealer.set_reveal_child(expanded)')
assert 'self.log_revealer.set_reveal_child(expanded)' in toggle_text

complete = next(node for node in window.body
                if isinstance(node, ast.FunctionDef)
                and node.name == '_on_log_reveal_complete')
complete_text = ast.get_source_segment(text, complete)
assert 'not revealer.get_reveal_child()' in complete_text
assert 'not revealer.get_child_revealed()' in complete_text
assert complete_text.index('self.log_panel.set_visible(False)') \
       < complete_text.index('self.log_toggle.set_margin_bottom(16)')

setter = next(node for node in window.body
              if isinstance(node, ast.FunctionDef)
              and node.name == '_set_log_revealed')
setter_text = ast.get_source_segment(text, setter)
assert 'self.log_toggle.set_active(bool(revealed))' in setter_text
assert 'set_expanded(' not in text
PY
...
namespace = {'subprocess': subprocess, 'sys': sys, 're': re}
...
# --- M01 firstboot-marker byte-count binding ---------------------------------
# noid-reboot-readiness runs unprivileged and can only stat M01's root-only
# .firstboot-cmdline-reboot-required record, so it pins the exact byte count
# that M01's writers produce. Execute every M01 writer and require the pin to
# equal its real output: an M01 schema change must fail here instead of
# silently turning every installed reboot verdict into `state-unsafe`.
M01_FILE="$PROJECT_ROOT/kickstart/snippets/01-bootloader.ks"
pinned_marker_size=$(grep -oE 'firstboot_marker" 2>/dev/null \|\| echo 0\)" -eq [0-9]+' \
    "$TMPDIR/noid-reboot-readiness" | grep -oE '[0-9]+$' || true)
assert_eq 1 "$(printf '%s\n' "$pinned_marker_size" | grep -c '^[0-9]\+$' || true)" \
    "readiness pins exactly one firstboot-marker byte count"
m01_writer_count=0
for m01_marker in ROOTFLAGS_REBIND_EOF CMDLINE_EOF; do
    extract_heredoc "$M01_FILE" "$m01_marker" "$TMPDIR/m01-$m01_marker" \
        || continue
    sed -n '/^write_reboot_state()/,/^}$/p' "$TMPDIR/m01-$m01_marker" \
        > "$TMPDIR/m01-$m01_marker.write-reboot-state"
    [ -s "$TMPDIR/m01-$m01_marker.write-reboot-state" ] || continue
    m01_writer_count=$((m01_writer_count + 1))
    assert_cmd_success "M01 $m01_marker write_reboot_state extracts" \
        bash -n "$TMPDIR/m01-$m01_marker.write-reboot-state"
    rm -rf -- "$TMPDIR/m01-state"
    mkdir -p "$TMPDIR/m01-state"
    actual_marker_size=$(
        STATE_DIR="$TMPDIR/m01-state" \
        REBOOT_STATE="$TMPDIR/m01-state/.firstboot-cmdline-reboot-required" \
        NOID_TEST_MODE=1 \
        bash -c '. "$1" && write_reboot_state "$2" "$3" "$4" 0 \
            && stat -c %s "$REBOOT_STATE"' _ \
            "$TMPDIR/m01-$m01_marker.write-reboot-state" \
            "$(printf 'a%.0s' {1..64})" "$(printf 'b%.0s' {1..64})" \
            12345678-1234-1234-1234-123456789abc 2>/dev/null || echo unwritable
    )
    assert_eq "$pinned_marker_size" "$actual_marker_size" \
        "M01 $m01_marker write_reboot_state output matches the readiness byte-count pin"
done
assert_eq 2 "$m01_writer_count" \
    "both M01 reboot-state writers were executed against the pin"

# --- marketplace transport-abort retry ---------------------------------------
# curl's own --retry covers a timeout and HTTP 408/429/5xx only. A CDN reset
# inside the response body surfaces as an exit code, so the observed
# `curl: (56) ... unexpected eof while reading` previously ended the refresh
# without a single retry. Execute the deployed helper against a recording fake
# curl: the transport-abort class must be retried and every other failure must
# stay exactly one attempt, so a 404 or a rejected certificate is never
# re-requested.
assert_grep_fixed 'curl_with_transport_retry -sS --proto' \
    "$TMPDIR/noid-update-all.sh" \
    "marketplace metadata request runs through the transport-abort retry"
assert_grep_fixed 'curl_with_transport_retry -fsSL --proto' \
    "$TMPDIR/noid-update-all.sh" \
    "marketplace XPI download runs through the transport-abort retry"
MARKETPLACE_FETCHER="$TMPDIR/fetch-marketplace-xpi.sh"
sed -n '/^fetch_marketplace_xpi() {$/,/^}$/p' \
    "$TMPDIR/noid-update-all.sh" > "$MARKETPLACE_FETCHER"
assert_cmd_success "marketplace fetcher extracts and parses" \
    bash -n "$MARKETPLACE_FETCHER"
assert_not_grep_fixed 'http_status=$(curl -sS --proto' \
    "$MARKETPLACE_FETCHER" \
    "marketplace metadata fetch cannot regress to raw curl"
assert_not_grep_fixed '    curl -fsSL --proto' \
    "$MARKETPLACE_FETCHER" \
    "marketplace XPI fetch cannot regress to raw curl"
assert_eq 1 "$(grep -cFx 'MARKETPLACE_TRANSPORT_RETRIES=2' \
    "$TMPDIR/noid-update-all.sh")" \
    "transport-abort recovery permits exactly two retries"
assert_eq 1 "$(grep -cFx 'MARKETPLACE_TRANSPORT_RETRY_DELAY=1' \
    "$TMPDIR/noid-update-all.sh")" \
    "transport-abort recovery waits exactly one second between attempts"

RETRY_HELPER="$TMPDIR/curl-transport-retry.sh"
sed -n '/^curl_with_transport_retry() {$/,/^}$/p' \
    "$TMPDIR/noid-update-all.sh" > "$RETRY_HELPER"
assert_cmd_success "transport-abort retry helper extracts and parses" \
    bash -n "$RETRY_HELPER"

RETRY_BIN="$TMPDIR/retry-bin"
mkdir -p "$RETRY_BIN"
# curl writes its --write-out status even when the transfer aborts. A partial
# response can therefore report 200 despite its non-zero exit, while an empty
# reply reports 000; the fake makes both cases selectable per attempt.
cat > "$RETRY_BIN/curl" <<'RETRY_FAKE_CURL_EOF'
#!/bin/bash
printf '%s\n' "$*" >> "${RETRY_ATTEMPTS:?}"
attempts=$(grep -c . "$RETRY_ATTEMPTS")
if [ -n "${RETRY_SUCCEED_ON:-}" ] && [ "$attempts" -ge "$RETRY_SUCCEED_ON" ]; then
    printf '200'
    exit 0
fi
printf '%s' "${RETRY_FAILURE_STATUS:-000}"
exit "${RETRY_EXIT:?}"
RETRY_FAKE_CURL_EOF
chmod 0755 "$RETRY_BIN/curl"

run_transport_retry() { # exit-code [succeed-on-attempt] [failed-write-out]
    : > "$TMPDIR/retry.attempts"
    env PATH="$RETRY_BIN:$PATH" \
        RETRY_ATTEMPTS="$TMPDIR/retry.attempts" \
        RETRY_EXIT="$1" RETRY_SUCCEED_ON="${2:-}" \
        RETRY_FAILURE_STATUS="${3:-000}" \
        MARKETPLACE_TRANSPORT_RETRIES=2 \
        MARKETPLACE_TRANSPORT_RETRY_DELAY=0 \
        bash -c 'set -euo pipefail
. "$1"
curl_with_transport_retry -sS https://example.invalid/x' \
        _ "$RETRY_HELPER"
}

for transport_abort in 18 52 56 92; do
    retry_rc=0
    run_transport_retry "$transport_abort" >/dev/null || retry_rc=$?
    assert_eq "$transport_abort" "$retry_rc" \
        "exhausted transport abort $transport_abort keeps curl's exit code"
    assert_eq 3 "$(grep -c . "$TMPDIR/retry.attempts")" \
        "transport abort $transport_abort is retried twice before giving up"
done

for hard_failure in 6 22 35 60 63; do
    retry_rc=0
    run_transport_retry "$hard_failure" >/dev/null || retry_rc=$?
    assert_eq "$hard_failure" "$retry_rc" \
        "non-transient curl failure $hard_failure is returned unchanged"
    assert_eq 1 "$(grep -c . "$TMPDIR/retry.attempts")" \
        "non-transient curl failure $hard_failure is never re-requested"
done

retry_rc=0
recovered_status=$(run_transport_retry 56 2 200) || retry_rc=$?
assert_eq 0 "$retry_rc" "a transport abort that clears on retry reports success"
assert_eq 2 "$(grep -c . "$TMPDIR/retry.attempts")" \
    "a cleared transport abort stops at the first successful attempt"
# The aborted attempt already received an HTTP 200 status before its body
# failed. Concatenating both attempts would hand fetch_marketplace_xpi
# `200200` and fail a recovered fetch as an HTTP error.
assert_eq 200 "$recovered_status" \
    "only the final attempt's write-out status reaches the caller"

retry_rc=0
empty_reply_status=$(run_transport_retry 52 2 000) || retry_rc=$?
assert_eq 0 "$retry_rc" "an empty reply that clears on retry reports success"
assert_eq 2 "$(grep -c . "$TMPDIR/retry.attempts")" \
    "a cleared empty reply stops at the first successful attempt"
assert_eq 200 "$empty_reply_status" \
    "a pre-header 000 status is also discarded before a successful retry"

retry_rc=0
run_transport_retry 0 >/dev/null || retry_rc=$?
assert_eq 0 "$retry_rc" "a first-attempt success needs no retry"
assert_eq 1 "$(grep -c . "$TMPDIR/retry.attempts")" \
    "a first-attempt success issues exactly one request"

assert_cmd_success "browser-version jumps and digest-bound recovery regressions" \
    python3 "$PROJECT_ROOT/tests/fixtures/extension_recovery.py"
test_finish
