...
assert_cmd_success "early initializer exit never inspects or unmounts another mount" \
    bash -c '
        findmnt() { printf "%s\\n" ext4; }
        logger() { :; }
        mountpoint() { exit 91; }
        ...
        source "$1"
    ' bash "$TMPDIR/snapper-init.sh"
if python3 - "$TMPDIR/snapper-init.sh" "$TMPDIR" <<'INIT_CLEANUP_PY'
...
cleanup = source.split("cleanup_init() {\n", 1)[1].split("# The supported Snapper", 1)[0]
cleanup = "cleanup_init() {\n" + cleanup
allocation = source.split("TOP=$(mktemp -d ", 1)[1].split('\n[ -d "$TOP" ]', 1)[0]
allocation = "TOP=$(mktemp -d " + allocation
allocation = allocation.replace("/run/noid-snapper-top.XXXXXX", shlex.quote(str(scratch / "top.XXXXXX")))
for action, expected in [("exit 0", 0), ("exit 37", 37),
    ...
    if (result.returncode != expected or result.stderr or created.parent != scratch
            or not created.name.startswith("top.") or created.exists()):
        raise SystemExit("initializer private-directory cleanup failed: " + action)
print("Initializer allocation/cleanup cases passed: 5")
INIT_CLEANUP_PY
then
    _pass "initializer removes only its allocated directory and preserves error/signal status"
else
    _fail "initializer allocation/cleanup lifecycle"
fi
...
# The legacy DNF4 Snapper plugin remains explicitly excluded (M20 owns the
# snapshot hooks itself).
