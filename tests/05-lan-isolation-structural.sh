...
def fixture_change(action, *, publish_error=False, named=True,
                   io_error=False, rollback_error=False):
    events = []
    ...
    def publish(mode):
        ...
        if io_error:
            raise OSError('fixture post-rename sync failed')
        if publish_error:
...
events, error = fixture_change('off', io_error=True)
assert error and 'previous DNS mode restored' in error
assert events.index(('publish', 'off')) < events.index(('restore', 'default'))
assert any(event[0] == 'physical-restore' for event in events)

events, error = fixture_change('off', io_error=True, rollback_error=True)
assert error and 'ROLLBACK ALSO FAILED' in error
assert 'previous DNS mode restored' not in error
...
# Restoring an unset saved profile must use its former per-device runtime
# state: NetworkManager's physical defaults can differ from global resolved.
ns['_is_live_boot'] = lambda: False
ns['_configured_mode'] = lambda: 'no'
restored = []
ns['_apply_physical_value'] = lambda profile, value: restored.append(value)
ns['_wait_physical'] = lambda expected, profiles: restored.append(expected)
default_profile = dict(active, configured='default')
ns['_restore_physical']([default_profile], {'wlan0': 'yes'})
assert restored == ['default', {'wlan0': 'yes'}]
try:
    ns['_restore_physical']([default_profile], {})
except ns['ModeError'] as exc:
    assert 'unknown physical DNS state' in str(exc)
else:
    raise AssertionError('missing runtime evidence cannot prove default restoration')
PY
...
# Force both first callers to observe absence, then hold the first native
# flock while the second attempts creation. They must retain one inode.
assert_cmd_success "concurrent first LAN transactions share one lock inode" \
    python3 - "$KS_FILE" "$TMPDIR" <<'LAN_LOCK_RACE_PY'
import json, os, re, subprocess, sys, tempfile, time
from pathlib import Path
source=Path(sys.argv[1]).read_text()
names=('trusted_state_directory','ensure_state_directory','acquire_exception_lock')
body='\n'.join(re.search(r'^'+n+r'\(\) \{\n.*?^}',source,re.M|re.S).group() for n in names)
needle='    if [ ! -e "$LAN_EXCEPTION_LOCK" ] && [ ! -L "$LAN_EXCEPTION_LOCK" ]; then\n'
assert body.count(needle)==1
body=body.replace(needle,needle+'        creation_barrier\n')
with tempfile.TemporaryDirectory(prefix='lan-lock-', dir=sys.argv[2]) as directory:
 root=Path(directory);state=root/'state';state.mkdir(mode=0o755);state.chmod(0o755)
 script=root/'actor.sh'
 script.write_text('''set -euo pipefail
umask 077
LAN_EXCEPTION_LOCK=$1
FIXTURE=$2
ROLE=$3
LAN_STATE_UID=$(id -u)
LAN_STATE_GID=$(id -g)
...
''')
 ...
 def wait_file(name):
  ...
  while not (root/name).exists():
   if time.monotonic()>end:raise RuntimeError('timeout: '+name)
   time.sleep(.01)
 try:
  for role in ('one','two'):
   processes.append(subprocess.Popen(['bash',str(script),str(state/'lan.lock'),str(root),role],stdout=subprocess.DEVNULL,stderr=subprocess.PIPE))
   wait_file('creation.'+role)
  (root/'permit.one').touch();wait_file('entered.one');inode=(state/'lan.lock').stat().st_ino
  (root/'permit.two').touch();time.sleep(.25)
  result=dict(overlap=(root/'entered.two').exists(),same_inode=(state/'lan.lock').stat().st_ino==inode)
  (root/'release.one').touch();wait_file('entered.two');(root/'release.two').touch()
  result['exit']=[p.wait(timeout=3) for p in processes]
  result['stderr_empty']=all(not p.stderr.read() for p in processes)
  print(json.dumps(result))
  assert result==dict(overlap=False,same_inode=True,exit=[0,0],stderr_empty=True),result
 finally:
  for p in processes:
   if p.poll() is None:p.terminate()
   p.wait(timeout=3);p.stderr.close()
LAN_LOCK_RACE_PY
