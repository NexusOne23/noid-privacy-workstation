...
bash -n "$SWEEP" && ok "PII sweep syntax" \
    || fail "PII sweep has a syntax error"
...
# Exercise the old bypass with both a neutral and an excepted parent path,
# and require clean content at those same paths to remain accepted.
for unusual_path in 'prefix:/home/alice/repository' \
    ...
    if leak_output=$("$SWEEP" "$leak_root" 2>&1); then
        fail "filename punctuation hid an unapproved identifier"
    else
        ok "filename punctuation cannot hide an unapproved identifier"
    fi
    [[ "$leak_output" != *'de:ad:be:ef:'"12:34"* ]] \
        && ok "identifier diagnostic does not disclose the matched value" \
        || fail "identifier diagnostic disclosed the matched value"
    [[ "$leak_output" == *':1: possible identifier or home path (redacted)'* ]] \
        && ok "identifier diagnostic retains its source line" \
        || fail "identifier diagnostic lost its source line"
    printf '%s\n' 'ab:cd:ef:12:34:56' > "$leak_root/fixture.txt"
    "$SWEEP" "$leak_root" >/dev/null 2>&1 \
        && ok "reviewed content stays accepted with filename punctuation" \
        || fail "filename punctuation rejected reviewed content"
done

if symlink_output=$("$SWEEP" "$tmp/rejected-symlink" 2>&1); then
    fail "private symlink target was accepted"
else
    [[ "$symlink_output" != *'/home/'"samplebuilder"* ]] \
        && ok "symlink diagnostic does not disclose the private target" \
        || fail "symlink diagnostic disclosed the private target"
fi
