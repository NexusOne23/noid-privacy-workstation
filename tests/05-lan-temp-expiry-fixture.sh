...
# Run the production minutes parser before any network mutation; capture the
# seconds delivered to the transaction instead of changing the firewall.
for spec in '10:600' '010:600' '08:480' '1440:86400' '0:' '1441:' '18446744073709551617:'; do
    minutes=${spec%%:*}
    expected=${spec#*:}
    : > "$FIXTURE/duration-record"
    duration_rc=0
    bash -c '
        source "$1"
        DURATION_RECORD=$3
        add_ip_exception() { printf "%s\n" "$3" > "$DURATION_RECORD"; }
        load_exception_state() { EX_STATE_EXPIRES=0; }
        add_temp_ip_exception 198.19.7.20 "$2"
    ' _ "$FIXTURE/functions.sh" "$minutes" "$FIXTURE/duration-record" \
        >"$FIXTURE/duration-output" 2>&1 || duration_rc=$?
    if [ -n "$expected" ]; then
        assert_eq 0 "$duration_rc" "decimal minute input $minutes succeeds"
    else
        assert_eq 1 "$duration_rc" "out-of-range minute input $minutes is rejected"
    fi
    assert_eq "$expected" "$(cat "$FIXTURE/duration-record")" \
        "minute input $minutes reaches the transaction only with the intended seconds"
done
