...
assert_grep_fixed '**Package and standards evidence last verified**: 2026-09-02.' \
...
assert_grep_fixed 'OpenVPN 2.7.6,' \
    "$TMPDIR/post-quantum-readiness.md" \
    "PQ package inventory retains the observed OpenVPN version"
assert_grep_fixed 'Firefox 154, Thunderbird 153, NSS 3.126, and GnuPG 2.4.9.' \
    "$TMPDIR/post-quantum-readiness.md" \
    "PQ package inventory covers the remaining load-bearing local transport versions"
assert_grep_fixed 'is not an exact v1.8 ISO package manifest;' \
    "$TMPDIR/post-quantum-readiness.md" \
...
assert_grep_fixed '**Endpoint probe observations last rerun**: 2026-09-02' \
    "$TMPDIR/post-quantum-readiness.md" \
...
assert_grep_fixed 'Just Perfection GNOME Shell extension v37 — GPL-3.0-only' \
    "$TMPDIR/licensing.md" \
