...
printf '%s\n' candidate-iso-bytes > \
    "$CANDIDATE/noid-privacy-workstation-44-v1.8-x86_64.iso"
ISO_NAME=noid-privacy-workstation-44-v1.8-x86_64.iso
...
if [[ -n \${NOID_TEST_ARCHIVE_RACE_ISO:-} \
      && \${!#} == */vm-test-signoff.txt ]]; then
    printf '\\nchanged after signature checks\\n' >> "\$NOID_TEST_ARCHIVE_RACE_ISO"
fi
EOF
cat > "$FIXTURE_REPO/bin/cp" <<'EOF'
#!/bin/bash
if [[ -n ${NOID_TEST_ARCHIVE_LATE_LINK:-} ]]; then
    ln -s "$NOID_TEST_ARCHIVE_LATE_TARGET" "$NOID_TEST_ARCHIVE_LATE_LINK" || exit 1
fi
exec /usr/bin/cp "$@"
EOF
...
exec /usr/bin/mv "$@"
EOF
chmod 0755 "$FIXTURE_REPO/bin/gpg" "$FIXTURE_REPO/bin/cp" "$FIXTURE_REPO/bin/mv"
...
# The original source is outside the private transaction. Mutating it after
# signature verification must not alter the bytes that reach the archive.
assert_cmd_success "archive authenticates its private copy despite a later source change" \
    env NOID_TEST_ARCHIVE_RACE_ISO="$CANDIDATE/$ISO_NAME" \
    "${ARCHIVE[@]}" "$CANDIDATE"
assert_cmd_failure "post-verification source mutation was actually injected" \
    cmp -s "$TMPDIR/iso.clean" "$CANDIDATE/$ISO_NAME"
assert_cmd_success "archived ISO retains the authenticated bytes" \
    cmp -s "$TMPDIR/iso.clean" \
    "$FIXTURE_REPO/build-archive/$FINAL_NAME/candidate/$ISO_NAME"
assert_cmd_success "archived public checksum still verifies after source mutation" \
    bash -c 'cd "$1" && sha256sum -c --strict --status SHA256SUMS' \
    _ "$FIXTURE_REPO/build-archive/$FINAL_NAME/candidate"
cp "$TMPDIR/iso.clean" "$CANDIDATE/$ISO_NAME"
rm -rf -- "$FIXTURE_REPO/build-archive/$FINAL_NAME"

# Regular-file manifests omit symlinks. Inject one after the source scan so
# only validation of the copied tree can reject it before publication.
printf '%s\n' outside-candidate-fixture > "$FIXTURE_REPO/outside-candidate"
assert_cmd_failure "copy-time symlink is rejected before publication" \
    env NOID_TEST_ARCHIVE_LATE_LINK="$CANDIDATE/late-link" \
    NOID_TEST_ARCHIVE_LATE_TARGET="$FIXTURE_REPO/outside-candidate" \
    "${ARCHIVE[@]}" "$CANDIDATE"
assert_cmd_success "copy-time symlink was actually injected" test -L "$CANDIDATE/late-link"
assert_cmd_failure "copy-time symlink leaves no published archive" \
    test -e "$FIXTURE_REPO/build-archive/$FINAL_NAME"
rm -f -- "$CANDIDATE/late-link" "$FIXTURE_REPO/outside-candidate"
rm -rf -- "$FIXTURE_REPO/build-archive/$FINAL_NAME"
