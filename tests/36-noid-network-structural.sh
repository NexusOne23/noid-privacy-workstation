...
# The aligned target also reserves padding for a peer with a larger tunnel
# MTU; it is conservative, not proof that every unaligned MTU fragments.
...
assert_grep_fixed 'Conservative local MTU target' \
    "$TMPDIR/noid-network-audit" \
    "the verdict is scoped to locally observable state, not the whole path"
if python3 - "$TMPDIR/noid-network-audit" "$TMPDIR/mtu-query-fixture" <<'PY'
import os
from pathlib import Path
import subprocess
import sys
...
root = Path(sys.argv[2])
root.mkdir()
# Exercise the actual MTU reader with controlled command responses. No host
# interface, sysfs file, profile, privilege route or key material is changed.
body = source[source.index("# WireGuard data-message encapsulation"):
              source.index("\naudit_wan()")]
header = r'''
set -uo pipefail
fmt_banner() { :; }; fmt_note() { :; }; fmt_step() { :; }
fmt_info() { :; }
fmt_ok() { printf 'PASS:%s\n' "$*"; }
fmt_err() { printf 'ERROR:%s\n' "$*"; }
IP=fixture_ip; WG=fixture_wg; CAT=fixture_cat
AWK=/usr/bin/awk; HEAD=/usr/bin/head; GREP=/usr/bin/grep; WC=/usr/bin/wc
fixture_ip() {
    if [ "$1" = -d ]; then
        case "$SCENARIO" in query-error) return 7;; no-tunnel) return 0;; esac
        printf '8: wgfixture0: <UP> mtu 1380\n'
        [ "$SCENARIO" != query-partial ] || return 7
    elif [ "$1" = route ]; then
        printf '%s\n' "$*" >> "$ROUTE_LOG"
        printf '203.0.113.2 dev outer0\n'
    else return 64; fi
}
fixture_wg() {
    case "$3" in
        fwmark)
            case "$SCENARIO" in
                mark-error) return 7;;
                mark-partial) printf '0x10\n'; return 7;;
                mark-empty) return 0;;
                mark-none) printf 'none\n';;
                mark-overflow) printf '0x100000000\n';;
                mark-multiline) printf '0x10\noff\n';;
                marked) printf '0x10\n';;
                *) printf 'off\n';;
            esac;;
        endpoints)
            [ "$SCENARIO" != endpoints-error ] || return 7
            printf 'fixture-key 203.0.113.2:51820\n'
            [ "$SCENARIO" != endpoints-partial ] || return 7;;
        *) return 64;;
    esac
}
fixture_cat() {
    case "$1" in
        */wgfixture0/mtu) printf '1380\n';;
        */outer0/mtu) printf '1500\n';;
        *) return 64;;
    esac
}
'''
runner = root / "run.sh"
runner.write_text(header + body + '\noverall=0\naudit_mtu\nexit "$overall"\n',
                  encoding="utf-8")
failures = ("query-error", "query-partial", "mark-error", "mark-partial",
            "mark-empty", "mark-none", "mark-overflow", "mark-multiline",
            "endpoints-error", "endpoints-partial")
for case in (*failures, "no-tunnel", "off", "marked"):
    route_log = root / "routes"
    route_log.write_text("")
    env = dict(os.environ, SCENARIO=case, ROUTE_LOG=str(route_log))
    result = subprocess.run(["bash", str(runner)], env=env, capture_output=True,
                            text=True, timeout=10)
    expected = 1 if case in failures else 0
    assert result.returncode == expected, (case, result)
    assert ("PASS:" in result.stdout) == (expected == 0), (case, result)
    assert ("ERROR:" in result.stdout) == (expected == 1), (case, result)
    assert not result.stderr, (case, result.stderr)
    routes = route_log.read_text()
    if case in failures or case == "no-tunnel":
        assert not routes, (case, routes)
    elif case == "marked":
        assert routes == "route get 203.0.113.2 mark 0x10\n", routes
    else:
        assert routes == "route get 203.0.113.2\n", routes
print("13 MTU query cases passed, including positive route controls")
PY
then
    _pass "MTU audit preserves query failures and requires exact fwmark evidence"
else
    _fail "MTU audit preserves query failures and requires exact fwmark evidence"
fi
...
try:
    for argv, section in (([], None), (['--section', 'lan'], 'lan'),
        ...
        if app.section != section or app.run_argv != ['noid-network'] + argv:
...
# The primary instance must validate forwarded requests before activation.
# Exercise the production handler without GTK; native two-process registration
# is also exercised when validating the running Fedora desktop.
app_node = next(node for node in tree.body
                if isinstance(node, ast.ClassDef) and node.name == 'NoIDNetworkApp')
...
class PrimaryProbe:
    ...
    def set_flags(self, flags):
        self.flags = flags
    def activate(self):
        self.selected.append(self._section)

namespace.update(noid_ui=SimpleNamespace(NoIDApplication=PrimaryProbe),
                 Gio=SimpleNamespace(ApplicationFlags=SimpleNamespace(
                     HANDLES_COMMAND_LINE=8)), APP_ID='test.network')
exec(compile(ast.Module(body=[app_node], type_ignores=[]), '<m36-app>', 'exec'),
     namespace)
primary = namespace['NoIDNetworkApp']()
assert primary.get_flags() & 8, 'secondary requests lack command-line forwarding'

class Request:
    def __init__(self, argv):
        self.argv = ['noid-network'] + argv
        self.errors = []
    def get_arguments(self):
        return self.argv
    def printerr_literal(self, message):
        self.errors.append(message)

for argv, expected in cases:
    request = Request(argv)
    assert primary.do_command_line(request) == 0
    assert primary.selected[-1] == expected and not request.errors
for argv in invalid + [['--section', '%s%n']]:
    request = Request(argv)
    previous = (primary._section, list(primary.selected))
    assert primary.do_command_line(request) == 2
    assert previous == (primary._section, primary.selected)
    assert len(request.errors) == 1, 'forwarded error must reach its caller'
print('ok')
SECTION_PROBE_EOF
}
