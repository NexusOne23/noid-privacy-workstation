...
for signature_tool in clang modinfo openssl gzip xz zstd; do
    assert_cmd_success "native signature fixture requires $signature_tool" \
        command -v "$signature_tool"
done
...
assert_cmd_success "native signed-module payload and exact-certificate verification" \
    python3 "$PROJECT_ROOT/tests/fixtures/nvidia_signatures.py" "$KS_FILE"
...
assert_grep_fixed 'rd.driver.blacklist=nouveau modprobe.blacklist=nouveau rd.driver.blacklist=nouveau,nova_core modprobe.blacklist=nouveau,nova_core nvidia-drm.modeset=1' "$TMP_SCRIPT" \
...
assert_cmd_success "native queue errors retain the guard and fail resume without scheduling" \
    python3 "$PROJECT_ROOT/tests/fixtures/nvidia_queue.py" "$KS_FILE"
