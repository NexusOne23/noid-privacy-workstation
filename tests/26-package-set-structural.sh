...
if python3 - "$KS_FILE" "$TMPDIR" <<'DNF_POLICY_PY'
import pathlib
import shlex
import subprocess
import sys
...
block = source.split('log "CHECK 3: persistent weak-dependency policy"\n', 1)[1]
block = block.split('# CHECK 4:', 1)[0]
cases = [
    ...
    ("default", "[main]\n", False),
    ("invalid-boolean", "[main]\ninstall_weak_deps=invalid\n", False),
]
for name, config, expected in cases:
    config_path = scratch / (name + ".conf")
    config_path.write_text(config)
    reader = "command dnf5 --config=" + shlex.quote(str(config_path))
    reader += " --setopt=logdir=" + shlex.quote(str(scratch)) + ' "$@"'
    script = 'set -euo pipefail\nlog() { :; }\nverify_ok=0\nverify_fail=0\n'
    script += 'dnf5() { ' + reader + '; }\n' + block
    script += '\nprintf "%s %s\\n" "$verify_ok" "$verify_fail"\n'
    result = subprocess.run(["bash"], input=script, text=True, capture_output=True, timeout=20)
    wanted = "1 0" if expected else "0 1"
    if result.returncode != 0 or result.stdout.strip() != wanted or result.stderr:
        raise SystemExit("native DNF policy case failed: " + name)
print("Native DNF policy cases passed: " + str(len(cases)))
DNF_POLICY_PY
then
    _pass "native DNF policy accepts disabled values and rejects precedence, section and query errors"
else
    _fail "native DNF policy regression"
fi
...
assert_grep_fixed '# CHECK 3: Persistent weak-dependency policy (native DNF main configuration)' \
