...
# Exercise the actual compose call site as well as its publisher. Conditional
# invocation suppresses Bash errexit inside the function, including label and
# staged sync failures. Only fixture paths, owners and external label/sync
# commands differ; the source's branch and pipeline execute unchanged.
m32_compose_root="$EXEC_FIXTURE_DIR/plymouth-compose"
mkdir -p "$m32_compose_root/bin"
for tool in restorecon matchpathcon sync; do
    cat >"$m32_compose_root/bin/$tool" <<'M32_COMPOSE_TOOL_EOF'
#!/bin/bash
set -eu
tool=${0##*/}
printf '%s\n' "$tool" >>"${M32_COMPOSE_LOG:?}"
if [ "${M32_COMPOSE_FAIL:-}" = "$tool" ]; then
    if [ "$tool" != sync ] || [[ "${!#}" == *.noid-publish.* ]]; then
        exit 71
    fi
fi
M32_COMPOSE_TOOL_EOF
    chmod 0700 "$m32_compose_root/bin/$tool"
done
{
    sed '$d' "$m32_parent_script" |
        sed "s|$m32_stamp_bin/|$m32_compose_root/bin/|g"
    printf '%s\n' 'check() { [ "$1" != fail ]; }'
    awk '
        /^        BGRT_PLY=/ { capture = 1 }
        /^        # Write plymouthd.conf/ { exit }
        capture { print }
    ' "$M32_FILE" |
        sed "s|/usr/share/plymouth/themes/bgrt/bgrt.plymouth|$m32_compose_root/bgrt.plymouth|g"
    printf '%s\n' 'printf "completed\n"'
} >"$m32_compose_root/compose.sh"
for failed_tool in none restorecon matchpathcon sync; do
    printf '%s\n' '[boot-up]' 'WatermarkVerticalAlignment=.91' \
        'VerticalAlignment=.66' 'UseFirmwareBackground=true' \
        >"$m32_compose_root/bgrt.plymouth"
    chmod 0644 "$m32_compose_root/bgrt.plymouth"
    : >"$m32_compose_root/tool.log"
    m32_compose_rc=0
    m32_compose_output=$(env PATH="$m32_compose_root/bin:$PATH" \
        M32_COMPOSE_FAIL="$failed_tool" \
        M32_COMPOSE_LOG="$m32_compose_root/tool.log" \
        bash "$m32_compose_root/compose.sh" 2>&1) || m32_compose_rc=$?
    if [ "$failed_tool" = none ]; then
        assert_eq 0 "$m32_compose_rc" "Plymouth compose positive control succeeds"
        assert_eq completed "$m32_compose_output" "Plymouth compose reaches completion"
        assert_grep_fixed 'WatermarkVerticalAlignment=.73' \
            "$m32_compose_root/bgrt.plymouth" "Plymouth compose publishes requested layout"
    else
        assert_eq 71 "$m32_compose_rc" "Plymouth compose preserves $failed_tool failure"
        assert_eq '' "$m32_compose_output" "Plymouth compose stops before success after $failed_tool failure"
        assert_grep_fixed "$failed_tool" "$m32_compose_root/tool.log" \
            "Plymouth negative control reaches $failed_tool"
    fi
done
