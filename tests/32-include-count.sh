...
# Read active directives and retain duplicates so repeated modules cannot hide.
includes=$(awk '$1 == "%include" && $2 ~ /^snippets\/[0-9a-zA-Z_.-]+\.ks$/ {print $2}' "$MASTER" | sort)
...
for f in "$SNIPPETS_DIR"/*.ks; do
    ...
    references=$(printf '%s\n' "$includes" | grep -cFx "$rel" || true)
    assert_eq 1 "$references" "snippet has exactly one active include: $rel"
