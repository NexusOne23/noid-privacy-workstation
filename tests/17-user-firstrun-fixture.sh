...
# Verify the user unit against its extracted executable, without depending on
# a previous NoID Privacy installation or the caller's live user-manager environment.
# The canonical executable path remains a separate source contract.
assert_grep_fixed 'ExecStart=/usr/local/libexec/noid-user-firstrun' "$SERVICE" \
    "first-login unit retains its canonical deployed executable"
UNIT_VERIFY_DIR="$TEST_TMPDIR/unit-verify"
mkdir -p "$UNIT_VERIFY_DIR/runtime"
VERIFY_SERVICE="$UNIT_VERIFY_DIR/noid-user-firstrun.service"
python3 - "$SERVICE" "$SCRIPT" "$VERIFY_SERVICE" <<'VERIFY_UNIT_EOF'
from pathlib import Path
import sys

source = Path(sys.argv[1]).read_text()
original = 'ExecStart=/usr/local/libexec/noid-user-firstrun\n'
assert source.count(original) == 1
executable = sys.argv[2].replace('\\', '\\\\').replace('"', '\\"').replace('%', '%%')
Path(sys.argv[3]).write_text(source.replace(original, f'ExecStart="{executable}"\n'))
VERIFY_UNIT_EOF
verify_firstrun_unit() {
    env XDG_RUNTIME_DIR="$UNIT_VERIFY_DIR/runtime" \
        SYSTEMD_UNIT_PATH="$UNIT_VERIFY_DIR:/usr/lib/systemd/user" \
        systemd-analyze --user --recursive-errors=no --generators=no verify "$1"
}
assert_cmd_success "first-login user service verifies against the extracted helper" \
    verify_firstrun_unit "$VERIFY_SERVICE"
cp "$VERIFY_SERVICE" "$UNIT_VERIFY_DIR/broken.service"
printf '\nNoIDInvalidFixtureDirective=yes\n' >> "$UNIT_VERIFY_DIR/broken.service"
assert_cmd_failure "first-login unit verification rejects an unknown directive" \
    verify_firstrun_unit "$UNIT_VERIFY_DIR/broken.service"
