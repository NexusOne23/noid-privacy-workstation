...
for markdown_block in "${markdown_bash_blocks[@]}"; do
    ...
    if grep -qE -- 'ollama_expected=|llama_cpp_expected=|llama_vscode_expected=|^model_id=ornith$|^lmstudio_filename=|^agent_workspace=' \
            "$markdown_block"; then
...
assert_eq 6 "$artifact_consuming_blocks" \
    "all six artifact-consuming examples are present"
...
assert_eq 5 "$(grep -c -- "--proto-redir '=https'" "$DOC")" \
    "every redirect-following artifact download remains HTTPS-only"
...
assert_grep_fixed 'rejects loopback, private,' "$DOC" \
    "reviewed Ollama redirect mitigation and limits are documented"
assert_grep_fixed 'routes the digest through `manifest.BlobsPath`' "$DOC" \
...
assert_grep_fixed "ollama_version=v0.34.2"          "$DOC" "current reviewed Ollama release is pinned exactly"
assert_grep_fixed "e155b83589986d2c581fdbf1381ea3ebdb16549883679cd5a0627f7cdc05b12b" "$DOC" \
    "current reviewed Ollama artifact is pinned independently"
assert_grep_fixed 'probe reported 0.34.2' "$DOC" \
...
assert_grep_fixed 'llama_cpp_build=b11067' "$DOC" \
    "current reviewed llama.cpp build is pinned exactly"
assert_grep_fixed 'llama-b11067-bin-ubuntu-vulkan-x64.tar.gz' "$DOC" \
    "reviewed Linux Vulkan artifact is named exactly"
assert_grep_fixed '9e2a3755dd3aaa1b061d2e6395041be06a3f29dabde7de56fa0dfb3a910798a3' "$DOC" \
    "reviewed llama.cpp artifact is pinned to the GitHub digest"
assert_grep_fixed '30,396,256-byte archive' "$DOC" \
    "reviewed llama.cpp artifact byte count is pinned exactly"
assert_grep_fixed 'development build 11067 at commit `932a68e06`' "$DOC" \
...
assert_grep_fixed 'llama_vscode_version=0.0.67' "$DOC" \
    "llama-vscode package version is pinned to the reviewed Open VSX release"
assert_grep_fixed '2bc07c4b27ade7b14592b211c458761b89d1d1d562a4898a2d37ae6a64088339' "$DOC" \
...
assert_grep_fixed 'without either ignore check or a workspace-boundary check' "$DOC" \
    "llama-vscode 0.0.67 incremental RAG defect is explicit"
...
assert_grep_fixed 'relative traversal paths' "$DOC" \
    "llama-vscode edit guard is not misrepresented as a filesystem boundary"
assert_grep_fixed 'auto-memory exception compares a string prefix' "$DOC" \
    "llama-vscode auto-memory edit exception limitation is explicit"
assert_grep_fixed 'delete tool normalizes the target' "$DOC" \
    "llama-vscode delete boundary remains distinguished from its edit guard"
...
assert_grep_fixed 'abdd624b12ebf020b767fff532ff44fe552b28c3' "$DOC" \
    "Ornith 1.5 9B candidate is tied to an immutable publisher revision"
assert_grep_fixed '70c112196e0b7023803c9762752e46d29e612a92c83f995bc3ba1ceb07e8fab6' "$DOC" \
    "Ornith 1.5 9B candidate bytes are pinned"
assert_grep_fixed '12393612fd4f730ff5aadc23e9b8f9648aa49ceb' "$DOC" \
    "Ornith 1.5 35B candidate is tied to an immutable publisher revision"
assert_grep_fixed '42739874cc2ccfdb8523b23fbe52e29b2a7555c8176737ca9ca0b5d59859d41f' "$DOC" \
...
assert_eq 6 "$(grep -c '^set -euo pipefail$' "$TMPDIR/28-local-ai.md")" \
    "all six artifact-consuming blocks stop on prerequisite failures"

# Execute the documented launch control flow in private paths. Stub only the
# final applications, not Bash, file validation, directory changes or copying.
assert_cmd_success "local-AI examples reject missing inputs before execution" \
    python3 - "$DOC" "$EXEC_TMPDIR" <<'AI_LAUNCH_FIXTURE_EOF'
...
server.chmod(0o755)
workspace = home/'path/to/repository'
model = home/'models/reviewed-tool-model.gguf'
model.parent.mkdir()
model.touch()
key = home/'.config/noid-local-ai/api-key'
key.parent.mkdir(parents=True)
key.write_text('SYNTHETIC-FIXTURE\n')
blocks = re.findall(r'^```bash\n(.*?)^```', doc, re.M | re.S)
agent = next(b for b in blocks if '\nagent_workspace=' in b)
app = next(b for b in blocks if '\nlmstudio_filename=' in b)
environment = os.environ | {
    'PATH': str(bindir)+':/usr/bin:/usr/sbin',
    'NOID_LAUNCH_CALLS': str(calls),
    'LM_STUDIO_FILENAME': 'fixture.AppImage',
}

def run(block, accepted):
    calls.unlink(missing_ok=True)
    # Substitute only private path roots; every source command stays intact.
    block = block.replace('/var/tmp/', str(root)+'/').replace('$HOME', str(home))
    result = subprocess.run(['bash','-c',block],env=environment,cwd=root,
                            capture_output=True,timeout=5)
    assert (result.returncode == 0) == accepted, result.stderr.decode()
    assert calls.exists() == accepted, 'unexpected application execution'

run(agent, False)  # no workspace: must not start in the caller's directory
...
key.write_text('SYNTHETIC-FIXTURE\n')
run(agent, True)
old = home/'.local/opt/lm-studio/LM-Studio.AppImage'
old.parent.mkdir(parents=True)
old.write_text(stub)
old.chmod(0o755)
run(app, False)  # missing new download must not execute the older artifact
download = root/'fixture.AppImage'
download.write_text(stub)
run(app, True)
# Version-specific installs must not overwrite an existing reviewed runtime.
# Exercise the exact native pre-extraction guards with existing and new roots.
for name in ('ollama_root','llama_cpp_root'):
    guard = re.search(r'^test ! -e "\$'+name+r'".*?^install -d[^\n]*',
    ...
    for exists in (True, False):
        ...
        result = subprocess.run(['bash','-c','set -euo pipefail\n'+name+'=$1\n'+
                                 guard+'\nprintf reached-extraction', '_', str(target)],
                                env=environment,capture_output=True,timeout=5)
        assert (result.returncode == 0) == (not exists)
        assert (result.stdout == b'reached-extraction') == (not exists)
print('ten launch and installation prerequisite cases passed')
AI_LAUNCH_FIXTURE_EOF
