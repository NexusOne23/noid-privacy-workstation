...
# Verify the unchanged units and their exact source helpers in a private root.
# Vendor units provide the dependency graph; generators and manpage lookup are
# outside this source contract. No unit or installed helper is executed.
UNIT_VERIFY_ROOT="$TMPDIR/systemd-verify"
install -d "$UNIT_VERIFY_ROOT/etc/systemd/system" \
    "$UNIT_VERIFY_ROOT/usr/lib/systemd" "$UNIT_VERIFY_ROOT/usr/local/bin" \
    "$UNIT_VERIFY_ROOT/usr/libexec" "$UNIT_VERIFY_ROOT/usr/bin"
cp -a /usr/lib/systemd/system "$UNIT_VERIFY_ROOT/usr/lib/systemd/"
M22_UNIT_NAMES=(noid-mount-hardening.service noid-live-mount-hardening.service
                btrfs-scrub.service btrfs-scrub.timer)
for unit in "${M22_UNIT_NAMES[@]}"; do
    install -m 0644 "$TMPDIR/$unit" "$UNIT_VERIFY_ROOT/etc/systemd/system/$unit"
done
install -m 0755 "$TMPDIR/mount-hardening.sh" \
    "$UNIT_VERIFY_ROOT/usr/local/bin/noid-mount-hardening.sh"
install -m 0755 "$TMPDIR/live-mount-script" \
    "$UNIT_VERIFY_ROOT/usr/local/bin/noid-live-mount-hardening.sh"
install -m 0755 "$TMPDIR/noid-btrfs-scrub" \
    "$UNIT_VERIFY_ROOT/usr/libexec/noid-btrfs-scrub"
install -m 0755 /usr/bin/findmnt "$UNIT_VERIFY_ROOT/usr/bin/findmnt"
M22_VERIFY=(env -i PATH=/usr/sbin:/usr/bin LC_ALL=C
    SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system
    systemd-analyze --root="$UNIT_VERIFY_ROOT" --generators=no --man=no
    --recursive-errors=no verify "${M22_UNIT_NAMES[@]}")
assert_cmd_success "all exact M22 units pass the systemd parser" "${M22_VERIFY[@]}"
for executable in usr/local/bin/noid-mount-hardening.sh \
                  usr/local/bin/noid-live-mount-hardening.sh \
                  usr/libexec/noid-btrfs-scrub usr/bin/findmnt; do
    chmod 0644 "$UNIT_VERIFY_ROOT/$executable"
    assert_cmd_failure "M22 unit parser rejects non-executable $executable" "${M22_VERIFY[@]}"
    chmod 0755 "$UNIT_VERIFY_ROOT/$executable"
done
printf '\nNoIDInvalidFixtureDirective=yes\n' \
    >> "$UNIT_VERIFY_ROOT/etc/systemd/system/noid-mount-hardening.service"
assert_cmd_failure "M22 unit parser rejects an unknown directive" "${M22_VERIFY[@]}"
...
{
    extract_function "fstab_opt_present" "$KS_FILE"
    echo ""
...
# The options column and its ordering determine the next mount. These cases
# retain comments/other fields while requiring each matching row to be safe.
# Generic-flag and propagation expectations follow native mount counterexamples.
OPTION_CASES="$TMPDIR/fstab-option-cases"
mkdir -p "$OPTION_CASES"
OPTION_HARNESS="$OPTION_CASES/run.sh"
{
    printf '#!/bin/bash\nset -euo pipefail\n'
    cat "$TMPDIR/helpers.sh"
    extract_function "check_opt_present" "$KS_FILE"
    cat <<'OPTION_HARNESS_EOF'
FSTAB=$1
required=$2
CHANGED=0
log() { :; }
if [ -n "${3:-}" ]; then
    failure_mode=$3
    # shellcheck disable=SC2317,SC2329 # called by the extracted option reader
    awk() {
        if [[ ${2:-} == pattern=* ]]; then
            if [ "$failure_mode" = full ]; then /usr/bin/awk "$@"; fi
            return 7
        fi
        /usr/bin/awk "$@"
    }
...
before_rc=0
check_opt_present /tmp "$required" || before_rc=$?
ensure_mount_options /tmp "$required"
check_opt_present /tmp "$required"
printf 'before=%s changed=%s\n' "$before_rc" "$CHANGED"
first=$(cat "$FSTAB")
CHANGED=0
ensure_mount_options /tmp "$required"
[ "$first" = "$(cat "$FSTAB")" ] && [ "$CHANGED" -eq 0 ]
OPTION_HARNESS_EOF
} > "$OPTION_HARNESS"
for option_case in canonical reversed overridden comment other-field duplicate-rows \
                   suid dev discard discard-sync discard-async shared recursive-shared; do
    required=noexec
    initial=defaults
    suffix=
    source=tmpfs
    case "$option_case" in
        canonical) initial=noexec ;;
        reversed) initial=exec,noexec ;;
        overridden) initial=noexec,exec ;;
        comment) suffix=' # noexec' ;;
        other-field) source=noexec ;;
        duplicate-rows) initial=noexec ;;
        suid) required=nosuid; initial=nosuid,suid ;;
        dev) required=nodev; initial=nodev,dev ;;
        discard) required=nodiscard; initial=nodiscard,discard ;;
        discard-sync) required=nodiscard; initial=nodiscard,discard=sync ;;
        discard-async) required=nodiscard; initial=nodiscard,discard=async ;;
        shared) required=private; initial=private,shared ;;
        recursive-shared) required=private; initial=private,rshared ;;
    esac
    case_file="$OPTION_CASES/$option_case.fstab"
    printf '%s /tmp tmpfs %s 0 0%s\n' "$source" "$initial" "$suffix" > "$case_file"
    if [ "$option_case" = duplicate-rows ]; then
        printf 'tmpfs /tmp tmpfs defaults 0 0\n' >> "$case_file"
    fi
    expected='before=1 changed=1'
    case "$option_case" in canonical|reversed) expected='before=0 changed=0' ;; esac
    option_output=$(bash "$OPTION_HARNESS" "$case_file" "$required") \
        || _fail "option case $option_case repairs and stays idempotent"
    assert_eq "$expected" "$option_output" \
        "option case $option_case rejects false evidence and repairs once"
    assert_cmd_success "option case $option_case preserves all required final tokens" \
        awk -v wanted="$required" '
            $2 == "/tmp" {
                count++
                split($4, values, ",")
                found=0
                for (i in values) if (values[i] == wanted) found=1
                if (!found) bad=1
            }
            END { exit (count && !bad ? 0 : 1) }
        ' "$case_file"
done
for failure_mode in empty full; do
    failed_file="$OPTION_CASES/query-$failure_mode.fstab"
    printf 'tmpfs /tmp tmpfs noexec 0 0\n' > "$failed_file"
    cp -- "$failed_file" "$failed_file.before"
    assert_cmd_failure "failed $failure_mode option query cannot approve fstab" \
        bash "$OPTION_HARNESS" "$failed_file" noexec "$failure_mode"
    assert_cmd_success "failed $failure_mode option query preserves fstab" \
        cmp -s "$failed_file.before" "$failed_file"
done
assert_grep_fixed '# noexec' "$OPTION_CASES/comment.fstab" \
    "fstab repair preserves the explanatory comment"
assert_grep_extended '^noexec /tmp tmpfs defaults,noexec 0 0$' \
    "$OPTION_CASES/other-field.fstab" "fstab repair preserves the source field"
...
# libmount verifies target usability as well as syntax. Supply every fixture
# target in a private namespace, including /boot/efi on non-UEFI test hosts.
# This harness contains only the fstab transaction, never live remounts.
# One flag set serves the capability probe and the fixture, so a probe that
# succeeds proves exactly the namespace shape the transaction needs.
fstab_transaction_bwrap() {
    bwrap --unshare-all --die-with-parent \
        --ro-bind /usr /usr --symlink usr/bin /bin --symlink usr/sbin /sbin \
        --symlink usr/lib /lib --symlink usr/lib64 /lib64 \
        --ro-bind /proc /proc --ro-bind /sys /sys \
        --dir /etc --dir /home --dir /tmp --dir /boot/efi --dir /dev/shm --dir /var/tmp \
        --bind "$TMPDIR" "$TMPDIR" \
        "$@"
}
run_fstab_transaction() {
    fstab_transaction_bwrap /usr/bin/bash "$TMPDIR/fstab-transaction.sh"
}

# Unprivileged user namespaces are a host property, not a source defect: the
# GitHub Actions job container denies them while the installed bubblewrap works
# on every ordinary host. A missing bubblewrap stays a failure (the release
# gate requires it); a denied namespace surfaces as a visible capability skip
# that keeps the structural coverage above.
if ! command -v bwrap >/dev/null 2>&1; then
    _fail "bubblewrap is required for the M22 fstab transaction fixture"
elif ! fstab_transaction_bwrap /usr/bin/true \
        >"$TMPDIR/fstab-namespace-probe.out" 2>&1; then
    printf '%s\n' \
        '  [SKIP] M22: fstab transaction namespace fixture unavailable; structural coverage retained'
    sed 's/^/    | /' "$TMPDIR/fstab-namespace-probe.out"
else
    TX_FSTAB="$TMPDIR/transaction-fstab"
    write_transaction_fstab "$TX_FSTAB"
    if TX_FSTAB="$TX_FSTAB" run_fstab_transaction \
