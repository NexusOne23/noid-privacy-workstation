...
assert_grep_fixed "marker_stat=\$(stat -c '%u:%d:%i:%Y:%F' -- \"\$marker\"" \
    "$TMPDIR/nm-scope.sh" \
    "account completion binds one checked marker identity snapshot"
...
assert_grep_fixed 'read -r up _rest < "$UPTIME_FILE"' \
    "$TMPDIR/nm-scope.sh" \
    "account-completion recovery uses boot-monotonic uptime"
assert_grep_fixed 'NOID-NM-SCOPE-WAIT-V1' "$TMPDIR/nm-scope.sh" \
    "account-completion recovery has closed boot-local evidence"
assert_grep_fixed '"$STATE_UID:$STATE_GID:755:directory"' \
    "$TMPDIR/nm-scope.sh" \
    "boot-local wait evidence requires its exact root-owned runtime boundary"
assert_grep_fixed '^STARTED_CS=(0|[1-9][0-9]{0,17})$' \
    "$TMPDIR/nm-scope.sh" \
    "boot-local monotonic evidence has a bounded canonical integer"
assert_grep_fixed '"$pending_inode" "$pending_mtime" "$monotonic_now"' \
    "$TMPDIR/nm-scope.sh" \
    "monotonic wait evidence is bound to marker identity and content time"
assert_grep_fixed 'monotonic_age=$((monotonic_now - WAIT_STARTED_CS))' \
    "$TMPDIR/nm-scope.sh" \
    "backward wall-clock recovery measures only monotonic elapsed time"
assert_grep_fixed '[ "$marker_bound_to_wait" -eq 0 ]' \
    "$TMPDIR/nm-scope.sh" \
    "a started stability window cannot be bypassed by forward wall-clock correction"
...
assert_grep_fixed \
    'ReadWritePaths=/var/lib/noid-privacy /run/noid-privacy /run/lock' \
    "$TMPDIR/nm-scope.service" \
    "account scoper can write only its boot-local monotonic evidence beside shared runtime state"
...
date() {
    if [ "$*" = '+%s' ] && [ -n "${NOID_TEST_NOW_EPOCH:-}" ]; then
        printf '%s\n' "$NOID_TEST_NOW_EPOCH"
    else
        command date "$@"
    fi
...
stat() {
    case "$*" in
        ...
        "-c %Y $NOID_TEST_RACE_MARKER")
            return 1
            ;;
        "-c %u:%d:%i:%Y:%F -- $NOID_TEST_RACE_MARKER")
            mv -- "$NOID_TEST_RACE_MARKER" \
                "$NOID_TEST_RACE_MARKER.disappeared"
            return 1
            ;;
...
export -f getent logger nmcli systemctl date

# A GNOME completion marker can be written before chrony corrects the wall
# clock backwards. The helper must not trust it immediately, but it also must
# not leave a physical profile system-wide until wall time catches up. Exercise
# the extracted production helper with a frozen future mtime and controlled
# /proc/uptime substitute: only 60 seconds of same-inode monotonic observation
# may authorize NetworkManager mutation.
future_scope_state="$TMPDIR/future-scope-state"
future_scope_wait="$TMPDIR/future-scope-runtime/account-wait.state"
future_scope_uptime="$TMPDIR/future-scope.uptime"
future_scope_calls="$TMPDIR/future-scope-nm.calls"
future_scope_nm_state="$TMPDIR/future-scope-nm.state"
future_scope_active="$TMPDIR/future-scope-active.state"
future_scope_systemctl="$TMPDIR/future-scope-systemctl.calls"
mkdir -p "$future_scope_state" "${future_scope_wait%/*}"
chmod 0755 "${future_scope_wait%/*}"
: > "$future_scope_calls"
: > "$future_scope_systemctl"
touch -d '2 hours' "$scope_home/.config/gnome-initial-setup-done"
printf '%s\n' '100.00 0.00' > "$future_scope_uptime"
assert_cmd_success "future account marker starts a monotonic recovery window" \
    env NOID_NMCLI_BIN=nmcli \
        NOID_NM_SCOPE_STATE_DIR="$future_scope_state" \
        NOID_NM_SCOPE_LOCK="$TMPDIR/future-scope.lock" \
        NOID_NM_SCOPE_WAIT_STATE="$future_scope_wait" \
        NOID_NM_SCOPE_UPTIME_FILE="$future_scope_uptime" \
        NOID_SYSTEMCTL_BIN=systemctl \
        NOID_TEST_SCOPE_HOME="$scope_home" \
        NOID_TEST_NM_STATE="$future_scope_nm_state" \
        NOID_TEST_NM_ACTIVE_STATE="$future_scope_active" \
        NOID_TEST_NM_CALLS="$future_scope_calls" \
        NOID_TEST_SYSTEMCTL_CALLS="$future_scope_systemctl" \
        bash "$TMPDIR/nm-scope.sh"
assert_file_exists "$future_scope_wait" \
    "future marker publishes boot-local wait evidence"
assert_eq 600 "$(stat -c '%a' "$future_scope_wait")" \
    "boot-local wait evidence is private"
assert_cmd_failure "future marker is not scoped immediately" \
    test -e "$future_scope_state/nm-physical-profiles-scoped.flag"
assert_eq 0 "$(wc -l < "$future_scope_calls")" \
    "future marker cannot reach NetworkManager before monotonic stability"

# Even if CLOCK_REALTIME now jumps far beyond the marker mtime, the already
# started window remains monotonic and cannot finish at 10 seconds.
printf '%s\n' '110.00 0.00' > "$future_scope_uptime"
future_marker_mtime=$(stat -c %Y \
    "$scope_home/.config/gnome-initial-setup-done")
assert_cmd_success "forward wall-clock correction preserves monotonic wait" \
    env NOID_NMCLI_BIN=nmcli \
        NOID_NM_SCOPE_STATE_DIR="$future_scope_state" \
        NOID_NM_SCOPE_LOCK="$TMPDIR/future-scope.lock" \
        NOID_NM_SCOPE_WAIT_STATE="$future_scope_wait" \
        NOID_NM_SCOPE_UPTIME_FILE="$future_scope_uptime" \
        NOID_SYSTEMCTL_BIN=systemctl \
        NOID_TEST_NOW_EPOCH="$((future_marker_mtime + 7200))" \
        NOID_TEST_SCOPE_HOME="$scope_home" \
        NOID_TEST_NM_STATE="$future_scope_nm_state" \
        NOID_TEST_NM_ACTIVE_STATE="$future_scope_active" \
        NOID_TEST_NM_CALLS="$future_scope_calls" \
        NOID_TEST_SYSTEMCTL_CALLS="$future_scope_systemctl" \
        bash "$TMPDIR/nm-scope.sh"
assert_cmd_failure "forward correction cannot complete a ten-second window" \
    test -e "$future_scope_state/nm-physical-profiles-scoped.flag"
assert_eq 0 "$(wc -l < "$future_scope_calls")" \
    "forward correction cannot reach NetworkManager before monotonic stability"

printf '%s\n' '159.99 0.00' > "$future_scope_uptime"
assert_cmd_success "future account marker remains pending before 60 monotonic seconds" \
    env NOID_NMCLI_BIN=nmcli \
        NOID_NM_SCOPE_STATE_DIR="$future_scope_state" \
        NOID_NM_SCOPE_LOCK="$TMPDIR/future-scope.lock" \
        NOID_NM_SCOPE_WAIT_STATE="$future_scope_wait" \
        ...
        NOID_TEST_SYSTEMCTL_CALLS="$future_scope_systemctl" \
        bash "$TMPDIR/nm-scope.sh"
assert_cmd_failure "59.99 monotonic seconds cannot authorize account scope" \
    test -e "$future_scope_state/nm-physical-profiles-scoped.flag"
assert_eq 0 "$(wc -l < "$future_scope_calls")" \
    "pre-deadline retry still cannot mutate NetworkManager"

printf '%s\n' '160.00 0.00' > "$future_scope_uptime"
assert_cmd_success "same future marker scopes after 60 monotonic seconds" \
    env NOID_NMCLI_BIN=nmcli \
        NOID_NM_SCOPE_STATE_DIR="$future_scope_state" \
        NOID_NM_SCOPE_LOCK="$TMPDIR/future-scope.lock" \
        NOID_NM_SCOPE_WAIT_STATE="$future_scope_wait" \
        ...
        NOID_TEST_SYSTEMCTL_CALLS="$future_scope_systemctl" \
        bash "$TMPDIR/nm-scope.sh"
assert_file_exists "$future_scope_state/nm-physical-profiles-scoped.flag" \
    "monotonic recovery closes future-marker profile scope"
assert_cmd_failure "consumed monotonic wait evidence is removed" \
    test -e "$future_scope_wait"
assert_grep_fixed \
    'connection modify fixture-uuid connection.permissions user:fixture' \
    "$future_scope_calls" \
    "monotonic recovery reaches the exact existing permission mutation"

# Replacing the marker at the old deadline must restart the full monotonic
# window. Keep the old inode allocated so inode reuse cannot make the fixture
# accidentally pass.
rm -f "$future_scope_state/nm-physical-profiles-scoped.flag" \
    "$future_scope_nm_state" "$future_scope_active"
: > "$future_scope_calls"
touch -d '2 hours' "$scope_home/.config/gnome-initial-setup-done"
printf '%s\n' '200.00 0.00' > "$future_scope_uptime"
assert_cmd_success "replacement fixture starts its first marker window" \
    env NOID_NMCLI_BIN=nmcli \
        NOID_NM_SCOPE_STATE_DIR="$future_scope_state" \
        NOID_NM_SCOPE_LOCK="$TMPDIR/future-scope.lock" \
        NOID_NM_SCOPE_WAIT_STATE="$future_scope_wait" \
        ...
        NOID_TEST_SYSTEMCTL_CALLS="$future_scope_systemctl" \
        bash "$TMPDIR/nm-scope.sh"
mv "$scope_home/.config/gnome-initial-setup-done" \
    "$scope_home/.config/gnome-initial-setup-done.old"
touch -d '2 hours' "$scope_home/.config/gnome-initial-setup-done"
printf '%s\n' '260.00 0.00' > "$future_scope_uptime"
assert_cmd_success "marker replacement restarts the monotonic window" \
    env NOID_NMCLI_BIN=nmcli \
        NOID_NM_SCOPE_STATE_DIR="$future_scope_state" \
        NOID_NM_SCOPE_LOCK="$TMPDIR/future-scope.lock" \
        NOID_NM_SCOPE_WAIT_STATE="$future_scope_wait" \
        ...
        NOID_TEST_SYSTEMCTL_CALLS="$future_scope_systemctl" \
        bash "$TMPDIR/nm-scope.sh"
assert_cmd_failure "replacement cannot inherit the prior marker deadline" \
    test -e "$future_scope_state/nm-physical-profiles-scoped.flag"
assert_eq 26000 "$(sed -n 's/^STARTED_CS=//p' "$future_scope_wait")" \
    "replacement marker receives a fresh monotonic start"
assert_eq 0 "$(wc -l < "$future_scope_calls")" \
    "replacement marker cannot mutate NetworkManager at the old deadline"

# Restore the ordinary wall-clock-ready fixture used by the remaining account
# scoper race and postcondition tests.
touch -d '2 minutes ago' "$scope_home/.config/gnome-initial-setup-done"
