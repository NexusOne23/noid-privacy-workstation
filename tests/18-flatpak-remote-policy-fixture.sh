...
# The verifier must interpret the same key-file dialect as native Flatpak.
# ConfigParser's case folding, DEFAULT inheritance and permissive booleans
# must never turn a missing native subset/URL into a successful trust proof.
for grammar_scope in default fedora; do
    for grammar_drift in key-case inherited-url colon-delimiter semicolon-comment \
                         invalid-boolean duplicate-key underscore-priority; do
        install_good_state
        if [ "$grammar_scope" = fedora ]; then
            "$POLICY" fedora-on >/dev/null
            grammar_action=fedora-status
        else
            grammar_action=verify-default
        fi
        python3 - "$CONFIG" "$grammar_scope" "$grammar_drift" <<'GRAMMAR_DRIFT_EOF'
from pathlib import Path
import sys
path, scope, drift = sys.argv[1:]
source = Path(path).read_text()
url = "oci+https://registry.fedoraproject.org" if scope == "fedora" else "https://dl.flathub.org/repo/"
if drift == "key-case":
    key = "xa.title-is-set" if scope == "fedora" else "xa.subset"
    source = source.replace(key + "=", key.upper() + "=")
elif drift == "inherited-url":
    source = "[DEFAULT]\nurl=" + url + "\n" + source.replace("url=" + url + "\n", "")
elif drift == "colon-delimiter":
    source = source.replace("url=" + url, "url:" + url)
elif drift == "semicolon-comment":
    source = "; not a GKeyFile comment\n" + source
elif drift == "invalid-boolean":
    key = "xa.title-is-set" if scope == "fedora" else "xa.subset-is-set"
    source = source.replace(key + "=true", key + "=yes")
elif drift == "duplicate-key":
    source += "url=" + url + "\n"
elif drift == "underscore-priority":
    source = source + "xa.prio=0_1\n" if scope == "fedora" else source.replace("xa.prio=2", "xa.prio=0_2")
else:
    raise SystemExit("unknown grammar fixture")
Path(path).write_text(source)
GRAMMAR_DRIFT_EOF
        assert_cmd_failure "$grammar_scope rejects native key-file drift: $grammar_drift" \
            "$POLICY" "$grammar_action"
    done
done

install_good_state
"$POLICY" fedora-on >/dev/null
sed -i 's/xa.title=Fedora Flatpaks/xa.title=Fedora\\sFlatpaks/' "$CONFIG"
assert_cmd_success "native escaped Fedora title retains its exact effective identity" \
    "$POLICY" fedora-status
