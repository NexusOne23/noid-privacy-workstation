...
# not apply to or mutate the host interface. Bind the existing proc tree read-only
# so nested containers retain their masked proc paths without requiring a new
# procfs mount. The parser still needs procfs to resolve its loopback device.
...
# One flag set serves the capability probe and the parser run, so a probe that
# succeeds proves exactly the namespace shape the parser needs.
link_parser_bwrap() {
    bwrap --unshare-all --share-net --die-with-parent \
    --ro-bind /usr /usr --symlink usr/bin /bin --symlink usr/lib /lib \
    --symlink usr/lib64 /lib64 --symlink usr/sbin /sbin --ro-bind /sys /sys \
    --ro-bind /proc /proc --dir /run --ro-bind "$TMPDIR/link-root/etc" /etc \
    "$@"
}
run_link_parser() {
    link_parser_bwrap --setenv SYSTEMD_LOG_LEVEL debug /usr/bin/udevadm \
        test-builtin net_setup_link /sys/class/net/lo
}
...
# that keeps the structural .link coverage above.
if ! command -v bwrap >/dev/null 2>&1; then
    _fail "bubblewrap is required for the M27 native .link parser fixture"
elif ! link_parser_bwrap /usr/bin/true \
        >"$TMPDIR/link-namespace-probe.out" 2>&1; then
    printf '%s\n' \
        '  [SKIP] M27: native .link parser namespace fixture unavailable; structural coverage retained'
    sed 's/^/    | /' "$TMPDIR/link-namespace-probe.out"
else
    link_parse_rc=0
    link_parse=$(run_link_parser 2>&1) || link_parse_rc=$?
    ...
    # The native parser may return success after ignoring an invalid setting.
    # Prove that the path-specific diagnostic check detects that case too.
    sed -i 's/^WakeOnLan=off$/WakeOnLan=invalid-fixture/' \
        "$TMPDIR/link-root/etc/systemd/network/10-noid-no-wol.link"
    invalid_link_rc=0
    invalid_link_parse=$(run_link_parser 2>&1) || invalid_link_rc=$?
    assert_eq "0" "$invalid_link_rc" \
        "invalid-link counterexample reaches the native parser"
    if grep -E '/etc/systemd/network/10-noid-no-wol\.link:.*(Failed|Invalid|Unknown|ignoring)' \
            <<<"$invalid_link_parse" >/dev/null; then
        _pass "native parser diagnostics reject an invalid WakeOnLan value"
    else
        _fail "native parser diagnostics missed an invalid WakeOnLan value"
    fi
fi
