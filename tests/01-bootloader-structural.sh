...
# scripts/build-iso.sh hands the same token set to lmc as --extra-boot-args.
# Bind it here so an M01 manifest change cannot desynchronize the Live boot
# line silently until a full compose fails its final boot-argument audit.
kspp_args=$(sed -n 's/^KSPP_KERNEL_ARGS="\([^"]*\)"$/\1/p' \
    "$PROJECT_ROOT/scripts/build-iso.sh")
assert_eq "$(tr ' ' '\n' <<<"$live_args" | sort)" \
    "$(tr ' ' '\n' <<<"$kspp_args" | sort)" \
    "build-iso.sh KSPP_KERNEL_ARGS equals M01's Live token set"
...
assert_grep_fixed 'active LUKS selector lacks one exact policy' \
    "$KARG_CONTRACT" \
    "runtime gate proves the LUKS retry policy active"
assert_grep_fixed 'required_kargs+=("$LUKS_KARG")' "$FIRSTBOOT_FIXTURE" \
    "firstboot records every active per-volume LUKS policy for postcheck"
assert_grep_fixed 'verify_required_kargs /etc/kernel/cmdline "${required_kargs[@]}"' \
    "$FIRSTBOOT_FIXTURE" \
    "firstboot postcheck consumes the complete required-karg array"

# Execute the exact deployed postcondition with two UUID policies. A scalar
# retaining only the loop's last token used to make the negative case pass.
REQUIRED_KARG_CHECK="$TEST_TMPDIR/verify-required-kargs.sh"
sed -n '/^verify_required_kargs() {$/,/^}$/p' \
    "$FIRSTBOOT_FIXTURE" > "$REQUIRED_KARG_CHECK"
first_luks_policy='rd.luks.options=11111111-2222-4333-8444-555555555555=tries=0,discard'
second_luks_policy='rd.luks.options=66666666-7777-4888-8999-aaaaaaaaaaaa=tries=0,discard'
required_karg_bytes="$TEST_TMPDIR/required-kargs.cmdline"
printf '%s %s\n' "$first_luks_policy" "$second_luks_policy" \
    > "$required_karg_bytes"
assert_cmd_success "multi-UUID postcheck accepts every required LUKS policy" \
    bash -c 'log() { :; }; . "$1"; verify_required_kargs "$2" "$3" "$4"' _ \
        "$REQUIRED_KARG_CHECK" "$required_karg_bytes" \
        "$first_luks_policy" "$second_luks_policy"
printf '%s\n' "$second_luks_policy" > "$required_karg_bytes"
assert_cmd_failure "multi-UUID postcheck rejects a missing first LUKS policy" \
    bash -c 'log() { :; }; . "$1"; verify_required_kargs "$2" "$3" "$4"' _ \
        "$REQUIRED_KARG_CHECK" "$required_karg_bytes" \
        "$first_luks_policy" "$second_luks_policy"
...
assert_grep_fixed "rd.luks.options=${luks_uuid}=tries=0,discard" \
    ...
    "canonical merger retains the per-volume unlimited-retry LUKS policy"
...
# Ambiguous invocations must stop before credential handling. In particular,
# --remove --help must never silently remove the existing credential.
grub_before_args=$(sha256sum "$GRUB_FIXTURE/boot/grub2/user.cfg" \
    "$GRUB_FIXTURE/boot/grub2/grub.cfg")
grub_rejects_arguments() {
    local status=0
    env PATH="$FAKE_BIN:$PATH" bash "$GRUB_HELPER_FIXTURE" "$@" \
        >"$TEST_TMPDIR/grub-args.out" 2>"$TEST_TMPDIR/grub-args.err" || status=$?
    [ "$status" -eq 2 ] && [ "$grub_before_args" = \
        "$(sha256sum "$GRUB_FIXTURE/boot/grub2/user.cfg" \
            "$GRUB_FIXTURE/boot/grub2/grub.cfg")" ]
}
assert_cmd_success "GRUB removal rejects combined help without mutation" \
    grub_rejects_arguments --remove --help
assert_cmd_success "GRUB removal rejects an extra empty argument" \
    grub_rejects_arguments --remove ''
assert_cmd_success "GRUB removal rejects an unknown extra argument" \
    grub_rejects_arguments --remove --unknown
assert_cmd_success "GRUB help rejects a combined mutation argument" \
    grub_rejects_arguments --help --remove
assert_cmd_success "GRUB setup rejects an explicitly empty argument" \
    grub_rejects_arguments ''
assert_cmd_success "GRUB helper rejects an unknown command" \
    grub_rejects_arguments --unknown
assert_cmd_success "exact GRUB help remains available" \
    env PATH="$FAKE_BIN:$PATH" bash "$GRUB_HELPER_FIXTURE" --help
