...
for pin_file in "$OFFLINE_PREP" "$BUILD_REDUCED"; do
    assert_grep_fixed 'readonly UBO_VERSION="1.75.0"' "$pin_file" \
        "reduced-dependency helper version pin matches M16"
    assert_grep_fixed 'readonly UBO_SHA256="5b74415860456370644bd80f16125e865b0e6c356bb5dfcfb84069967eaa5287"' \
        "$pin_file" "reduced-dependency helper SHA-256 pin matches M16"
    assert_grep_fixed 'readonly UBO_SIZE="4650100"' "$pin_file" \
        "reduced-dependency helper size pin matches M16"
done
assert_grep_fixed 'local expected_size=4650100' "$BUILD_ISO" \
    "canonical builder cache size pin matches M16"
assert_grep_fixed 'local expected_sha="5b74415860456370644bd80f16125e865b0e6c356bb5dfcfb84069967eaa5287"' \
    "$BUILD_ISO" "canonical builder cache SHA-256 pin matches M16"

# --- Extension paths: langpack activation dir + uBO system-scope staging ----
# uBO itself is delivered profile-locally (distribution-bundled scanning does
# not auto-install regular extensions); distribution/extensions carries only
# the Fedora langpack XPIs.
assert_grep_fixed '/usr/lib64/firefox/distribution/extensions' "$KS_FILE" \
    "Firefox langpack activation path (distribution/extensions)"
assert_grep_fixed 'mozilla/extensions/{ec8030f7-c20a-464f-9b0e-13a3a9e97384}' "$KS_FILE" \
    "uBO system-scope staging path"
...
if [ -f "$FIREFOX_JS" ]; then
    ...
    assert_grep_fixed ',-JSLocalePrompt,-JSLocale");' "$FIREFOX_JS" \
        "FPP overrides exclude JSLocalePrompt and JSLocale (English-spoof rewrite disarmed)"
    assert_not_grep '^user_pref("intl\.accept_languages"' "$FIREFOX_JS" \
        "web-content language is left to the system locale, never pinned in user.js"
...
USER_DEFAULTS_PYEOF
    ...
    assert_grep_fixed 'user_pref("browser.urlbar.quicksuggest.enabled", false);' "$FIREFOX_JS" \
        "Firefox Suggest (address-bar quick suggest) master gate is off"
    assert_grep_fixed 'user_pref("browser.urlbar.suggest.quicksuggest.all", false);' "$FIREFOX_JS" \
        "Firefox Suggest FF146+ all-results switch is off on the user branch"
    assert_grep_fixed 'user_pref("browser.urlbar.suggest.quicksuggest.sponsored", false);' "$FIREFOX_JS" \
        "Firefox Suggest sponsored address-bar results are off"
    assert_grep_fixed 'user_pref("browser.urlbar.amp.featureGate", false);' "$FIREFOX_JS" \
        "adMarketplace sponsored suggestion feature gate is off"
    assert_grep_fixed 'user_pref("browser.urlbar.wikipedia.featureGate", false);' "$FIREFOX_JS" \
        "Wikipedia address-bar suggestion feature gate is off"
...
# Completeness must reject a generator failure even when its emitted prefix
# exactly matches user.js. Exercise the extracted library with real files;
# shell command substitution must not normalize trailing-newline differences.
COMPOSITION_FIXTURE="$TMPDIR2/composition"
mkdir -m 700 "$COMPOSITION_FIXTURE" "$COMPOSITION_FIXTURE/profile"
printf '%s\n' 'user_pref("fixture.base", true);' > "$COMPOSITION_FIXTURE/base.js"
printf '%s\n' 'user_pref("fixture.playground", true);' > "$COMPOSITION_FIXTURE/playground.js"
cp "$TMPDIR2/drm-overrides.js" "$COMPOSITION_FIXTURE/drm.js"
cp "$COMPOSITION_FIXTURE/base.js" "$COMPOSITION_FIXTURE/profile/user.js"
chmod 600 "$COMPOSITION_FIXTURE/profile/user.js"
run_composition_fixture() {
    bash -c '
        . "$1"
        NOID_FF_USERJS_BASE="$2/base.js"
        NOID_FF_USERJS_PLAYGROUND_OVERRIDES="$2/playground.js"
        NOID_FF_USERJS_DRM_OVERRIDES="$2/drm.js"
        if [ "$4" = failed-read ]; then
            # Emit the real file bytes, then inject the producer error.
            cat() { command cat "$@" || return; return 74; }
        fi
        profile_userjs_supported "$3" "$2/profile"
    ' bash "$TMPDIR2/firefox-profiles.sh" "$COMPOSITION_FIXTURE" "$1" "${2:-normal}"
}
assert_cmd_success "exact base composition satisfies the read-only profile check" \
    run_composition_fixture work
printf '\n' >> "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_failure "profile comparison preserves extra trailing-newline differences" \
    run_composition_fixture work
printf '%s' 'user_pref("fixture.base", true);' > "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_failure "profile comparison requires the canonical final newline" \
    run_composition_fixture work
cp "$COMPOSITION_FIXTURE/base.js" "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_failure "matching generator bytes cannot hide a producer error" \
    run_composition_fixture work failed-read
cat "$COMPOSITION_FIXTURE/playground.js" >> "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_success "complete Playground composition satisfies the profile check" \
    run_composition_fixture playground
rm -f -- "$COMPOSITION_FIXTURE/playground.js"
cp "$COMPOSITION_FIXTURE/base.js" "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_failure "missing Playground source cannot bless a matching base prefix" \
    run_composition_fixture playground
printf '%s\n' NOID_FIREFOX_DRM_OPT_IN_V1 \
    > "$COMPOSITION_FIXTURE/profile/.noid-drm-enabled"
chmod 600 "$COMPOSITION_FIXTURE/profile/.noid-drm-enabled"
cat "$COMPOSITION_FIXTURE/drm.js" >> "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_success "complete consented DRM composition satisfies the profile check" \
    run_composition_fixture work
rm -f -- "$COMPOSITION_FIXTURE/drm.js"
cp "$COMPOSITION_FIXTURE/base.js" "$COMPOSITION_FIXTURE/profile/user.js"
assert_cmd_failure "missing consented DRM source cannot bless a matching base prefix" \
    run_composition_fixture work
...
printf '%s\n' '{"addons":[{"id":"uBlock0@raymondhill.net","version":"1.75.0","type":"extension","signedState":2,"active":true,"visible":true,"path":"/fixture/uBO.xpi"}]}' \
...
assert_cmd_success "Firefox signed-state parser accepts exact native evidence" \
    python3 "$SIGNED_STATE_PARSER" "$TMPDIR2/signed-state.json" \
        uBlock0@raymondhill.net 1.75.0 /fixture/uBO.xpi
...
create_test_xpi "$TMPDIR2/ubo-valid.xpi" uBlock0@raymondhill.net 1.75.0 128.0 '200.*' 1
assert_cmd_success "signed compatible uBO fixture validates" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-valid.xpi" \
        uBlock0@raymondhill.net 1.75.0 1 150.0
create_test_xpi "$TMPDIR2/ubo-unbounded.xpi" uBlock0@raymondhill.net 1.75.0 128.0 '*' 1
assert_cmd_success "literal wildcard maximum is treated as unbounded" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-unbounded.xpi" \
        uBlock0@raymondhill.net 1.75.0 1 152.0
create_test_xpi "$TMPDIR2/ubo-unsigned.xpi" uBlock0@raymondhill.net 1.75.0 - - 0
assert_cmd_failure "uBO validator rejects a missing Mozilla signature container" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-unsigned.xpi" \
        uBlock0@raymondhill.net 1.75.0 1 150.0
create_test_xpi "$TMPDIR2/ubo-wrong-id.xpi" attacker@example.invalid 1.75.0 - - 1
assert_cmd_failure "uBO validator rejects a manifest identity mismatch" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-wrong-id.xpi" \
        uBlock0@raymondhill.net 1.75.0 1 150.0
create_test_xpi "$TMPDIR2/ubo-incompatible.xpi" uBlock0@raymondhill.net 1.75.0 999.0 - 1
...
assert_cmd_success "native process-query fixture requires pgrep" command -v pgrep
run_firefox_query_fixture() {
    env FIREFOX_PROFILE_LIBRARY="$TMPDIR2/firefox-profiles.sh" \
        FIREFOX_QUERY_MODE="$1" \
        FIREFOX_MISSING_PROGRAM="$TMPDIR2/absent-bin/pgrep" bash -c '
        . "$FIREFOX_PROFILE_LIBRARY"
        case "$FIREFOX_QUERY_MODE" in
        ...
        pgrep() {
            case "$FIREFOX_QUERY_MODE" in
                ...
                missing-command) "$FIREFOX_MISSING_PROGRAM" ;;
...
assert_cmd_failure "native no-match process query permits an inactive result" \
    run_firefox_query_fixture absent
for query_case in native-error missing-command partial-error inconsistent-no-match \
    ...
    assert_cmd_success "Firefox mutation is blocked for $query_case process evidence" \
        run_firefox_query_fixture "$query_case"
done
...
case "$*" in
    ...
    *FILESTATES*FILELINKTOS*FILEDIGESTS*)
        ...
        # Keep the manifest larger than a normal pipe buffer. The reassert
        # helper must consume it completely rather than closing the producer
        # after an early match and turning a valid lookup into SIGPIPE/141.
        printf '%262144s\n' ''
        ;;
    *FILEDIGESTS*)
...
ff_mock_langpack_manifest_bytes=$(
    PATH="$FF_OVERLAY_FIXTURE/mock-bin:$PATH" \
        rpm -q --qf \
        '[%{FILENAMES}\t%{FILESTATES}\t%{FILELINKTOS}\t%{FILEDIGESTS}\n]' \
        firefox-langpacks | wc -c
)
if [ "$ff_mock_langpack_manifest_bytes" -gt 262144 ]; then
    _pass "Firefox langpack RPM fixture exceeds the pipe buffer regression threshold"
else
    _fail "Firefox langpack RPM fixture is too small to exercise the pipefail regression"
fi
if PATH="$FF_OVERLAY_FIXTURE/mock-bin:$PATH" bash "$FF_OVERLAY_FIXTURE/reassert.sh"; then
    _pass "Firefox overlay generator consumes an oversized pristine RPM manifest"
else
    _fail "Firefox overlay generator consumes an oversized pristine RPM manifest"
fi
assert_eq \
    "$(printf '%s\n' \
...
assert_grep_fixed 'Firefox Suggest in the address bar' "$TMPDIR2/firefox-hardening.md" \
    "Firefox documentation states that address-bar Firefox Suggest is off"
...
create_test_xpi "$HARDEN_SOURCE/ubo.xpi" uBlock0@raymondhill.net 1.75.0 - - 1
...
create_test_xpi "$FF_SETUP_SOURCE/ubo.xpi" uBlock0@raymondhill.net 1.75.0 - - 1
...
create_test_xpi "$FF_SETUP_PROFILE/extensions/uBlock0@raymondhill.net.xpi" \
    uBlock0@raymondhill.net 1.75.0 - - 1
...
PAYLOAD_FIXTURE_PATH="$FIXTURE_CACHE/ubo/1.75.0/uBlock0_1.75.0.firefox.signed.xpi"
...
if [ ! -e "$FIXTURE_BAD_CACHE/MANIFEST.txt" ] && \
   [ ! -e "$FIXTURE_BAD_CACHE/ubo/1.75.0/uBlock0_1.75.0.firefox.signed.xpi" ]; then
...
assert_cmd_failure "prep rejects a symlinked cache component before traversal" \
    env CACHE_DIR="$REDUCED_FIXTURE/component-cache" \
        "$FIXTURE_REPO/scripts/offline-prep.sh"
if [ ! -e "$REDUCED_FIXTURE/component-target/1.75.0" ]; then
