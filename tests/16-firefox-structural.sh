...
for pin_file in "$OFFLINE_PREP" "$BUILD_REDUCED"; do
    assert_grep_fixed 'readonly UBO_VERSION="1.74.0"' "$pin_file" \
        "reduced-dependency helper version pin matches M16"
    assert_grep_fixed 'readonly UBO_SHA256="175756d74468c9ba45863f7fc333d3be670f82d5b066314e915814dd547d1652"' \
        "$pin_file" "reduced-dependency helper SHA-256 pin matches M16"
    assert_grep_fixed 'readonly UBO_SIZE="4617614"' "$pin_file" \
        "reduced-dependency helper size pin matches M16"
done
assert_grep_fixed 'local expected_size=4617614' "$BUILD_ISO" \
    "canonical builder cache size pin matches M16"
assert_grep_fixed 'local expected_sha="175756d74468c9ba45863f7fc333d3be670f82d5b066314e915814dd547d1652"' \
...
printf '%s\n' '{"addons":[{"id":"uBlock0@raymondhill.net","version":"1.74.0","type":"extension","signedState":2,"active":true,"visible":true,"path":"/fixture/uBO.xpi"}]}' \
...
assert_cmd_success "Firefox signed-state parser accepts exact native evidence" \
    python3 "$SIGNED_STATE_PARSER" "$TMPDIR2/signed-state.json" \
        uBlock0@raymondhill.net 1.74.0 /fixture/uBO.xpi
...
create_test_xpi "$TMPDIR2/ubo-valid.xpi" uBlock0@raymondhill.net 1.74.0 128.0 '200.*' 1
assert_cmd_success "signed compatible uBO fixture validates" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-valid.xpi" \
        uBlock0@raymondhill.net 1.74.0 1 150.0
create_test_xpi "$TMPDIR2/ubo-unbounded.xpi" uBlock0@raymondhill.net 1.74.0 128.0 '*' 1
assert_cmd_success "literal wildcard maximum is treated as unbounded" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-unbounded.xpi" \
        uBlock0@raymondhill.net 1.74.0 1 152.0
create_test_xpi "$TMPDIR2/ubo-unsigned.xpi" uBlock0@raymondhill.net 1.74.0 - - 0
assert_cmd_failure "uBO validator rejects a missing Mozilla signature container" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-unsigned.xpi" \
        uBlock0@raymondhill.net 1.74.0 1 150.0
create_test_xpi "$TMPDIR2/ubo-wrong-id.xpi" attacker@example.invalid 1.74.0 - - 1
assert_cmd_failure "uBO validator rejects a manifest identity mismatch" \
    python3 "$TMPDIR2/validate-webextension.py" "$TMPDIR2/ubo-wrong-id.xpi" \
        uBlock0@raymondhill.net 1.74.0 1 150.0
create_test_xpi "$TMPDIR2/ubo-incompatible.xpi" uBlock0@raymondhill.net 1.74.0 999.0 - 1
...
create_test_xpi "$HARDEN_SOURCE/ubo.xpi" uBlock0@raymondhill.net 1.74.0 - - 1
...
create_test_xpi "$FF_SETUP_SOURCE/ubo.xpi" uBlock0@raymondhill.net 1.74.0 - - 1
...
create_test_xpi "$FF_SETUP_PROFILE/extensions/uBlock0@raymondhill.net.xpi" \
    uBlock0@raymondhill.net 1.74.0 - - 1
...
PAYLOAD_FIXTURE_PATH="$FIXTURE_CACHE/ubo/1.74.0/uBlock0_1.74.0.firefox.signed.xpi"
...
if [ ! -e "$FIXTURE_BAD_CACHE/MANIFEST.txt" ] && \
   [ ! -e "$FIXTURE_BAD_CACHE/ubo/1.74.0/uBlock0_1.74.0.firefox.signed.xpi" ]; then
...
assert_cmd_failure "prep rejects a symlinked cache component before traversal" \
    env CACHE_DIR="$REDUCED_FIXTURE/component-cache" \
        "$FIXTURE_REPO/scripts/offline-prep.sh"
if [ ! -e "$REDUCED_FIXTURE/component-target/1.74.0" ]; then
