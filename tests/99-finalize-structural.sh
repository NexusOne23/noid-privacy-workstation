...
ACL_UNIT_ROOT=""
trap 'rm -rf "$TEST_TMPDIR" "${ACL_UNIT_ROOT:-}"' EXIT
...
# Every section is a named, dated release. The closed ordered inventory
# rejects sections that are not assigned to an exact numbered release.
all_release_headings=$(grep '^## \[' "$PROJECT_ROOT/CHANGELOG.md")
actual_release_headings=$(grep '^## \[v' "$PROJECT_ROOT/CHANGELOG.md")
...
assert_eq "$expected_release_headings" "$actual_release_headings" \
    "changelog contains exactly nine named and dated releases, newest first"
assert_eq "$expected_release_headings" "$all_release_headings" \
    "changelog assigns every section to an exact numbered release"
...
assert_eq "## [$branding_version] - 2026-09-02" \
...
# Keep the real ExecStart and helper in an offline root. /var/tmp permits
# executable-file checks on the live host; /tmp is deliberately noexec.
# Vendor units supply the dependency graph, without requiring their manpages
# or borrowing a helper from an already installed NoID Privacy image.
ACL_UNIT_ROOT=$(mktemp -d /var/tmp/noid-acl-unit-test.XXXXXX)
install -d "$ACL_UNIT_ROOT/etc/systemd/system" \
    "$ACL_UNIT_ROOT/usr/lib/systemd" "$ACL_UNIT_ROOT/usr/libexec"
cp -a /usr/lib/systemd/system "$ACL_UNIT_ROOT/usr/lib/systemd/"
install -m 0644 "$TEST_TMPDIR/noid-live-payload-acl-restore.service" \
    "$ACL_UNIT_ROOT/etc/systemd/system/noid-live-payload-acl-restore.service"
install -m 0755 "$TEST_TMPDIR/noid-restore-live-payload-acls" \
    "$ACL_UNIT_ROOT/usr/libexec/noid-restore-live-payload-acls"
ACL_UNIT_VERIFY=(env -i PATH=/usr/sbin:/usr/bin LC_ALL=C
    SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system
    systemd-analyze --root="$ACL_UNIT_ROOT" --generators=no --man=no
    --recursive-errors=no verify noid-live-payload-acl-restore.service)
assert_cmd_success "ACL early-boot unit validates" \
    "${ACL_UNIT_VERIFY[@]}"
chmod 0644 "$ACL_UNIT_ROOT/usr/libexec/noid-restore-live-payload-acls"
assert_cmd_failure "ACL unit verification rejects a non-executable source helper" \
    "${ACL_UNIT_VERIFY[@]}"
chmod 0755 "$ACL_UNIT_ROOT/usr/libexec/noid-restore-live-payload-acls"
...
# Execute the actual finalizer block with its three source payloads redirected
# into private scratch. bash -n accepts only one script; extra paths become
# arguments, so a multi-path invocation silently ignores later syntax errors.
if python3 - "$KS_FILE" "$M10_KS_FILE" "$ACL_UNIT_ROOT" <<'HISTORY_SYNTAX_PY'
from pathlib import Path
import subprocess
import sys

finalize, producer = (Path(p).read_text() for p in sys.argv[1:3])
scratch = Path(sys.argv[3]) / "history-syntax"
scratch.mkdir(mode=0o700)
specs = [
    ("/etc/profile.d/98-noid-bash-history.sh", "HIST_EOF"),
    ("/usr/local/libexec/noid-bash-history-compact", "HIST_COMPACT_EOF"),
    ("/usr/local/bin/noid-toggle-bash-history", "TOGGLE_HIST_EOF"),
]
paths = {source: str(scratch / Path(source).name) for source, _ in specs}
payloads = {}
for source, marker in specs:
    body = producer.split("<<'" + marker + "'\n", 1)[1].split(
        "\n" + marker + "\n", 1)[0] + "\n"
    for original, fixture in paths.items():
        body = body.replace(original, fixture)
    payloads[paths[source]] = body
block = finalize[finalize.index("M10_FC_PREFIX_LITERAL=$(cat "):
                 finalize.index("for m10_history_meta in ")]
for original, fixture in paths.items():
    block = block.replace(original, fixture)
probe = ('set -euo pipefail\nfail=0\n'
         'log() { printf "%s\\n" "$*"; }\n' + block + 'exit "$fail"\n')
for invalid in [None, *payloads]:
    for filename, body in payloads.items():
        path = Path(filename)
        path.write_text(body + ("\nif then\n" if filename == invalid else ""))
        path.chmod(0o700)
    result = subprocess.run(["bash", "-c", probe], capture_output=True, text=True)
    expected = 0 if invalid is None else 1
    if result.returncode != expected:
        raise SystemExit(f"history gate: invalid={invalid}, "
                         f"expected={expected}, got={result.returncode}")
    if invalid and "Module 10 locked/atomic Bash history contract invalid" not in result.stdout:
        raise SystemExit("history gate missed its failure accumulator/diagnostic")
HISTORY_SYNTAX_PY
then
    _pass "M99 accepts valid history scripts and rejects a syntax error in each one"
else
    _fail "M99 must syntax-check every history script independently"
fi
...
assert_grep_fixed 'native permission policy/action metadata or trigger count invalid' \
    "$KS_FILE" "final gate requires exact native artifacts and six triggers"
...
# Bind the finalizer's trigger count and package loop to M10's real action
# inventory: a trigger added to M10 without this parity check would only
# surface as a failed compose.
M10_FILE="$PROJECT_ROOT/kickstart/snippets/10-pam-login.ks"
extract_heredoc "$M10_FILE" PERMISSION_ACTION_EOF \
    "$TEST_TMPDIR/permission-policy.actions" \
    || _fail "M10 permission action extraction"
m10_trigger_count=$(grep -c '^post_transaction:' \
    "$TEST_TMPDIR/permission-policy.actions" || true)
m99_trigger_pin=$(grep -oE 'noid-permission-policy\.actions 2>/dev/null \|\| true\)" -ne [0-9]+' \
    "$KS_FILE" | grep -oE '[0-9]+$' || true)
assert_eq "$m10_trigger_count" "$m99_trigger_pin" \
    "finalizer trigger-count pin equals M10's action inventory"
m99_trigger_pkgs=$(sed -n 's/^for permission_pkg in \(.*\); do$/\1/p' "$KS_FILE" \
    | tr ' ' '\n' | sort)
m10_trigger_pkgs=$(sed -n 's/^post_transaction:\([^:]*\):.*/\1/p' \
    "$TEST_TMPDIR/permission-policy.actions" | sort)
assert_eq "$m10_trigger_pkgs" "$m99_trigger_pkgs" \
    "finalizer trigger package loop equals M10's action inventory"
...
assert_grep_fixed '175756d74468c9ba45863f7fc333d3be670f82d5b066314e915814dd547d1652' \
...
# git). This pre-LOCK semantic test must accept REVIEW so the required gate
# can run before attestation. 00-status-metadata still requires LOCKED for
# the complete release tree.
assert_grep_extended '^# Status: (REVIEW|LOCKED) [0-9]{4}-[0-9]{2}-[0-9]{2} \(v[0-9]+(\.[0-9]+)*\) — .+\.$' "$KS_FILE"
assert_not_grep_extended \
