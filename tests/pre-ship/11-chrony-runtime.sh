...
cleanup() {
    local rc=${1:-$?} restore_failed=0
    trap - EXIT HUP INT TERM
    if [[ $restore_cookie_backup -eq 1 && -n $cookie_backup \
          && -d $cookie_backup && ! -L $cookie_backup ]]; then
        if ! systemctl stop chronyd-restricted.service >/dev/null 2>&1; then
            echo "FAIL: cannot stop chronyd for recovery; backup retained at $cookie_backup" >&2
            exit 1
        fi
        if ! find /var/lib/chrony -maxdepth 1 -type f -name '*.nts' -print0 \
                > "$TMPDIR/current-cookies" \
           || ! find "$cookie_backup" -maxdepth 1 -type f -name '*.nts' -print0 \
                > "$TMPDIR/backup-cookies"; then
            systemctl start chronyd-restricted.service >/dev/null 2>&1 || true
            echo "FAIL: cannot inventory cookie recovery; backup retained at $cookie_backup" >&2
            exit 1
        fi
        while IFS= read -r -d '' cookie; do
            rm -f -- "$cookie" || restore_failed=1
        done < "$TMPDIR/current-cookies"
        while IFS= read -r -d '' cookie; do
            mv -- "$cookie" /var/lib/chrony/ || restore_failed=1
        done < "$TMPDIR/backup-cookies"
        systemctl start chronyd-restricted.service >/dev/null 2>&1 || rc=1
        if [[ $restore_failed -eq 1 ]]; then
            echo "FAIL: cookie recovery incomplete; backup retained at $cookie_backup" >&2
            exit 1
        fi
