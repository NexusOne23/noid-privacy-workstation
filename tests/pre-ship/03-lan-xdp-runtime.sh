...
cleanup() {
    ...
    set +e
    [ "$owned_va" -eq 0 ] || ip link del "$va" >/dev/null 2>&1
    [ "$owned_vc" -eq 0 ] || ip link del "$vc" >/dev/null 2>&1
...
owned_vc=1
ip link set "$va" netns "$pid_a"
owned_va=0  # The owned namespace now cleans up this pair, including its peer.
ip link set "$vb" netns "$pid_b"
ip link set "$vc" netns "$pid_c"
owned_vc=0
ip link set "$vd" netns "$pid_d"
...
# Reserve our pin root exclusively; cleanup must never adopt an existing tree.
pin=$(mktemp -d /sys/fs/bpf/noid_xdp_preship_XXXXXXXX)
mkdir "$pin/progs" "$pin/maps"
...
# The WPA four-way exchange uses unicast EAPOL-Key frames after association.
# Before TC has observed the station's newly selected local MAC, XDP must not
# pass such a frame merely because its EAPOL envelope is structurally valid.
rotated_eapol_before=$(stat_count 2)
rotated_default_before=$(stat_count 8)
in_namespace "$pid_a" python3 "$FIXTURES" --interface "$va" \
    --suite eapol-key-rotated >/dev/null
[ $(( $(stat_count 2) - rotated_eapol_before )) -eq 0 ] || {
    echo "FAIL  $TEST_NAME: unlearned rotated-MAC EAPOL-Key took the pass path" >&2
    exit 1
}
[ $(( $(stat_count 8) - rotated_default_before )) -eq 1 ] || {
    echo "FAIL  $TEST_NAME: unlearned rotated-MAC EAPOL-Key did not fail closed" >&2
    exit 1
}
in_namespace "$pid_b" python3 "$FIXTURES" --interface "$vb" \
...
# Once the exact shared TC egress program has learned that MAC, the same
# realistic unicast EAPOL-Key frame must pass. This is the repository-level
# regression for the X13 observation; it never attaches to a host Wi-Fi link.
rotated_eapol_before=$(stat_count 2)
rotated_default_before=$(stat_count 8)
in_namespace "$pid_a" python3 "$FIXTURES" --interface "$va" \
    --suite eapol-key-rotated >/dev/null
[ $(( $(stat_count 2) - rotated_eapol_before )) -eq 1 ] || {
    echo "FAIL  $TEST_NAME: learned rotated-MAC EAPOL-Key did not pass" >&2
    exit 1
}
[ $(( $(stat_count 8) - rotated_default_before )) -eq 0 ] || {
    echo "FAIL  $TEST_NAME: learned rotated-MAC EAPOL-Key hit the default-drop path" >&2
    exit 1
}
...
echo "PASS  $TEST_NAME: verifier+generic+TC+36 invalid/unsupported+direction matrix+static-address unsolicited DHCP drop+7 baseline valid+unicast EAPOL-Key stable-MAC rotation+multi-NIC binding; native=$native"
