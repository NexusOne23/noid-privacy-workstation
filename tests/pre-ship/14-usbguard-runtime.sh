...
STOP_RECOVERY=/etc/systemd/system/usbguard.service.d/10-timeout-abort.conf
...
[ -f "$STOP_RECOVERY" ] && [ ! -L "$STOP_RECOVERY" ] || \
    fail "USBGuard stop-recovery drop-in is missing, non-regular or symlinked"
[ "$(stat -Lc '%U:%G:%a:%h' "$STOP_RECOVERY")" = root:root:644:1 ] || \
    fail "USBGuard stop-recovery drop-in metadata differs"
matchpathcon -V "$STOP_RECOVERY" >/dev/null 2>&1 || \
    fail "USBGuard stop-recovery drop-in SELinux label differs"
cmp -s "$STOP_RECOVERY" <(printf '%s\n' \
    '[Service]' \
    'TimeoutStopSec=15s' \
    'TimeoutStopFailureMode=kill') || \
    fail "USBGuard stop-recovery drop-in bytes differ"
...
[ "$(systemctl show usbguard.service -P TimeoutStopUSec)" = 15s ] || \
    fail "USBGuard effective stop timeout is not 15 seconds"
[ "$(systemctl show usbguard.service -P TimeoutStopFailureMode)" = kill ] || \
    fail "USBGuard effective stop timeout still takes the SIGABRT path"
