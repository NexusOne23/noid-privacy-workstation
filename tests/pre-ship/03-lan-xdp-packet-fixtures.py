...
def eapol_key(*, destination_mac: bytes = ROTATED_MAC) -> bytes:
    """A bounded 95-byte EAPOL-Key body addressed to the current station."""
    body = b"\x02" + b"\0" * 94
    header = struct.pack("!BBH", 2, 3, len(body))
    return ethernet(header + body, 0x888E, destination_mac)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "dhcp-reply-rotated", "eapol-key-rotated",
        ),
    )
    ...
    elif args.suite == "dhcp-reply-rotated":
        frames = [(
            "dhcp-reply-rotated",
            dhcp_reply(client_mac=ROTATED_MAC, xid=ROTATED_DHCP_XID),
        )]
    else:
        frames = [("eapol-key-rotated", eapol_key())]
