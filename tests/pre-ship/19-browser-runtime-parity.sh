...
def verify_firefox_common(ff):
    ...
    # Firefox Suggest (address-bar quick suggest) stays off on the user branch:
    # Firefox 156 flips the default branch on by region/locale (DE/FR/IT/GB/US).
    assert ff["browser.urlbar.quicksuggest.enabled"] is False
    assert ff["browser.urlbar.suggest.quicksuggest.all"] is False
    assert ff["browser.urlbar.suggest.quicksuggest.sponsored"] is False
    assert ff["browser.urlbar.amp.featureGate"] is False
    assert ff["browser.urlbar.wikipedia.featureGate"] is False
    ...
    assert ff["privacy.fingerprintingProtection.overrides"].endswith(
        ",-JSLocalePrompt,-JSLocale"
    )
    assert "intl.accept_languages" not in ff
...
addon(ff_extensions, "uBlock0@raymondhill.net", "1.75.0", ff_ubo)
