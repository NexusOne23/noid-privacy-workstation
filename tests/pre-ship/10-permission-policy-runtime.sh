...
[[ $(grep -c '^post_transaction:' "$action") -eq 6 ]] || {
    echo "FAIL: permission dnf5 action trigger count is not six" >&2
    exit 1
}
...
permission_versions=$(
    for pkg in util-linux shadow-utils pam usermode glusterfs-fuse libgtop2 cronie crontabs sudo; do
