...
for tool in grep mktemp nft python3 rm setpriv stat systemctl wc; do
...
# A failed table query is neither absence nor proof of an authorization denial.
nft_table_state() {
    local inventory
    inventory=$(nft --json list tables) || return 1
    python3 -c '
import json, sys
rows = json.load(sys.stdin)["nftables"]
if not isinstance(rows, list):
    raise SystemExit("invalid nft table inventory")
found = False
for row in rows:
    if not isinstance(row, dict):
        raise SystemExit("invalid nft table record")
    if "table" in row:
        table = row["table"]
        if not isinstance(table, dict) or not all(isinstance(table.get(k), str) for k in ("family", "name")):
            raise SystemExit("invalid nft table identity")
        found |= table["family"] == "inet" and table["name"] == sys.argv[1]
    elif "metainfo" not in row:
        raise SystemExit("unexpected nft inventory record")
print("present" if found else "absent")
' "$1" <<<"$inventory"
}

if [ "$runtime_status" = MODE=DISABLED ]; then
    table_state=$(nft_table_state noid_wan_strict) || fail "cannot inspect DISABLED nft postcondition"
    [[ $table_state == absent ]] || fail "DISABLED postcondition still has the M06 nft table"
...
# The same capability-empty identity must not mutate the candidate namespace.
# Refuse a collision before installing cleanup; never delete an existing table.
# NOID_NFT_BOUNDARY_BEGIN
table_state=$(nft_table_state "$PROBE_TABLE") || fail "cannot inventory nft probe target"
[[ $table_state == absent ]] || fail "nft probe table already exists; preserved"
probe_owned=false
probe_diagnostics=$(mktemp -p /var/tmp noid-wan-nft-probe.XXXXXX) || fail "cannot allocate nft diagnostics"
cleanup_probe() {
    if $probe_owned; then
        nft delete table inet "$PROBE_TABLE" >/dev/null 2>&1 || true
    fi
    rm -f -- "$probe_diagnostics"
}
trap cleanup_probe EXIT
# A root positive control proves this exact operation is supported before a
# rejection from the capability-empty identity is interpreted.
nft create table inet "$PROBE_TABLE" || fail "root nft positive control allocation failed"
probe_owned=true
nft add table inet "$PROBE_TABLE" || fail "root nft positive control failed"
nft delete table inet "$PROBE_TABLE" || fail "cannot remove root nft positive control"
probe_owned=false
nft_rc=0
setpriv --reuid=65534 --regid=65534 --clear-groups \
    --inh-caps=-all --ambient-caps=-all --bounding-set=-all \
    nft add table inet "$PROBE_TABLE" >"$probe_diagnostics" 2>&1 || nft_rc=$?
table_state=$(nft_table_state "$PROBE_TABLE") || fail "cannot inspect nft probe postcondition"
if [[ $table_state == present ]]; then
    probe_owned=true
    fail "unprivileged nft probe left a table"
fi
[[ $nft_rc -eq 1 ]] || fail "nft probe returned unexpected status $nft_rc"
python3 - "$probe_diagnostics" "$PROBE_TABLE" <<'NFT_DENIAL_PY' || \
    fail "nft failure was not an exact native permission denial"
from pathlib import Path
import re
import sys

text = Path(sys.argv[1]).read_text()
table = re.escape(sys.argv[2])
rule_error = (r"Error: Could not process rule: (Operation not permitted|Permission denied)\n"
              r"add table inet " + table + r"\n\^+\n?")
cache_error = (r"(?:Operation not permitted \(you must be root\)\n)?"
               r"netlink: Error: cache initialization failed: Operation not permitted\n?")
if not (re.fullmatch(rule_error, text) or re.fullmatch(cache_error, text)):
    raise SystemExit("unrecognized nft rejection")
NFT_DENIAL_PY
rm -f -- "$probe_diagnostics"
trap - EXIT
# NOID_NFT_BOUNDARY_END
note "UID 65534 cannot mutate the host nft namespace"

echo "PASS  $TEST_NAME [$PASS_ID]: mode=${runtime_status#MODE=}, raw=permission-denied, nft-mutation=permission-denied"
