...
read_software_plugins() {
    python3 - "$1" <<'SOFTWARE_ENVIRONMENT_PY'
...
if not re.fullmatch(r"[0-9]+", pid):
    raise SystemExit("invalid software PID")
data = Path(f"/proc/{pid}/environ").read_bytes()
if not data or not data.endswith(b"\0"):
    raise SystemExit("software environment is empty or truncated")
prefix = b"GNOME_SOFTWARE_PLUGINS_ALLOWLIST="
values = [record[len(prefix):] for record in data.split(b"\0") if record.startswith(prefix)]
if len(values) != 1 or not re.fullmatch(rb"[a-z0-9_-]+(?:,[a-z0-9_-]+)*", values[0]):
    raise SystemExit("software plugin assignment is missing, duplicated or invalid")
print(values[0].decode("ascii"))
SOFTWARE_ENVIRONMENT_PY
}
...
# /var/lib/fwupd is root-only. An unprivileged test -d cannot distinguish
# an absent child from one hidden by that intentional access boundary.
if ! sudo -n /usr/bin/python3 - <<'FWUPD_MUTABLE_EVIDENCE_PY'
from pathlib import Path
import stat

base = Path("/var/lib/fwupd")
for directory in (base, base / "remotes.d"):
    try:
        metadata = directory.lstat()
    except FileNotFoundError:
        if directory == base:
            raise SystemExit("fwupd state directory is missing")
        continue
    if (not stat.S_ISDIR(metadata.st_mode) or metadata.st_uid != 0
            or metadata.st_gid != 0 or metadata.st_mode & 0o022):
        raise SystemExit("unsafe fwupd mutable directory")
    if directory != base and any(entry.name.endswith(".conf") for entry in directory.iterdir()):
        raise SystemExit("unexpected mutable fwupd remote definition")
FWUPD_MUTABLE_EVIDENCE_PY
then
    fail "privileged fwupd mutable-source verification failed"
fi
...
process_plugins=$(read_software_plugins "$software_pid") || \
    fail "Fedora-RPM process environment is unreadable or invalid"
...
process_plugins=$(read_software_plugins "$software_pid") || \
    fail "post-RPM process environment is unreadable or invalid"
