...
for required_command in \
        ...
        readlink rm rpm sed sha256sum stat systemctl systemd-run tail timeout; do
...
python3 -I - "$shell_pid" <<'GSK_SHELL_PROCESS_PYEOF' || \
    fail "could not verify the GNOME Shell process evidence"
import pathlib
import sys
...
try:
    ...
    reject("cannot read complete GNOME Shell process records")

if not command.endswith(b"\0"):
    reject("GNOME Shell command record is empty or truncated")
argv = command[:-1].split(b"\0")
if len(argv) < 2 or argv[:2] != [b"/usr/bin/gnome-shell", b"--mode=user"]:
    reject("systemd MainPID is not the user GNOME Shell")
if environment and not environment.endswith(b"\0"):
    reject("GNOME Shell environment record is truncated")
if any(entry.startswith(b"GSK_RENDERER=") for entry in environment.split(b"\0")):
    reject("GNOME Shell inherited the application-only renderer override")
GSK_SHELL_PROCESS_PYEOF
