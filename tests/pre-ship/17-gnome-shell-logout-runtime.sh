...
audit_shell_abends() {
    local output error_file rc=0
    error_file=$(mktemp -p /var/tmp noid-f284-audit.XXXXXX) || \
        fail "cannot allocate audit-query diagnostics"
    output=$(LC_ALL=C ausearch -m ANOM_ABEND --start boot --raw 2>"$error_file") || rc=$?
    # Native --raw returns 1 with empty stdout/stderr when there are no matches.
    # The same status also covers input/config errors; diagnostics invalidate it.
    if [[ -s $error_file || ( $rc -ne 0 && $rc -ne 1 ) \
          || ( $rc -eq 1 && -n $output ) ]]; then
        rm -f -- "$error_file"
        fail "ausearch could not read current-boot anomalies"
    fi
    rm -f -- "$error_file"
    awk '$1 == "type=ANOM_ABEND" && /(^|[[:space:]])exe="\/usr\/bin\/gnome-shell"([[:space:]]|$)/ { count++ }
...
if ! python3 - "$journal_file" "$old_pid" "$TEST_UID" <<'PY'
import json
import pathlib
import sys
...
for number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
    ...
    if (exe == "/usr/bin/gnome-shell"
            and row.get("_PID") == old_pid
            and row.get("_UID") == expected_uid
            and message == "Shutting down GNOME Shell"):
        shutdown_seen = True
