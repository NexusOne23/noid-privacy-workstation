...
assert_cmd_success "libvirt probe lifecycle preserves existing domains and cleans up owned probes" \
    python3 "$PROJECT_ROOT/tests/fixtures/libvirt_lifecycle.py" "$LIBVIRT_RUNTIME"
...
assert_eq 6 "$(grep -c '^post_transaction:' "$TMPDIR/permission-policy.actions")" \
    "dnf5 action has one trigger per owning package family"
for pkg in util-linux shadow-utils glusterfs-fuse cronie crontabs sudo; do
...
if python3 - "$TMPDIR/history-toggle" <<'HISTORY_ARGV_PY'
...
cases = [(97, ["ephemeral"]), (97, ["default"]), (0, []), (0, ["status"]),
         (2, [""]), (2, ["unknown"]), (2, ["status", "extra"]),
         (2, ["ephemeral", "extra"]), (2, ["default", "extra"])]
for expected, args in cases:
    result = subprocess.run(["/bin/bash", "-c", source, "history-fixture", *args],
                            stdin=subprocess.DEVNULL, capture_output=True,
                            timeout=5, check=False)
    assert result.returncode == expected, (args, result.returncode, expected)
HISTORY_ARGV_PY
then
    _pass "history toggle validates nine argument cases before mutation"
else
    _fail "history toggle validates nine argument cases before mutation"
fi
...
assert_cmd_success "history gate stops after signals and cleans its private state" \
    python3 - "$HISTORY_RUNTIME" <<'HISTORY_SIGNAL_PY'
from pathlib import Path
import signal
import subprocess
import sys
...
for sig in (None, signal.SIGHUP, signal.SIGINT, signal.SIGTERM):
    with tempfile.TemporaryDirectory() as temporary:
        ...
        owned.mkdir()
        command = "" if sig is None else f"kill -{sig.value} $$\n"
        result = subprocess.run(
            ["bash", "-c", "set -euo pipefail\ntmp=$1\n" + cleanup
             + "\n" + command + "printf CONTINUED\n", "fixture", str(owned)],
            capture_output=True, text=True, timeout=5,
        )
        assert result.returncode == (0 if sig is None else 128 + sig.value), (
            sig, result.returncode, result.stderr,
        )
        assert ("CONTINUED" in result.stdout) == (sig is None)
        assert not owned.exists()
print("PASS: normal completion and three interruption cases")
HISTORY_SIGNAL_PY
