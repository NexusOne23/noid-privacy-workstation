...
if [ -f "$HORLOGESKYNET_LICENSE" ] && \
   [ "$(sha256sum "$HORLOGESKYNET_LICENSE" | awk '{print $1}')" \
    ...
    ok "exact HorlogeSkynet v140.3 MIT notice retained in repository"
else
    err "HorlogeSkynet v140.3 MIT notice missing or hash-drifted"
fi
if cmp -s \
...
#!/usr/bin/python3
import pathlib, sys
payload, identity, expected, require_signature, browser = sys.argv[1:6]
...
printf '%s\n' '{}' > "$TB_OVERLAY_FIXTURE/cache/dkim-compatibility.json"
chmod 0644 "$TB_OVERLAY_FIXTURE/cache/dkim-compatibility.json"
...
sed \
    ...
    -e "s#/var/lib/noid-privacy/managed-extensions/dkim-compatibility.json#$TB_OVERLAY_FIXTURE/dkim-current-compat.json#g" \
...
chmod 0644 "$TB_OVERLAY_FIXTURE/dkim-current.xpi"
if PATH="$TB_OVERLAY_FIXTURE/mock-bin:$PATH" bash "$TB_OVERLAY_FIXTURE/reassert.sh"; then
...
# Use native pgrep/ps failures and owned live/zombie children. The full CLI
# counterexamples keep two private registered profiles unchanged, before and
# after the operation lock, for apply, automatic and removal workflows.
if python3 "$REPO_ROOT/tests/fixtures/thunderbird_process.py" "$TB_KS"; then
    ok "native process evidence and complete CLI preservation counterexamples pass"
else
    err "process-query failures can authorize Thunderbird profile changes"
fi
TB_PROCESS_SELF_CHECK="$TB_TEST_TMP/process-guard-post.sh"
awk '
    /^if ! bash -s -- "\$HARDEN_PROFILE_CANDIDATE" <<.TB_PROCESS_GUARD_CHECK_EOF./ { copy=1; next }
    copy && /^TB_PROCESS_GUARD_CHECK_EOF$/ { exit }
    copy { print }
' "$TB_KS" > "$TB_PROCESS_SELF_CHECK"
if [ -s "$TB_PROCESS_SELF_CHECK" ] && \
   bash "$TB_PROCESS_SELF_CHECK" "$TB_HARDEN_TMP"; then
    ok "M35 post checks the real candidate guard with no-match, dead and query-failure cases"
else
    err "M35 process-guard post check failed or is missing"
fi
printf '%s\n' 'user_pref("_noid.thunderbird.hardening.version", "fixture");' \
...
# Match an actual assignment: the header records why the upstream v140.3 pref is
# not adopted, and that explanation must not read as the override itself.
if grep -qE '^[[:space:]]*(user_pref|defaultPref)\("mail\.openpgp\.load_untested_gpgme_version"' \
    "$REPO_ROOT/thunderbird/noid-thunderbird-hardening.js" \
    "$REPO_ROOT/thunderbird/mozilla.cfg"; then
