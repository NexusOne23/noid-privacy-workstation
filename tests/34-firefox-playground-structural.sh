...
for firefox_notice_arg in \
    '--urgency=normal' \
    '--icon="$NOID_LOGO"' \
    '--app-name="NoID Privacy"'; do
    assert_grep_fixed "$firefox_notice_arg" \
        "$TMPDIR/noid-firefox-playground-init.sh" \
        "Firefox-ready notification carries the shared NoID Privacy identity: $firefox_notice_arg"
done
assert_not_grep '--transient' "$TMPDIR/noid-firefox-playground-init.sh" \
    "Firefox-ready notification remains available in GNOME's notification list"
