...
assert_cmd_failure "whitespace-only section value exits non-zero" \
    bash "$TMPDIR/noid-integrity-check" --section $' \t\n'
# A file named after a valid section must not turn a literal wildcard into a scan.
mkdir "$TMPDIR/section-glob"
touch "$TMPDIR/section-glob/oauth"
assert_cmd_failure "section selection never expands caller-directory filenames" \
    bash -c 'cd -- "$1" && bash "$2" --section "*"' _ \
        "$TMPDIR/section-glob" "$TMPDIR/noid-integrity-check"
...
assert_grep_fixed '1 unowned system cron entry requires attribution' \
    <(printf '%s\n' "$unowned_cron_out") \
...
assert_grep_fixed '1 SUID/SGID file(s) (PARTIAL — find rc=1;' \
    <(printf '%s\n' "$partial_suid_out") \
