...
discovery schemas, `gzip`, `jq`, `ksvalidator`, `modinfo`, `openssl`, `patch`,
...
`usbguard-rule-parser`, `visudo`, `dbus-run-session`, `pipewire`, `wireplumber`,
`pw-cli`, `pw-dump`, `wpctl`, and `spa-json-dump`.
On Fedora 44:

```bash
sudo dnf install ShellCheck aide acl audit binutils bubblewrap checkpolicy clang dbus-daemon dconf desktop-file-utils git glib2 gvfs gzip jq kernel-headers kmod libbpf-devel openssl patch pipewire pipewire-utils policycoreutils procps-ng pykickstart python3 python3-audit sudo systemd systemd-udev usbguard usbguard-tools usbguard-notifier wireplumber xz zstd
```
...
Full suite: **85 tests** via `run-all.sh` + **4 bwrap smoke tests** under
`tests/smoke/` = **89 test scripts total** (plus `lib.sh` + `run-all.sh`
...
| `99-finalize-structural.sh` | M99 cross-module sanity, native ACL unit/source-helper validation in a private root, complete four-app release gate and rejection of compose-created AIDE trust state | app/runtime/desktop contract, missing unit helpers, stamp-read loop or trust-boundary drift |
...
malformed and seven baseline-valid raw Ethernet/IPv4/UDP/TCP/ICMP/EAPOL/ARP/DHCP
frames. Two DHCP frames exercise Stable-MAC rotation; a realistic unicast
EAPOL-Key frame must hit the fail-closed verdict before TC learns that rotated
local MAC and the EAPOL pass verdict afterwards. Every malformed reachable
frame must increment the exact drop class and remain invisible to a concurrent
AF_PACKET capture; every valid exceptional branch must increment its exact
pass class. These checks use disposable namespaces and never attach to a live
