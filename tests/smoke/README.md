...
Complement to the source and behavioral tests in `tests/NN-*.sh`.
Smoke tests execute a Module's extracted **%post block** inside a bubblewrap
sandbox against a prepared Fedora rootfs. Individual tests document scoped
substitutions for unavailable operations, such as M17's offline extension
fixture; a passing smoke test does not attest those substituted operations.
...
| `tests/NN-*.sh` | source invariants and native/isolated behavioral fixtures | seconds to minutes, 85 test programs |
...
- **SELinux**: the harness does not disable the host's SELinux policy. A
  sandboxed `getenforce` result is not a host-policy check. Some smoke tests
  check file labels against the prepared rootfs policy; booted policy and
  domain-transition behavior still require the candidate VM gates.
- **systemctl**: offline unit-file operations such as `enable` and `mask`
  modify the disposable rootfs. There is no running systemd manager in the
  sandbox, so a `start` request can fail; it is not generally a successful
  no-op. Tests must scope and document any unavailable-operation substitution.
