...
mkdir -p "$TMPDIR/usbguard.service.d"
extract_heredoc "$KS_FILE" "USBGUARD_STOP_RECOVERY_EOF" \
    "$TMPDIR/usbguard.service.d/10-timeout-abort.conf" \
    || _fail "USBGuard stop-recovery drop-in extraction"
...
extract_heredoc "$M41_FILE" "HOST_IDENTITY_EOF" "$TMPDIR/noid-host-identity" \
    || _fail "host-identity helper extraction"
extract_heredoc "$M41_FILE" "CLEANUP_EOF" "$TMPDIR/noid-anaconda-cleanup.sh" \
    || _fail "Anaconda cleanup helper extraction"
...
assert_cmd_success "native quoted device attributes cannot alter manager safeguards" \
    python3 "$PROJECT_ROOT/tests/fixtures/usbguard_manager_attributes.py" \
        "$TMPDIR/usbguard-devices.py"
assert_cmd_success "USBGuard allow-device helper is valid bash" \
    bash -n "$TMPDIR/allow-device.sh"
assert_cmd_success "native admission attributes cannot be supplied by quoted device text" \
    python3 "$PROJECT_ROOT/tests/fixtures/usbguard_admission_attributes.py" \
...
# Verify unchanged units and executable payloads at their deployed paths in
# an offline candidate root. A prior NoID Privacy installation must not supply missing
# helpers. Vendor units provide the dependency graph; recursive diagnostics
# are scoped to every explicitly listed owned unit and usbguard.service.
# Manpage availability is separate from unit correctness (Fedora's container
# image installs packages with tsflags=nodocs). No service or helper is run.
M14_ROOT="$TMPDIR/system-unit-root"
install -d "$M14_ROOT/etc/systemd/system/usbguard.service.d" \
    "$M14_ROOT/usr/lib/systemd"
cp -a /usr/lib/systemd/system "$M14_ROOT/usr/lib/systemd/"
M14_UNITS=(usbguard.service noid-host-identity.service
    noid-anaconda-cleanup.service noid-usbguard-firstboot.service
    noid-usbguard-live-init.service noid-usbguard-add-user.service
    noid-usbguard-remove-gnome-wildcard.service)
for unit in "${M14_UNITS[@]:1}"; do
    install -m 0644 "$TMPDIR/$unit" "$M14_ROOT/etc/systemd/system/$unit"
done
install -m 0644 "$TMPDIR/usbguard.service.d/10-timeout-abort.conf" \
    "$M14_ROOT/etc/systemd/system/usbguard.service.d/10-timeout-abort.conf"
install -D -m 0755 "$TMPDIR/firstboot.sh" \
    "$M14_ROOT/usr/local/bin/noid-usbguard-firstboot.sh"
install -D -m 0755 "$TMPDIR/add-user.sh" \
    "$M14_ROOT/usr/local/bin/noid-usbguard-add-user.sh"
install -D -m 0755 "$TMPDIR/remove-gnome-wildcard.sh" \
    "$M14_ROOT/usr/local/sbin/noid-usbguard-remove-gnome-wildcard"
install -D -m 0755 "$TMPDIR/noid-host-identity" \
    "$M14_ROOT/usr/local/bin/noid-host-identity"
install -D -m 0755 "$TMPDIR/noid-anaconda-cleanup.sh" \
    "$M14_ROOT/usr/libexec/noid-anaconda-cleanup.sh"
install -D -m 0755 /usr/bin/usbguard-daemon \
    "$M14_ROOT/usr/bin/usbguard-daemon"
M14_VERIFY=(env -i PATH=/usr/sbin:/usr/bin LC_ALL=C
    SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system
    systemd-analyze --root="$M14_ROOT" --generators=no --man=no
    --recursive-errors=no verify)
assert_cmd_success "USBGuard system-unit graph parses with the candidate definitions" \
    "${M14_VERIFY[@]}" "${M14_UNITS[@]}"
candidate_unit="$M14_ROOT/etc/systemd/system/noid-usbguard-firstboot.service"
printf '\nNoIDInvalidFixtureDirective=yes\n' >> "$candidate_unit"
assert_cmd_failure "USBGuard unit verifier rejects an unknown directive" \
    "${M14_VERIFY[@]}" "${M14_UNITS[@]}"
install -m 0644 "$TMPDIR/noid-usbguard-firstboot.service" "$candidate_unit"
chmod 0644 "$M14_ROOT/usr/local/bin/noid-usbguard-firstboot.sh"
assert_cmd_failure "USBGuard unit verifier rejects a non-executable source helper" \
    "${M14_VERIFY[@]}" "${M14_UNITS[@]}"
chmod 0755 "$M14_ROOT/usr/local/bin/noid-usbguard-firstboot.sh"
rm -f "$M14_ROOT/usr/libexec/noid-anaconda-cleanup.sh"
assert_cmd_failure "USBGuard unit verifier checks the required cleanup helper" \
    "${M14_VERIFY[@]}" "${M14_UNITS[@]}"
install -D -m 0755 "$TMPDIR/noid-anaconda-cleanup.sh" \
    "$M14_ROOT/usr/libexec/noid-anaconda-cleanup.sh"
assert_cmd_success "restored USBGuard candidate graph verifies again" \
    "${M14_VERIFY[@]}" "${M14_UNITS[@]}"

# systemd merges differently named top-level service.d and unit-specific
# drop-ins lexicographically. Reproduce Fedora's real global abort policy with
# its exact basename and ask systemd's own unit parser to prove that the
# higher-priority candidate copy shadows it and supplies the effective values.
mkdir -p "$M14_ROOT/etc/systemd/system/service.d"
printf '%s\n' \
    '[Service]' \
    'TimeoutStopSec=45s' \
    'TimeoutStopFailureMode=abort' \
    > "$M14_ROOT/etc/systemd/system/service.d/10-timeout-abort.conf"
if env -i PATH=/usr/sbin:/usr/bin LC_ALL=C \
        SYSTEMD_LOG_LEVEL=debug \
        SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system \
        systemd-analyze --root="$M14_ROOT" --generators=no --man=no \
            --recursive-errors=no verify usbguard.service \
        > "$TMPDIR/stop-recovery-systemd-debug.log" 2>&1; then
    _pass "systemd parses the competing Fedora and candidate stop drop-ins"
else
    sed -n '1,160p' "$TMPDIR/stop-recovery-systemd-debug.log" >&2
    _fail "systemd parses the competing Fedora and candidate stop drop-ins"
fi
assert_grep_fixed \
    "Skipping overridden file '$M14_ROOT/etc/systemd/system/service.d/10-timeout-abort.conf'." \
    "$TMPDIR/stop-recovery-systemd-debug.log" \
    "unit-specific candidate shadows Fedora's exact top-level abort drop-in"
assert_grep_fixed 'TimeoutStopSec: 15s' \
    "$TMPDIR/stop-recovery-systemd-debug.log" \
    "systemd resolves the effective USBGuard stop timeout to 15 seconds"
assert_grep_fixed 'TimeoutStopFailureMode: kill' \
    "$TMPDIR/stop-recovery-systemd-debug.log" \
    "systemd resolves the effective USBGuard stop failure mode to kill"
...
assert_grep_fixed 'USBGUARD_RESTART_REQUIRED=0' "$TMPDIR/firstboot.sh" \
    "steady-state USBGuard activation starts without a restart request"
assert_grep_fixed 'USBGUARD_RESTART_REQUIRED=1' "$TMPDIR/firstboot.sh" \
    "policy publication and ambiguous initializing recovery request reload"
assert_grep_fixed 'already running with the unchanged committed policy' \
    "$TMPDIR/firstboot.sh" \
    "stable real state leaves the loaded USBGuard daemon untouched"
...
assert_grep_extended '^TimeoutStopSec=15s$' \
    "$TMPDIR/usbguard.service.d/10-timeout-abort.conf" \
    "USBGuard policy reload cannot delay boot for Fedora's full default timeout"
assert_grep_extended '^TimeoutStopFailureMode=kill$' \
    "$TMPDIR/usbguard.service.d/10-timeout-abort.conf" \
    "a stuck USBGuard stop skips the disabled coredump/SIGABRT path"
assert_not_grep 'TimeoutStopFailureMode=abort' \
    "$TMPDIR/usbguard.service.d/10-timeout-abort.conf" \
    "USBGuard stop recovery never requests an unavailable crash artifact"
assert_grep_fixed "stat -Lc '%U:%G:%a:%h' \"\$usbguard_stop_recovery\"" \
    "$KS_FILE" "USBGuard stop-recovery postcondition checks owner, mode and link count"
assert_grep_fixed 'matchpathcon -V "$usbguard_stop_recovery"' \
    "$KS_FILE" "USBGuard stop-recovery postcondition checks its SELinux label"
assert_grep_fixed 'cmp -s "$usbguard_stop_recovery" <(printf' "$KS_FILE" \
    "USBGuard stop-recovery postcondition compares exact unit bytes"
assert_grep_fixed 'STOP_RECOVERY=/etc/systemd/system/usbguard.service.d/10-timeout-abort.conf' \
    "$USBGUARD_RUNTIME" "candidate runtime gate checks the installed stop-recovery drop-in"
assert_grep_fixed 'systemctl show usbguard.service -P TimeoutStopUSec' \
    "$USBGUARD_RUNTIME" "candidate runtime gate checks the effective USBGuard stop timeout"
assert_grep_fixed 'systemctl show usbguard.service -P TimeoutStopFailureMode' \
    "$USBGUARD_RUNTIME" "candidate runtime gate rejects the inherited SIGABRT path"
...
# Execute the production service-activation block with a stateful systemctl
# fake. Stable `real` state must not recycle either service; a replaced or
# ambiguous policy must restart the daemon and its D-Bus frontend together.
USBGUARD_ACTIVATION_BLOCK="$TMPDIR/usbguard-service-activation.block"
sed -n '/^# BEGIN USBGUARD_SERVICE_ACTIVATION$/,/^# END USBGUARD_SERVICE_ACTIVATION$/p' \
    "$TMPDIR/firstboot.sh" | sed '1d;$d' > "$USBGUARD_ACTIVATION_BLOCK"
assert_cmd_success "USBGuard production service-activation block parses" \
    bash -n "$USBGUARD_ACTIVATION_BLOCK"

USBGUARD_SERVICE_FIXTURE="$TMPDIR/usbguard-service-fixture"
mkdir -p "$USBGUARD_SERVICE_FIXTURE/bin" "$USBGUARD_SERVICE_FIXTURE/state"
cat > "$USBGUARD_SERVICE_FIXTURE/bin/systemctl" <<'USBGUARD_SYSTEMCTL_FIXTURE_EOF'
#!/bin/bash
set -eu
root=${USBGUARD_SERVICE_FIXTURE_ROOT:?}
printf '%s\n' "$*" >> "$root/calls"
if [ "${1:-}" = --no-reload ]; then
    shift
fi
case "${1:-}" in
    unmask|enable|daemon-reload)
        exit 0
        ;;
    is-active)
        [ "${2:-}" = --quiet ] && [ -f "$root/state/${3:-}.active" ]
        ;;
    is-enabled)
        [ "${2:-}" = --quiet ]
        ;;
...
USBGUARD_SYSTEMCTL_FIXTURE_EOF
chmod 0755 "$USBGUARD_SERVICE_FIXTURE/bin/systemctl"

USBGUARD_ACTIVATION_HARNESS="$USBGUARD_SERVICE_FIXTURE/harness.sh"
{
    cat <<'USBGUARD_ACTIVATION_HARNESS_EOF'
#!/bin/bash
set -euo pipefail
USBGUARD_RESTART_REQUIRED=${USBGUARD_RESTART_REQUIRED:-0}
log() { printf '%s\n' "$*" >> "$USBGUARD_SERVICE_FIXTURE_ROOT/log"; }
USBGUARD_ACTIVATION_HARNESS_EOF
    cat "$USBGUARD_ACTIVATION_BLOCK"
} > "$USBGUARD_ACTIVATION_HARNESS"
chmod 0755 "$USBGUARD_ACTIVATION_HARNESS"
assert_cmd_success "USBGuard service-activation behavioral harness parses" \
    bash -n "$USBGUARD_ACTIVATION_HARNESS"

reset_usbguard_service_fixture() {
    rm -f "$USBGUARD_SERVICE_FIXTURE"/calls "$USBGUARD_SERVICE_FIXTURE"/log \
        "$USBGUARD_SERVICE_FIXTURE"/state/*.active
    : > "$USBGUARD_SERVICE_FIXTURE/calls"
    : > "$USBGUARD_SERVICE_FIXTURE/log"
}

reset_usbguard_service_fixture
: > "$USBGUARD_SERVICE_FIXTURE/state/usbguard.service.active"
: > "$USBGUARD_SERVICE_FIXTURE/state/usbguard-dbus.service.active"
assert_cmd_success "stable committed USBGuard state performs no service recycle" \
    env PATH="$USBGUARD_SERVICE_FIXTURE/bin:$PATH" \
        USBGUARD_SERVICE_FIXTURE_ROOT="$USBGUARD_SERVICE_FIXTURE" \
        USBGUARD_RESTART_REQUIRED=0 bash "$USBGUARD_ACTIVATION_HARNESS"
assert_not_grep_extended '^(start|restart) usbguard' \
    "$USBGUARD_SERVICE_FIXTURE/calls" \
    "steady-state firstboot cannot trigger the USBGuard stop race"
assert_grep_fixed 'already running with the unchanged committed policy' \
    "$USBGUARD_SERVICE_FIXTURE/log" \
    "stable daemon preservation is observable"

reset_usbguard_service_fixture
: > "$USBGUARD_SERVICE_FIXTURE/state/usbguard.service.active"
: > "$USBGUARD_SERVICE_FIXTURE/state/usbguard-dbus.service.active"
assert_cmd_success "replaced USBGuard policy restarts both active service layers" \
    env PATH="$USBGUARD_SERVICE_FIXTURE/bin:$PATH" \
        USBGUARD_SERVICE_FIXTURE_ROOT="$USBGUARD_SERVICE_FIXTURE" \
        USBGUARD_RESTART_REQUIRED=1 bash "$USBGUARD_ACTIVATION_HARNESS"
assert_eq 1 "$(grep -cFx 'restart usbguard.service' \
    "$USBGUARD_SERVICE_FIXTURE/calls")" \
    "a replaced policy reloads the daemon exactly once"
assert_eq 1 "$(grep -cFx 'restart usbguard-dbus.service' \
    "$USBGUARD_SERVICE_FIXTURE/calls")" \
    "the D-Bus frontend reconnects after the daemon transition"

reset_usbguard_service_fixture
assert_cmd_success "inactive USBGuard services are started without restart" \
    env PATH="$USBGUARD_SERVICE_FIXTURE/bin:$PATH" \
        USBGUARD_SERVICE_FIXTURE_ROOT="$USBGUARD_SERVICE_FIXTURE" \
        USBGUARD_RESTART_REQUIRED=0 bash "$USBGUARD_ACTIVATION_HARNESS"
assert_eq 1 "$(grep -cFx 'start usbguard.service' \
    "$USBGUARD_SERVICE_FIXTURE/calls")" \
    "inactive USBGuard daemon starts exactly once"
assert_eq 1 "$(grep -cFx 'start usbguard-dbus.service' \
    "$USBGUARD_SERVICE_FIXTURE/calls")" \
    "inactive USBGuard D-Bus frontend starts exactly once"

reset_usbguard_service_fixture
: > "$USBGUARD_SERVICE_FIXTURE/state/usbguard.service.active"
: > "$USBGUARD_SERVICE_FIXTURE/state/usbguard-dbus.service.active"
assert_cmd_failure "failed required USBGuard policy reload remains retryable" \
    env PATH="$USBGUARD_SERVICE_FIXTURE/bin:$PATH" \
        USBGUARD_SERVICE_FIXTURE_ROOT="$USBGUARD_SERVICE_FIXTURE" \
        USBGUARD_RESTART_REQUIRED=1 FAIL_RESTART_UNIT=usbguard.service \
        bash "$USBGUARD_ACTIVATION_HARNESS"
assert_grep_fixed 'ERROR: failed to restart usbguard.service' \
    "$USBGUARD_SERVICE_FIXTURE/log" \
    "required policy-reload failure remains explicit"
...
# The shared lexical reader places its parse backstop before the call-site
# program. gawk runs END rules in source order and stops END processing at the
# first exit, so a call site with its own END rule would otherwise consume the
# exit status and report a malformed record as an ordinary empty attribute.
sed -n '/^usbguard_rule_awk() {$/,/^}$/p' "$TMPDIR/allow-device.sh" \
    > "$TMPDIR/usbguard-rule-awk.sh"
assert_cmd_success "shared rule reader extraction is valid bash" \
    bash -n "$TMPDIR/usbguard-rule-awk.sh"
assert_cmd_success "parse backstop precedes the call-site program" \
    awk -v marker='"$program"' '
        index($0, "device attributes are unverified") { backstop = NR }
        index($0, marker) { program = NR }
        END { exit(backstop > 0 && program > 0 && backstop < program ? 0 : 1) }
    ' "$TMPDIR/usbguard-rule-awk.sh"
assert_cmd_success "malformed record fails closed through a call-site END rule" \
    bash -c '
        . "$1"
        set +e
        output=$(usbguard_rule_awk "$2" <<< "$3" 2>&1)
        result=$?
        [ "$result" -eq 1 ] || exit 1
        case "$output" in *"device attributes are unverified"*) ;; *) exit 1 ;; esac
    ' _ "$TMPDIR/usbguard-rule-awk.sh" \
    '{ print field_value($0, "name") } END { exit 0 }' \
    '1: allow name "unterminated'
assert_cmd_success "well-formed record still reaches the call-site END rule" \
    bash -c '
        . "$1"
        output=$(usbguard_rule_awk "$2" <<< "$3") || exit 1
        [ "$output" = "My Device" ]
    ' _ "$TMPDIR/usbguard-rule-awk.sh" \
    '{ line = $0; seen++ } END { if (seen != 1) exit 1; print field_value(line, "name") }' \
    '1: allow id 1d6b:0002 name "My Device" hash "YWJjZA=="'
...
# The native admission-attribute fixture above executes preservation of
# labelled, conditional and reduced operator policy, including quoted text
# containing those keywords. Free substring searches are not valid guards.
...
for published_path in \
    ...
    /etc/systemd/system/usbguard.service.d/10-timeout-abort.conf \
