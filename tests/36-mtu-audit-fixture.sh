...
run_case "oversized IPv4 tunnel reports the aligned target and the command" 1 \
    "Conservative local MTU target: 1392" \
    "exceeds the conservative local target by 28 bytes" \
...
run_case "correctly sized tunnel passes and prints no command" 0 \
    "fits the conservative local MTU target" \
    '!ip link set'
...
run_case "multiple outer links select the strictest peer ceiling" 1 \
    "Conservative local MTU target: 1392" \
    "outer outer1" \
    "exceeds the conservative local target by 8 bytes"
...
run_case "route MTU overrides a larger device MTU" 1 \
    "Conservative local MTU target: 1328" \
    "route-mtu"
