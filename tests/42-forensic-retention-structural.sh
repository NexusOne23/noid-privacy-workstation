...
: > "$TMPDIR/install-fixture/ks-young.log"
touch -d '29 days ago' "$TMPDIR/install-fixture/ks-young.log"
...
assert_file_exists "$TMPDIR/install-fixture/ks-young.log" \
...
assert_cmd_success "native AIDE report creation and exact retention agree" \
    python3 -B -I "$PROJECT_ROOT/tests/fixtures/aide_reports.py" "$KS_M13" "$KS_M42" "$KS_M99"
...
# Verify the unchanged units against their source helpers in an offline root.
# The running image must not provide missing helpers, and container manpage
# availability is separate from unit correctness. No helper/service is run.
M42_UNIT_ROOT="$EXEC_TMPDIR/unit-root"
install -d "$M42_UNIT_ROOT/etc/systemd/system" \
    "$M42_UNIT_ROOT/usr/lib/systemd" "$M42_UNIT_ROOT/usr/local/bin"
cp -a /usr/lib/systemd/system "$M42_UNIT_ROOT/usr/lib/systemd/"
ln -s bin "$M42_UNIT_ROOT/usr/local/sbin"
M42_UNIT_NAMES=()
for unit_spec in "${m42_unit_specs[@]}"; do
    unit_name=${unit_spec#*:}
    install -m 0644 "$TMPDIR/m42-units/$unit_name" \
        "$M42_UNIT_ROOT/etc/systemd/system/$unit_name"
    M42_UNIT_NAMES+=("$unit_name")
done
for helper_arg_spec in "${helper_arg_specs[@]}"; do
    helper_marker=${helper_arg_spec%%:*}
    helper_name=${helper_arg_spec#*:}
    install -m 0755 "$TMPDIR/${helper_marker}.sh" \
        "$M42_UNIT_ROOT/usr/local/bin/$helper_name"
done
M42_VERIFY=(env -i PATH=/usr/sbin:/usr/bin LC_ALL=C
    SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system
    systemd-analyze --root="$M42_UNIT_ROOT" --generators=no --man=no
    --recursive-errors=no verify "${M42_UNIT_NAMES[@]}")
assert_cmd_success "all ten exact M42 units pass systemd-analyze verify" "${M42_VERIFY[@]}"
chmod 0644 "$M42_UNIT_ROOT/usr/local/bin/noid-install-logs-prune.sh"
assert_cmd_failure "M42 unit verification rejects a non-executable source helper" "${M42_VERIFY[@]}"
chmod 0755 "$M42_UNIT_ROOT/usr/local/bin/noid-install-logs-prune.sh"
printf '\nNoIDInvalidFixtureDirective=yes\n' \
    >> "$M42_UNIT_ROOT/etc/systemd/system/noid-install-logs-prune.service"
assert_cmd_failure "M42 unit verification rejects an unknown directive" "${M42_VERIFY[@]}"
...
# Run the actual SWTPM selection and rotation against private source logs.
# Failed writes can otherwise leave a partial stanza that logrotate accepts.
assert_cmd_success "SWTPM rotation rejects failed preparation and preserves source logs" \
    python3 "$PROJECT_ROOT/tests/fixtures/swtpm_rotation.py" "$KS_M42"
