...
assert_grep_fixed 'manager_environment=$(systemctl --user show-environment)' \
    "$SESSION_HELPER" "session helper captures complete user-manager queries"
assert_cmd_success "session helper preserves evidence across native manager read failures" \
    python3 -I "$PROJECT_ROOT/tests/fixtures/gsk_session_environment.py" "$KS_FILE"
...
assert_cmd_success "runtime gate rejects native process-read failures and preserves NUL records" \
    python3 -I "$PROJECT_ROOT/tests/fixtures/gsk_runtime_process.py" "$RUNTIME_GATE"
...
assert_cmd_success "topology matcher rejects native read failures and incomplete inventories" \
    python3 -I "$PROJECT_ROOT/tests/fixtures/gsk_topology.py" "$KS_FILE"
make_gpu() {
