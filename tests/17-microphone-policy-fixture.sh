...
#!/usr/bin/env bash
if [[ -n ${NOID_FIXTURE_DUMP_FILE:-} ]]; then
    cat "$NOID_FIXTURE_DUMP_FILE" || exit 1
else
    printf '%s\n' \
      '[{"id":7,"type":"PipeWire:Interface:Node","info":{"props":{"media.class":"Audio/Source"}}}]'
fi
exit "${NOID_FIXTURE_DUMP_RC:-0}"
EOF
...
# A malformed or incomplete source inventory cannot prove that microphones
# are absent. Validate the complete snapshot before publishing any source IDs.
python3 - "$PARSER_FIXTURE" <<'INVENTORY_CASES_EOF'
import json
from pathlib import Path
import sys

root = Path(sys.argv[1])
node = {"id": 7, "type": "PipeWire:Interface:Node",
        "info": {"props": {"media.class": "Audio/Source"}}}
cases = {
    "valid": [node], "absent": [], "object": {}, "string": "",
    "driver": [dict(node, info={"props": {"node.name": "Dummy-Driver"}})],
    "empty-record": [{}], "invalid-type": [dict(node, type=None)],
    "invalid-class": [dict(node, info={"props": {"media.class": False}})],
    "missing-id": [{k: v for k, v in node.items() if k != "id"}],
    "negative-id": [dict(node, id=-1)], "float-id": [dict(node, id=7.0)],
    "string-id": [dict(node, id="7")], "boolean-id": [dict(node, id=True)],
    "overflow-id": [dict(node, id=2**32)], "null-info": [dict(node, info=None)],
    "missing-props": [dict(node, info={})], "non-object": [None],
    "valid-prefix": [node, dict(node, id=-1)],
}
for name, payload in cases.items():
    (root / (name + ".json")).write_text(json.dumps(payload), encoding="utf-8")
INVENTORY_CASES_EOF

for inventory_case in valid absent driver; do
    inventory_output=$(env HOME="$PARSER_FIXTURE/home" \
        NOID_FIXTURE_GNOME=true NOID_FIXTURE_ACTIVE=true NOID_FIXTURE_SAVED=true \
        NOID_FIXTURE_VOLUME='Volume: 1.00 [MUTED]' \
        NOID_FIXTURE_DUMP_FILE="$PARSER_FIXTURE/$inventory_case.json" \
        "$PARSER_FIXTURE/toggle" status) || _fail "valid source snapshot $inventory_case"
    assert_eq microphone=off "$(grep '^microphone=' <<<"$inventory_output")" \
        "complete $inventory_case source snapshot permits a coherent verdict"
done
for inventory_case in object string missing-id negative-id float-id string-id \
                      boolean-id overflow-id null-info missing-props non-object valid-prefix query-error empty-record invalid-type invalid-class; do
    dump_rc=0
    dump_file="$PARSER_FIXTURE/$inventory_case.json"
    if [[ $inventory_case == query-error ]]; then
        dump_rc=7
        dump_file="$PARSER_FIXTURE/valid.json"
    fi
    inventory_rc=0
    inventory_output=$(env HOME="$PARSER_FIXTURE/home" \
        NOID_FIXTURE_GNOME=true NOID_FIXTURE_ACTIVE=true NOID_FIXTURE_SAVED=true \
        NOID_FIXTURE_VOLUME='Volume: 1.00 [MUTED]' \
        NOID_FIXTURE_DUMP_FILE="$dump_file" NOID_FIXTURE_DUMP_RC="$dump_rc" \
        "$PARSER_FIXTURE/toggle" status 2>"$PARSER_FIXTURE/inventory.stderr") || inventory_rc=$?
    assert_eq 1 "$inventory_rc" "source snapshot $inventory_case fails visibly"
    assert_eq microphone=degraded "$(grep '^microphone=' <<<"$inventory_output")" \
        "source snapshot $inventory_case cannot report privacy policy enforced"
    assert_eq capture_source_mute=unavailable \
        "$(grep '^capture_source_mute=' <<<"$inventory_output")" \
        "source snapshot $inventory_case is unavailable rather than absent"
    assert_not_grep_fixed Traceback "$PARSER_FIXTURE/inventory.stderr" \
        "source snapshot $inventory_case has no parser traceback"
done
...
if ((${#missing_commands[@]} > 0)); then
    printf 'ERROR: microphone runtime prerequisites missing: %s\n' "${missing_commands[*]}" >&2
    exit 2
fi
for command in dbus-run-session pipewire wireplumber wpctl pw-cli pw-dump \
               python3 gsettings spa-json-dump; do
