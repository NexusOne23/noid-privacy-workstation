...
M01_KS_FILE="$PROJECT_ROOT/kickstart/snippets/01-bootloader.ks"
M17_KS_FILE="$PROJECT_ROOT/kickstart/snippets/17-gnome-hardening.ks"
M20_KS_FILE="$PROJECT_ROOT/kickstart/snippets/20-snapper.ks"
...
assert_file_exists "$M01_KS_FILE"
assert_file_exists "$M17_KS_FILE"
assert_file_exists "$M20_KS_FILE"
...
extract_heredoc "$M17_KS_FILE" FIRSTRUN_SCRIPT_EOF "$TMPDIR/noid-user-firstrun" \
    || _fail "transactional first-login executable extraction"
extract_heredoc "$M20_KS_FILE" SNAPPER_INIT_SERVICE_EOF \
    "$TMPDIR/noid-snapper-init.service" \
    || _fail "Snapper prerequisite unit extraction"
extract_heredoc "$M20_KS_FILE" SNAPPER_INIT_EOF "$TMPDIR/noid-snapper-init.sh" \
    || _fail "Snapper prerequisite executable extraction"
extract_heredoc "$M01_KS_FILE" CMDLINE_SVC_EOF \
    "$TMPDIR/noid-firstboot-cmdline.service" \
    || _fail "cmdline prerequisite unit extraction"
extract_heredoc "$M01_KS_FILE" CMDLINE_EOF "$TMPDIR/noid-firstboot-cmdline.sh" \
    || _fail "cmdline prerequisite executable extraction"
...
assert_eq 52 "$deny" "52 canonical loadable denies"
assert_eq 8 "$builtin" "8 explicitly unaffected built-ins"
assert_eq 44 "$absent" "44 explicitly absent target-kernel identities"
...
assert_eq 52 "$(grep -c '^blacklist ' "$TMPDIR/generated.conf")" \
    "generated blacklist cardinality"
assert_eq 52 "$(grep -c '^install .* /bin/false$' "$TMPDIR/generated.conf")" \
...
assert_cmd_success "BLS selection requires one exact version and a complete native scan" \
    python3 "$PROJECT_ROOT/tests/fixtures/bls_source_selection.py" "$KS_FILE"
...
# Preserve the actual M01 -> M20 -> M21 graph in a clean CI image. The static
# verifier checks executable presence, so stage the original helper payloads
# and change only the disposable units' ExecStart paths; never launch them.
chmod 0755 "$TMPDIR/noid-firstboot-cmdline.sh" \
    "$TMPDIR/noid-snapper-init.sh" "$TMPDIR/noid-user-firstrun"
sed -i "s#^ExecStart=/usr/local/sbin/noid-firstboot-cmdline.sh\$#ExecStart=$TMPDIR/noid-firstboot-cmdline.sh#" \
    "$TMPDIR/noid-firstboot-cmdline.service"
sed -i "s#^ExecStart=/usr/local/bin/noid-snapper-init.sh\$#ExecStart=$TMPDIR/noid-snapper-init.sh#" \
    "$TMPDIR/noid-snapper-init.service"
sed -i "s#^ExecStart=/usr/local/libexec/noid-user-firstrun\$#ExecStart=$TMPDIR/noid-user-firstrun#" \
    "$TMPDIR/noid-user-firstrun.service"
