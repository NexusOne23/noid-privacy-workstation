...
#
# Checks:
#   - MASK_LIST has critical units (atd, fprintd, systemd-coredump.socket, etc.);
#     bluetooth.service is deliberately absent (disable + rfkill design)
...
TMPDIR="$(mktemp -d /var/tmp/noid-m08-test.XXXXXXXX)"
CODIUM_LAUNCH_FIXTURE=
OPENH264_DNS_FIXTURE=
cleanup_test_fixtures() {
    rm -rf -- "$TMPDIR"
    if [[ -n $CODIUM_LAUNCH_FIXTURE ]]; then
        rm -rf -- "$CODIUM_LAUNCH_FIXTURE"
    fi
    if [[ -n $OPENH264_DNS_FIXTURE ]]; then
        rm -rf -- "$OPENH264_DNS_FIXTURE"
    fi
}
...
# Validate both ExecStart and ExecStopPost against the actual source helper.
# The private root supplies source units; no codec transaction is executed.
CODEC_UNIT_ROOT="$TMPDIR/codec-unit-root"
install -d "$CODEC_UNIT_ROOT/etc/systemd/system" \
    "$CODEC_UNIT_ROOT/usr/lib/systemd"
cp -a /usr/lib/systemd/system "$CODEC_UNIT_ROOT/usr/lib/systemd/"
install -m 0644 "$TMPDIR/firstboot.service" \
    "$CODEC_UNIT_ROOT/etc/systemd/system/noid-firstboot-setup.service"
install -D -m 0755 "$TMPDIR/firstboot.sh" \
    "$CODEC_UNIT_ROOT/usr/local/bin/noid-firstboot-setup.sh"
CODEC_VERIFY=(env -i PATH=/usr/sbin:/usr/bin LC_ALL=C
    SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system
    systemd-analyze --root="$CODEC_UNIT_ROOT" --generators=no --man=no
    --recursive-errors=no verify noid-firstboot-setup.service)
assert_cmd_success "codec service passes the native systemd unit verifier" \
    "${CODEC_VERIFY[@]}"
chmod 0644 "$CODEC_UNIT_ROOT/usr/local/bin/noid-firstboot-setup.sh"
assert_cmd_failure "codec unit rejects a non-executable source helper" \
    "${CODEC_VERIFY[@]}"
...
direct_codec_transactions=$(grep -cE \
    '^[[:space:]]*run_system_dnf[[:space:]]+(install|reinstall|swap)[[:space:]]' \
    "$TMPDIR/firstboot.sh")
openh264_codec_transactions=$(grep -cE \
    '^[[:space:]]*run_openh264_dnf[[:space:]]+(install|reinstall)[[:space:]]' \
    "$TMPDIR/firstboot.sh")
assert_eq 11 "$((direct_codec_transactions + openh264_codec_transactions))" \
    "all eleven codec package transactions use a scoped DNF wrapper"
assert_eq 0 \
...
assert_grep_fixed 'run_openh264_dnf reinstall -y $oh264_pkgs' \
    "$TMPDIR/firstboot.sh" \
    "receipt-less desired OpenH264 state replays through bounded DNS recovery"
assert_grep_fixed 'OH264_DNS_HOST=ciscobinary.openh264.org' \
    "$TMPDIR/firstboot.sh" \
    "automatic DNS recovery is bound to Cisco's exact package hostname"
assert_grep_fixed '9.9.9.10#dns10.quad9.net' \
    "$TMPDIR/firstboot.sh" \
    "temporary recovery uses Quad9's authenticated no-threat-blocking endpoint"
assert_grep_fixed '149.112.112.10#dns10.quad9.net' \
    "$TMPDIR/firstboot.sh" \
    "temporary recovery carries both reviewed Quad9 IPv4 endpoints"
assert_grep_fixed 'query --cache=no --stale-data=no --legend=no' \
    "$TMPDIR/firstboot.sh" \
    "Cisco recovery decisions bypass cached and stale resolver answers"
assert_grep_fixed 'ExecStopPost=/usr/local/bin/noid-firstboot-setup.sh --restore-openh264-dns' \
    "$TMPDIR/firstboot.service" \
    "systemd gets an independent post-stop DNS restoration path"
assert_grep_fixed 'trap cleanup_openh264_dns EXIT' "$TMPDIR/firstboot.sh" \
    "ordinary codec exits synchronously restore any temporary DNS override"
assert_not_grep 'noid-dns-mode unfiltered' "$TMPDIR/firstboot.sh" \
    "codec recovery never persists a different user-selected DNS mode"
for repair_pkg in intel-media-driver mesa-va-drivers-freeworld; do
    assert_grep_fixed "run_system_dnf reinstall -y $repair_pkg" \
        "$TMPDIR/firstboot.sh" \
        "receipt-less desired state replays $repair_pkg scriptlets"
done

# Execute the production recovery functions against closed file-backed fakes.
# This proves exact filtered-only retry and restoration, an unrelated DNF
# failure with no DNS mutation, and independent post-stop recovery.
OPENH264_BLOCK="$TMPDIR/openh264-dns-recovery.block"
sed -n '/^# BEGIN OPENH264_DNS_RECOVERY$/,/^# END OPENH264_DNS_RECOVERY$/p' \
    "$TMPDIR/firstboot.sh" | sed '1d;$d' > "$OPENH264_BLOCK"
assert_cmd_success "OpenH264 DNS recovery function block parses" \
    bash -n "$OPENH264_BLOCK"

DNS_FIXTURE=$(mktemp -d /var/tmp/noid-openh264-dns-test.XXXXXX)
OPENH264_DNS_FIXTURE=$DNS_FIXTURE
DNS_FIXTURE_BIN="$DNS_FIXTURE/bin"
mkdir -p "$DNS_FIXTURE_BIN" "$DNS_FIXTURE/run"
chmod 0755 "$DNS_FIXTURE" "$DNS_FIXTURE_BIN" "$DNS_FIXTURE/run"
install -m 0600 /dev/null "$DNS_FIXTURE/run/dns-mode.lock"

cat > "$DNS_FIXTURE_BIN/noid-dns-mode" <<'DNS_MODE_FIXTURE_EOF'
#!/bin/bash
[ "$#" -eq 1 ] && [ "$1" = --status-machine ] || exit 2
cat <<'EOF'
NOID-DNS-MODE-V2
selection=default
configured=yes
...
link_mode=none
EOF
DNS_MODE_FIXTURE_EOF

cat > "$DNS_FIXTURE_BIN/nmcli" <<'NMCLI_FIXTURE_EOF'
#!/bin/bash
printf '%s\n' 'wlan0:802-11-wireless'
NMCLI_FIXTURE_EOF

cat > "$DNS_FIXTURE_BIN/resolvectl" <<'RESOLVECTL_FIXTURE_EOF'
#!/bin/bash
set -eu
printf '%s\n' "$*" >> "$DNS_FIXTURE_ROOT/resolvectl.calls"
state=$(cat "$DNS_FIXTURE_ROOT/resolver.state")
filtered_json() {
    if [ "${FOREIGN_LINK_DNS:-0}" = 1 ]; then
        cat <<'EOF'
[{"servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"},{"addressString":"2620:fe::fe","name":"dns.quad9.net"},{"addressString":"2620:fe::9","name":"dns.quad9.net"}],"dnsOverTLS":"yes"},{"ifname":"wlan0","servers":[],"searchDomains":[],"dnsOverTLS":"yes"},{"ifname":"proton0","servers":[{"addressString":"10.2.0.1","name":null}],"searchDomains":[],"dnsOverTLS":"no"}]
EOF
        return
    fi
    cat <<'EOF'
[{"servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"},{"addressString":"2620:fe::fe","name":"dns.quad9.net"},{"addressString":"2620:fe::9","name":"dns.quad9.net"}],"dnsOverTLS":"yes"},{"ifname":"wlan0","servers":[],"searchDomains":[],"dnsOverTLS":"yes"}]
EOF
}
physical_json() {
    if [ "${FOREIGN_LINK_DNS:-0}" = 1 ]; then
        cat <<'EOF'
[{"servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"},{"addressString":"2620:fe::fe","name":"dns.quad9.net"},{"addressString":"2620:fe::9","name":"dns.quad9.net"}],"dnsOverTLS":"yes"},{"ifname":"wlan0","servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"}],"searchDomains":[{"name":".","routeOnly":true}],"dnsOverTLS":"yes"},{"ifname":"proton0","servers":[{"addressString":"10.2.0.1","name":null}],"searchDomains":[],"dnsOverTLS":"no"}]
EOF
        return
    fi
    cat <<'EOF'
[{"servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"},{"addressString":"2620:fe::fe","name":"dns.quad9.net"},{"addressString":"2620:fe::9","name":"dns.quad9.net"}],"dnsOverTLS":"yes"},{"ifname":"wlan0","servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"}],"searchDomains":[{"name":".","routeOnly":true}],"dnsOverTLS":"yes"}]
EOF
}
unfiltered_json() {
    cat <<'EOF'
[{"servers":[{"addressString":"9.9.9.9","name":"dns.quad9.net"},{"addressString":"149.112.112.112","name":"dns.quad9.net"},{"addressString":"2620:fe::fe","name":"dns.quad9.net"},{"addressString":"2620:fe::9","name":"dns.quad9.net"}],"dnsOverTLS":"yes"},{"ifname":"wlan0","servers":[{"addressString":"9.9.9.10","name":"dns10.quad9.net"},{"addressString":"149.112.112.10","name":"dns10.quad9.net"}],"searchDomains":[{"name":".","routeOnly":true}],"dnsOverTLS":"yes"}]
EOF
}
case "${1:-}" in
    status)
        case "$state" in
            unfiltered) unfiltered_json ;;
    ...
    query)
        if [ "$state" = unfiltered ] \
           || [ "${FILTERED_QUERY_SUCCEEDS:-0}" = 1 ]; then
            exit 0
        fi
        exit 1
        ;;
    dns)
        case " $* " in
            *' 9.9.9.10#dns10.quad9.net '*)
                printf '%s\n' unfiltered > "$DNS_FIXTURE_ROOT/resolver.state"
                ;;
            *)
                if [ "${RESTORE_TO_PHYSICAL:-0}" = 1 ]; then
                    printf '%s\n' physical > "$DNS_FIXTURE_ROOT/resolver.state"
                else
                    printf '%s\n' filtered > "$DNS_FIXTURE_ROOT/resolver.state"
                fi
                ;;
        esac
        ;;
    dnsovertls|domain|flush-caches) ;;
    revert)
        [ "${RESTORE_FAIL:-0}" = 0 ] || exit 70
        printf '%s\n' filtered > "$DNS_FIXTURE_ROOT/resolver.state"
        ;;
    *) exit 2 ;;
esac
RESOLVECTL_FIXTURE_EOF

cat > "$DNS_FIXTURE_BIN/dnf" <<'DNF_FIXTURE_EOF'
#!/bin/bash
set -u
printf '%s\n' "$*" >> "$DNS_FIXTURE_ROOT/dnf.calls"
if [ ! -e "$DNS_FIXTURE_ROOT/dnf-first-attempt" ]; then
    : > "$DNS_FIXTURE_ROOT/dnf-first-attempt"
    exit 42
fi
exit "${RETRY_DNF_RC:-0}"
DNF_FIXTURE_EOF
chmod 0755 "$DNS_FIXTURE_BIN"/*

DNS_HARNESS="$DNS_FIXTURE/harness.sh"
{
    cat <<'DNS_HARNESS_PREFIX'
#!/bin/bash
set -euo pipefail
umask 077
EXPECTED_UID=$(id -u)
...
NMCLI=nmcli
DNS_MODE_CLI=noid-dns-mode
PYTHON=/usr/bin/python3
DNS_RUNTIME_DIR="$DNS_FIXTURE_ROOT/run"
DNS_MODE_LOCK="$DNS_RUNTIME_DIR/dns-mode.lock"
OH264_DNS_STATE="$DNS_RUNTIME_DIR/openh264-dns-recovery.state"
OH264_DNS_HOST=ciscobinary.openh264.org
QUAD9_FILTERED_V4=(9.9.9.9#dns.quad9.net 149.112.112.112#dns.quad9.net)
QUAD9_UNFILTERED_V4=(9.9.9.10#dns10.quad9.net 149.112.112.10#dns10.quad9.net)
DNS_LOCK_HELD=0
OH264_DNS_RESTORE_FAILED=0
LOG_READY=1
LOG_FILE="$DNS_FIXTURE_ROOT/recovery.log"
path_exists() { [ -e "$1" ] || [ -L "$1" ]; }
log() { printf '%s\n' "$*" >> "$LOG_FILE"; }
run_system_dnf() { ( umask 022; "$DNF" "$@" ); }
DNS_HARNESS_PREFIX
    cat "$OPENH264_BLOCK"
    cat <<'DNS_HARNESS_SUFFIX'
case "${1:-}" in
    run)
        set +e
        run_openh264_dnf install -y openh264
        rc=$?
        set -e
        printf '%s\n' "$rc" > "$DNS_FIXTURE_ROOT/result"
        printf '%s\n' "$OH264_DNS_RESTORE_FAILED" \
            > "$DNS_FIXTURE_ROOT/restore-failed"
        ;;
    restore)
        open_dns_mode_lock
        restore_openh264_dns
        close_dns_mode_lock
        ;;
    match)
        dns_runtime_matches "${EXPECTED_DNS_RUNTIME:?}" wlan0
        ;;
    *) exit 2 ;;
esac
DNS_HARNESS_SUFFIX
} > "$DNS_HARNESS"
chmod 0755 "$DNS_HARNESS"
assert_cmd_success "OpenH264 DNS recovery behavioral harness parses" \
    bash -n "$DNS_HARNESS"
export DNS_FIXTURE_ROOT="$DNS_FIXTURE"

reset_dns_fixture() {
    rm -f -- "$DNS_FIXTURE"/dnf.calls "$DNS_FIXTURE"/resolvectl.calls \
        "$DNS_FIXTURE"/dnf-first-attempt "$DNS_FIXTURE"/recovery.log \
        "$DNS_FIXTURE"/result "$DNS_FIXTURE"/restore-failed \
        "$DNS_FIXTURE/run/openh264-dns-recovery.state"
    printf '%s\n' filtered > "$DNS_FIXTURE/resolver.state"
}

reset_dns_fixture
assert_cmd_success "confirmed filtered-only Cisco failure retries and restores" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FILTERED_QUERY_SUCCEEDS=0 \
        RESTORE_FAIL=0 bash "$DNS_HARNESS" run
assert_eq 0 "$(cat "$DNS_FIXTURE/result")" \
    "successful one-shot OpenH264 retry preserves DNF success"
assert_eq 2 "$(wc -l < "$DNS_FIXTURE/dnf.calls")" \
    "confirmed filtered-only failure performs exactly one DNF retry"
assert_eq filtered "$(cat "$DNS_FIXTURE/resolver.state")" \
    "successful retry restores the exact original filtered resolver topology"
assert_cmd_failure "successful retry leaves no DNS recovery record" \
    test -e "$DNS_FIXTURE/run/openh264-dns-recovery.state"
assert_grep_fixed 'confirmed Quad9 filtered-only Cisco resolution failure' \
    "$DNS_FIXTURE/recovery.log" \
    "automatic compatibility retry is visible to the user"
assert_grep_fixed 'original Quad9 Strict filtered DNS path restored' \
    "$DNS_FIXTURE/recovery.log" \
    "successful restoration is visible to the user"

reset_dns_fixture
assert_cmd_success "unrelated DNF failure completes the harness without mutation" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FILTERED_QUERY_SUCCEEDS=1 \
        RESTORE_FAIL=0 bash "$DNS_HARNESS" run
assert_eq 42 "$(cat "$DNS_FIXTURE/result")" \
    "a resolvable Cisco name preserves the original DNF failure status"
assert_eq 1 "$(wc -l < "$DNS_FIXTURE/dnf.calls")" \
    "an unconfirmed resolver condition performs no DNF retry"
assert_not_grep '^dns ' "$DNS_FIXTURE/resolvectl.calls" \
    "an unconfirmed resolver condition never changes DNS servers"
assert_cmd_failure "unconfirmed resolver condition leaves no recovery record" \
    test -e "$DNS_FIXTURE/run/openh264-dns-recovery.state"

reset_dns_fixture
assert_cmd_success "failed synchronous restoration remains externally recoverable" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FILTERED_QUERY_SUCCEEDS=0 \
        RESTORE_FAIL=1 bash "$DNS_HARNESS" run
assert_eq 1 "$(cat "$DNS_FIXTURE/result")" \
    "unproven DNS restoration overrides a successful retry status"
assert_eq 1 "$(cat "$DNS_FIXTURE/restore-failed")" \
    "restoration failure raises the fatal orchestration guard"
assert_file_exists "$DNS_FIXTURE/run/openh264-dns-recovery.state" \
    "failed restoration retains its closed recovery evidence"
assert_cmd_success "fresh ExecStopPost-style process restores retained DNS state" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FILTERED_QUERY_SUCCEEDS=0 \
        RESTORE_FAIL=0 bash "$DNS_HARNESS" restore
assert_eq filtered "$(cat "$DNS_FIXTURE/resolver.state")" \
    "independent recovery returns to filtered Quad9"
assert_cmd_failure "independent recovery consumes its state only after proof" \
    test -e "$DNS_FIXTURE/run/openh264-dns-recovery.state"

# Restoration must prove only the resolver state owned by this transaction.
# A VPN or VM link can legitimately appear while DNF runs, and systemd-resolved
# represents some address-only servers with JSON name=null. The initial
# filtered-only detection remains strict; these fixtures enter only through a
# pre-existing closed recovery record, as ExecStopPost would after interruption.
printf '%s\n' filtered > "$DNS_FIXTURE/resolver.state"
assert_cmd_failure "initial DNS detection still rejects unrelated link state" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FOREIGN_LINK_DNS=1 \
        EXPECTED_DNS_RUNTIME=filtered-global \
        bash "$DNS_HARNESS" match

cat > "$DNS_FIXTURE/run/openh264-dns-recovery.state" <<'EOF'
NOID-OH264-DNS-RECOVERY-V1
SCOPE=global
IFACE=wlan0
EOF
chmod 0600 "$DNS_FIXTURE/run/openh264-dns-recovery.state"
printf '%s\n' unfiltered > "$DNS_FIXTURE/resolver.state"
assert_cmd_success "global DNS restore ignores an unrelated link resolver" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FOREIGN_LINK_DNS=1 \
        RESTORE_FAIL=0 bash "$DNS_HARNESS" restore
assert_eq filtered "$(cat "$DNS_FIXTURE/resolver.state")" \
    "global restore reverts the exact temporary candidate-link override"
assert_cmd_failure "global restore consumes recovery evidence with a foreign link" \
    test -e "$DNS_FIXTURE/run/openh264-dns-recovery.state"
...
chmod 0600 "$DNS_FIXTURE/run/openh264-dns-recovery.state"
printf '%s\n' unfiltered > "$DNS_FIXTURE/resolver.state"
assert_cmd_success "physical DNS restore ignores an unrelated link resolver" \
    env PATH="$DNS_FIXTURE_BIN:$PATH" FOREIGN_LINK_DNS=1 \
        RESTORE_TO_PHYSICAL=1 RESTORE_FAIL=0 \
        bash "$DNS_HARNESS" restore
assert_eq physical "$(cat "$DNS_FIXTURE/resolver.state")" \
    "physical restore proves strict filtered DNS on the candidate link"
assert_cmd_failure "physical restore consumes recovery evidence with a foreign link" \
    test -e "$DNS_FIXTURE/run/openh264-dns-recovery.state"
unset DNS_FIXTURE_ROOT
...
assert_grep_fixed 'The NoID Privacy Workstation repository is multi-license, not repo-wide GPL.' \
    "$POLICY_FLAT" \
...
assert_grep_fixed 'but never run `aide --init`, `aide --update`, replace `aide.db*`, or start any rebaseline workflow.' "$POLICY_FLAT" \
    "the policy forbids agents from changing the AIDE baseline"
assert_grep_fixed 'the full-system-update boundary detailed in §8: launch only on an explicit user request for that invocation;' \
    "$POLICY_FLAT" "boundary summary retains the explicit per-run update exception"
assert_grep_fixed 'Never launch `noid-update-all.sh` or an equivalent full-system-update workflow unless the user explicitly requests that invocation.' \
    "$POLICY_FLAT" "full updates require an explicit user request for that invocation"
assert_grep_fixed 'General permission to audit, fix, finish, or continue work is not consent, and neither is a request to change this policy.' \
    "$POLICY_FLAT" "general work and policy-edit requests never authorize updates"
assert_grep_fixed 'A request authorizes only the specified run, not future runs.' \
    "$POLICY_FLAT" "update consent cannot turn into standing permission"
assert_grep_fixed 'Never bypass this boundary through a GUI, wrapper, or substitute command.' \
    "$POLICY_FLAT" "alternate update entry points retain the same consent boundary"
assert_grep_fixed "consent to run updates does not authorize firmware writes, reboots, or AIDE rebaselining, and the workflow's own firmware prompt needs its own explicit user answer." \
    "$POLICY_FLAT" "update consent covers neither firmware writes nor the workflow's own firmware prompt"
assert_not_grep_extended 'an agent never launches that workflow|holds even on direct request' \
    "$POLICY_FLAT" "the former unconditional update ban cannot contradict the explicit exception"
...
assert_cmd_success "FSS examples preserve prior keys and reject incomplete verification input" \
    python3 - "$TMPDIR/fss-verify.md" "$TMPDIR" <<'PY'
import os
import pathlib
import re
import subprocess
...
document = pathlib.Path(sys.argv[1]).read_text()
blocks = re.findall(r'```bash\n(.*?)```', document, re.S)
copy_blocks = [block for block in blocks if 'cp --update=none-fail' in block]
verify_blocks = [block for block in blocks if 'fss_key_file=' in block]
assert len(copy_blocks) == len(verify_blocks) == 1
root = pathlib.Path(sys.argv[2]) / 'fss-doc-fixture'
root.mkdir()
source = root / 'source.key'
target = root / 'noid-fss-verify.key'
key = 'aaaaaa-bbbbbb-cccccc-dddddd/1111-2222'
source.write_text(key + '\n')
prefix = '''mountpoint() { return "${MOUNT_RC:-0}"; }
sudo() {
...
def run(block, mount_rc=0):
    script = prefix + block.replace('/etc/noid-privacy/journal-verify.key', str(source)).replace('/mnt/usb', str(root))
    return subprocess.run(['bash', '-c', script], capture_output=True, text=True,
                          env={**os.environ, 'MOUNT_RC': str(mount_rc)})

result = run(copy_blocks[0], mount_rc=1)
assert result.returncode != 0 and not target.exists()
result = run(copy_blocks[0])
assert result.returncode == 0 and target.read_text() == key + '\n', result.stderr
assert target.stat().st_mode & 0o777 == 0o400
source.write_text('another key\n')
result = run(copy_blocks[0])
assert result.returncode != 0 and target.read_text() == key + '\n'
for content, accepted in ((key + '\n', True), ('', False),
                          ('not a key\n', False), (key + '\n' + key + '\n', False),
                          (None, False)):
    target.unlink(missing_ok=True)
    if content is not None:
        target.write_text(content)
    result = run(verify_blocks[0])
    assert (result.returncode == 0) == accepted, (content, result.stderr)
    assert ('VERIFIED ' in result.stdout) == accepted, (content, result.stdout)
    if accepted:
        assert '<--verify-key=' + key + '>' in result.stdout
target.write_text(key + '\n')
result = run(verify_blocks[0], mount_rc=1)
assert result.returncode != 0 and 'VERIFIED ' not in result.stdout
PY
assert_cmd_success "privacy and gaming toggles reject extra arguments before mutation" \
    python3 - "$TMPDIR" <<'PY'
import pathlib
import subprocess
import sys
...
for name, sentinel in (('toggle-bt', 'prepare_mutation'),
                       ('toggle-loc', 'gset'),
                       ('toggle-gaming', 'require_installed_system')):
    source = (root / name).read_text().replace(
        '/usr/local/lib/noid-privacy/agent-install-format.sh',
        str(root / 'absent-format-library'))
    prefix, suffix = source.split('case "$ACTION" in\n', 1)
    # Retain the actual CLI argument parser. Neutralize only identity and the
    # first mutation boundary, with a positive control proving it is reached
    # for a valid invocation. No services, settings or boot state are changed.
    fixture = root / (name + '-argument-fixture')
    fixture.write_text(
        prefix + 'require_root() { :; }\n' + sentinel +
        '() { echo MUTATION_REACHED; exit 71; }\n' +
        'case "$ACTION" in\n' + suffix)
    for args in (('on', '--help'), ('off', 'extra'), ('status', 'extra')):
        result = subprocess.run(['bash', str(fixture), *args],
                                capture_output=True, text=True)
        assert result.returncode == 2, (name, args, result.stderr)
        assert 'MUTATION_REACHED' not in result.stdout, (name, args)
    for action in ('on', 'enable', 'off', 'disable'):
        result = subprocess.run(['bash', str(fixture), action],
                                capture_output=True, text=True)
        assert result.returncode == 71 and 'MUTATION_REACHED' in result.stdout, (
            name, action, result.stderr)
PY
...
# Sanity: BT CLI multi-layer state machine has all layers. The toggle starts
# the service directly while retaining Fedora's target/D-Bus enablement;
# rfkill plus the root-owned flag implement the radio default.
...
assert_not_grep 'systemctl enable --now "\$SERVICE"' "$TMPDIR/toggle-bt" \
    "Bluetooth toggle preserves existing target/D-Bus enablement"
...
assert_cmd_success "repo recovery state preserves filesystem failures and unsafe targets" \
    python3 - "$TMPDIR" <<'PY'
import grp
import os
...
root = Path(sys.argv[1]) / 'repo-recovery-errors'
root.mkdir()
owner = pwd.getpwuid(os.getuid()).pw_name
group = grp.getgrgid(os.getgid()).gr_name
source = (Path(sys.argv[1]) / 'toggle-thirdparty-repos').read_text()
source = source.replace('/usr/local/lib/noid-privacy/agent-install-format.sh', str(root / 'absent-format'))
source = source.replace('-o root -g root', f'-o {os.getuid()} -g {os.getgid()}')
source = source.replace('chown root:root', f'chown {os.getuid()}:{os.getgid()}')
source = source.replace('root:root:', f'{owner}:{group}:')
source = source.replace('[ "$EUID" -eq 0 ]', '[ 0 -eq 0 ]')
prefix, dispatch = source.rsplit('case "${1:-}" in\n', 1)
dispatch = 'case "${1:-}" in\n' + dispatch

def run(state, suffix, *args):
    return subprocess.run(['bash', '-c', prefix + suffix, 'fixture', *args],
                          capture_output=True, text=True,
                          env={**os.environ, 'NOID_REPO_TOGGLE_STATE_DIR': str(state)})

state = root / 'write'
result = run(state, 'write_state_file thirdparty\n')
assert result.returncode == 0, result.stderr
record = state / 'thirdparty-repos-minimal.state'
expected = b'NOID_THIRDPARTY_MINIMAL_V1\nthirdparty\n'
assert record.read_bytes() == expected
result = run(state, 'printf() { return 71; }\nwrite_state_file replacement\n')
assert result.returncode != 0 and record.read_bytes() == expected, result
assert not list(state.glob('.thirdparty-repos-minimal.*'))
...
for path in (link, outside):
    result = run(path, 'write_state_file thirdparty\n')
    assert result.returncode != 0, (path, result.stderr)
    assert outside.stat().st_mode & 0o777 == 0o700
    assert not list(outside.iterdir())

record.unlink()
record.symlink_to(root / 'absent-record-target')
result = run(state, 'validate_state_file\n')
assert result.returncode == 2 and record.is_symlink()

dependencies = '''load_effective_repo_state() {
    ALL_REPO_IDS=(thirdparty); ENABLED_REPO_IDS=(thirdparty)
}
set_repo_state() { return 0; }
'''
for label, fault, success, retained in (
        ('positive', '', True, False),
        ('unlink', 'rm() { return 71; }\n', False, True),
        ('sync', 'sync() { return 72; }\n', False, False)):
    state = root / ('restore-' + label)
    state.mkdir(mode=0o755)
    record = state / 'thirdparty-repos-minimal.state'
    record.write_bytes(expected)
    record.chmod(0o644)
    result = run(state, dependencies + fault + dispatch, 'restore')
    assert (result.returncode == 0) == success, (label, result.stderr)
    assert ('Done.' in result.stdout) == success, (label, result.stdout)
    assert record.exists() == retained, label
PY

assert_cmd_success "codec wrapper validates argv before authorization and GPU gate retains probe failure" \
    python3 - "$TMPDIR" "$CODEC_RUNTIME" <<'PY'
import os
from pathlib import Path
import subprocess
import sys
...
source = (root / 'complete-setup').read_text()
source = source.replace('/usr/local/lib/noid-privacy/agent-install-format.sh', str(root / 'absent-format'))
source = source.replace('if [ "$(id -u)" -eq 0 ]; then', 'if false; then')
source = source.replace('FLAG_FILE=/var/lib/noid-privacy/firstboot-setup-done.flag', 'FLAG_FILE=' + str(root / 'absent-codec-flag'))
source = source.replace('"$SUDO" -v', 'bash -c "echo AUTH_REACHED; exit 71"')
for args in (('',), ('', '--help'), ('--help', 'extra'), ('--dry-run', 'extra')):
    result = subprocess.run(['bash', '-c', source, 'fixture', *args],
                            capture_output=True, text=True)
    assert result.returncode == 2, (args, result.stderr)
    assert 'AUTH_REACHED' not in result.stdout, args
for args, authorization in ((('--help',), False), ((), True)):
    result = subprocess.run(['bash', '-c', source, 'fixture', *args],
                            capture_output=True, text=True,
                            env={**os.environ, 'NOID_WELCOME_SPAWN': '1'})
    assert result.returncode == (1 if authorization else 0), result.stderr
    assert ('AUTH_REACHED' in result.stdout) == authorization

gate = Path(sys.argv[2]).read_text()
start = gate.index('GPU_CONTROLLER_RE=')
end = gate.index('if ! path_exists "$FLAG";')
block = gate[start:end]
for probe, rc, expected in (
        ('return 73', 1, None),
        ('return 0', 0, '0:0'),
        ('printf "%s\\n" "0000:00:02.0 VGA compatible controller: Intel fixture"', 0, '1:0')):
    script = 'set -euo pipefail\nfail() { exit 1; }\nlspci() { ' + probe + '; }\n'
    script += block + 'printf "%s:%s\\n" "$intel_gpu" "$amd_gpu"\n'
