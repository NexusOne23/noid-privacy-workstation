...
# systemd verifies executable access to source helpers; /tmp may be noexec.
TMPDIR="$(mktemp -d /var/tmp/noid-m41-test.XXXXXXXX)"
M41_STAMP_TEST_ROOT=""
...
# One flag set serves the capability probe and both boundary runs, so a probe
# that succeeds proves exactly the namespace shape the fixture needs.
evidence_boundary_bwrap() {
...
# that keeps the extracted predicate's structural coverage.
if ! command -v bwrap >/dev/null 2>&1; then
    _fail "bubblewrap is required for the M41 installer-evidence boundary fixture"
elif ! evidence_boundary_bwrap /bin/true \
        >"$TMPDIR/evidence-namespace-probe.out" 2>&1; then
    printf '%s\n' \
        '  [SKIP] M41: installer-evidence boundary namespace fixture unavailable; structural coverage retained'
    sed 's/^/    | /' "$TMPDIR/evidence-namespace-probe.out"
else
    assert_cmd_success \
...
# Validate the exact source units and helpers without a prior NoID Privacy install.
# Fedora's GDM and systemd packages supply the real login dependency graph.
# Only the unit parser runs: helpers, daemons and generators are not executed.
M41_UNIT_ROOT="$TMPDIR/systemd-root"
M41_UNIT_DIR="$M41_UNIT_ROOT/etc/systemd/system"
install -d "$M41_UNIT_DIR/gdm.service.d" \
    "$M41_UNIT_DIR/systemd-user-sessions.service.d" \
    "$M41_UNIT_ROOT/usr/lib/systemd"
cp -a /usr/lib/systemd/system "$M41_UNIT_ROOT/usr/lib/systemd/"
install -m 0644 "$TMPDIR/noid-host-identity.service" \
    "$M41_UNIT_DIR/noid-host-identity.service"
install -m 0644 "$TMPDIR/cleanup.service" \
    "$M41_UNIT_DIR/noid-anaconda-cleanup.service"
install -m 0644 "$TMPDIR/maintenance.service" \
    "$M41_UNIT_DIR/noid-anaconda-maintenance.service"
install -m 0644 "$TMPDIR/gdm-gate.conf" \
    "$M41_UNIT_DIR/gdm.service.d/40-noid-anaconda-cleanup.conf"
install -m 0644 "$TMPDIR/user-sessions-gate.conf" \
    "$M41_UNIT_DIR/systemd-user-sessions.service.d/40-noid-anaconda-cleanup.conf"
install -D -m 0755 "$TMPDIR/noid-host-identity" \
    "$M41_UNIT_ROOT/usr/local/bin/noid-host-identity"
install -D -m 0755 "$TMPDIR/cleanup.sh" \
    "$M41_UNIT_ROOT/usr/libexec/noid-anaconda-cleanup.sh"
for binary in /usr/bin/gdm /bin/kill /usr/lib/systemd/systemd-user-sessions; do
    install -D -m 0755 "$binary" "$M41_UNIT_ROOT$binary"
done
M41_VERIFY=(env -i PATH=/usr/sbin:/usr/bin LC_ALL=C
    SYSTEMD_UNIT_PATH=/etc/systemd/system:/usr/lib/systemd/system
    systemd-analyze --root="$M41_UNIT_ROOT" --generators=no --man=no
    --recursive-errors=no verify noid-host-identity.service
    noid-anaconda-cleanup.service noid-anaconda-maintenance.service
    gdm.service systemd-user-sessions.service)
assert_cmd_success "generated cleanup/login unit graph verifies" \
    "${M41_VERIFY[@]}"
chmod 0644 "$M41_UNIT_ROOT/usr/libexec/noid-anaconda-cleanup.sh"
assert_cmd_failure "unit graph rejects a non-executable cleanup helper" \
    "${M41_VERIFY[@]}"
chmod 0755 "$M41_UNIT_ROOT/usr/libexec/noid-anaconda-cleanup.sh"
printf '\n[Unit]\nNoIDInvalidFixtureDirective=yes\n' \
    >> "$M41_UNIT_DIR/noid-anaconda-cleanup.service"
assert_cmd_failure "unit parser rejects an unknown directive" \
    "${M41_VERIFY[@]}"
...
# Exercise all three GDM policy branches against isolated files. GDM 50.2
...
package_gate_fixture="$TMPDIR/installed-package-gate.sh"
{
    printf '%s\n' '#!/usr/bin/env bash' 'set -euo pipefail' \
        ...
        'rpm() {' \
        '    [ "$1" = -qa ] || return 64' \
        '    [ "${FAKE_RPM_QUERY_FAIL:-0}" -eq 0 ] || return 65' \
        '    printf "%s\n" bash dbus-tools' \
        '    [ -z "${FAKE_INSTALLED_PACKAGE:-}" ] || printf "%s\n" "$FAKE_INSTALLED_PACKAGE"' \
        '    return 0' \
        '}' \
        'dnf5() {' \
...
assert_cmd_failure "installed package gate rejects an RPM inventory failure" \
    env FAKE_RPM_QUERY_FAIL=1 bash "$package_gate_fixture"
