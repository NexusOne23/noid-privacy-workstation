...
assert_grep_fixed 'JP_SIZE="237442"' "$KS_FILE" \
...
assert_grep_fixed 'JP_ENTRY_COUNT="77"' "$KS_FILE" \
    "Just-Perfection archive entry count is pinned"
assert_grep_fixed 'JP_EXPANDED_SIZE="888116"' "$KS_FILE" \
...
assert_grep_fixed 'if str(metadata.get("version", "")) != expected_version:' \
    "$KS_FILE" "extension metadata version is closed to v37"
...
# Execute both pristine generators before the invocation-boundary instrumentation.
# Existing matching directories make accidental shell glob expansion observable.
if python3 - "$NOARG_TMP" <<'XDG_PATH_FIXTURE_PY'
import os
...
for name in ("entry-a", "entry-b"):
    (root / name).mkdir()
literal = str(root / "entry-*")
cases = (
    (None, "/usr/local/share:/usr/share"),
    ("", "/usr/local/share:/usr/share"),
    ("/usr/local/share:/usr/share/", "/usr/local/share:/usr/share"),
    ("/:/usr/share/", "/:/usr/share"),
    ("/opt/app///:/usr/share/", "/opt/app:/usr/share"),
    (literal + ":/usr/share/", literal + ":/usr/share"),
    ("/opt/with space/:/usr/share/", "/opt/with space:/usr/share"),
    (":/usr/share/::", ":/usr/share::"),
)
for helper in ("xdg-system", "xdg-user"):
    for value, expected in cases:
        env = os.environ.copy()
        env.pop("XDG_DATA_DIRS", None)
        if value is not None:
            env["XDG_DATA_DIRS"] = value
        result = subprocess.run(
            ["/usr/bin/sh", str(root / helper)], env=env,
            text=True, capture_output=True, timeout=5,
        )
        assert result.returncode == 0 and not result.stderr, (helper, value, result)
        assert result.stdout == f"XDG_DATA_DIRS={expected}\n", (helper, value, result.stdout)
XDG_PATH_FIXTURE_PY
then
    _pass "both XDG generators preserve literal paths while trimming trailing slashes"
else
    _fail "XDG generator path normalization"
fi
...
assert_cmd_success "logout evidence rejects audit failures and foreign Shell shutdowns" \
    python3 "$PROJECT_ROOT/tests/fixtures/logout_evidence.py" "$MUTTER_LOGOUT_RUNTIME"
...
assert_cmd_success "JIT gate checks native environment fields on the managed process" \
    python3 "$PROJECT_ROOT/tests/fixtures/jit_runtime_process.py" "$JIT_RUNTIME"
