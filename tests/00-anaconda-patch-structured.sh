...
if [ -n "$REAL_ISO" ]; then
    ...
    # Empty command substitutions must never make two failed builds compare equal.
    if [ ! -s "$first" ] || [ ! -s "$second" ]; then
        _fail "authenticated builds must produce two nonempty images"
        test_finish
        exit 1
    fi
    first_hash=$(sha256sum "$first")
    second_hash=$(sha256sum "$second")
