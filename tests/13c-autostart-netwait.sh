...
for tool in sleep logger cat grep timeout; do
...
[ -z "${MOCK_NM_HUNG:-}" ] || exec /bin/sleep 20
case "$*" in
...
# The original helper hung in both cases despite its one-second budget.
if timeout 5 env PATH="$MOCKBIN" MOCK_NM_HUNG=1 "$NETWAIT" \
        --timeout 1 --quiet -- /bin/echo started > "$TMPDIR/hung.out" 2>&1 \
   && grep -qx started "$TMPDIR/hung.out"; then
    _pass "a hung NetworkManager client cannot prevent application exec"
else
    _fail "a hung NetworkManager client cannot prevent application exec"
fi
sed "s|/proc/uptime|$TMPDIR/missing-clock|g" "$NETWAIT" > "$TMPDIR/no-clock"
if timeout 5 env PATH="$MOCKBIN" /bin/bash "$TMPDIR/no-clock" \
        --timeout 1 --quiet -- /bin/echo started > "$TMPDIR/no-clock.out" 2>&1 \
   && grep -qx started "$TMPDIR/no-clock.out"; then
    _pass "an unreadable monotonic clock starts the application immediately"
else
    _fail "an unreadable monotonic clock starts the application immediately"
fi
...
scenario 'wlp3s0:wifi:100 (connected):yes'
decimal_output=$(PATH="$MOCKBIN" "$NETWAIT" --timeout 08 --settle 00 \
    --quiet -- /bin/echo started 2>"$TMPDIR/decimal.err")
assert_eq "started" "$decimal_output" "leading-zero durations still launch the app"
assert_eq "" "$(cat "$TMPDIR/decimal.err")" "decimal durations cause no arithmetic errors"

# Observe the parsed values at the existing check/execute boundary, before
# any network query or sleep. No implementation of normalization is mocked.
python3 - "$NETWAIT" "$TMPDIR/duration-probe" <<'DURATION_PROBE_PY'
from pathlib import Path
import sys
source = Path(sys.argv[1]).read_text()
anchor = 'if [ "$check_only" -eq 1 ]; then'
assert source.count(anchor) == 1
source = source.replace(anchor, 'printf "%s:%s\\n" "$timeout_s" "$settle_s"; exit 0\n' + anchor)
Path(sys.argv[2]).write_text(source)
DURATION_PROBE_PY
for duration_case in 08:8 010:10 00:0 86400:86400 86401:default \
    999999999999999999999999:default nonsense:default; do
    value=${duration_case%:*}
    expected=${duration_case#*:}
    if [ "$expected" = default ]; then expected=30:0; else expected=$expected:$expected; fi
    actual=$(PATH="$MOCKBIN" /bin/bash "$TMPDIR/duration-probe" \
        --timeout "$value" --settle "$value" -- /bin/true)
    assert_eq "$expected" "$actual" "decimal duration/default policy: $value"
done
...
wanted_funcs = {
    '_xdg_app_dirs', '_parse_desktop_file', '_scan_available_apps',
    '_autostart_remove',
...
# Synthetic XDG roots exercise priority without touching a real user profile.
data = Path(workdir) / 'data'
system_data = Path(workdir) / 'system-data'
for directory in (data / 'applications', system_data / 'applications'):
    directory.mkdir(parents=True)
class XdgPaths:
    get_user_data_dir = staticmethod(lambda: str(data))
    get_system_data_dirs = staticmethod(lambda: [str(system_data), '/', 'relative'])
namespace['GLib'] = XdgPaths
app_dirs = namespace['_xdg_app_dirs']()
check(data / 'applications' == app_dirs[0], 'picker honors XDG_DATA_HOME first')
check(all(p.is_absolute() for p in app_dirs), 'picker never searches relative application paths')
XdgPaths.get_user_data_dir = staticmethod(lambda: 'relative')
check(all(p.is_absolute() for p in namespace['_xdg_app_dirs']()),
      'invalid relative XDG_DATA_HOME falls back to the normal user data root')
XdgPaths.get_user_data_dir = staticmethod(lambda: str(data))
namespace['_xdg_app_dirs'] = lambda: [data / 'applications', system_data / 'applications']
valid = '[Desktop Entry]\nType=Application\nName=Probe\nExec=/usr/bin/true\n'
for key in ('Hidden', 'NoDisplay'):
    name = key.lower() + '.desktop'
    (data / 'applications' / name).write_text(valid + key + '=true\n')
    (system_data / 'applications' / name).write_text(valid)
(data / 'applications/custom.desktop').write_text(valid)
(system_data / 'applications/visible.desktop').write_text(valid)
check({e['filename'] for e in namespace['_scan_available_apps']()} ==
      {'custom.desktop', 'visible.desktop'},
      'hidden higher-priority launchers suppress lower copies; visible controls remain')
dangling = autostart / 'dangling.desktop'
dangling.symlink_to(Path(workdir) / 'absent.desktop')
check(namespace['_autostart_remove'](dangling.name) and not dangling.is_symlink(),
      'removing a broken local autostart link removes that exact entry')
