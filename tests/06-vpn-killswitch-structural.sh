...
assert_cmd_success "endpoint publication preserves a matching visible file/nft pair" \
    python3 - "$TMPDIR/endpoint-engine.py" "$TMPDIR" <<'PY'
import ast
import os
import pathlib
import types
...
root = pathlib.Path(sys.argv[2]) / 'publication-faults'
root.mkdir()
for fault in ('none', 'stage', 'nft', 'rename', 'directory-open', 'directory-sync'):
    state = root / ('state-' + fault)
    state.write_text('old')
    nft = ['old']
    staged = root / ('staged-' + fault)

    def stage(records):
        if fault == 'stage':
            raise OSError('injected staging failure')
        staged.write_text(records[0])
        return staged

    def apply(records, *args):
        if fault == 'nft':
            raise EndpointError('injected nft failure')
        nft[:] = records
    ...
    namespace = {
        'Record': str, 'STATE': state, 'load_state': lambda: ['old'],
        'reconciliation_grace': lambda: False, 'stage_state': stage,
        'apply_nft': apply, 'fail': EndpointError,
        'os': types.SimpleNamespace(
            replace=replace, open=open_directory, close=os.close, fsync=sync,
            O_RDONLY=os.O_RDONLY, O_DIRECTORY=os.O_DIRECTORY),
    }
    exec(compile(module, '<actual-endpoint-publisher>', 'exec'), namespace)
    error = ''
    try:
        namespace['publish'](['new'], set(), False, 1)
    except (EndpointError, OSError) as exc:
        error = str(exc)
    assert bool(error) == (fault != 'none'), (fault, error)
    expected = 'new' if fault in ('none', 'directory-open', 'directory-sync') else 'old'
    assert state.read_text() == nft[0] == expected, (fault, state.read_text(), nft)
    assert not staged.exists(), fault
    if fault.startswith('directory-'):
        assert 'persistence unconfirmed' in error and 'rolled back' not in error
PY

assert_cmd_success "WAN pause validates decimal durations and preserves an existing timer" \
    python3 - "$TMPDIR/wan-strict-cli.sh" "$EXEC_TMPDIR" <<'PY'
import os
import pathlib
import subprocess
import sys
...
body = source.split('cmd_pause() {\n', 1)[1].split('\ncmd_resume()', 1)[0]
root = pathlib.Path(sys.argv[2]) / 'pause-contract'
root.mkdir()
fixture = root / 'pause.sh'
...
systemctl() {
    printf '%s\\n' "systemctl $*" >> "$CASE_LOG"
    if [[ $1 == stop ]]; then printf stopped > "$TIMER_STATE"; fi
}
systemd-run() {
    printf '%s\\n' "systemd-run $*" >> "$CASE_LOG"
    if [[ $SCHEDULE_RC != 0 ]]; then return "$SCHEDULE_RC"; fi
    printf active > "$TIMER_STATE"
}
engine() { printf '%s\\n' "engine $*" >> "$CASE_LOG"; return "$ENGINE_RC"; }
logger() { :; }
cmd_pause() {
''' + body + '\ncmd_pause "$1"\n')

def run(value, schedule=0, engine=0):
    log = root / 'calls'
    timer = root / 'timer'
    log.write_text('')
    timer.write_text('active')
    result = subprocess.run(
        ['bash', str(fixture), value], text=True, capture_output=True,
        env={**os.environ, 'CASE_LOG': str(log), 'TIMER_STATE': str(timer),
             'SCHEDULE_RC': str(schedule), 'ENGINE_RC': str(engine)})
    return result, log.read_text(), timer.read_text()

for value, seconds in (('', 300), ('10', 600), ('010', 600), ('08', 480), ('1440', 86400)):
    result, calls, timer = run(value)
    assert result.returncode == 0, (value, result.stderr)
    assert '--on-active=' + str(seconds) + ' ' in calls, (value, calls)
    assert calls.count('engine pause') == 1 and timer == 'active'
    assert 'systemctl stop' not in calls
for value in ('0', '1441', '18446744073709551617', '-1', '1+1', '1.5'):
    result, calls, timer = run(value)
    assert result.returncode != 0 and calls == '' and timer == 'active', (value, calls)
result, calls, timer = run('10', schedule=1)
assert result.returncode != 0 and timer == 'active', (result.stderr, calls)
assert 'systemctl stop' not in calls and 'engine pause' not in calls
assert 'existing policy and timer retained' in result.stderr
result, calls, timer = run('10', engine=1)
assert result.returncode != 0 and timer == 'stopped', calls
assert calls.index('systemd-run ') < calls.index('engine pause') < calls.index('systemctl stop')
PY
...
assert_cmd_success "nft boundary distinguishes permission denial, query failure and existing tables" \
    python3 "$PROJECT_ROOT/tests/fixtures/nft_boundary_evidence.py" \
        "$PROJECT_ROOT/tests/pre-ship/21-wan-threat-boundary-runtime.sh"
