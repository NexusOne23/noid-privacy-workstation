...
# (kept in sync with m13_page_order() below):
#   1. Critical — Do These First       9. Printing
#   2. VPN — Install Before Updates   10. Gaming Mode
#   3. System Updates                 11. App Autostart
#   4. GNOME Software Sources         12. Security Notifications
#   5. Hardware Privacy               13. Companion Apps
#   6. Devices                        14. Reference
#   7. AI Development Tools           15. Project & Ecosystem
#   8. Media & Graphics                + Conditional finish/restart action
...
# Native rule resolution plus synthetic target-comparison vectors. The
# fixture never initializes, updates or replaces an AIDE baseline. Module
# 13's step-10.0a also proves the complete SECURE superset invariant against
# the installed vendor configuration during compose.
if command -v aide >/dev/null 2>&1; then
    assert_cmd_success "native AIDE rule and target vectors preserve the evidence boundary" \
        python3 -B -I "$PROJECT_ROOT/tests/fixtures/aide_rule_vectors.py" "$KS_FILE"
else
    echo '  [SKIP] native AIDE rule and target vectors (aide not installed)'
fi
...
WELCOME_WAIT="$TMPDIR/wait-fedora-welcome.sh"
extract_heredoc "$KS_FILE" "WAIT_HELPER_EOF" "$WELCOME_WAIT" || \
    _fail "Fedora Welcome wait-helper extraction"
...
assert_cmd_success "Fedora Welcome wait-helper parses" bash -n "$WELCOME_WAIT"
assert_grep_fixed 'IFS='"'"' '"'"' read -r uptime remainder < /proc/uptime' \
    "$WELCOME_WAIT" \
    "bounded Welcome sequencing reads the boot-monotonic kernel clock"
assert_grep_fixed 'start=$(noid_boot_uptime_seconds) || return 0' \
    "$WELCOME_WAIT" \
    "Welcome wait fails open if its monotonic start cannot be read"
assert_grep_fixed 'now=$(noid_boot_uptime_seconds) || return 0' \
    "$WELCOME_WAIT" \
    "every Welcome deadline check uses boot-monotonic time"
assert_not_grep 'date +%s' "$WELCOME_WAIT" \
    "wall-clock corrections cannot extend the bounded Welcome wait"
for welcome_notice_arg in \
    "'--urgency=normal'" \
    "f'--icon={LOGO_PATH}'" \
    "'--app-name=NoID Privacy'"; do
    assert_grep_fixed "$welcome_notice_arg" "$SCRIPT" \
        "Welcome notification carries the shared NoID Privacy identity: $welcome_notice_arg"
done
assert_not_grep "'--transient'" "$SCRIPT" \
    "Welcome notification remains available in GNOME's notification list"
...
assert_grep_fixed 'CLAUDE_VERSION="2.1.265"' "$CLAUDE_INSTALLER" \
    "Claude native seed matches the reviewed vendor release"
assert_grep_fixed 'CLAUDE_SHA256="e14738e3a58d1fc6ccc23b9c919451b4846bc27074a3fb48db976a7d595bdeeb"' \
    "$CLAUDE_INSTALLER" \
    "Claude 2.1.265 native seed is bound to the reviewed vendor bytes"
assert_grep_fixed 'CLAUDE_SIZE="215940592"' "$CLAUDE_INSTALLER" \
    "Claude 2.1.265 native seed is bound to the reviewed byte count"
...
assert_grep_fixed 'EXT_VERSION="2.1.265"' "$CLAUDE_INSTALLER" \
    "Claude VSCodium seed matches the reviewed fixed client release"
assert_grep_fixed 'EXT_SHA256="23983f5044003e30bd29c707ef8bec9a865f156f8eb12a811ac49fba2543e107"' \
    "$CLAUDE_INSTALLER" \
    "Claude 2.1.265 VSIX seed is bound to the reviewed Open VSX bytes"
assert_grep_fixed 'EXT_SIZE="100804434"' "$CLAUDE_INSTALLER" \
    "Claude 2.1.265 VSIX seed is bound to the reviewed byte count"
...
assert_grep_fixed 'CODEX_VERSION="0.153.4"' "$CODEX_INSTALLER" \
    "Codex standalone seed matches the reviewed stable release"
assert_grep_fixed 'CODEX_SHA256="a822187e1a2420c61c5926721bfbd878701ed95547c9bb0d4de4498a16ba1821"' \
    "$CODEX_INSTALLER" \
    "Codex 0.153.4 standalone seed matches the official release digest"
assert_grep_fixed 'CODEX_SIZE="126196303"' "$CODEX_INSTALLER" \
    "Codex 0.153.4 standalone seed is bound to the reviewed byte count"
assert_grep_extended '^CODEX_SHA256="[0-9a-f]{64}"$' "$CODEX_INSTALLER" \
    "Codex installer pins standalone SHA256"
assert_grep_fixed 'EXT_VERSION="26.5901.22334"' "$CODEX_INSTALLER" \
    "Codex VSCodium seed matches the reviewed Open VSX release"
assert_grep_fixed 'EXT_SHA256="cd9cd06c5bfcc8e18972587d04ac9d08b04152ebf6de426233ddc812b05933ff"' \
    "$CODEX_INSTALLER" \
    "Codex VSCodium seed matches the Open VSX published digest"
assert_grep_fixed 'EXT_SIZE="242285816"' "$CODEX_INSTALLER" \
...
assert_cmd_success "VPN uninstall preserves package/query/removal errors and removes dangling links" \
    python3 "$PROJECT_ROOT/tests/fixtures/vpn_uninstall.py" "$KS_FILE"
...
assert_eq 16 "$(grep -c 'exit 14' "$AIDE_CHECK_WRAPPER")" \
...
assert_grep_fixed 'chmod 0755 /usr/local/sbin/noid-aide-baseline-review' \
    "$KS_FILE" "AIDE review helper is executable by an unprivileged caller"
assert_grep_fixed '/usr/local/sbin/noid-aide-baseline-review|755' \
    "$KS_FILE" "AIDE review helper manifest preserves unprivileged help access"
chmod 0755 "$AIDE_REVIEW"
aide_review_exec=$(mktemp /var/tmp/noid-aide-review-help.XXXXXX)
cp -- "$AIDE_REVIEW" "$aide_review_exec"
chmod 0755 "$aide_review_exec"
if aide_review_help=$("$aide_review_exec" --help 2>&1); then
    _pass "AIDE baseline review help executes without root"
else
    _fail "AIDE baseline review help executes without root: $aide_review_help"
fi
rm -f -- "$aide_review_exec"
if grep -qF 'Usage: sudo noid-aide-baseline-review COMMAND [SHA256]' \
        <<< "$aide_review_help"; then
    _pass "AIDE baseline review help prints the complete usage contract"
else
    _fail "AIDE baseline review help prints the complete usage contract"
fi
