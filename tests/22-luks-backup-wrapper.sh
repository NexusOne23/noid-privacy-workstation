    ...
    case "$output" in
        ...
        SOURCE)
            case "$TOPO_SCENARIO" in
                verified_usb|same_disk|mixed_target) printf '%s\n' /dev/mock-usb1 ;;
...
assert_eq '/dev/sdb,/dev/nvme1n1' "$MOUNT_DISKS" \
    "mixed-target counterexample actually contains both physical disks"
    ...
    if [ "$cmd" = "stat" ]; then
        ...
        if [ -d "$last" ] && \
           [ "$last" = "${BACKUP_STAGE_HANDLE:-}" ]; then
            # Only the root identity is simulated. Preserve the native mode
            # and stat's symlink/dereference behavior for the pinned descriptor.
            if [ "$SCENARIO" = "stage_meta_fail" ]; then
                command chmod 0755 "$last" || return 1
            fi
            local native_metadata
            native_metadata=$(command stat "$@") || return 1
            printf 'root:root:%s\n' "${native_metadata##*:}"
            return 0
        fi
