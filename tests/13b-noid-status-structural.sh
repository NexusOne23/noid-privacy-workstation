...
for policy in loadable blacklisted; do
    for spec in 'mei-me-bound:available-platform-results-vary' 'loaded-unbound:conditional'; do
        write_valid_platform_fixture
        state=${spec%%:*}
        visibility=${spec#*:}
        sed -i -e "s/^MEI_CORE_POLICY=.*/MEI_CORE_POLICY=$policy/" \
            -e "s/^MEI_STATE=.*/MEI_STATE=$state/" \
            -e "s/^MEI_FWUPD_VISIBILITY=.*/MEI_FWUPD_VISIBILITY=$visibility/" \
            "$PLATFORM_FIXTURE_FILE"
        assert_eq "Intel MEI=$state; core-policy=$policy; submodules=none; KT/SOL=enforced; fwupd=$visibility" \
            "$(read_platform_status "$PLATFORM_FIXTURE_FILE" "$platform_file_meta" \
                "$platform_dir_meta" 6.99.1-test "$policy_a")" \
            "observed $state remains visible under $policy policy"
        sed -i 's/^MEI_FWUPD_VISIBILITY=.*/MEI_FWUPD_VISIBILITY=unavailable/' \
            "$PLATFORM_FIXTURE_FILE"
        assert_eq 'unknown (invalid platform-status schema)' \
            "$(read_platform_status "$PLATFORM_FIXTURE_FILE" "$platform_file_meta" \
                "$platform_dir_meta" 6.99.1-test "$policy_a")" \
            "inconsistent $state visibility is rejected under $policy policy"
    done
done
write_valid_platform_fixture
sed -i -e 's/^MEI_STATE=.*/MEI_STATE=unknown/' \
    -e 's/^MEI_FWUPD_VISIBILITY=.*/MEI_FWUPD_VISIBILITY=unknown/' "$PLATFORM_FIXTURE_FILE"
assert_eq 'Intel MEI=unknown; core-policy=loadable; submodules=none; KT/SOL=enforced; fwupd=unknown' \
    "$(read_platform_status "$PLATFORM_FIXTURE_FILE" "$platform_file_meta" \
        "$platform_dir_meta" 6.99.1-test "$policy_a")" \
    "failed metadata without an absence proof remains unknown"
write_valid_platform_fixture
