...
# Extracted executables must share the checkout's executable filesystem;
# the installed image deliberately mounts /tmp with noexec.
TMPDIR="$(mktemp -d "$PROJECT_ROOT/.test-mei.XXXXXX")"
...
if command -v udevadm >/dev/null 2>&1 && udevadm verify --help >/dev/null 2>&1; then
    ...
    printf '%s\n' '  [SKIP] udev rule parser unavailable'
fi
...
# The native unit verifier and policy hasher need the source payload at its
# deployed paths. Build those paths in a disposable user namespace: source
# bytes and ownership checks remain unchanged, and a prior NoID Privacy installation
# cannot supply missing test inputs. SELinux deployment labels are outside
# this namespace fixture's scope and belong to the installed-image gates.
M15_ROOT="$TMPDIR/rootfs"
install -D -m 0644 "$TMPDIR/mei-submodules.conf" \
    "$M15_ROOT/etc/modprobe.d/noid-mei-submodules.conf"
install -D -m 0644 "$TMPDIR/99-noid-mei-kt-block.rules" \
    "$M15_ROOT/etc/udev/rules.d/99-noid-mei-kt-block.rules"
install -D -m 0644 "$TMPDIR/noid-mei-blacklist.conf" \
    "$M15_ROOT/etc/dracut.conf.d/noid-mei-blacklist.conf"
install -D -m 0644 "$TMPDIR/noid-mei-kt-enforce.service" \
    "$M15_ROOT/etc/systemd/system/noid-mei-kt-enforce.service"
install -D -m 0644 "$TMPDIR/noid-cpu-vendor-detect.service" \
    "$M15_ROOT/etc/systemd/system/noid-cpu-vendor-detect-firstboot.service"
install -D -m 0755 "$TMPDIR/noid-mei-kt-enforce" \
    "$M15_ROOT/usr/libexec/noid-mei-kt-enforce"
install -D -m 0755 "$TMPDIR/noid-platform-policy-sha256" \
    "$M15_ROOT/usr/libexec/noid-platform-policy-sha256"
install -D -m 0755 "$TMPDIR/noid-cpu-vendor-detect" \
    "$M15_ROOT/usr/local/sbin/noid-cpu-vendor-detect.sh"
M15_NAMESPACE=(bwrap --unshare-user --uid 0 --gid 0 --die-with-parent --unshare-net
    --tmpfs / --ro-bind /usr /usr --ro-bind "$M15_ROOT/etc" /etc
    --ro-bind "$M15_ROOT/usr/libexec" /usr/libexec
    --ro-bind "$M15_ROOT/usr/local" /usr/local
    --symlink usr/bin /bin --symlink usr/sbin /sbin
    --symlink usr/lib /lib --symlink usr/lib64 /lib64
    --ro-bind /dev /dev --proc /proc --chdir / --dir /tmp --dir /run
    --setenv HOME /)
if command -v bwrap >/dev/null 2>&1 && \
   bwrap --unshare-user --uid 0 --gid 0 --die-with-parent --unshare-net \
       --ro-bind / / --ro-bind /dev /dev --proc /proc /usr/bin/true \
       >/dev/null 2>&1; then
    if command -v systemd-analyze >/dev/null 2>&1; then
        M15_VERIFY=("${M15_NAMESPACE[@]}" systemd-analyze
            --recursive-errors=no --generators=no verify)
        assert_cmd_success "both platform units verify against the extracted source" \
            "${M15_VERIFY[@]}" /etc/systemd/system/noid-mei-kt-enforce.service \
            /etc/systemd/system/noid-cpu-vendor-detect-firstboot.service
        cp "$M15_ROOT/etc/systemd/system/noid-mei-kt-enforce.service" \
            "$M15_ROOT/etc/systemd/system/broken.service"
        printf '\nNoIDInvalidFixtureDirective=yes\n' \
            >> "$M15_ROOT/etc/systemd/system/broken.service"
        assert_cmd_failure "platform unit verifier rejects an unknown directive" \
            "${M15_VERIFY[@]}" /etc/systemd/system/broken.service
        rm -f "$M15_ROOT/etc/systemd/system/broken.service"
    else
        printf '%s\n' '  [SKIP] systemd-analyze unavailable for platform unit verification'
    fi

    M15_HASH=("${M15_NAMESPACE[@]}" /usr/libexec/noid-platform-policy-sha256)
    policy_hash=$("${M15_HASH[@]}")
    assert_cmd_success "platform policy helper hashes the extracted source inputs" \
        grep -qxE '[a-f0-9]{64}' <<< "$policy_hash"
    policy_input="$M15_ROOT/etc/modprobe.d/noid-mei-submodules.conf"
    printf '\n# changed fixture policy\n' >> "$policy_input"
    changed_hash=$("${M15_HASH[@]}")
    assert_cmd_success "changed policy still yields a complete digest" \
        grep -qxE '[a-f0-9]{64}' <<< "$changed_hash"
    assert_cmd_failure "source policy changes alter the digest" \
        test "$policy_hash" = "$changed_hash"
    chmod 0600 "$policy_input"
    assert_cmd_failure "policy hash rejects unexpected input permissions" "${M15_HASH[@]}"
    chmod 0644 "$policy_input"
    ln "$policy_input" "$TMPDIR/policy-hardlink"
    assert_cmd_failure "policy hash rejects a hard-linked input" "${M15_HASH[@]}"
    rm -f "$TMPDIR/policy-hardlink"
    mv "$policy_input" "$policy_input.saved"
    assert_cmd_failure "policy hash rejects a missing input" "${M15_HASH[@]}"
    ln -s noid-mei-submodules.conf.saved "$policy_input"
    assert_cmd_failure "policy hash rejects a symlink input" "${M15_HASH[@]}"
    rm -f "$policy_input"
    mv "$policy_input.saved" "$policy_input"
    assert_eq "$changed_hash" "$("${M15_HASH[@]}")" \
        "restored regular input restores the complete digest"
else
    printf '%s\n' '  [SKIP] platform unit and policy-hash user namespace unavailable'
fi
...
# Exercise the real argument dispatch, replacing its first policy/root boundary
# with exit 97. No fixture can read policy, prompt or mutate the live system.
if python3 - "$TMPDIR" <<'MEI_ARGV_PY'
from pathlib import Path
import subprocess
import sys

root = Path(sys.argv[1])
guard = '[ "$(id -u)" -eq 0 ] || fail "must run as root (use sudo)"'
checks = 0
for name, expected_guards in (("noid-mei-restore-submodules", 1),
                              ("noid-mei-lockdown", 2)):
    source = (root / name).read_text()
    assert source.count(guard) == expected_guards
    assert source.count("validate_conf() {\n") == 1
    source = source.replace(guard, "exit 97")
    source = source.replace("validate_conf() {\n", "validate_conf() {\n    exit 97\n")
    fixture = root / (name + "-argv")
    fixture.write_text(source)

    cases = [(0, ["--help"]), (0, ["-h"]), (97, []),
             (97, ["--list"]), (97, ["status"]),
             (1, [""]), (1, ["", "extra"]), (1, ["--unknown"]),
             (1, ["--help", "extra"]), (1, ["--list", "extra"])]
    if name.endswith("restore-submodules"):
        for module in ("hdcp", "pxp", "wdt"):
            cases.extend([(97, [module]), (97, ["--block", module]),
                          (97, ["--restore", module])])
        cases.extend([(97, ["--block", "hdcp", "pxp", "wdt"]),
                      (1, ["--block"]), (1, ["--restore"]),
                      (1, ["--block", "hdcp pxp"]),
                      (1, ["--restore", "pxp wdt"]),
                      (1, ["--block", "hdcp", "hdcp"]),
                      (1, ["--block", ""]), (1, ["--block", "hdcp\npxp"])])
    else:
        for action in ("--status", "--undo", "undo"):
            cases.extend([(97, [action]), (1, [action, "extra"])])
        cases.extend([(1, ["--status\n--undo"]), (1, ["--undo", ""])])
    for expected, args in cases:
        result = subprocess.run(["/bin/bash", str(fixture), *args],
                                stdin=subprocess.DEVNULL, capture_output=True,
                                timeout=5, check=False)
        assert result.returncode == expected, (name, args, result.returncode, expected)
        checks += 1
print(f"MEI argument boundary: {checks} cases passed")
MEI_ARGV_PY
then
    _pass "MEI argument contracts reject malformed input before policy access"
else
    _fail "MEI argument contracts reject malformed input before policy access"
fi
...
assert_cmd_success "MEI runtime state remains distinct from configured policy" \
    python3 "$PROJECT_ROOT/tests/fixtures/mei_state.py" "$KS_FILE"
