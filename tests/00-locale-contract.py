...
the image ships thirteen langpacks (twelve non-English), so a helper that
...
This check walks every deployed-helper heredoc in the Kickstart sources —
the same writer set the sandbox-path contract audits (`cat >`, `cat >>`,
`tee`, `stage_root_file`, `write_file`, `publish_root_file`) — finds the ones
that read a locale-sensitive field, and requires an effective top-level locale
pin in the same script body.
"""
...
HEREDOC_OPEN = re.compile(
    r"(?:cat\s+>{1,2}\s*|(?:sudo\s+)?tee\s+|stage_root_file\s+|write_file\s+"
    r"|publish_root_file\s+)(\S+)(?:\s+\S+)*?\s*<<-?['\"]?([A-Za-z0-9_]+)['\"]?"
)
