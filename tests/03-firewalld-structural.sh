...
chmod 0755 "$TMPDIR/noid-lan-topology-refresh.sh" \
    "$TMPDIR/noid-lan-topology-boot-refresh.sh" "$TMPDIR/noid-lan-xdp-notify"
...
assert_grep_fixed 'unlearned rotated-MAC EAPOL-Key did not fail closed' \
    "$PROJECT_ROOT/tests/pre-ship/03-lan-xdp-runtime.sh" \
    "runtime gate proves a rotated station MAC cannot pass EAPOL before TC learning"
assert_grep_fixed 'learned rotated-MAC EAPOL-Key did not pass' \
    "$PROJECT_ROOT/tests/pre-ship/03-lan-xdp-runtime.sh" \
    "runtime gate proves realistic unicast WPA EAPOL passes after TC learning"
assert_grep_fixed 'it never attaches to a host Wi-Fi link' \
    "$PROJECT_ROOT/tests/pre-ship/03-lan-xdp-runtime.sh" \
    "WPA regression remains confined to disposable repository test interfaces"
assert_grep_fixed 'eapol-key-rotated' \
    "$PROJECT_ROOT/tests/pre-ship/03-lan-xdp-packet-fixtures.py" \
    "packet fixture CLI exposes the realistic rotated-MAC EAPOL-Key suite"
...
# The ERR trap records the failing command's line for the EXIT diagnostic. Bash
# does not inherit that trap inside functions unless errtrace (-E) is active.
assert_grep_fixed 'set -Eeuo pipefail' \
    "$TMPDIR/noid-lan-topology-refresh.sh" \
    "topology refresh inherits its ERR line recorder inside functions"
...
assert_grep_fixed 'trap '"'"'cleanup_and_report "$?"'"'"' EXIT' \
    "$TMPDIR/noid-lan-topology-refresh.sh" \
    "an EXIT guard reports future bare-command failures through both sinks"
...
# Exercise both an explicit validation failure and an unhandled set -e failure.
# The fake logger is the positive control proving that the syslog sink was
# actually reached; checking stderr alone caused the prior false green.
diag_bin="$TMPDIR/topology-diag-bin"
diag_log="$TMPDIR/topology-diag.logger"
diag_stderr="$TMPDIR/topology-diag.stderr"
mkdir -p "$diag_bin"
cat > "$diag_bin/logger" <<'TOPOLOGY_DIAG_LOGGER_EOF'
#!/bin/bash
printf '%s\n' "$*" >> "${NOID_TEST_LOG:?}"
TOPOLOGY_DIAG_LOGGER_EOF
chmod 0755 "$diag_bin/logger"
env PATH="$diag_bin:$PATH" NOID_TEST_LOG="$diag_log" \
    logger -t noid-test positive-control
assert_grep_fixed 'positive-control' "$diag_log" \
    "dual-sink fixture can detect a logger invocation"
: > "$diag_log"
set +e
env PATH="$diag_bin:$PATH" NOID_TEST_LOG="$diag_log" \
    NOID_LAN_LOGGER_BIN="$diag_bin/logger" \
    NOID_LAN_RETRY_DEGRADED_XDP=2 \
    "$TMPDIR/noid-lan-topology-refresh.sh" 2>"$diag_stderr"
diag_status=$?
set -e
assert_eq 2 "$diag_status" \
    "invalid degraded-XDP selector retains its usage-error status"
assert_grep_fixed 'FAILED: invalid degraded-XDP retry selector' "$diag_stderr" \
    "explicit fatal validation reaches stderr"
assert_grep_fixed 'FAILED: invalid degraded-XDP retry selector' "$diag_log" \
    "explicit fatal validation reaches the syslog sink"

cat > "$diag_bin/chown" <<'TOPOLOGY_DIAG_CHOWN_EOF'
#!/bin/bash
exit 70
TOPOLOGY_DIAG_CHOWN_EOF
chmod 0755 "$diag_bin/chown"
diag_state="$TMPDIR/topology-diag-state"
mkdir -m 0700 "$diag_state"
: > "$diag_log"
: > "$diag_stderr"
set +e
env PATH="$diag_bin:$PATH" NOID_TEST_LOG="$diag_log" \
    NOID_LAN_LOGGER_BIN="$diag_bin/logger" \
    NOID_LAN_TOPOLOGY_LOCK_FILE="$diag_state/refresh.lock" \
    "$TMPDIR/noid-lan-topology-refresh.sh" 2>"$diag_stderr"
diag_status=$?
set -e
assert_eq 70 "$diag_status" \
    "unhandled lock-publication failure preserves its command status"
assert_grep_fixed 'FAILED: unhandled topology-refresh error' "$diag_stderr" \
    "unhandled set -e failure reaches stderr"
assert_grep_fixed 'FAILED: unhandled topology-refresh error' "$diag_log" \
    "unhandled set -e failure reaches the syslog sink"

# Exercise the load-bearing function-inheritance case too. Inject one bare
# failure immediately after the production traps in a copy of the extracted
# helper, then require its exact body line in both diagnostic sinks. A
# top-level-only ERR trap leaves this as `unknown` and fails the fixture.
diag_function_helper="$TMPDIR/noid-lan-topology-refresh-function-failure.sh"
awk '
    { print }
    /^trap .*cleanup_and_report.* EXIT$/ {
        print "unhandled_function_fixture() {"
        print "    /bin/false # injected function-body failure"
        print "}"
        print "unhandled_function_fixture"
        injected=1
    }
    END { if (!injected) exit 1 }
' "$TMPDIR/noid-lan-topology-refresh.sh" > "$diag_function_helper" \
    || _fail "function-body diagnostic fixture injection"
chmod 0755 "$diag_function_helper"
diag_function_line=$(grep -nF \
    '/bin/false # injected function-body failure' "$diag_function_helper" \
    | cut -d: -f1)
: > "$diag_log"
: > "$diag_stderr"
set +e
env NOID_TEST_LOG="$diag_log" \
    NOID_LAN_LOGGER_BIN="$diag_bin/logger" \
    "$diag_function_helper" 2>"$diag_stderr"
diag_status=$?
set -e
assert_eq 1 "$diag_status" \
    "unhandled function-body failure preserves its command status"
assert_grep_fixed \
    "FAILED: unhandled topology-refresh error at script line $diag_function_line (status 1)" \
    "$diag_stderr" \
    "function-body failure reports its exact line to stderr"
assert_grep_fixed \
    "FAILED: unhandled topology-refresh error at script line $diag_function_line (status 1)" \
    "$diag_log" \
    "function-body failure reports its exact line to the syslog sink"
assert_not_grep 'script line unknown' "$diag_stderr" \
    "function-body stderr diagnostic never loses its line number"
assert_not_grep 'script line unknown' "$diag_log" \
    "function-body syslog diagnostic never loses its line number"
...
assert_grep_fixed 'NOID_LAN_TOPOLOGY_LOCK_HELD=1 /bin/bash "$0" "$@"' \
    "$TMPDIR/noid-lan-topology-refresh.sh" \
    "topology transaction runs once under the parent-held lock"
...
# Real first-start concurrency at the production lock boundary. Stop before
# network inspection; two callers must serialize on the same inode.
assert_cmd_success "concurrent initial topology refreshes share one lock inode" \
    env PATH=/usr/bin:/usr/sbin python3 - "$KS_FILE" "$TMPDIR" <<'TOPOLOGY_LOCK_RACE_EOF'
import json
...
helper = source.split("<<'LAN_TOPOLOGY_REFRESH_EOF'\n", 1)[1].split('\nLAN_TOPOLOGY_REFRESH_EOF', 1)[0]
body = helper.split('unset NOID_LAN_TOPOLOGY_LOCK_HELD', 1)[0] + 'unset NOID_LAN_TOPOLOGY_LOCK_HELD\n'
body = body.replace('    if [ ! -e "$LOCK_FILE" ] && [ ! -L "$LOCK_FILE" ]; then\n',
                    '    if [ ! -e "$LOCK_FILE" ] && [ ! -L "$LOCK_FILE" ]; then\n        creation_barrier\n', 1)
with tempfile.TemporaryDirectory(prefix='topology-lock-race-', dir=sys.argv[2]) as directory:
    root = Path(directory)
    state = root/'state'
...
set -euo pipefail
umask 077
export NOID_LAN_TOPOLOGY_LOCK_FILE=$1/refresh.lock
export FIXTURE=$2 ROLE=$3
export NOID_LAN_STATE_UID=$(id -u) NOID_LAN_STATE_GID=$(id -g)
export NOID_LAN_LOGGER_BIN=/bin/true
err() { printf '%s\\n' "$*" >&2; }
creation_barrier() {
...
''')
    ...
    try:
        ...
        inode = (state/'refresh.lock').stat().st_ino
        (root/'permit.two').touch()
        time.sleep(.25)
        result = {'overlap': (root/'entered.two').exists(),
                  'same_inode': (state/'refresh.lock').stat().st_ino == inode}
