...
TMPDIR=$(mktemp -d /var/tmp/noid-test-03d.XXXXXX)
...
#!/bin/bash
printf 'ip %s\n' "$*" >> "$MOCK_LOG"
[ "${MOCK_ADDR_FAIL:-0}" = 0 ] || exit 1
[ "$*" = '-j -4 addr show dev eth1 scope global' ] || exit 2
printf '%s\n' "${MOCK_ADDR_JSON:-[]}"
MOCK
cat > "$MOCKBIN/tc" <<'MOCK'
#!/bin/bash
...
# A subnet-only change must update gateway map membership even when every
# interface retains its name, index and MAC. Query failure must preserve both
# committed files and all attachments, not silently reuse the old generation.
mkdir -m 0755 "$TMPDIR/arp"
cat > "$TMPDIR/arp/identity" <<'ARP'
ENABLED=1
WAN_IFACE=eth0
GATEWAY_IP=192.0.2.1
GATEWAY_MAC=02:00:00:00:00:03
LEARNED_AT=2026-09-06T00:00:00Z
ARP
chmod 0644 "$TMPDIR/arp/identity"
onlink='[{"ifname":"eth1","addr_info":[{"family":"inet","local":"192.0.2.20","prefixlen":24}]}]'
offlink='[{"ifname":"eth1","addr_info":[{"family":"inet","local":"198.51.100.20","prefixlen":24}]}]'
EXTRA_ENV=(NOID_ARP_STATE_FILE="$TMPDIR/arp/identity" "MOCK_ADDR_JSON=$offlink")
expect_rebuild "gateway baseline builds" 0 --iface eth0 --iface eth1
assert_not_grep 'noid_xdp_gateway_macs key hex 0c 00 00 00' "$MOCK_LOG" \
    "off-link interface receives no gateway map entry"
expect_fastpath "unchanged subnet membership converges" 0 --iface eth0 --iface eth1
EXTRA_ENV=(NOID_ARP_STATE_FILE="$TMPDIR/arp/identity" "MOCK_ADDR_JSON=$onlink")
expect_rebuild "joining gateway subnet rebuilds" 0 --iface eth0 --iface eth1
assert_grep 'noid_xdp_gateway_macs key hex 0c 00 00 00' "$MOCK_LOG" \
    "newly on-link interface receives gateway map entry"
expect_fastpath "on-link membership converges" 0 --iface eth0 --iface eth1
EXTRA_ENV=(NOID_ARP_STATE_FILE="$TMPDIR/arp/identity" "MOCK_ADDR_JSON=$offlink")
expect_rebuild "leaving gateway subnet rebuilds" 0 --iface eth0 --iface eth1
assert_not_grep 'noid_xdp_gateway_macs key hex 0c 00 00 00' "$MOCK_LOG" \
    "new generation excludes the former on-link gateway entry"
expect_fastpath "off-link membership reconverges" 0 --iface eth0 --iface eth1

cp "$STATE" "$TMPDIR/state.before"
cp "$DIGEST" "$TMPDIR/digest.before"
for address_case in failed malformed wrong-type wrong-interface invalid-prefix invalid-address; do
    EXTRA_ENV=(NOID_ARP_STATE_FILE="$TMPDIR/arp/identity" "MOCK_ADDR_JSON=$offlink")
    case "$address_case" in
        failed) EXTRA_ENV+=(MOCK_ADDR_FAIL=1) ;;
        malformed) EXTRA_ENV+=(MOCK_ADDR_JSON='{') ;;
        wrong-type) EXTRA_ENV+=(MOCK_ADDR_JSON='{}') ;;
        wrong-interface) EXTRA_ENV+=("MOCK_ADDR_JSON=${offlink/eth1/eth2}") ;;
        invalid-prefix) EXTRA_ENV+=("MOCK_ADDR_JSON=${offlink/24/true}") ;;
        invalid-address) EXTRA_ENV+=("MOCK_ADDR_JSON=${offlink/198.51.100.20/invalid}") ;;
    esac
    assert_cmd_failure "$address_case address inventory rejects sync" \
        sync_run 0 --iface eth0 --iface eth1
    assert_not_grep 'bpftool prog loadall\|bpftool net attach\|bpftool map update\|^tc ' \
        "$MOCK_LOG" "$address_case inventory makes no attachment or map changes"
    assert_cmd_success "$address_case inventory preserves authoritative state" \
        cmp -s "$STATE" "$TMPDIR/state.before"
    assert_cmd_success "$address_case inventory preserves committed digest" \
        cmp -s "$DIGEST" "$TMPDIR/digest.before"
done
EXTRA_ENV=(NOID_ARP_STATE_FILE="$TMPDIR/arp/identity" 'MOCK_ADDR_JSON=[{"ifname":"eth1","addr_info":[]}]')
expect_fastpath "valid empty address list still proves off-link membership" \
    0 --iface eth0 --iface eth1
EXTRA_ENV=(NOID_ARP_STATE_FILE="$TMPDIR/arp/identity" 'MOCK_ADDR_JSON=[]')
expect_fastpath "native ip -4 empty inventory proves no matching addresses" \
    0 --iface eth0 --iface eth1
