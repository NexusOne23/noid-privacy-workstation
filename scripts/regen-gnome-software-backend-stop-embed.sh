#!/usr/bin/env bash
# Keep M17's privileged DNF5 idle-stop helper identical to repo source.
set -euo pipefail
...
TARGET=${NOID_GS_BACKEND_STOP_M17:-$REPO_ROOT/kickstart/snippets/17-gnome-hardening.ks}
SOURCE=${NOID_GS_BACKEND_STOP_SOURCE:-$REPO_ROOT/scripts/noid-gnome-software-backend-stop.sh}
OPENING="cat > /usr/local/sbin/noid-gnome-software-backend-stop <<'NOID_GS_BACKEND_STOP_EOF'"
CLOSING=NOID_GS_BACKEND_STOP_EOF

log() { echo "[regen-gnome-software-backend-stop-embed] $*"; }
usage() { echo "Usage: scripts/regen-gnome-software-backend-stop-embed.sh [--check]"; }
...
if noid_generator_block_matches "$TARGET" "$SOURCE" "$OPENING" "$CLOSING"; then
    log "IN SYNC: M17 DNF5 idle-stop helper matches canonical source"
    exit 0
fi
[[ $NOID_GENERATOR_MODE != check ]] || {
    log "DRIFT DETECTED: run scripts/regen-gnome-software-backend-stop-embed.sh"
    exit 1
}
...
log "OK: M17 DNF5 idle-stop helper published atomically"
