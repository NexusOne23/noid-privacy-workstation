...
# Required tools
for tool in curl flock git isoinfo ksflatten livemedia-creator losetup mount mountpoint ss \
        patch python3 unsquashfs umount xorriso; do
    if ! command -v "$tool" >/dev/null 2>&1; then
        log "ERROR: required tool missing: $tool"
        log "  Install: sudo dnf install lorax-lmc-virt lorax-lmc-novirt anaconda pykickstart genisoimage patch git curl squashfs-tools util-linux-core iproute xorriso"
        exit 3
    fi
done
...
# The pinned Fedora Lorax stops its virtio-log handler as soon as shutdown sets the
...
# adds them to the Live-ISO bootloader configs (the BIOS and UEFI grub.cfg of
# the FINAL ISO) → the Live-Boot gets the same KSPP hardening as
...
stage_reduced_dependency_cache() {
    ...
    local expected_sha="175756d74468c9ba45863f7fc333d3be670f82d5b066314e915814dd547d1652"
    local expected_size=4617614
...
stage_noid_audit() {
    ...
    local source_commit="706cb0a92a715181d47a7176f104389ab21d7e73"
    local source_url="https://raw.githubusercontent.com/NexusOne23/noid-privacy-linux/${source_commit}/noid-privacy-linux.sh"
    local expected_size="639310"
    local expected_sha="30ce5f49a8d119a8801620c342e146da087210d68aa920d06083571f05d7546c"
