...
AUDITOR_SOURCE_VERSION='v3.7.2'
AUDITOR_SOURCE_COMMIT='706cb0a92a715181d47a7176f104389ab21d7e73'
AUDITOR_SOURCE_SIZE='639310'
AUDITOR_SOURCE_SHA256='30ce5f49a8d119a8801620c342e146da087210d68aa920d06083571f05d7546c'
...
verify_auditor_bundle() {
    local bundle=$1 tag_ref="refs/tags/$AUDITOR_SOURCE_VERSION"
    local tag_object tag_commit actual_refs expected_refs
    tag_object=$(git -C "$AUDITOR_REPO" rev-parse --verify "$tag_ref") \
        || die 'auditor release tag is missing'
    tag_commit=$(git -C "$AUDITOR_REPO" rev-parse --verify "$tag_ref^{commit}") \
        || die 'auditor release tag does not resolve to a commit'
    [ "$tag_commit" = "$AUDITOR_SOURCE_COMMIT" ] \
        || die 'auditor release tag differs from the pinned commit'
    git -C "$AUDITOR_REPO" bundle verify "$bundle" >/dev/null \
        || die 'auditor bundle verification failed'
    actual_refs=$(git bundle list-heads "$bundle" | sort) \
        || die 'auditor bundle refs are unreadable'
    expected_refs=$(printf '%s HEAD\n%s %s\n' \
        "$AUDITOR_SOURCE_COMMIT" "$tag_object" "$tag_ref" | sort)
    [ "$actual_refs" = "$expected_refs" ] \
        || die 'auditor bundle must advertise only pinned HEAD and its release tag'
}

create_auditor_bundle() {
    local bundle=$1 head_commit
    # git bundle records the ref name it was given and silently drops a raw
    # commit argument, so the pinned commit can only travel as HEAD. Verify the
    # checkout position here instead of relying on the caller: this function is
    # also exercised standalone, and without this guard an off-pin checkout
    # surfaces as a downstream ref mismatch that names the bundle rather than
    # the actual cause.
    head_commit=$(git -C "$AUDITOR_REPO" rev-parse --verify HEAD) \
        || die 'auditor checkout HEAD is unreadable'
    [ "$head_commit" = "$AUDITOR_SOURCE_COMMIT" ] \
        || die "auditor checkout HEAD $head_commit is not the pinned commit $AUDITOR_SOURCE_COMMIT"
    git -C "$AUDITOR_REPO" bundle create "$bundle" \
        HEAD "refs/tags/$AUDITOR_SOURCE_VERSION" \
        || die 'auditor bundle creation failed'
    chmod 0644 "$bundle" || die 'auditor bundle permissions could not be set'
    verify_auditor_bundle "$bundle"
}
...
# self-contained sibling bundle with the exact HEAD and the release tag used
# by M40's pin-parity gate. HEAD alone loses the tag during an offline clone.
create_auditor_bundle "$STAGE/noid-privacy-linux.bundle"
...
verify_auditor_bundle "$auditor_bundle"
...
[ "$(git -C "$AUDITOR_BUNDLE_CHECKOUT/repo" rev-parse --verify \
    "refs/tags/$AUDITOR_SOURCE_VERSION^{commit}")" = "$AUDITOR_SOURCE_COMMIT" ] \
    || die 'auditor bundle checkout release tag drifted'
