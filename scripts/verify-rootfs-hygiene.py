...
def verify(args: argparse.Namespace) -> tuple[dict[str, object], int]:
    ...
    if lstat(log_root) is not None:
        ...
        try:
            for current, directory_names, file_names in os.walk(
                log_root, topdown=True, onerror=log_walk_error, followlinks=False
            ):
    ...
    ssh_metadata = lstat(ssh_dir)
    if ssh_metadata is not None:
        if not stat.S_ISDIR(ssh_metadata.st_mode):
            raise VerificationError("SSH configuration path is not a real directory")
        for candidate in relative_entries(ssh_dir):
            if candidate.name.startswith("ssh_host_") and candidate.name.endswith("_key"):
