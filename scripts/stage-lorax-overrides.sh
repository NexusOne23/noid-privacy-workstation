...
EXPECTED_NEVRA='lorax-44.7-1.fc44.x86_64'
EXPECTED_MONITOR_SHA256='47931372d5fb992a62abc05d83db2b2b362cd53ca7498518fa1d2f28cbaef91c'
EXPECTED_CREATOR_SHA256='1c21bbecb29d8c8d561e2cce47d05d5aab5caba75ab0d59b0d0b5e74c77266ff'
...
if ! destination_entry=$(find "$DESTINATION" -mindepth 1 -maxdepth 1 -print -quit); then
    echo 'ERROR: cannot inspect Lorax override destination' >&2
    exit 2
fi
[ -z "$destination_entry" ] || {
...
if ! rpm_inventory=$("${NOID_RPM[@]}" -ql lorax \
            | awk -v root="$SOURCE_ROOT/pylorax" \
                '$0 == root || index($0, root "/") == 1' \
            | sort); then
    echo 'ERROR: cannot read the Lorax RPM-owned inventory' >&2
    exit 3
fi
if ! actual_inventory=$(find "$SOURCE_ROOT/pylorax" -xdev -print | sort); then
    echo 'ERROR: cannot read the installed pylorax tree inventory' >&2
    exit 3
fi
if [ "$rpm_inventory" != "$actual_inventory" ]; then
