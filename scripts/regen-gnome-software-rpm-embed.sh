#!/usr/bin/env bash
# Keep M17's Fedora-RPM one-shot helper identical to repo source.
set -euo pipefail
...
TARGET=${NOID_GS_RPM_M17:-$REPO_ROOT/kickstart/snippets/17-gnome-hardening.ks}
SOURCE=${NOID_GS_RPM_SOURCE:-$REPO_ROOT/scripts/noid-gnome-software-rpm.sh}
OPENING="cat > /usr/local/bin/noid-gnome-software-rpm <<'NOID_GS_RPM_EOF'"
CLOSING=NOID_GS_RPM_EOF

log() { echo "[regen-gnome-software-rpm-embed] $*"; }
usage() { echo "Usage: scripts/regen-gnome-software-rpm-embed.sh [--check]"; }
...
if noid_generator_block_matches "$TARGET" "$SOURCE" "$OPENING" "$CLOSING"; then
    log "IN SYNC: M17 GNOME Software Fedora-RPM helper matches canonical source"
    exit 0
fi
[[ $NOID_GENERATOR_MODE != check ]] || {
    log "DRIFT DETECTED: run scripts/regen-gnome-software-rpm-embed.sh"
    exit 1
}
...
log "OK: M17 GNOME Software Fedora-RPM helper published atomically"
