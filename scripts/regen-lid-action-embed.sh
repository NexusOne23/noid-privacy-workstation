#!/usr/bin/env bash
# Keep M17's native lid-action helper identical to its canonical repo source.
set -euo pipefail
...
TARGET=${NOID_LID_M17:-$REPO_ROOT/kickstart/snippets/17-gnome-hardening.ks}
SOURCE=${NOID_LID_SOURCE:-$REPO_ROOT/scripts/noid-toggle-lid-action.sh}
OPENING="cat > /usr/local/bin/noid-toggle-lid-action <<'NOID_LID_ACTION_EOF'"
CLOSING=NOID_LID_ACTION_EOF

log() { echo "[regen-lid-action-embed] $*"; }
usage() { echo "Usage: scripts/regen-lid-action-embed.sh [--check]"; }
...
if noid_generator_block_matches "$TARGET" "$SOURCE" "$OPENING" "$CLOSING"; then
    log "IN SYNC: M17 lid-action helper matches canonical source"
    exit 0
fi
[[ $NOID_GENERATOR_MODE != check ]] || {
    log "DRIFT DETECTED: run scripts/regen-lid-action-embed.sh"
    exit 1
}
...
log "OK: M17 lid-action helper published atomically"
