...
EXPECTED_NEVRA='lorax-templates-generic-44.7-1.fc44.x86_64'
...
if ! destination_entry=$(find "$DESTINATION" -mindepth 1 -maxdepth 1 -print -quit); then
    echo 'ERROR: cannot inspect Lorax template destination' >&2
    exit 2
fi
[ -z "$destination_entry" ] || {
...
if ! rpm_inventory=$("${NOID_RPM[@]}" -ql lorax-templates-generic \
            | awk -v root="$SOURCE_ROOT" \
                '$0 == root || index($0, root "/") == 1' \
            | sort); then
    echo 'ERROR: cannot read the Lorax template RPM-owned inventory' >&2
    exit 3
fi
if ! actual_inventory=$(find "$SOURCE_ROOT" -xdev -print | sort); then
    echo 'ERROR: cannot read the installed Lorax template tree inventory' >&2
    exit 3
fi
