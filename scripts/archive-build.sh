...
validate_candidate_tree() {
    local tree=$1 unsupported_entry unreadable newline_name
    unsupported_entry=$(find "$tree" -mindepth 1 \
        ! -type d ! -type f -print -quit) || die "cannot inspect candidate entries"
    [ -z "$unsupported_entry" ] \
        || die "candidate archive input contains an unsupported entry: $unsupported_entry"
    unreadable=$(find "$tree" -type f ! -readable -print -quit) \
        || die "cannot inspect candidate readability"
    [ -z "$unreadable" ] || die "unreadable candidate file: $unreadable"
    newline_name=$(find "$tree" -depth -print0 | \
        python3 -c 'import os,sys; p=sys.stdin.buffer.read().split(b"\0"); print(next((os.fsdecode(x) for x in p if b"\n" in x), ""))') \
        || die "cannot inspect candidate pathnames"
    [ -z "$newline_name" ] || die "newline in candidate pathname"
}
validate_candidate_tree "$CANDIDATE_DIR"
...
# The source may change while it is being archived. Copy into the private
# transaction first, then authenticate those exact bytes before publication.
# Recheck entry types: hashes of regular files do not cover a late symlink.
INPUT_MANIFEST="$TRANSACTION/INPUT-SHA256SUMS"
OUTPUT_MANIFEST="$TRANSACTION/OUTPUT-SHA256SUMS"
ARCHIVE_CANDIDATE="$TRANSACTION/candidate"
(
    cd "$CANDIDATE_DIR"
    find . -type f -print0 | LC_ALL=C sort -z | xargs -0 -r sha256sum --
) > "$INPUT_MANIFEST"
mkdir "$ARCHIVE_CANDIDATE"
cp -a --reflink=auto "$CANDIDATE_DIR/." "$ARCHIVE_CANDIDATE/"
validate_candidate_tree "$ARCHIVE_CANDIDATE"
(
    cd "$ARCHIVE_CANDIDATE"
    find . -type f -print0 | LC_ALL=C sort -z | xargs -0 -r sha256sum --
) > "$OUTPUT_MANIFEST"
cmp -s "$INPUT_MANIFEST" "$OUTPUT_MANIFEST" \
    || die "archived candidate bytes differ from input"

mapfile -t ISO_FILES < <(find "$ARCHIVE_CANDIDATE" -maxdepth 1 -type f \
...
ISO_NAME=${ISO_PATH##*/}
PUBLIC_MANIFEST="$ARCHIVE_CANDIDATE/SHA256SUMS"
PUBLIC_SIGNATURE="$ARCHIVE_CANDIDATE/SHA256SUMS.asc"
SIGNOFF="$ARCHIVE_CANDIDATE/vm-test-signoff.txt"
SIGNOFF_SIGNATURE="$ARCHIVE_CANDIDATE/vm-test-signoff.txt.asc"
