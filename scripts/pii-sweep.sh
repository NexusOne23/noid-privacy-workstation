...
# Keep filenames out of the exception decision entirely. A filename can
# contain colons and newlines, so no delimiter-based split of grep's filename
# prefix can safely recover the match. Read find's NUL-delimited paths and
# request only line-number/match records from grep for each individual file.
# Diagnostics identify the location without copying private values into CI
# logs or agent context. Read/traversal errors must never become a clean scan.
scan_regular_files() {
    local path matches grep_status record line_number match_text
    while IFS= read -r -d '' path; do
        grep_status=0
        matches=$(grep -aihEno -- "$PATTERNS" "$path") || grep_status=$?
        case "$grep_status" in
            0) ;;
            1) continue ;;
            *) printf '[pii-sweep] ERROR: cannot scan file: %q\n' "$path" >&2
               return 2 ;;
        esac
        while IFS= read -r record; do
            line_number=${record%%:*}
            match_text=${record#*:}
            if [[ ! ${match_text,,} =~ $EXCEPTIONS ]]; then
                printf '%q:%s: possible identifier or home path (redacted)\n' \
                    "$path" "$line_number"
            fi
        done <<< "$matches"
    done
...
if [ "${#symlink_leaks[@]}" -gt 0 ]; then
    printf '%q: possible identifier or home path in symlink target (redacted)\n' \
        "${symlink_leaks[@]}"
...
if [ "$default_scan" -eq 1 ]; then
    ...
    scan_output=$(default_find -type f \
        ...
        ! -path "$ROOT/.tools/*" \
        -print0 | scan_regular_files) || scan_status=$?
else
    ...
    scan_status=0
    scan_output=$(find "${existing[@]}" -type f -print0 \
        | scan_regular_files) || scan_status=$?
fi
[ "$scan_status" -eq 0 ] || {
    echo "[pii-sweep] ERROR: source scan incomplete" >&2
    exit 2
}
