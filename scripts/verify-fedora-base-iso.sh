...
# Even --show-keys opens a keyring. Use the private home for every GPG call,
# including fingerprint inspection, so a fresh account needs no user keyring.
gpg --batch --no-options --homedir "$tmp_dir" --with-colons \
    --show-keys "$FEDORA_KEY" >"$tmp_dir/key-info" 2>"$tmp_dir/key.stderr" \
    || die "could not inspect the Fedora signing key"
mapfile -t key_fprs < <(awk -F: '
...
gpg --batch --no-options --homedir "$tmp_dir" --import "$FEDORA_KEY" >/dev/null 2>&1 \
    || die "could not import the pinned Fedora 44 key"
if ! gpg --batch --no-options --homedir "$tmp_dir" --status-fd=3 \
