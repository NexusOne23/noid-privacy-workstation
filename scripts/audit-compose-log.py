...
def load_policy(path: pathlib.Path):
    ...
    if type(policy["schema_version"]) is not int or policy["schema_version"] != 4:
    ...
    for section in compiled:
        ...
        for row in rows:
            ...
            compiled_rule = {
                ...
                "after_success_marker": row.get("after_success_marker"),
            }
            if "after_success_marker" in row and (
                section != "allowed_events"
                or not isinstance(row["after_success_marker"], str)
                or not re.fullmatch(r"[a-z0-9_]+", row["after_success_marker"])
            ):
                raise PolicyError(f"invalid success-order contract for {identifier}")
            compiled[section].append(compiled_rule)
            if correlation_id is not None:
                correlations.setdefault(correlation_id, []).append(compiled_rule)
    success_ids = {rule["id"] for rule in compiled["success_markers"]}
    for rule in compiled["allowed_events"]:
        required_marker = rule["after_success_marker"]
        if required_marker is not None and required_marker not in success_ids:
            raise PolicyError(f"unknown success marker for {rule['id']}")
    ...
    for index, line in enumerate(lines):
        for rule in compiled["success_markers"]:
            ...
            if line_match and context_matches(rule, lines, index, line_match):
                counts[rule["id"]] += 1
                first_success_lines.setdefault(rule["id"], index)
        ...
        for rule in compiled["allowed_events"]:
            ...
            if (required_marker is not None
                    and first_success_lines.get(required_marker, index) >= index):
                continue
            line_match = rule["line"].fullmatch(line)
            if line_match and context_matches(rule, lines, index, line_match):
...
def main() -> int:
    ...
    try:
        ...
        print(f"compose-log-audit: ERROR: {exc}", file=sys.stderr)
        return 2
    atomic_report(args.report, report)
    if not passed:
        print(
