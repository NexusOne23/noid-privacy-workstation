...
# List into a file first: a `grep -q` early exit would otherwise SIGPIPE
# unsquashfs under pipefail and misclassify a nested LiveOS image as a flat
# root (a spurious gate failure, never a false pass).
unsquashfs -ll "$STAGE/squashfs.img" >"$STAGE/squashfs.list" \
        2>"$STAGE/unsquashfs-ll.err" || {
    sed -n '1,20p' "$STAGE/unsquashfs-ll.err" >&2
    echo "ERROR: cannot list the final SquashFS image" >&2
    exit 3
}
if grep -qE '/LiveOS/rootfs\.img$' "$STAGE/squashfs.list"; then
