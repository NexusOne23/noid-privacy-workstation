...
| `ntsync` | not blocked, loads on demand | NT synchronization support for compatible Wine/Proton builds (not in M21's deny inventory) |
...
Module 12 sets the `selinuxuser_execmod` SELinux boolean **off**. Some
Wine/Proton DLL-loading paths need executable file mappings after modification,
which can require the SELinux `execmod` permission. A denial can prevent
`wineboot` or a title from starting. Check the actual AVC and runner version;
this does not mean that every Wine/Proton title requires this permission.
...
- **Trade-off:** re-opens W^X for unconfined home content while Gaming-Mode is
  ...
  changes.
- **Why a global boolean:** the helper uses the supported SELinux policy
  boolean as its explicit compatibility trade-off. It does not ship a
  game-specific confinement or labeling policy. Such a policy would need
  separate validation of each affected file and execution path.

The 32-bit ABI setting is relevant to 32-bit native programs and the documented
Steam launcher. A 64-bit native Linux game does not inherently need Gaming Mode.
Wine/Proton requirements depend on the runner and title; this helper deliberately
selects both compatibility settings rather than diagnosing individual games.

The kernel's [NT synchronization interface](https://docs.kernel.org/userspace-api/ntsync.html)
is a distinct interface; driver availability alone does not prove that a runner
uses it or establish a performance improvement.
