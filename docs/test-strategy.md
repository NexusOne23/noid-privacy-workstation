...
**Run all**: `bash tests/run-all.sh` (host-dependent; currently about six
minutes on the reference host, with no intentional system modification). The unfiltered release gate
...
the USBGuard tools used by the hermetic notifier graph, and native `pgrep`
process queries. NVIDIA signature fixtures also require native OpenSSL, kmod
and gzip/xz/zstd decompression. The microphone fixture requires a private
D-Bus session plus native PipeWire/WirePlumber tools; missing tools are an
error, not a passing deferred lifecycle test. On Fedora 44 install
them with `sudo dnf install ShellCheck aide acl audit binutils bubblewrap checkpolicy
clang dbus-daemon dconf desktop-file-utils gdm git glib2 gvfs gzip jq kernel-headers kmod libbpf-devel openssl
patch pipewire pipewire-utils policycoreutils procps-ng pykickstart python3 python3-audit sudo systemd systemd-udev
usbguard usbguard-tools usbguard-notifier wireplumber xz zstd`. A missing prerequisite exits as a
harness error before a partial result is printed.

The runner returns 1 for failed tests and 2 for harness errors. In verbose
mode, failure to create, write, read or remove a test log stops the run as a
harness error; it cannot be counted as a successful test or zero skipped checks.

The M13 AIDE fixture resolves the shipped rules with native `--path-check`
and runs unprivileged `--compare` operations on synthetic input vectors.
It checks the complete NORMAL-to-SECURE attribute superset separately from
the projected symlink-target comparison, including the withdrawn rule as a
negative control. It performs no filesystem baseline scan, initialization,
update or database replacement. Fedora's audit-enabled AIDE may write
...
**Coverage**: 85 structural tests total, with hundreds of assertions. Every Module
...
**Run**: manual; no automation is shipped.
...
unsupported raw fixtures, seven baseline-valid frames, two additional DHCP
frames for Stable-MAC rotation and the same realistic unicast EAPOL-Key frame
both before and after TC learns the rotated local MAC, covering reachable truncation, declared
...
DHCP/EAPOL, fail-closed pre-learning plus post-learning WPA admission,
Stable-MAC rotation, related-ICMP and fragment cases;
...
enforcing, the established Fedora 44 kernel-facing schema baseline, readable
policy-capability/unknown-handling state, audit immutable, zero lost audit
events and zero AVC/USER_AVC records for the current boot. It also reports
whether the newer kernel extensions are complete or remain at Fedora 44's
documented partial vendor boundary. An enabled capability with missing schema
or any baseline regression fails closed. Build-installer permissive AVC
classification never acts as an installed-system allowlist.
...
2. **`tests`** — Fedora 44 job container with target-release ShellCheck,
   ...
   full `bash tests/run-all.sh` (all 85 structural). Both checkouts, including
   Git metadata, are copied into a workspace owned by the ordinary `noid-ci`
   test account. Desktop and permission-denial fixtures run as that account.
   Every test failure and any failure to retain the test log fails the job;
   there are no whole-test exceptions. GDM is installed so the cleanup/login
   unit parser uses Fedora's real vendor units and the helpers from the source
   under test. Precise capability skips remain visible in the test output;
   actual boot and login behavior still requires the candidate-image gates.
3. **`shellcheck`** — warning/error gate over every shell script directly
   under `tests/`, `tests/pre-ship/`, `tests/smoke/`, `scripts/`,
   `scripts/lib/`, `scripts/anaconda-patch/` and `branding/icons/`, with the workflow's
...
Not covered by CI (the default GitHub runners cannot provide it):
...
- **bwrap helper smoke tests** (`tests/smoke/run-all.sh`, M02/M17/M23/M27) —
  ...
  smoke-test sources.
- **Layer 3 VM smoke test** — manual (QEMU + 16-point matrix in
...
- **Layer 4 reproducibility smoke test** — manual pre-release. Needs
  ...
  digest-only `sha256sum` comparison on a controlled host.
- **Actual ISO build in CI** — `livemedia-creator` needs KVM, which the
  default GitHub runners do not provide; the build runs on a KVM-capable host.
