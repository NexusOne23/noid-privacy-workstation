...
release omits the trailing `.0` (`v1.8` corresponds to SemVer `1.8.0`), while a
hotfix adds the patch component (`v1.8.1`).

| Component | Meaning | Example bump |
|-----------|---------|--------------|
| **Major**  | Breaking public contract or compatibility change, including a removed Module | `v1.8` → `v2.0` |
| **Minor**  | Additive change — new Module, new user docs, hardening added | `v1.7` → `v1.8` |
| **Patch**  | Bug fix, doc fix, CVE mitigation within a Module | `v1.8` → `v1.8.1` |

There are no pre-release version strings: the canonical builder accepts only
`vX.Y[.Z]` from M32's `NOID_VERSION`, and unpublished candidates are identified
by their unsigned candidate build ID, never by a version suffix.
...
don't provide, and no QEMU automation is shipped (see `docs/test-strategy.md`,
"Not covered by CI"). For v1.x releases
...
- [ ] Run `sudo bash tests/pre-ship/21-dracut-hostonly-runtime.sh` with the
      ...
      host-only candidate is the one-shot next entry. Complete `fresh-install`,
      perform the first normal reboot and pass `reboot` before starting the
      ...
      image, fallback image or state bytes. The reboot pass must verify that
      the central guard returns exactly `basis=hostonly`. Retain all three logs;
      content inspection alone is not boot evidence.
...
- [ ] Run `sudo bash tests/pre-ship/20-snapper-rollback-runtime.sh live` in the
      ...
      source/RPM evidence before activation. After M21's `reboot` pass succeeds
      in the same disposable installed VM, run the Snapper `fresh-install`
      pass **last before a second, separate rollback reboot**: it creates a
      ...
      exact prepared/pending/ready evidence. After that second reboot, run its
      `reboot` pass and require the selected snapshot to be both active/default, the
...
- [ ] Run `tests/pre-ship/30-live-payload-acl-parity.sh` against the retained
      ...
      SELinux enforcing, the Fedora 44 kernel-facing schema baseline, immutable
      audit, zero lost events and zero current-boot AVC/USER_AVC records. Record
      the gate's extension-coverage and `handle-unknown` evidence. Current F44
      may report the documented partial vendor boundary; a baseline regression,
      malformed flag or capability/schema inconsistency is a release blocker.
...
```
# VM-test sign-off for noid-privacy-workstation-44-v1.8-x86_64
...
Results:
  ...
  8. SELinux enforcing + kernel/policy baseline + recorded extension coverage +
     tests/pre-ship/31-installed-enforcing-avc.sh PASS [ ]
 ...
 14. Snapper baseline + native default model    [ ]
     ...
     - M21 fresh-install + first reboot pass completed before Snapper fresh-install
     - Snapper fresh-install ran last before the second, rollback reboot
...
Sign the candidate's existing checksum and completed VM sign-off, then verify
exactly one signature from the release key for each. Run this Bash recipe from
the repository root; its subshell stops on command or pipeline failures.
...
gpg --batch --no-options --status-fd=1 --verify SHA256SUMS.asc SHA256SUMS \
  | awk -v fpr="$RELEASE_KEY_ID" \
      '$1=="[GNUPG:]" && ($2=="EXPKEYSIG"||$2=="REVKEYSIG"||$2=="EXPSIG"||$2=="ERRSIG"||$2=="BADSIG") {bad=1}
       $1=="[GNUPG:]" && $2=="GOODSIG" {good++}
       $1=="[GNUPG:]" && $2=="VALIDSIG" {total++; if ($3==fpr) ok++}
       END {exit !(total==1 && ok==1 && good==1 && !bad)}'
...
own download host; its public download entry is
<https://noid-privacy.com/linux.html>, which redirects to the download page
`hardened-linux-privacy-os.html`:

- Upload the builder's exact `noid-privacy-workstation-44-${TAG}-x86_64.iso`,
  `SHA256SUMS`, and
  `SHA256SUMS.asc` to the website's `downloads/` area.
- Keep the verification instructions beside the download aligned with the
  complete [README recipe](../README.md#-download-the-iso): independently
  confirm the fingerprint, require the exact signer and usable signature/key
  status, and stop on any command or pipeline failure before checking hashes.
  Verify the download host's force-download and resume (`Accept-Ranges`)
  behavior for the published artifact.
...
Use the complete [download-verification instructions](https://github.com/NexusOne23/noid-privacy-workstation#-download-the-iso).
Confirm the release fingerprint independently and require every verification
step to succeed before booting the image.
...
1. Branch off the release tag: `git checkout -b hotfix/1.8.1 v1.8`
...
6. Merge hotfix back to main: `git checkout main && git merge hotfix/1.8.1`
