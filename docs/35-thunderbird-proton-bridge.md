...
- **"Authentication failed"**: Check the username and **Bridge-generated** password in Bridge's mailbox details; the Proton account password is not the mail-client password.
- **"Cannot connect to server"**: Open Bridge and check the account status and the host/ports in its mailbox details; confirm Thunderbird uses those values.
- **"Certificate untrusted"**: Check the expected Bridge endpoint and certificate before confirming Thunderbird's exception dialog. The warning alone does not establish that a certificate expired.
