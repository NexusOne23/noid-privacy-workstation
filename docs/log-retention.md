...
limits. Each run gets a timestamped report. Scheduled check reports additionally
contain the systemd invocation ID, so they can be associated with the exact
service run even after a long mutex wait. Direct checks without that ID retain
the timestamp/random-suffix format. Both exact filename forms have the same
30-day report retention; the shared log's timestamp alone cannot identify a run.
Preparing a candidate never makes
