...
- Active egress: `drop` (= the dropped physical interfaces: Ethernet and Wi-Fi)
...
Listing a `gateway-*` object does not establish that it processes traffic.
Check the effective disabled state, for example:
...
- Their regular zone is `libvirt-routed`, which is distinct from the default
  NAT network's `libvirt` zone. Activation depends on the policy's enabled
  state and zone bindings, not the number of running VMs or a bridge's name.
- Inspect `firewall-cmd --get-active-policies` and the actual zone assignments.
  A network can retain its bridge and firewall bindings without a running VM.
- The derived `block-lan-out-vms` policy binds `libvirt` ingress to `drop`
  egress. It does not automatically cover `libvirt-routed` or custom zones;
  those require their own complete forwarding review. The separate topology
  guard and WAN-strict layer have their own scopes described above.

See firewalld's [policy activation contract](https://firewalld.org/documentation/man-pages/firewalld.policies.html).

## Common questions

### "Why does `firewall-cmd --info-zone=drop` show empty services + ports?"

The image deliberately leaves those allowance lists empty. The `DROP` target
handles traffic that has not matched an earlier applicable rule; it does not
prevent explicit allowances or established replies. Zone rules govern host
ingress, while policies such as `block-lan-out` also govern the documented
egress and forwarding paths. Inspect all applicable layers before concluding
that a particular flow is blocked.
