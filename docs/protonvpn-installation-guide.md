...
The three cases below describe previously observed causes, not an exhaustive
diagnosis. Compare app, device and firewall evidence before selecting a remedy.
A WAN-strict pause permits direct egress and does not establish the cause of a
connection failure.
...
```bash
# 1 — inspect this table's IPv4 drop counter; compare before/after the probe
...
physical device. This supports an early-start diagnosis. A zero counter alone
does not exclude other filters or drops before the current table was loaded.
...
step. In the investigated client, Proton gives `add_connection_async` ten seconds; past that,
`kill_switch.enable()` raises and no further attempt is ever scheduled, so the
app stalls until it is restarted by hand. One confirmed image-side cause was a
...
**Mode C — investigate a WAN-strict drop.** `wan_blocked_v4` rises while the app probes, `noid-wan-strict
status` reports `STRICT_EMPTY`, and the journal says `reconciled 0 transient
bootstrap route tuple(s)`. The counter is shared by traffic through that rule;
correlate the destination and timing before attributing a drop to this probe.

`STRICT_EMPTY` means armed with no durable endpoint. A provider that creates
profiles only during connection can leave no saved profile to reconcile after
reboot. Resolver bootstrap and bounded handshake/route candidates are separate
exceptions; see [WAN egress strict architecture](wan-egress-strict.md).

If the verified failure needs temporary unrestricted egress, this recovery
command pauses WAN-strict for three minutes. **Direct WAN traffic becomes
possible during the pause**; other controls, including Proton's own kill
switch, keep their independent state. This is a deliberate temporary privacy
trade-off:
...
Then connect in the app. Auto-resume is scheduled before the pause is applied.
Restore strict mode as soon as the connection is established and verify it:
...
The same connectivity failure can also prevent chrony's NTS handshakes.
Inspect its actual status separately; a VPN probe failure does not prove that
time synchronization failed or justify relaxing authentication.
