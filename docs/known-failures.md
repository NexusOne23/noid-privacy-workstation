...
The Fedora OpenH264 repository downloads its signed payloads from Cisco's
distribution hostname. If the first codec transaction fails while NoID Privacy can
prove all of the following at once — the selected and effective resolver is
the exact Quad9 Strict filtered path, an uncached Cisco lookup fails there,
and the same lookup succeeds after a runtime-only switch to Quad9's
no-threat-blocking service — Task 2 retries exactly once through authenticated
strict DoT at `dns10.quad9.net`. The helper explains the temporary exception
in its live output and restores the original filtered path before Task 3.
Ordinary network/DNF failures do not change DNS. A failed restoration is fatal,
retains a root-only `/run` recovery record and is retried independently by the
service's post-stop hook; it is never reported as successful codec completion.
...
If chrony steps a previously incorrect system clock backwards after GNOME
wrote its completion marker, the scoper does not wait for wall time to catch
up and does not trust the future timestamp immediately. It binds a root-only
boot-local wait record to that marker's owner, device, inode and mtime, then
accepts it only after 60 seconds of monotonic boot time. Replacing or rewriting
the marker restarts the stability window, and a later forward wall-clock step
cannot shorten it. During that minute the profile may
still show “available to all users”; the active two-minute timer completes the
normal one-time user binding on its next pass. A later deliberate administrator
change of an already completed profile back to all-users remains an allowed
choice and is not silently reverted.
...
The installed-candidate gate reports newer SELinux kernel-extension coverage
separately from its hard Fedora 44 baseline. Current F44 policy may report
`extension-coverage=partial` with `handle-unknown=allow`: `getenforce` still
says `Enforcing`, but the missing `memfd_file`, delegated-BPF and newer
system/io_uring vectors are a documented vendor MAC-coverage boundary. This is
not mislabeled as complete coverage and does not make the supported F44 release
impossible. A missing baseline vector, malformed flag, or enabled
`memfd_class`/`bpf_token_perms` capability without its required schema remains
a hard failure. Recheck after aligned Fedora kernel/policy updates. Do not turn
on `deny_unknown` against an incompatible policy and do not try to declare base
security classes from the local NoID Privacy module; either can break normal
boot or application paths without repairing the vendor policy schema.
...
Close all Firefox windows, including Playground, before reapplying profile files.
...
Confirm the exact profile path and inspect `about:addons` after reopening Firefox;
file presence is not proof that an extension loaded
...
```bash
lsblk -f
for path in / /home /tmp /var/tmp; do
    findmnt --target "$path" -no TARGET,SOURCE,FSTYPE,OPTIONS
done
sudo cryptsetup luksDump "${NOID_LUKS_DEVICE:?set the reviewed LUKS backing device from lsblk}"
```
...
Update All retries a transient official-marketplace failure twice within a
bounded window. Two separate conditions qualify. `curl --retry` owns a timeout
and the HTTP 408/429/5xx statuses. An interrupted response can instead finish
with a curl exit code outside that retry set, even when its HTTP status already
arrived — observed as `curl: (56) OpenSSL SSL_read: ... unexpected eof while
reading` — so Update All additionally retries exit codes 18, 52, 56 and 92.
Every other curl failure stays a single attempt: DNS, a rejected TLS
certificate, an HTTP 404 and the response-size cap are never re-requested.
If the metadata or XPI endpoint is still unavailable, the run reports one
warning plus a `Deferred:` entry for that check, retains the already validated
installed XPI, and does not claim that its current release was authenticated. A
digest, identity, compatibility, or publication failure remains a hard update
error.
...
for database in passwd shadow group gshadow; do
    result=0
    sudo getent -s files "$database" liveuser >/dev/null || result=$?
    printf '%s lookup exit: %s\n' "$database" "$result"
done
...
The account checks print only exit statuses: `0` means an entry remains, `2`
means the key was not found, and another status needs investigation. They do
not print password hashes or other account fields.
...
the outer link MTU and reports a conservative, 16-byte-aligned inner target:

```
 - Peer routing evaluated with WireGuard fwmark 0x150c17a6
 - Conservative local MTU target: 1392 (outer wlp0s20f0u1 MTU 1456, inet endpoint, overhead 60; 16-byte aligned)
ERR: proton0 exceeds the conservative local target by 28 bytes
```
...
The aligned target is computed, never assumed:
...
Linux caps outgoing padding at the sending tunnel's MTU. With both peers set
to 1396, a 1396-byte inner packet therefore fits an IPv4 outer MTU of 1456
without fragmentation. A peer still set to 1420 can instead pad its
1396-byte reply to 1408, exceeding that outer budget. The 1392 target leaves
room for either sender's padding. An isolated two-peer Linux packet test
confirmed both cases; this explains the conservative choice without claiming
that every 1396-byte tunnel necessarily fragments or identifying a past TLS
failure's cause from MTU alone. The peer's actual MTU and a smaller remote
path MTU remain outside this passive audit's evidence.
