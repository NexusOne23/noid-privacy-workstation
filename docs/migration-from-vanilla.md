...
- **M08 service masking** — apply only reviewed entries from M08's explicit
  ...
  copied count.
  Without review, masking may break avahi/wsdd discovery or CUPS printing.
  Thermald is not in M08's mask list. Review each service before applying a mask.
...
## No migration script is shipped

There is no supported automated migration tool. Treat the list above as a
review map, not as commands to copy: an in-place migration needs a backup of
every original configuration, exact recovery commands for every change and a
manual decision for every Tier-2+ item.
