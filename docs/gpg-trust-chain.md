...
- **Source**: `thunderbird/noid-thunderbird-hardening.js` checked into this
  repository (reviewed derivative of tagged HorlogeSkynet v140.3, MIT base).
- **Verification**: `scripts/regen-thunderbird-embed.sh --check` enforces
  ...
  and post-v140.3 Thunderbird preferences.
...
- **Source**: GitHub release
  `gorhill/uBlock/releases/download/1.75.0/uBlock0_1.75.0.firefox.signed.xpi`
...
- **Signs**: the detached `SHA256SUMS.asc` over the release ISO's `SHA256SUMS`.
  Before booting an ISO, use the complete
  [download-verification recipe](../README.md#-download-the-iso): key import,
  exactly one good signature from the pinned primary key, rejection of
  expired signatures or expired/revoked keys, then strict checksum verification.
  Every command and pipeline must succeed.
