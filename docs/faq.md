...
For a signed release, follow the complete
[download-verification recipe](../README.md#-download-the-iso), including the
pinned signing key and strict signature/checksum checks. Cross-check the key
fingerprint through an independent public channel as described in
[`gpg-trust-chain.md`](gpg-trust-chain.md).

A locally built unsigned candidate has checksums but no release signature.
Checksum equality verifies its recorded bytes; it does not authenticate a
publisher or establish that the candidate passed the release gate.
...
Depends on your employer's policy. Licenses differ by component; see
[`LICENSING.md`](../LICENSING.md), including third-party and branding exceptions.
No warranty. No compliance claims (not HIPAA, PCI, SOC2).
