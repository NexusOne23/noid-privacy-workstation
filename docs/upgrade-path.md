...
| Firefox/Thunderbird integration | cached source re-deployed by Module 25; Update All additionally advances the profile-local uBlock Origin and DKIM Verifier add-ons through their official AMO/ATN channels with signature/digest checks and recorded version + SHA-256 evidence | cached hardening pins/config are not automatically replaced by a newer repo revision; the add-on channel is an owner-started mutable-pin exception |
| Claude/Codex CLIs and editor extensions | Update All invokes the M13 helpers' `--update` mode: opted-in components refresh over the vendor channel with identity/structure validation and recorded version + SHA-256 evidence | never-opted-in components stay untouched; install-time pins change only with a reviewed NoID Privacy release. The managed GNOME extension's cached image seed remains exact-pin checked, but an owner-started Update All may replace the installed tree from its fixed EGO identity without an upstream artifact signature; together with the browser add-on and Open VSX extension channels this is one of the documented mutable-pin exceptions |
