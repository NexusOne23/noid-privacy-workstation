...
This inventory covers exact upstream artifact pins, signing-key fingerprints
and reviewed Fedora payloads. Separately fetched artifacts use fixed versions
or commits, SHA-256 and, for large binaries, byte counts from their official
channels. Ordinary Fedora packages follow the configured signed repositories;
they are not all version-locked. This page lists **where each pin lives and how to
...
| Local-AI reference GGUF (Ornith) | immutable publisher revision on huggingface.co | canonical `docs/28-local-ai.md`, generated `kickstart/snippets/28-local-ai-docs.ks`, and `tests/28-local-ai-structural.sh`; reproduced reference-host record pinned by revision, size and SHA-256, never image downloads |
...
user-started channel (Firefox/Thunderbird add-ons, the managed GNOME Shell
extension from its fixed EGO identity, non-agent VSCodium extensions from Open
VSX, and opted-in Claude/Codex components through Update All), the pin is a
recovery seed and that explicit
